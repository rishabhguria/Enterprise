  ...
  <xsl:template name="Option">
    ...
    <xsl:param name="Suffix"/>
      ...
      <xsl:variable name="StrikePrice">
        <xsl:value-of select="format-number(substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL7),'/'),'/'),' '),' '),2),'##.00')"/>
      </xsl:variable>
          ...
          <PositionMaster>
            ...
            <xsl:variable name="varSedol">
			  ...
			  <xsl:variable name="varSymbol">
				  <xsl:choose>
					  <xsl:when test="contains(COL38,'_')">
						  <xsl:value-of select="substring-before(COL38,'_')"/>
					  </xsl:when>

					  <xsl:when test="contains(COL38,'.')">
						  <xsl:value-of select="substring-before(COL38,'.')"/>
					  </xsl:when>
