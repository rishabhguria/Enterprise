...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		...
		<Groups>
			...
			<Group
		   OrderNumber ="OrderNumber" Cancelcorrectindicator ="Cancelcorrectindicator" AccountNumber="AccountNumber" SecurityIdentifier="SecurityIdentifier"
		   ...
		   GenevaLotNumber1="GenevaLotNumber1" GainsKeeperLotNumber1="GainsKeeperLotNumber1" LotDate1="LotDate1" LotQty1="LotQty1" LotPrice1="LotPrice1"
		   GenevaLotNumber2 ="GenevaLotNumber2" GainsKeeperLotNumber2="GainsKeeperLotNumber2" LotDate2="LotDate2" LotQty2="LotQty2" LotPrice2="LotPrice2"
		   GenevaLotNumber3="GenevaLotNumber3" GainsKeeperLotNumber3="GainsKeeperLotNumber3" LotDate3="LotDate3" LotQty3="LotQty3" LotPrice3="LotPrice3"
		   GenevaLotNumber4 ="GenevaLotNumber4" GainsKeeperLotNumber4="GainsKeeperLotNumber4" LotDate4="LotDate4" LotQty4="LotQty4" LotPrice4="LotPrice4"
		   GenevaLotNumber5="GenevaLotNumber5" GainsKeeperLotNumber5="GainsKeeperLotNumber5" LotDate5="LotDate5" LotQty5="LotQty5" LotPrice5="LotPrice5"

		   EntityID="{EntityID}" TaxLotState="{TaxLotState}" TaxLotState1="" FileHeader="FALSE" FileFooter="FALSE" RowHeader="false"/>
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="varTransactionType">
			<xsl:choose>
				...
				<xsl:when test ="IsSwapped='true' and (Side = 'Sell short' or Side = 'Sell')">
					<xsl:value-of select="'SELL'"/>
				</xsl:when>
				<xsl:when test="Side = 'Sell to Open' and Asset = 'EquityOption'">
					<xsl:value-of select="'SHRT'"/>
				</xsl:when>
		...
		<xsl:variable name="varSymbol">
			<xsl:choose>
				...
				<xsl:when test="SEDOLSymbol != '*' and SEDOLSymbol != '' and Asset = 'Equity'">
					<xsl:value-of select="SEDOLSymbol"/>
				</xsl:when>
				<xsl:when test="ISINSymbol != '*' and ISINSymbol != '' and Asset = 'Equity'">
					  <xsl:value-of select="ISINSymbol"/>
				</xsl:when>
				<xsl:when test="CUSIPSymbol != '*' and CUSIPSymbol != '' and Asset = 'Equity'">
					<xsl:value-of select="CUSIPSymbol"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name ="varAvgPrice">
			<xsl:choose>
				<xsl:when test="SettlementCurrency = CurrencySymbol">
					<xsl:value-of select="format-number(AveragePrice,'##.########')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="format-number(SettlCurrAmt,'##.########')"/>
				</xsl:otherwise>				
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="varCommission">
			<xsl:choose>
				<xsl:when test="SettlCurrFxRate=0">
					<xsl:value-of select="format-number($varTotalCommission,'0.00')"/>
				</xsl:when>
				<xsl:when test="SettlCurrFxRate!=0 and FXConversionMethodOperator='M'">
					<xsl:value-of select="format-number($varTotalCommission * FXRate_Taxlot,'0.00')"/>
				</xsl:when>
				<xsl:when test="SettlCurrFxRate!=0 and FXConversionMethodOperator='D'">
					<xsl:value-of select="format-number($varTotalCommission div FXRate_Taxlot,'0.00')"/>
				</xsl:when>				
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<xsl:variable name ="varAccount">
			<xsl:choose>
				<xsl:when test ="IsSwapped='true'">					
					<xsl:value-of  select="'050033786'"/>
				</xsl:when>
				<xsl:otherwise>					
					<xsl:value-of  select="'002674034'"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<Group
				OrderNumber ="" Cancelcorrectindicator ="{$varAllocationState}" AccountNumber="{$varAccount}" SecurityIdentifier="{$varSymbol}"
				Broker="{CounterParty}" Custodian="GSCO" TransactionType="{$varTransactionType}" CurrencyCode="{$PB_SettleCurrency}"
				TradeDate = "{TradeDate}" SettleDate = "{SettlementDate}"
				Quantity="{AllocatedQty}" Commission="{$varCommission}" Price="{$varAvgPrice}" AccruedInterest="{$varAccruedInterest}"
				TradeTax="" MiscMoney="" NetAmount="" Principal="{GrossAmount}" Description=""
