...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[contains(AccountName,'VABI')]">


				<xsl:if test="AccountNo!='038CAAPD2' and AccountNo!='0581CHFY9' and AccountNo!='06178WSG8'">
					...
					<ThirdPartyFlatFileDetail>
						...
						<ClearingBroker>
							<xsl:choose>
								<xsl:when test="contains(AccountName,'GS')">
									<xsl:value-of select="'GS'"/>
								</xsl:when>
								<xsl:when test="contains(AccountName,'MS')">
						...
						<ClearerAccountNumber>
							<xsl:choose>
								<xsl:when test="Asset='Equity' and IsSwapped='true'">
									<xsl:value-of select="'06178WXN7'"/>
								</xsl:when>
								<xsl:when test="contains(Asset,'FX')">
									<xsl:value-of select="'0581CHXC7'"/>
								</xsl:when>
								<xsl:when test="(Asset='Equity' or Asset='EquityOption') and AccountName='VABI - GS - PB'">
									<xsl:value-of select="'002604387'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="'038CDKDA6'"/>
								</xsl:otherwise>
							</xsl:choose>
						</ClearerAccountNumber>
						...
						<xsl:variable name="TotalFee">
							<xsl:value-of select="CommissionCharged + SoftCommissionCharged + OtherBrokerFee + ClearingBrokerFee + StampDuty + TransactionLevy + ClearingFee + TaxOnCommissions + MiscFees + SecFee + OccFee + OrfFee"/>
						</xsl:variable>

						<Commission>
							<xsl:choose>
								<xsl:when test="number($TotalFee)">
							...
							<!--<xsl:call-template name="Conversion">
								<xsl:with-param name="Value" select="$TotalFee"/>
