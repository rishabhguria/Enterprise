...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
  ...
  <xsl:template match="/">
    <Groups>
      ...
      <xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail[(contains(FundName, 'Swap') = false and TaxLotState = 'Allocated') and (UDASecurityTypeName!='cfeu' and UDASecurityTypeName!='cfus' and UDASecurityTypeName!='ffus')]">
  ...
  <xsl:template name="TaxLotIDBuilder">
    ...
    <xsl:variable name="varClientAccount">
	  ...
	  <xsl:variable name ="varAccountNo">
		  <xsl:choose>
			  <xsl:when test ="FundName= 'GS Newtyn Partners LP:002486561'">
				  <xsl:value-of select ="'002486561'"/>
			  </xsl:when>
			  <xsl:when test ="FundName= 'GS Newtyn TE Partners LP:002486553'">
    ...
    <xsl:variable name="varFeeValue2">
	  ...
	  <xsl:variable name="PRANA_SYMBOL_NAME">
		  <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='NewtynEODSymbols']/SymbolData[@PBCompanyName=$PB_SYMBOL_NAME]/@PranaSymbol"/>
	  </xsl:variable>
	  ...
	  <xsl:variable name ="varAvgPrice">
		  <xsl:choose>
			  ...
			  <xsl:when test ="CounterParty = 'ENSK' and number(ForexRate)">
				  <xsl:value-of  select="format-number(AveragePrice,'0.######')*ForexRate"/>
			  </xsl:when>
			  <xsl:otherwise>
	  ...
	  <xsl:variable name ="PB_CounterParty">
		  <xsl:value-of select="document('../ReconMappingXml/ThirdPartyCounterPartyMappingWithCurrency.xml')/CounterPartyMapping/PB[@Name='GS']/CounterPartyData[(@PranaCounterParty = $Cpty) and (@CurrencySymbol=$Curr)]/@PBCounterParty"/>
	  </xsl:variable>

	  <xsl:variable name ="PB_CounterPartyWOCurr">
		  <xsl:value-of select="document('../ReconMappingXml/ThirdPartyCounterPartyMappingWithCurrency.xml')/CounterPartyMapping/PB[@Name='GS']/CounterPartyData[@PranaCounterParty = $Cpty]/@PBCounterParty"/>
	  </xsl:variable>
	  ...
	  <xsl:variable name="varCounterParty">
        ...
        <xsl:when test="CounterParty='CGOA'">
          <xsl:value-of select="'CITI'"/>
			  ...
			  <xsl:when test="$PB_CounterPartyWOCurr!=''">
				  <xsl:value-of select="$PB_CounterPartyWOCurr"/>
			  </xsl:when>
			  <xsl:otherwise>
	  ...
	  <Group 
			  OrderNumber ="" Cancelcorrectindicator ="" AccountNumber="{FundAccntNo}" SecurityIdentifier="{$varSymbol}"
			  Broker="{$varCounterParty}" Custodian="GSCO" TransactionType="{$varTransactionType}" CurrencyCode="{$PB_SettleCurrency}"
			  ...
			  TradeTax="{$varTradeTax}" MiscMoney="{MiscFees}" NetAmount="{$varNetAmount}" Principal="{GrossAmount}" Description="{Description}"
