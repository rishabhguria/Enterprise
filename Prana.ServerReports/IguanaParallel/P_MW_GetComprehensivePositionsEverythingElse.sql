...
alter Procedure [dbo].[P_MW_GetComprehensivePositionsEverythingElse]                                                                                    
(                                                                                                               
 @Date datetime,                     
 @fund varchar(max),        
 @IncludeCash bit,        
...
INSERT INTO #t(SecurityID,SecurityDescription,UnderlyingSecurity, Fund, Quantity, SideMultiplier, Multiplier,UnitCost, UnitCostLocal, TotalCost, TotalCostLocal,       
...
End as SecurityDescription,        
 UnderlyingSymbol, MAX(Fund), SUM(BeginningQuantity*SideMultiplier), MAX(SideMultiplier), Multiplier,AVG(UnitCostBase), AVG(UnitCostLocal), SUM(TotalCost_Base), SUM(TotalCost_Local), AVG(EndingPriceBase), AVG(EndingPriceLocal), SUM(EndingMarketValueBase),
...
AND (Fund in (SELECT * FROM dbo.split(@fund, ',')))        
GROUP BY Symbol,UnderlyingSymbol,Multiplier            
...
CREATE TABLE #temp(        
 ...
 [ISINSymbol] [varchar](50),     [SEDOLSymbol] [varchar](50),        
...
UPDATE #t        
SET PercentOfEquity = Case When @NetEquity <> 0 Then MarketValue / @NetEquity Else 0 End            
