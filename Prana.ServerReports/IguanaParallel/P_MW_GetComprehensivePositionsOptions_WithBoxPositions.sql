...
alter Procedure [dbo].[P_MW_GetComprehensivePositionsOptions_WithBoxPositions]                                                                                            
(                                                                                                                       
 @Date datetime,                             
 @fund varchar(max),                
 @IncludeCash bit,                     
...
INSERT INTO #closed(Symbol, Quantity,CountSides)                
SELECT DISTINCT Symbol, SUM(BeginningQuantity*SideMultiplier),Count(distinct Side)as CountSides FROM T_MW_GenericPNL                
WHERE RunDate = @Date                
AND Asset = 'Equity'                
AND Open_CloseTag = 'O'                
...
INSERT INTO #t(SecurityID, SecurityDescription, UnderlyingSecurity, Fund, Quantity, SideMultiplier, UnitCost, UnitCostLocal, TotalCost, TotalCostLocal, MarketPrice, MarketPriceLocal, MarketValue, MarketValueLocal, Delta, DeltaAdjusted, DeltaAdjustedLocal,
...
End as SecurityName,                 
 ...
 MAX(UnderlyingSymbol), MAX(Fund), SUM(BeginningQuantity*SideMultiplier), MAX(SideMultiplier), AVG(UnitCostBase), AVG(UnitCostLocal), SUM(TotalCost_Base), SUM(TotalCost_Local), AVG(EndingPriceBase), AVG(EndingPriceLocal), SUM(EndingMarketValueBase),      
...
WHERE RunDate = @Date                 
AND Open_CloseTag = 'O'                
AND Asset = 'EquityOption'                
AND UDASubSector != 'Indices' --Hierarchy: indices > naked options                
AND (UnderlyingSymbol NOT IN (SELECT DISTINCT Symbol FROM T_MW_GenericPNL                
        WHERE RunDate = @Date                
        AND Open_CloseTag = 'O'                
        AND Fund in (SELECT * FROM dbo.split(@fund, ',')))                
     --OR UnderlyingSymbol IN (SELECT DISTINCT Symbol from #closed)          
 )                
AND (Fund in (SELECT * FROM dbo.split(@fund, ',')))                
GROUP BY Symbol                
ORDER BY Symbol                
...
delete del from #t del          
inner join #Closed on (del.SecurityID = #Closed.Symbol)          
where #Closed.CountSides = 1               
