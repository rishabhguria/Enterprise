  ...
  <xsl:template name="Option">
    <xsl:param name="Symbol"/>
      ...
      <xsl:variable name="UnderlyingSymbol">
        <xsl:value-of select="substring-before(normalize-space(COL53),' ')"/>
      </xsl:variable>
      <xsl:variable name="ExpiryDay">
        <xsl:value-of select="substring(substring-after(normalize-space(COL53),' '),5,2)"/>
      </xsl:variable>
      <xsl:variable name="ExpiryMonth">
        <xsl:value-of select="substring(substring-after(normalize-space(COL53),' '),3,2)"/>
      </xsl:variable>
      <xsl:variable name="ExpiryYear">
        <xsl:value-of select="substring(substring-after(normalize-space(COL53),' '),1,2)"/>
      </xsl:variable>
      <xsl:variable name="StrikePrice">
        <xsl:value-of select="format-number(substring(substring-after(normalize-space(COL53),' '),8) div 1000,'##.00')"/>
      </xsl:variable>
      <xsl:variable name="PutORCall">
        <xsl:value-of select="substring(substring-after(normalize-space(COL53),' '),7,1)"/>
      </xsl:variable>
  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="substring-before(COL12,' ')='CALL' or substring-before(COL12,' ')='PUT'">
          <PositionMaster>
            <xsl:variable name="varOption">
              <xsl:call-template name="Option">
                <xsl:with-param name="Symbol" select="COL53"/>
              </xsl:call-template>
            </xsl:variable>
            <TickerSymbol>
            ...
            <PutOrCall>
              <xsl:choose>
                <xsl:when test="substring(substring-after(normalize-space(COL53),' '),7,1)='P'">
                  <xsl:value-of select="'0'"/>
                </xsl:when>
                <xsl:when test="substring(substring-after(normalize-space(COL53),' '),7,1)='C'">
            ...
            <xsl:variable name="varDescription">
              <xsl:value-of select="concat(COL8,COL12)"/>
            </xsl:variable>
            <LongName>
