...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:param name="Suffix"/>
		<xsl:if test="contains(substring(substring-after(normalize-space(COL4),' '),7,1),'C') or contains(substring(substring-after(normalize-space(COL4),' '),7,1),'P')">
				...
				<xsl:if test="number(COL8) and normalize-space(COL5)!='U.S. DOLLARS'">
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="string-length(COL4) &gt; 20">
