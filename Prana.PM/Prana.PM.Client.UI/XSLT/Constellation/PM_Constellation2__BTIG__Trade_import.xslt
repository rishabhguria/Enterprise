			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<SideTagValue>							
							<xsl:choose>
								<xsl:when test="contains(normalize-space(COL17),'B')">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL17),'S')">
									<xsl:value-of select="'2'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL17),'BC')">
									<xsl:value-of select="'B'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL17),'SS')">
