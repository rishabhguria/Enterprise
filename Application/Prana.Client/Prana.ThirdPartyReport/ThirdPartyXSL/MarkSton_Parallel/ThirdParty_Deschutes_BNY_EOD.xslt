...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<Group 
		  ...
		  Netmoney="{concat('$',$NetAmountSumG)}"  Broker="{$CP}" DTC="0226" EntityID="{EntityID}" TaxLotState="{TaxLotState}">
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<ThirdPartyFlatFileDetail
					...
					Netmoney="{concat('$',$NetAmount)}"	Broker="{$CP}" DTC="{$AccountId}" EntityID="{EntityID}" />
			</xsl:for-each>
		</Group>
	</xsl:template>
</xsl:stylesheet>