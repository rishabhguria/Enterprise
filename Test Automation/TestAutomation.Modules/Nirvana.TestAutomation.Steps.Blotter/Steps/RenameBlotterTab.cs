...
{
    public class RenameBlotterTab : BlotterUIMap, ITestStep
    {
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                if (testData != null)
                {
                    ...
                    {
                        WorkingSubsTab.MsaaName = dr[TestDataConstants.RENAME_OLD_TAB_NAME].ToString();
                        WorkingSubsTab.WaitForVisible();
                        if (WorkingSubsTab.IsValid)
                        {
                            WorkingSubsTab.Click();
                            CtrlBlotterMain00.Click(MouseButtons.Right);
                            RenameTab.Click(MouseButtons.Left);
                            if (TextBox1.IsEnabled)
                            {
                                if (TextBox1.IsValid)
                                {
                                    TextBox1.Click(MouseButtons.Left);
                                    TextBox1.Properties["Text"] = dr[TestDataConstants.RENAME_NEW_TAB_NAME].ToString();
                                }
                            }
