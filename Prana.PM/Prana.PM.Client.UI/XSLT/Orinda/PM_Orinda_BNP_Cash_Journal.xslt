			...
			<xsl:for-each select ="//PositionMaster">
				<xsl:variable name="Cash">
					<xsl:value-of select="COL14"/>
	         		...
	         		<xsl:if test="number($Cash) and  COL9 = 0 ">
					<PositionMaster>
						...
						<xsl:variable name="PRANA_ACRONYM_NAME">
							<xsl:choose>
	         						<xsl:when test="COL19!='*'  and $Cash &gt; 0 ">								
										<xsl:value-of select="'Interest_Income'"/>
								</xsl:when>
								<xsl:when test="COL19!='*'  and $Cash &lt; 0 ">
									<xsl:value-of select="'Interest_Expense'"/>								
      							</xsl:when>
								<xsl:when test="COL7='Expense' and COL19='*'  and $Cash &gt; 0 ">
									<xsl:value-of select="'MISC_INC'"/>
								</xsl:when>
								<xsl:when test="COL7='Income' and COL19='*'  and $Cash &lt; 0 ">
