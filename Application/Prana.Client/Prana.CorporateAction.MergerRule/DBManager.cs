...
namespace Prana.CorporateAction.MergerRule
{
    ...
    {
        internal static bool SaveCAMerger(List<CAOnProcessObjects> caOnProcessObjectList, IAllocationServices allocationServices, IClosingServices closingServices, int userID)
        {
            bool isSaved = true;
            foreach (CAOnProcessObjects obj in caOnProcessObjectList)
            {
                ...
                Logger.LoggerWrite("Corporate Action (Merger) data saving starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);

                Int64 symbol_PK_Underlying = SecurityMasterSymbolIDGenerator.GenerateSymbolPKID();

                //Saves data in securitymaster
                Logger.LoggerWrite("Corporate Action (Merger): saving in Security Master DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                ...
                Logger.LoggerWrite("Corporate Action (Merger): saved in Security Master DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                ...
                {
                    Logger.LoggerWrite("Corporate Action (Merger): New groups from Chunk " + chunkNo + " saving in client DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    affectedPositions2 = allocationServices.SaveGroups(taxlotSave, userID);
                    chunkNo++;
                }
                Logger.LoggerWrite("Corporate Action (Merger): New groups saved in client DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                //Saves the closing data into PM_Taxlots and PM_Taxlotclosing
                Logger.LoggerWrite("Corporate Action (Merger): Closed data saving in client DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                affectedPositions3 = closingServices.SaveCloseTradesData(obj.ClosingData);
                Logger.LoggerWrite("Corporate Action (Merger): Closed data saved in client DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                ...
                Logger.LoggerWrite("Corporate Action (Merger): Taxlots affected due to Corporate action, saving in client DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                affectedPositions4 = AllocationDataManager.SaveCAWiseCloseData(caCloseData);
                Logger.LoggerWrite("Corporate Action (Merger): Taxlots affected due to Corporate action, saved in client DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);

                Logger.LoggerWrite("Corporate Action (Merger) data saving completed", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
        ...
        internal static bool UndoNameChange(CAOnProcessObjects caOnProcessObject, IClosingServices closingServices, IAllocationServices allocationServices)
        {
            ...
            Logger.LoggerWrite("Undo Corporate Action (Merger) Delete Data from Security Master starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            int affectedPositions1 = SecMasterDataManager.UndoNameChange(caIds);

            Logger.LoggerWrite("Undo Corporate Action (Merger) Unwinding positions starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            bool isSuccessfullyUnwinded = UnWindPositions(caIds, allocationServices);

            Logger.LoggerWrite("Undo Corporate Action (Merger) Delete Groups starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            int affectedPositions2 = allocationServices.DeleteGroupsFromCA(FetchGroupsToDeleteForCAs(caIds));

            Logger.LoggerWrite("Undo Corporate Action (Merger) Delete CA Information from client side starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            int affectedPositions3 = DeleteNameChangeData(caIds);
            Logger.LoggerWrite("Corporate Action (Merger): Undo data completed", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
