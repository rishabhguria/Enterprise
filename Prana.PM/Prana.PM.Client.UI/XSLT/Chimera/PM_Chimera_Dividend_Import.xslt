	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($varAmount) and normalize-space(COL2) = '038CAOOJ0'">
					...
					<PositionMaster>
						...
						<xsl:variable name="varActivityType">
							<xsl:choose>
								<xsl:when test="COL6='DividendTax'">
						...
						<xsl:variable name="varDecription">
							<xsl:choose>
								<xsl:when test="$varActivityType='WithholdingTax'">
									<xsl:value-of select="'Withholding Tax'"/>
								</xsl:when>								
								<xsl:when test="$varActivityType='DividendExpense'">
