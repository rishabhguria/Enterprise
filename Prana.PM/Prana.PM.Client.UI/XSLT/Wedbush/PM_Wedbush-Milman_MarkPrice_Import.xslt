  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($MarkPrice) and COL1='38511651'">
        <!--<xsl:if test="normalize-space(COL2)!='Cusip' ">-->
          <!--normalize-space(COL2)!='Cusip' and $PRANA_FUND_NAME!='' and $PRANA_FUND_NAME!=''-->
          <PositionMaster>
           <xsl:variable name ="varCallPut">
            ...
            <xsl:variable name="PB_SYMBOL_NAME">
              <xsl:choose>
                <xsl:when test="contains(COL3,$apos) or contains(COL6,'WTS') ">
            ...
            <xsl:variable name="PB_CUSIP_SYMBOL" >
              <xsl:choose>
                <xsl:when test="substring(COL2,1,1)='0'">
                  <xsl:value-of select="COL2"/>
                </xsl:when>
                <xsl:when test="not(number(COL2))">
                  <xsl:value-of select="COL2"/>
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:variable name="PRANA_CUSIP_SYMBOL">
              <xsl:value-of select="document('../ReconMappingXml/CusipMapping.xml')/SymbolMapping/PB[@Name='Wedbush']/SymbolData[@CusipSymbol=$PB_CUSIP_SYMBOL]/@PranaSymbol"/>
            </xsl:variable>
            <xsl:variable name="PRANA_SYMBOL">
              <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='Wedbush']/SymbolData[@PBCompanyName=$PB_SYMBOL and @CusipSymbol=$PB_CUSIP_SYMBOL]/@PranaSymbol"/>
            </xsl:variable>
            <xsl:variable name="Apos">'</xsl:variable>
