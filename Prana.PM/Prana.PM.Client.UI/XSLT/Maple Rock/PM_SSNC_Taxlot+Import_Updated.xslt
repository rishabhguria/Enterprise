	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and COL10!='Currency'">
					<PositionMaster>
						...
						<xsl:variable name ="Asset">
							<xsl:choose>
								<xsl:when test=" COL14='Put Option' and string-length(COL6 &gt; 20)">
						...
						<xsl:variable name="CostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$TotalCostBasis div $Position "/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
