      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($NetPosition)" >
          <PositionMaster>
            ...
            <SideTagValue>
              <xsl:choose>
                <xsl:when test="normalize-space(COL6)='Buy'">
                  <xsl:value-of select="'1'"/>
                </xsl:when>
                
                <xsl:when test="normalize-space(COL6)='Sell'">
            ...
            <xsl:variable name="varODay">
              <xsl:choose>
                <xsl:when test="contains(COL20,'/')">
                  <xsl:choose>
                    <xsl:when test="string-length(substring-before(substring-after(normalize-space(COL20),'/'),'/'))=1">
                      <xsl:value-of select="concat(0,substring-before(substring-after(COL20,'/'),'/'))"/>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <xsl:variable name="varOMonth">
              <xsl:choose>
                <xsl:when test="contains(COL20,'/')">
                  <xsl:choose>
                    <xsl:when test="string-length(substring-before(normalize-space(COL20),'/'))=1">
                      <xsl:value-of select="concat(0,substring-before(COL20,'/'))"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="substring-before(COL20,'/')"/>
                    </xsl:otherwise>
                  </xsl:choose>
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:variable name="varSettleDay">
              <xsl:choose>
                <xsl:when test="contains(COL19,'/')">
                  <xsl:choose>
                    <xsl:when test="string-length(substring-before(substring-after(normalize-space(COL19),'/'),'/'))=1">
            ...
            <xsl:variable name="varSettleMonth">
              <xsl:choose>
                <xsl:when test="contains(COL19,'/')">
                  <xsl:choose>
                    <xsl:when test="string-length(substring-before(normalize-space(COL19),'/'))=1">
            ...
            <xsl:variable name="varTradeDay">
              <xsl:choose>
                <xsl:when test="contains(COL18,'/')">
                  <xsl:choose>
                    <xsl:when test="string-length(substring-before(substring-after(normalize-space(COL18),'/'),'/'))=1">
            ...
            <xsl:variable name="varTradeMonth">
              <xsl:choose>
                <xsl:when test="contains(COL18,'/')">
                  <xsl:choose>
                    <xsl:when test="string-length(substring-before(normalize-space(COL18),'/'))=1">
