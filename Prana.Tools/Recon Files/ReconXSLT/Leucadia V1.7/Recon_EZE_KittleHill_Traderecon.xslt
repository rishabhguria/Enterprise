				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name ="PB_FUND_NAME">
							<xsl:value-of select ="concat(COL6,COL11)"/>
						</xsl:variable>
						...
						<xsl:variable name="PRANA_BROKER_ID">
							<xsl:value-of select="document('../ReconMappingXml/ExecBrokerMapping.xml')/BrokerMapping/PB[@Name=$varPBName]/BrokerData[@PranaBrokerCode=$PB_BROKER_NAME]/@PranaBroker"/>
						</xsl:variable>
						...
						<CounterParty>
							<xsl:choose>
								...
								<xsl:when test="COL9='COWNS'">
									<xsl:value-of select="'COWEN'"/>
								</xsl:when>
								<xsl:when test="COL9='BTIGS'">
									<xsl:value-of select="'BTIG'"/>
								</xsl:when>
								<xsl:when test="COL9='RAYM'">
									<xsl:value-of select="'RAYM'"/>
								</xsl:when>
								<xsl:when test="COL9='PWJC'">
									<xsl:value-of select="'PWJC'"/>
								</xsl:when>
								<xsl:when test="COL9='WCHV'">
									<xsl:value-of select="'WCHV'"/>
								</xsl:when>
								<!--<xsl:when test="$PRANA_BROKER_ID">
									<xsl:value-of select="$PRANA_BROKER_ID"/>
								</xsl:when>
