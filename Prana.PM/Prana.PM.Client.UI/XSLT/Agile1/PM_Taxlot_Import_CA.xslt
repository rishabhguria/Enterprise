		...
		<DocumentElement>
          ...
          <xsl:call-template name="Translate">
            <xsl:with-param name="Number" select="(COL7 * 3)"/>
				...
				<xsl:if test="number($Position) and not(contains(COL3,'MONEY MARKET')) ">
					...
					<PositionMaster>
						...
						<xsl:variable name = "PB_SYMBOL_NAME" >
							<xsl:value-of select ="translate(COL5, $smallcase, $uppercase)"/>
						</xsl:variable>
                  ...
                  <xsl:value-of select="''"/>
						...
						<SideTagValue>
							<xsl:choose>
								...
								<xsl:when test="($Position &gt; 0) and COL3='EQUITY OPTION'">
									<xsl:value-of select="'A'"/>
								</xsl:when>
						...
						<xsl:variable name="Commission">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL411"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="Fees">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL412"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="FXRate" select="COL41"/>
