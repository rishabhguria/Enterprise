...
{
    class VerifyAggregationSymbolWithNav : PopUpUIMap, ITestStep
    {
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                PopUpUIMap.DumpWindow("AggregationSymbolWithNav");
                StringBuilder errorBuilder = new StringBuilder();
                string DevPath = Path.GetFullPath(Path.Combine(Environment.CurrentDirectory, @"..\..\..\.."));
                DataTable xmltoDataTable = CommonMethods.ReadXML(DevPath + TestDataConstants.DUMP_PATH + "AggregationSymbolWithNav.xml");
