...
namespace Prana.Admin
{
    ...
    public class Fix : System.Windows.Forms.Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.grdFix.UseFlatMode = Infragistics.Win.DefaultableBoolean.True;
        ...
        private void BindFixGrid()
        {
            ...
            {
                Fixs nullFixs = new Fixs();
                nullFixs.Add(new Prana.Admin.BLL.Fix(int.MinValue, ""));
                grdFix.DataSource = nullFixs;
            }
        }
        ...
        private int SaveFix()
        {
            ...
            Prana.Admin.BLL.Fix fix = new Prana.Admin.BLL.Fix();
            ...
            if (newFixID == -1)
            {
                MessageBox.Show("Fix with the same name or tag already exists.", "Prana Alert", MessageBoxButtons.OK);
            }
        ...
        private void btnDelete_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (grdFix.Rows.Count > 0)
                {
                    string fixName = grdFix.ActiveRow.Cells["FixVersion"].Text.ToString();
                    if (fixName != "")
                    {
                        //Asking the user to be sure about deleting the fix.
                        if (MessageBox.Show(this, "Do you want to delete this Fix?", "Prana Alert", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                            ...
                            bool chkVarraible = Fixmanager.DeleteFix(fixID);
                            if (!(chkVarraible))
                            {
                                MessageBox.Show(this, "Fix is referenced in CounterpartyVenue/Client.\n Please remove references first to delete it.", "Prana Alert");
                            }
                            else
                            {
                                BindFixGrid();
