...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="MonthNo">
		...
		<xsl:choose>
			<xsl:when test ="$varMonth='JAN' or $varMonth='01'">
				<xsl:value-of select ="'01'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='FEB' or $varMonth='02'">
				<xsl:value-of select ="'02'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='MAR' or $varMonth='03'">
				<xsl:value-of select ="'03'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='APR' or $varMonth='04'">
				<xsl:value-of select ="'04'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='MAY' or $varMonth='05'">
				<xsl:value-of select ="'05'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='JUN' or $varMonth='06'">
				<xsl:value-of select ="'06'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='JUL' or $varMonth='07'">
				<xsl:value-of select ="'07'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='AUG' or $varMonth='08'">
				<xsl:value-of select ="'08'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='SEP' or $varMonth='09'">
				<xsl:value-of select ="'09'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='OCT' or $varMonth='10'">
				<xsl:value-of select ="'10'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='NOV' or $varMonth='11'">
				<xsl:value-of select ="'11'"/>
			</xsl:when>
			<xsl:when test ="$varMonth='DEC' or $varMonth='12'">
				...
				<xsl:if test ="number(COL16)">
					...
					<Comparision>
						...
						<xsl:variable name="TotalCommissionandFees">
							<xsl:value-of select="number(COL19 - COL20 - COL21)"/>
						</xsl:variable>
