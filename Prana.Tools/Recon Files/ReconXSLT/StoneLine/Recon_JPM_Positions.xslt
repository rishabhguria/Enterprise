	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name="PB_SYMBOLS_NAME">
				...
				<xsl:if test="number($Quantity)">
					<PositionMaster>
						...
						<xsl:variable name="varCusip" select="COL26"/>
						<xsl:variable name="varISIN" select="COL25"/>


						<xsl:variable name="varCurrency" select="normalize-space(substring(COL1,708,3))"/>
						<Symbol>
							<xsl:choose>
