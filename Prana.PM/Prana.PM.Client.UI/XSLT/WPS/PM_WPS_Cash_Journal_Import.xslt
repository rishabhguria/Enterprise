      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($varCash)">
          ...
          <PositionMaster>
            ...
            <JournalEntries>
              <xsl:choose>
                <xsl:when test="$varTransactionType = 'DEP'">
                  <xsl:value-of select="concat('Cash',':', $varAmount , '|','CASH_DEP', ':' , $varAmount)"/>
                </xsl:when>
                <xsl:when test="$varCash &gt; 0">
                  <xsl:value-of select="concat('Cash',':', $varAmount , '|','MISC_EXP', ':' , $varAmount)"/>
                </xsl:when>
                <xsl:when test="$varCash &lt; 0">
                  <xsl:value-of select="concat('MISC_INC',':', $varAmount , '|','Cash', ':' , $varAmount)"/>
                </xsl:when>
              </xsl:choose>
            </JournalEntries>
