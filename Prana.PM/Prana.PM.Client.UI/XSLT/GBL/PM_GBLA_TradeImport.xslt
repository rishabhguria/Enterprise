	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($NetPosition) and COL84!='MISC.'">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="contains(COL84,'OPTION')">
									<xsl:value-of select="'EquityOption'"/>
								</xsl:when>
								
								
								
								<xsl:when test="contains(COL84,'FUTFOP')">
									<xsl:value-of select="'FutureOption'"/>
								</xsl:when>
								<xsl:when test="contains(COL84,'FUTURE')">
							...
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:call-template name ="Option">
										<xsl:with-param name="Symbol" select="COL72"/>
						...
						<xsl:variable name="Futures">
							<xsl:choose>
								<xsl:when test="$Asset='Future'">
									<xsl:choose>
										<xsl:when test="contains(string-length(COL27),'10')">
											<xsl:value-of select="concat(substring(COL27,1,4),' ',substring(COL27,9,2))"/>
										</xsl:when>
										<xsl:when test="contains(string-length(COL27),'9')">
											<xsl:value-of select="concat(substring(COL27,2,2),' ',substring(COL27,8,2))"/>
										</xsl:when>
										<!--<xsl:when test="contains(string-length(COL27),'8')">
											<xsl:value-of select="concat(substring(COL27,2,2),' ',substring(COL27,7,2))"/>
										</xsl:when>-->
										<xsl:when test="contains(string-length(COL27),'6')">
											<xsl:value-of select="concat(substring(COL27,1,4),' ',substring(COL27,5,2))"/>
										</xsl:when>
										<xsl:when test="contains(string-length(COL27),'5')">
											<xsl:value-of select="concat(substring(COL27,1,3),' ',substring(COL27,4,2))"/>
										</xsl:when>
										<xsl:when test="contains(string-length(COL27),'4')">
											<xsl:value-of select="concat(substring(COL27,1,2),' ',substring(COL27,3,2))"/>
										</xsl:when>
										<xsl:when test="contains(string-length(COL27),'3')">
											<xsl:value-of select="concat(substring(COL27,1,1),' ',substring(COL27,2,2))"/>
										</xsl:when>
										<!--<xsl:otherwise>
						...
						<Symbol>
							<xsl:choose>
								...
								<!--<xsl:when test="$Asset='Future'">
									<xsl:value-of select="concat(substring(COL27,1,2),' ',substring(COL27,4,2))"/>
								</xsl:when>-->
								<xsl:when test="$Asset='Future'">
								...
								<xsl:when test="$Asset='FutureOption'">
									<xsl:value-of select="concat(substring(substring-before(COL26,' '),1,2),' ',substring(substring-before(COL26,' '),3,2),substring(substring-before(COL26,' '),5,1),substring-before(substring-after(normalize-space(COL26),' '),' '))"/>
								</xsl:when>
						...
						<IDCOOptionSymbol>
							<xsl:choose>
								...
								<xsl:when test="string-length(COL31) &gt; 20">
