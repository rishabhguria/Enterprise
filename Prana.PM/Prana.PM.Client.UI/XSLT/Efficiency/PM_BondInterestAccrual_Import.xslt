      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:variable name="varCash">
          <xsl:call-template name="Translate">
            <xsl:with-param name="Number" select="COL83"/>
          </xsl:call-template>
        </xsl:variable>
           ...
           <Symbol>
            ...
            <xsl:variable name="DR_ACRONYM_NAME">
              <xsl:choose>
                ...
                <xsl:when test="$varCash &lt; 0">
                  <xsl:value-of select="'Bond_Interest'"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="''"/>
                </xsl:otherwise>
            ...
            <JournalEntries>
              <xsl:choose>
                <xsl:when test="number($Dramount) or number($Cramount)">
            ...
            <Description>
              <xsl:value-of select="'Interest Adjustment'"/>
            </Description>
