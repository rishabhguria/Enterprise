			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name="Quantity">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="COL122"/>
					</xsl:call-template>
				</xsl:variable>
				
		  <xsl:choose>
		  <xsl:when test="number($Quantity) and (contains(COL113,'Purchase') or contains(COL113,'Sale')) and not(contains(COL113,'Dividend')) ">
			...
			<PositionMaster>
				...
				<xsl:variable name="AvgPrice">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="COL121"/>
					</xsl:call-template>
				</xsl:variable>
				...
				<Side>
					<xsl:choose>
						<xsl:when test="contains(COL113,'Purchase')">
							<xsl:value-of select="'Buy'"/>
						</xsl:when>


						<xsl:when test="contains(COL113,'Sale')">
