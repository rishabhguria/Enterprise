...
using Prana.BusinessObjects.AppConstants; // for Thread class
using System;
...
namespace Cryptography.GnuPG
{
    ...
    public class GnuPGException : Exception
    {
        ...
        protected GnuPGException(SerializationInfo info, StreamingContext context) : base(info, context)
        {
        }
    }
    ...
    public class GnuPGWrapper
    {
        ...
        public string passphrase
        {
            ...
            {
                _passphrase = value;
                if (_passphrase != "" && _passphrase != null)
                {
        ...
        //TDOD: Decrypting should be done using this class.
        //You can take help from following links
        //http://www.codeproject.com/Articles/3118/Gnu-Privacy-Guard-GPG-PGP-for-NET-v?msg=2279071#xx2279071xx

        //Following method is referred from http://www.codeproject.com/Articles/22055/Automated-File-Decryption-Using-GnuPG-and-C
        public bool DecryptFile(ref string encryptedFilePath, string decryptedFilePath, string extensionToAdd)
        {
            #region Enritch FileName before decrypting
            if (!string.IsNullOrEmpty(extensionToAdd) && File.Exists(encryptedFilePath))
            {
                //delete encrypted file if exists
                if (File.Exists(encryptedFilePath + extensionToAdd))
                {
                    File.Delete(encryptedFilePath + extensionToAdd);
                }
                //Enrich fileName (Need to add .pgp extension)
                File.Move(encryptedFilePath, encryptedFilePath + extensionToAdd);
                encryptedFilePath = encryptedFilePath + extensionToAdd;
            }
            #endregion
            ...
            psi.WorkingDirectory = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + @"\MiscUtilities\SupportFiles";
            _processObject = System.Diagnostics.Process.Start(psi);
            // actually NEED to set this as a local string variable
            // and pass it - bombs otherwise!

            string sCommandLine = "echo " + _passphrase + "| gpg.exe --passphrase-fd 0 -o \"" + decryptedFilePath + "\" --decrypt \"" + encryptedFilePath + "\"";
