				...
				<xsl:if test="number($Quantity)">
					<PositionMaster>
						...
						<xsl:variable name="GrossNotionalValue">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring(COL1,83,16)"/>
							</xsl:call-template>
						</xsl:variable>
						<GrossNotionalValue>
							<xsl:choose>
						...
						<xsl:variable name="NetNaotionalValue">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="format-number(substring(COL1,205,16),'##.##')"/>
							</xsl:call-template>
						</xsl:variable>
						<NetNaotionalValue>
							<xsl:choose>
						...
						<xsl:variable name="NetNaotionalValueBase">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="format-number(substring(COL1,288,15),'##.##')"/>
							</xsl:call-template>
						</xsl:variable>
						<NetNaotionalValueBase>
							<xsl:choose>
						...
						<xsl:variable name="varAccrued">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring(COL1,129,15)"/>
							</xsl:call-template>
						</xsl:variable>
						<Accrued>
							<xsl:choose>
								<xsl:when test="$varAccrued &gt; 0">
									<xsl:value-of select="$varAccrued"/>
								</xsl:when>
								<xsl:when test="$varAccrued &lt; 0">
									<xsl:value-of select="$varAccrued * (-1)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
