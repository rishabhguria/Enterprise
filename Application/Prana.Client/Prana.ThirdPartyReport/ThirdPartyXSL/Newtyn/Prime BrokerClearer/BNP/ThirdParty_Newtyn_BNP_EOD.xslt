...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<!--<xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail[(contains(FundName, 'Swap') = false and TaxLotState = 'Allocated') and (FundAccntNo!='49100040' and FundAccntNo!='49100041')]">-->
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name ="varAccountNo">
			<xsl:choose>
				<xsl:when test ="FundName = 'BNP Newtyn Partners LP:491-00040'">
					<xsl:value-of select ="'49100040'"/>
				</xsl:when>
				<xsl:when test ="FundName = 'BNP Newtyn TE Partners LP:491-00041'">
		...
		<xsl:variable name="varSecurityID">
			<xsl:choose>
				...
				<xsl:when test="SEDOLSymbol != ''">
					<xsl:value-of select="SEDOLSymbol"/>
				</xsl:when>				
				<xsl:when test="CUSIPSymbol != ''">
		...
		<xsl:variable name="PRANA_SYMBOL_TYPE">
			<xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='NewtynEODSymbols']/SymbolData[@PBCompanyName=$PB_SYMBOL_NAME]/@SymbolType"/>
		</xsl:variable>
          ...
          <xsl:choose>
            <xsl:when test="CounterParty='TOUR'">
              <xsl:value-of select="'TOPA'"/>
				...
				<xsl:when test="CounterParty='GSCC'">
		...
		<Group 
			...
			price="{$varAvgPrice}" Account="{FundAccntNo}" executing_broker="{$Broker}" trade_date = "{TradeDate}" security_id_type="{$varSecurityIdType}"
