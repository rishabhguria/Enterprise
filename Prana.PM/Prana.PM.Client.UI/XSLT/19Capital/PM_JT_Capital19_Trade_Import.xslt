				...
				<xsl:if test="number($NetPosition) ">
					...
					<PositionMaster>
						...
						<xsl:variable name="SecFee">
							<xsl:choose>
								<xsl:when test="COL5='SellShort' or COL5='Sell'">
									<xsl:choose>
										<xsl:when test="$Asset='Equity'">
											<xsl:value-of select="($NetPosition * $CostBasis) * 0.0000218 "/>
										</xsl:when>
										<xsl:when test="$Asset='EquityOption'">
											<xsl:value-of select="($NetPosition * $CostBasis * 100) * 0.0000218"/>
										</xsl:when>
									</xsl:choose>
								</xsl:when>
						...
						<xsl:variable name="OrfFeeOption">
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:value-of select="$NetPosition * 0.0407"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="''"/>
								</xsl:otherwise>
						...
						<OrfFee>
							<xsl:choose>
								<xsl:when test="$OrfFeeOption &gt; 0">
									<xsl:value-of select="$OrfFeeOption"/>
								</xsl:when>
								<xsl:when test="$OrfFeeOption &lt; 0">
									<xsl:value-of select="$OrfFeeOption * (-1)"/>
								</xsl:when>
