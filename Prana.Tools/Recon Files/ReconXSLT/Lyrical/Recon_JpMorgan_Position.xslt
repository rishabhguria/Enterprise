...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select="//Comparision[substring(COL1,40,1) = 'O' or substring(COL1,40,1) = 'C' or substring(COL1,40,1) = 'A' or substring(COL1,40,1) = 'G']">
        ...
        <C>
          <xsl:value-of select="substring(COL1,73,2)"/>
				...
				<xsl:if test="$varFormatPrice >= 0 and $varFormatQty > 0 and (substring(COL1,4,2) ='42') and $PRANA_FUND_NAME!=''">
                  ...
                  <xsl:value-of select="string-length($varSymbol)"/>
						...
						<Quantity>
							<xsl:choose>
								...
								<xsl:when test="$varSide='S' and number($varFormatQty)">
									<xsl:value-of select="format-number(-1*$varFormatQty,'####.00000')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
						...
						<MarketValue>
							<xsl:choose>
								<xsl:when test ="number($varFormatPrice) and number($varFormatQty) and ($varSide='S' and $varSide != '')">
									<xsl:value-of select="format-number($varFormatPrice*$varFormatQty, '####.0000000')*-1"/>
								</xsl:when>
								<xsl:when test ="number($varFormatPrice) and number($varFormatQty) ">
									<xsl:value-of select="format-number($varFormatPrice*$varFormatQty, '####.0000000')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
	...
	<xsl:template match="/">
        ...
        <C>
          <xsl:value-of select="substring(COL1,73,2)"/>
				...
				<xsl:if test="$varFormatPrice >= 0 and $varFormatQty > 0 and (substring(COL1,4,2) ='42') ">
					<PositionMaster>
