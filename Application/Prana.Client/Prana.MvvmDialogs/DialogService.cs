using Prana.MvvmDialogs.DialogTypeLocators;
using Prana.MvvmDialogs.FrameworkDialogs;
using Prana.MvvmDialogs.FrameworkDialogs.FolderBrowser;
using Prana.MvvmDialogs.FrameworkDialogs.OpenFile;
using Prana.MvvmDialogs.FrameworkDialogs.SaveFile;
using Prana.MvvmDialogs.Properties;
using Prana.MvvmDialogs.Reflection;
...
namespace Prana.MvvmDialogs
{
    ...
    public class DialogService : IDialogService
    {
        ...
        private static DialogService _dialogServiceInstance;
        private static readonly object _padlock = new object();
        public static DialogService DialogServiceInstance
        {
            get
            {
                lock (_padlock)
                {
                    if (_dialogServiceInstance == null)
                    {
                        lock (_padlock)
                        {
                            _dialogServiceInstance = new DialogService();
                        }
                    }
                }
                return _dialogServiceInstance;
            }
        }
        /// <summary>
        ...
        /// </param>
        /// <param name="isWindowsHosted">
        /// A <see cref="MessageBoxResult"/> value that specifies the default value stating whete it is wpf hosted or windows hosted
        ///  <see>
        ///      <cref>false</cref>
        ///  </see>
        /// </param>
        ...
        public MessageBoxResult ShowMessageBox(
            ...
            MessageBoxResult defaultResult = MessageBoxResult.None, bool isWindowsHosted = false)
        {
            if (ownerViewModel == null)
                throw new ArgumentNullException("ownerViewModel");
            if (isWindowsHosted)
            ...
            return MessageBox.Show(
                (Window)FindOwnerWindow(ownerViewModel),
                messageBoxText,
                caption,
                button,
                icon,
            ...
            OpenFileDialogSettings settings)
        {
            ...
            return dialog.ShowDialog((Window)FindOwnerWindow(ownerViewModel));
        }
            ...
            FolderBrowserDialogSettings settings)
        {
            ...
            {
                DialogResult result = dialog.ShowDialog(new WindowWrapper((Window)FindOwnerWindow(ownerViewModel)));
            ...
            INotifyPropertyChanged viewModel)
        {
            ...
            if (FindOwnerWindow(ownerViewModel) is Form)
            {
                WindowInteropHelper helper = new WindowInteropHelper(dialog)
                {
                    Owner = ((Form)FindOwnerWindow(ownerViewModel)).Handle
                };
            }
            else
            {
                dialog.Owner = (Window)FindOwnerWindow(ownerViewModel);
            }
        ...
        /// </summary>
        private static object FindOwnerWindow(INotifyPropertyChanged viewModel)
        {
            ...
            if (view.Source is IViewWinForms)
            {
                ...
                {
                    owner = ((IViewWinForms)view.Source).ParentWinformControl;
                }
                else
                {
                    owner = view.GetOwner();
