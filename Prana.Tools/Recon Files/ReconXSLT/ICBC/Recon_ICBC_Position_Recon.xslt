...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="Option">
    ...
    <xsl:variable name="StrikePrice">
      <xsl:choose>
        <xsl:when test ="format-number(substring(substring-after(normalize-space($Symbol),' '),8,8) div 1000,'##.00') &gt; 1">
          <xsl:value-of select ="format-number(substring(substring-after(normalize-space($Symbol),' '),8,8) div 1000,'##.00')"/>
        </xsl:when>
        <xsl:when test ="format-number(substring(substring-after(normalize-space($Symbol),' '),8,8) div 1000,'##.00') = 1">
          <xsl:value-of select ="format-number(substring(substring-after(normalize-space($Symbol),' '),8,8) div 1000,'##.00')"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="concat('0',format-number(substring(substring-after(normalize-space($Symbol),' '),8,8) div 1000,'##.00'))"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name ="PB_FUND_NAME">
					<xsl:value-of select ="concat(COL2,COL4,COL5)"/>
				</xsl:variable>
                    ...
                    <xsl:otherwise>
                      <xsl:value-of select ="normalize-space(COL9)"/>
								...
								<xsl:when test="contains(COL9,'.')">
									<xsl:choose>
										<xsl:when test="string-length(substring-after(COL9,'.'))=3">
											<xsl:value-of select="concat(substring-before(COL9,'.'),'/',substring(substring-after(COL9,'.'),1,1),substring(substring-after(COL9,'.'),3,1))"/>
										</xsl:when>
										<xsl:when test="string-length(substring-after(COL9,'.'))=2">
											<xsl:value-of select="concat(substring-before(COL9,'.'),'/',substring(substring-after(COL9,'.'),1,1))"/>
										</xsl:when>
										<xsl:otherwise>
              ...
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:value-of select ="COL9"/>
						...
						<Side>
							<xsl:choose>
								<xsl:when test="COL11='O' and $varQuantity &gt; 0">
                ...
                <xsl:with-param name="Number" select="COL16"/>
						...
						<AvgPX>
							<xsl:choose>
								...
								<xsl:when test="COL11='O'">

									<xsl:choose>
										<xsl:when test ="$varAvgPX &gt;0">
											<xsl:value-of select ="$varAvgPX div 100"/>
										</xsl:when>

										<xsl:otherwise>
											<xsl:value-of select ="($varAvgPX*-1) div 100"/>
										</xsl:otherwise>
									</xsl:choose>
