...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
  ...
  <xsl:template name="TaxLotIDBuilder">
    ...
    <xsl:variable name="varClientAccount">
	  ...
	  <xsl:variable name ="Custodian11">
		  <xsl:choose>
			  <xsl:when test ="contains(AccountName,'- 002')">
    ...
    <xsl:variable name="varFeeValue2">
 	...
 	<xsl:variable name ="varAllocationState">
	  ...
	  <xsl:variable name="VarComm">
		  <xsl:choose>
			  ...
			  <xsl:when test="SettlCurrFxRate!=0 and SettlCurrFxRateCalc='M'">
				  <xsl:value-of select="format-number($varCommission * SettlCurrFxRate,'##.00')"/>
			  </xsl:when>

			  <xsl:when test="SettlCurrFxRate!=0 and SettlCurrFxRateCalc='D'">
				  <xsl:value-of select="format-number($varCommission div SettlCurrFxRate,'##.00')"/>
			  </xsl:when>
			  <xsl:otherwise>
				  <xsl:value-of select="''"/>
			  </xsl:otherwise>
	  ...
	  <xsl:variable name="Account123">
		  <xsl:choose>
			  <xsl:when test ="AccountNo= '033512260'">
				  <xsl:value-of select ="'002264166'"/>
			  </xsl:when>
			  <xsl:otherwise>
	  ...
	  <Group 
			  OrderNumber ="{EntityID}" Cancelcorrectindicator ="{$varAllocationState}" AccountNumber="{$Account123}" SecurityIdentifier="{$varSymbol}"
			  Broker="{$Broker}" Custodian="{$Custodian11}" TransactionType="{$varTransactionType}" CurrencyCode="{SettlCurrency}"
