    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:variable name="varQuantityValue">
          <xsl:choose>
            <xsl:when test="contains(COL3,'NAV')">
              <xsl:value-of select="substring-before(COL3,'NAV')"/>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:if test="number($varQuantity)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PB_SYMBOL_NAME">
              <xsl:value-of select="substring-before(normalize-space(COL6),'Registrar')"/>
            </xsl:variable>

            <xsl:variable name="varPB_SYMBOL_NAME">
              <xsl:call-template name="ReplaceSpecialCharater">
                <xsl:with-param name="String" select="$PB_SYMBOL_NAME"/>
              </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:variable name="Symbol">
              <xsl:value-of select="substring-before(normalize-space(COL1),'PAN')"/>
            </xsl:variable>
            ...
            <xsl:variable name="varMarkPrice">
              <xsl:choose>
                <xsl:when test="contains(COL4,'V')">
                  <xsl:value-of select="substring-before(COL4,'V')"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="0"/>
                </xsl:otherwise>
