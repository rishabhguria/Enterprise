...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[Symbol !='BYMA AR' and (TaxLotState != 'Sent' and TaxLotState = 'Allocated') and (UDASecurityTypeName!='cfeu' and UDASecurityTypeName!='cfus')]">
                ...
                <xsl:choose>
                  <xsl:when test="CounterParty='TOUR'">
					...
					<xsl:variable name="NetAmountFX">
						<xsl:choose>
							<xsl:when test="FXConversionMethodOperator_Trade='M'">
								<xsl:value-of select="$NetAmount_Swap * $SettleFx"/>
							</xsl:when>

							<xsl:when test="FXConversionMethodOperator_Trade='D'">
								<xsl:value-of select="$NetAmount_Swap div $SettleFx"/>
							</xsl:when>
					...
					<NetAmount>
						<xsl:value-of select="$NetAmount_Swap"/>
					</NetAmount>
