...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring-before(COL28,' '),'Put') or contains(substring-before(COL28,' '),'Call')">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL4),' '),1,5),'#.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="contains(substring-before(COL28,' '),'Put') or contains(substring-before(COL28,' '),'Call')">
							...
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:choose>
										...
										<xsl:when test="$Side='BO' or $Side='BC'">
											<xsl:value-of select="'B'"/>
										</xsl:when>
										
										<xsl:when test="$Side='SC' or $Side='SO'">
											<xsl:value-of select="'C'"/>
										</xsl:when>
