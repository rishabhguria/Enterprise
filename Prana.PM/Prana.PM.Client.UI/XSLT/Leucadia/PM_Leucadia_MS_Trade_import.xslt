				...
				<xsl:if test="number(COL34) and COL26='Trade' ">
					<PositionMaster>
						...
						<xsl:variable name="PRANA_Exchange">
							<xsl:value-of select="document('../../../MappingFiles/ReconMappingXml/SymbolSuffixMapping.xml')/SymbolMapping/PB[@Name='MS']/SymbolData[@PBSuffixCode=$PB_Suffix]/@PranaSuffixCode"/>
						</xsl:variable>
						...
						<xsl:variable name="Month1">
							<xsl:call-template name="MonthName">
								<xsl:with-param name="Month" select="substring-before(COL20,' ')"/>
							</xsl:call-template>
						</xsl:variable>

						<!--<PositionSettlementDate>
							<xsl:value-of select="concat($Month1,'/',substring-before(substring-after(COL20,' '),','),'/',substring-after(substring-after(COL20,' '),' '))"/>
						</PositionSettlementDate>-->
