			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($NetPosition)">
					<PositionMaster>
						...
						<xsl:variable name="PB_FUND_NAME" select="concat(COL6,COL11)"/>
						...
						<xsl:variable name="Comm">
							<xsl:choose>
								<xsl:when test="COL9='MSCOS'">
									<xsl:value-of select="COL4 * 0.005"/>
								</xsl:when>
								<xsl:when test="COL9='CCMB'">
									<xsl:value-of select="COL4 * 0.004"/>
								</xsl:when>
