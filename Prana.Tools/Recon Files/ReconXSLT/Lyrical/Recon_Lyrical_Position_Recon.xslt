	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and normalize-space(COL1) != 'CASH AND CASH EQUIVALENTS'">
					...
					<PositionMaster>
						...
						<xsl:variable name="PB_FUND_NAME" select="'Macomb County Health Care: 11063343'"/>
						...
						<xsl:variable name="varAvgPX">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="normalize-space(COL7) div normalize-space(COL5)"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="MarkPriceBase">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$MarketValueBase div $Quantity"/>
							</xsl:call-template>
						</xsl:variable>
