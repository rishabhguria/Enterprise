...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL37,'OPTION')">
			...
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring-after(substring-after($Symbol,'/'),'/')"/>
			</xsl:variable>
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after($Symbol,$UnderlyingSymbol),' ' ),1,2),'#.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($NetPosition)">
					...
					<PositionMaster>
						...
						<xsl:variable name="PB_ROOT_NAME">
							<xsl:value-of select="normalize-space(substring(COL33,1,2))"/>
						</xsl:variable>
						...
						<xsl:variable name="Future" select="concat($Underlying,' ',$MonthYearCode)"/>

						<xsl:variable name="Strike">
							<xsl:choose>
								<xsl:when test="contains(COL42,'.')">
						...
						<xsl:variable name="FutureOp" select="concat($Future,COL41,$Strike)"/>

						<xsl:variable name="Symbol" select="normalize-space(COL28)"/>
							...
							<xsl:choose>
								...
								<xsl:when test="COL37='OPTION'">
									<xsl:call-template name="Option">
										<xsl:with-param name="Symbol" select="COL28"/>
									</xsl:call-template>
								</xsl:when>
