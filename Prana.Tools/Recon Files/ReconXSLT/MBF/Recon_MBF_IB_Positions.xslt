	...
	<xsl:template match="/DocumentElement">
		<DocumentElement>
			<xsl:for-each select="Comparision">
				...
				<xsl:if test ="COL1 ='DATA' and COL18='SUMMARY'">
					...
					<PositionMaster>
						...
						<xsl:choose>
							<xsl:when test ="number(translate(COL8,',','')) &lt; 0">
								<Side>
									<xsl:value-of select="'Sell'"/>
								</Side>
								<Quantity>
									<xsl:value-of select="number(translate(COL8,',',''))*(-1)"/>
								</Quantity>
							</xsl:when>
							<xsl:otherwise>
								<Side>
								...
								<Quantity>
									<xsl:value-of select="number(translate(COL8,',',''))"/>
								</Quantity>
							</xsl:otherwise>
						</xsl:choose>
