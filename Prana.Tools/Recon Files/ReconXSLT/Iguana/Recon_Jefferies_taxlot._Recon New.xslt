	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Position) and contains(COL9,'Cash')!='true'">
					...
					<PositionMaster>
						...
						<xsl:variable name="AssetType" select="normalize-space(COL9)"/>
						...
						<Side>
							...
							<xsl:choose>
								...
								<xsl:when test="$Side='Long' and $AssetType='Equity'">
									<xsl:value-of select="'Buy'"/>
								</xsl:when>

								<xsl:when test="$Side='Short' and $AssetType='Equity'">
									<xsl:value-of select="'Sell'"/>
								</xsl:when>

								<xsl:when test="$Side='Long' and $AssetType='Options'">
									<xsl:value-of select="'Buy'"/>
								</xsl:when>

								<xsl:when test="$Side='Short' and $AssetType='Options'">
									<xsl:value-of select="'Sell'"/>
								</xsl:when>
						...
						<!--<xsl:variable name="TradeDay" select="substring-before(substring-after(COL8,'/'),'/')"/>
						<xsl:variable name="TradeMonth" select="substring-before(COL8,'/')"/>
						<xsl:variable name="TradeYear" select="substring(substring-before(substring-after(substring-after(COL8,'/'),'/'),' '),3,2)"/>-->

						<TradeDate>
							<!--<xsl:value-of select="concat($TradeMonth,'/',$TradeDay,'/',$TradeYear)"/>-->
						...
						<UnitCost>
							...
							<xsl:choose>
								...
								<xsl:when test="$AvgPX &gt; 0">
									<xsl:value-of select="format-number($AvgPX,'#.####')"/>
								</xsl:when>

								<xsl:when test="$AvgPX &lt; 0">
									<xsl:value-of select="format-number($AvgPX * (-1),'#.####')"/>
								</xsl:when>
						...
						<xsl:variable name="CompanyName" select="COL7"/>
