				...
				<xsl:choose>
					<xsl:when test ="TaxLotStateID = 2 and TranCode != ';' ">
						...
						<ThirdPartyFlatFileDetail>
							...
							<Broker>
								<xsl:choose>
									<xsl:when test ="Broker = 'btigalgo'">
										<xsl:value-of select ="'btig'"/>
									</xsl:when>

									<xsl:when test ="Broker = 'hasf'">
										<xsl:value-of select ="'hayu'"/>
									</xsl:when>
									<xsl:otherwise>
					...
					<xsl:when test ="TaxLotStateID=3">
						<ThirdPartyFlatFileDetail>
							...
							<SecuritySymbol>
								<xsl:choose>
									<xsl:when test="SecuritySymbol='almed-eeb'">
										<xsl:value-of select="'almed-pf'"/>
									</xsl:when>
									<xsl:otherwise>
								<xsl:value-of select="SecuritySymbol"/>
									</xsl:otherwise>
