...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail[AccountName!='Rebal Risk Analysis']">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<Group 
				...
				Quantity="{AllocatedQty}" Commission="{$varCommission}" Price="{$varAvgPrice}" AccruedInterest="{AccruedInterest}" 
				TradeTax="{$Stamp}" MiscMoney="{MiscFees}" NetAmount="{$varNetAmount}" Principal="{GrossAmount}" Description="{CompanyName}"
