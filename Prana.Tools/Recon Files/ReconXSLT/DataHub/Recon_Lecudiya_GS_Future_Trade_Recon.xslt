				...
				<xsl:if test ="number($varQuantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="PRANA_UNDERLYING_NAME">
							...
							<xsl:value-of select="document('../ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name=$PB_NAME]/SymbolData[@PBCode=$BB_Code]/@UnderlyingCode"/>
						</xsl:variable>
						...
						<xsl:variable name="PRANA_EXCHANGE_NAME">
							...
							<xsl:value-of select="document('../ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name=$PB_NAME]/SymbolData[@PBCode=$BB_Code]/@ExchangeCode"/>
						</xsl:variable>
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="COL17 !='*' ">
									<xsl:value-of select="normalize-space(concat(COL20,' ',$Month_Code,$Year,$Suffix))"/>
								</xsl:when>-->
