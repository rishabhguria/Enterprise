...
Create PROCEDURE [dbo].[P_GetClosingTalxlot_ThirdParty] (  
 @thirdPartyID INT  
...
SELECT VT.TaxLotID AS TaxLotID  
 ...
 ,ISNULL(ClearingBrokerFee, 0) AS ClearingBrokerFee  
 ,ISNULL(SecFee, 0) AS SecFee  
 ,ISNULL(OccFee, 0) AS OccFee  
 ,ISNULL(OrfFee, 0) AS OrfFee  
 ,VT.AUECLocalDate  
...
SELECT TDT.TaxLotID AS TaxLotID  
 ...
 ,ISNULL(TDT.ClearingBrokerFee, 0) AS ClearingBrokerFee  
...
SELECT VT.TaxLotID  
 ...
 ,VT.LotID  
 ,CASE   
  WHEN Closing.PositionalTaxlotId IS NULL  
   THEN VT.TaxLotID  
  ELSE Closing.PositionalTaxlotId  
  END AS UserTranId  
 ,VT.IsSwapped AS IsSwapped  
