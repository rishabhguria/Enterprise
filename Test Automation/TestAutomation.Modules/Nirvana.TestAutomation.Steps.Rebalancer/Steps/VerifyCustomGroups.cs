...
{
    class VerifyCustomGroups : IUIAutomationTestStep
    {
        public TestResult RunUIAutomationTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("VerifyCustomGroups failed as DataSet is empty.");
                }
                ...
                errors = Recon.RunRecon(ds.Tables["CustomGroups"], testData.Tables[0], columns, 0.01);
                    if (errors.Count > 0)
