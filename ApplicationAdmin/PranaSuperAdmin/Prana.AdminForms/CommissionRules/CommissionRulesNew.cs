...
namespace Prana.AdminForms
{
    public partial class CommissionRulesNew : Form
    {
        ...
        Prana.BusinessObjects.CommissionRule _previousNode = null;
        ...
        {
            Prana.BusinessObjects.CommissionRule newCommissionRule = ctrlCommissionRuleobj.AddNewCommissionRuleOnUI(null);
        ...
        private bool IsCommissionRuleNameExists(Prana.BusinessObjects.CommissionRule commissionRule)
        {
            List<Prana.BusinessObjects.CommissionRule> commissionRuleCollections = CommissionRulesCacheManager.GetInstance().GetAllCommissionRules();

            foreach (Prana.BusinessObjects.CommissionRule commRule in commissionRuleCollections)
            {
                if (commRule.RuleName == commissionRule.RuleName && commRule.RuleID != commissionRule.RuleID)
                {
                    MessageBox.Show("Commission Rule with the same Name already exists.", "Prana Admin ", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return false;
                }
            }
            return true;
        ...
        private void RebindTreeNodes(Prana.BusinessObjects.CommissionRule addedCommissionRule)
        {
            List<Prana.BusinessObjects.CommissionRule> AllCommissionRules = CommissionRulesCacheManager.GetInstance().GetAllCommissionRules();
            ...
            treeNodeParent.Tag = new Prana.BusinessObjects.CommissionRule();
            Prana.BusinessObjects.CommissionRule commissionRuleNew = new Prana.BusinessObjects.CommissionRule();

            foreach (Prana.BusinessObjects.CommissionRule objcommissionRule in AllCommissionRules)
            {
        ...
        // List<Prana.BusinessObjects.CommissionRule> currentRules = CommissionRulesCacheManager.GetInstance().GetAllCommissionRules();
        private void RefreshCommissionRule()
        {
            ctrlCommissionRuleobj.InitControls();
        }
        ...
        {
            bool isUnSavedChanges = ctrlCommissionRuleobj.IsUnSavedChanges;
            Prana.BusinessObjects.CommissionRule lastSelectedNode = (Prana.BusinessObjects.CommissionRule)trvCommissionRule.SelectedNode.Tag;
            ...
            Prana.BusinessObjects.CommissionRule checkedCommissionRule = ctrlCommissionRuleobj.AddNewCommissionRuleOnUI(lastSelectedNode);
            // get Modified Rules if ...
            List<Prana.BusinessObjects.CommissionRule> AllModifiedRules = CommissionRulesCacheManager.GetInstance().GetAllModifiedCommissionRules();
            ...
            {
                int result = 0;
                _blnWantToSave = false;
                result = CommissionDBManager.SaveAndUpdateCommissionRule(AllModifiedRules);

                if (result > 0)
                {
                    // MessageBox.Show("Commission Rule(s) saved.", "Prana Admin");
                    return false;
                }
            }
            ...
            {
                DialogResult result = MessageBox.Show("Do you want to save the changes ?", "Prana Admin", MessageBoxButtons.YesNoCancel);
            ...
            else if (checkedCommissionRule == null && prevRuleName.Equals(""))
            {
                if (AllModifiedRules.Count > 0)
                {
                    if (MessageBox.Show("Do you want to save the changes ?", "Prana Admin", MessageBoxButtons.YesNoCancel) == DialogResult.Yes)
                    {
                        int result = 0;
                        result = CommissionDBManager.SaveAndUpdateCommissionRule(AllModifiedRules);
            ...
            else if (checkedCommissionRule != null)
            {
                if (AllModifiedRules.Count > 0 && isUnSavedChanges)
                {
                    DialogResult dialogResult = MessageBox.Show("Do you want to save the changes ?", "Prana Admin", MessageBoxButtons.YesNoCancel);
            ...
            //Bharat Kumar Jangir (21 April 2014)
            //For preventing default save commission rules
            //else
            //{
            //    int result = 0;
            //    result = CommissionDBManager.SaveAndUpdateCommissionRule(AllModifiedRules);
            ...
            try
            {
                if (trvCommissionRule.SelectedNode != null)
                {
                    ...
                    else
                    {
                        ...
                        Prana.BusinessObjects.CommissionRule commissionRule = (Prana.BusinessObjects.CommissionRule)trvCommissionRule.SelectedNode.Tag;
                        // retain the value of current Rule Name
                        string currentRuleName = commissionRule.RuleName;
                        // check the validation on the Control except the Rule Name
                        Prana.BusinessObjects.CommissionRule addedCommissionRule = ctrlCommissionRuleobj.AddNewCommissionRuleOnUI(commissionRule);
        ...
        private void btnDeleteCommissionRule_Click(object sender, EventArgs e)
        {
            ...
            if (trvCommissionRule.SelectedNode != null)
            {
                if (trvCommissionRule.SelectedNode == trvCommissionRule.Nodes[0])
                {
                    MessageBox.Show("Please select a Valid Rule to delete", "Prana Admin", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                else if (MessageBox.Show(this, "Do you want to delete the selected Rule ?", "Prana Admin", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    Prana.BusinessObjects.CommissionRule selectedCommissionRule = (Prana.BusinessObjects.CommissionRule)trvCommissionRule.SelectedNode.Tag;
                    ...
                    if (result == 1)
                    {
                        ...
                        //MessageBox.Show("Selected commission rule deleted.", "Prana Alert");
                    }
                    else if (result == -1)
                    {
                        MessageBox.Show("Selected commission rule can not be deleted, it is referenced.", "Prana Admin");
                        return;
                    }
        ...
        private void btnSaveCommissionRule_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (AllModifiedRules.Count > 0)
                {
                    ctrlCommissionRuleobj.IsUnSavedChanges = false;
                    int result = 0;
                    result = CommissionDBManager.SaveAndUpdateCommissionRule(AllModifiedRules);
        ...
        private void trvCommissionRule_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
        {
            try
            {
                if (trvCommissionRule.SelectedNode != null)
                {
                    ...
                    if (trvCommissionRule.SelectedNode != trvCommissionRule.Nodes[0])
                    {
                        Prana.BusinessObjects.CommissionRule currentNodeValue = (Prana.BusinessObjects.CommissionRule)trvCommissionRule.SelectedNode.Tag;

                        _previousNode = currentNodeValue;

                        //ctrlCommissionRuleobj.CommissionRuleProperties = currentNodeValue;
                        ctrlCommissionRuleobj.SetCommissionRuleDetails(currentNodeValue);
            ...
            if (trvCommissionRule.SelectedNode != trvCommissionRule.Nodes[0] && trvCommissionRule.SelectedNode != null)
            {
                try
                {
                    if (_previousNode != null)
                    {
                        ...
                        Prana.BusinessObjects.CommissionRule updatedCommissionRule = ctrlCommissionRuleobj.AddNewCommissionRuleOnUI(_previousNode);
                        ...
                        else
                        {
                            ...
                            {
                                if (!preRuleName.Equals(updatedCommissionRule.RuleName))
                                {
                                    trvCommissionRule.SelectedNode.Text = updatedCommissionRule.RuleName;
                                }
                ...
                finally
                {
                    ...
                    Logger.LoggerWrite("trvCommissionRule_BeforeSelect",
                        LoggingConstants.LOG_CATEGORY_UI, 1, 1, System.Diagnostics.TraceEventType.Information,
