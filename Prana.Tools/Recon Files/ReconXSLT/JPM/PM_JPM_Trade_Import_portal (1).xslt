        ...
        <xsl:if test="number($Position) and contains(COL33,'CASH')!='true'">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_SYMBOL_NAME">
			  ...
			  <xsl:variable name = "PB_ExSymbol" >
				  <xsl:value-of select ="substring-before(COL43,' ')"/>
			  </xsl:variable>

			  <xsl:variable name="PRANA_ExSymbol">
				  <xsl:value-of select="document('../ReconMappingXml/ExSymbol.xml')/ExSymbolMapping/PB[@Name=$PB_NAME]/ExSymbolData[@ExSymbolName=$PB_ExSymbol]/@PranaExSymbol"/>
			  </xsl:variable>
              ...
              <xsl:choose>
                ...
                <xsl:when test="$Symbol!='*'">
				  ...
				  <xsl:when test="contains(COL106,'Foreign Currency (F/X)')">
					  <xsl:value-of select="concat($PRANA_ExSymbol,'USD SPOT')"/>
				  </xsl:when>
            ...
            <xsl:variable name="Side" select="normalize-space(COL58)"/>
			  ...
			  <xsl:variable name="TradePrice">
				  <xsl:call-template name="Translate">
					  <xsl:with-param name="Number" select="COL108 div COL107"/>
				  </xsl:call-template>
			  </xsl:variable>
