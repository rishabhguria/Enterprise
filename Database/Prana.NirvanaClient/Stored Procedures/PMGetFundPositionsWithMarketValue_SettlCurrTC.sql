...
*/      
CREATE PROCEDURE [dbo].[PMGetFundPositionsWithMarketValue_SettlCurrTC] (      
...
End AS Asset        
  ...
  WHEN (      
   ...
   OR (#PositionTable.AssetID = 3)    
  )      
  THEN    
  (     
   CASE       
     WHEN     
    (  #PositionTable.SettlCurrency = 'USD')     
     THEN     
      ((#PositionTable.Quantity * dbo.GetSideMultiplier(#PositionTable.SideID) * #PositionTable.Multiplier * DayEndFxRate.Val * IsNull(coalesce(MP.FinalMarkPrice, MP1.FinalMarkPrice), 0)) - (SettlCurrFxRate1.Val * NetNotionalValue.Val))     
    --(EODSettlFxRate.Val * MarketValue.Val) - (SettlCurrFxRate1.Val * NetNotionalValue.Val)     
    WHEN     
    (  #PositionTable.SettlCurrency <> 'USD')    
    Then       
    (MarketValue.Val - #PositionTable.TotalCommissionandFees)    
     END    
  )     
      
  ELSE (MarketValue.Val - NetNotionalValue.Val)    
    
 END  AS SettlementCurrencyMarketValue       
 ...
 CROSS APPLY (     
  SELECT CASE       
    WHEN (      
     #PositionTable.AssetID =3     
      )     
   THEN IsNull((#PositionTable.Quantity * IsNull(#PositionTable.AvgPx, 0) * #PositionTable.Multiplier * dbo.GetSideMultiplier(#PositionTable.SideID)), 0)      
   ELSE IsNull((#PositionTable.Quantity * IsNull(#PositionTable.AvgPx, 0) * #PositionTable.Multiplier * dbo.GetSideMultiplier(#PositionTable.SideID) + #PositionTable.TotalCommissionandFees), 0)      
   END    
   ) AS NetNotionalValue(Val)     
  -- IsNull((#PositionTable.Quantity * IsNull(#PositionTable.AvgPx, 0) * #PositionTable.Multiplier * dbo.GetSideMultiplier(#PositionTable.SideID) + #PositionTable.TotalCommissionandFees), 0)       
 CROSS APPLY (      
  SELECT CASE       
 ...
 CROSS APPLY (      
  ...
  WHEN #PositionTable.SettlCurrency <> CUR.CurrencySymbol      
      THEN CASE     
   WHEN #PositionTable.FXConversionMethodOperator <> 'D'      
    THEN IsNull(#PositionTable.FXRate, 0)      
   ELSE COALESCE(1 / NULLIF(#PositionTable.FXRate, 0), 0)      
   END    
  ELSE 1    
    END      
