...
>
	...
	<xsl:template name="Option">
		<xsl:param name="Symbol"/>
		<xsl:param name="Suffix"/>
		<xsl:if test="contains(substring-after(substring-after(substring-after(COL8,' '),' '),' '),C) or contains(substring-after(substring-after(substring-after(COL11,' '),' '),' '),P) ">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL8,' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(COL8,' '),1,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(COL8,' '),3,4)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-after(COL8,' '),6,7)"/>
			</xsl:variable>

			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring-after(substring-after(substring-after(COL8,' '),' '),' ')"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<!--<xsl:value-of select =" substring-before(substring-after(substring-after(normalize-space(COL8),' '),' '),' ')"/>-->
				<xsl:value-of select="format-number(substring-before(substring-after(substring-after(normalize-space(COL8),' '),' '),' '),'#.0')"/>
			</xsl:variable>
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($MarkPrice)and COL8!='CASH' and COL8!='INTACC' ">



					<PositionMaster>
						<xsl:variable name="PB_NAME">
							<xsl:value-of select="'Whitman'"/>
						</xsl:variable>
						...
						<xsl:variable name="PRANA_SYMBOL_NAME">
							...
							<xsl:variable name ="Asset">
								<xsl:choose>
									...
									<xsl:when test="contains(COL9,'FUT')">
										<xsl:value-of select="'Future'"/>
									</xsl:when>

									<xsl:when test="contains(COL9,'FOP')">
									...
									<xsl:when test="contains(COL9,'STK')">
						...
						<xsl:variable name="Symbol">
							<xsl:choose>
								<xsl:when test="$Asset='Future'">
									<xsl:value-of select="concat(substring(COL5,1,2),' ',substring(COL5,3,4))"/>
								</xsl:when> 
            ...
            <xsl:variable name="Month" select="substring-before(substring-after(normalize-space(COL5),' '),' ')"/>

						<!--<xsl:variable name ="ExpiryMonth" select="substring(substring-after(normalize-space(COL8),' '),3,3)"/>-->
            <xsl:variable name ="varMonthCode">
			<xsl:call-template name ="MonthCodeVar">
				<xsl:with-param name ="varMonth" select ="$Month"/>
            ...
            <xsl:variable name="Future">
              <xsl:choose>
                <xsl:when test="$Asset='Future'">
                  <xsl:choose>
                    <xsl:when test="contains(substring-after(substring-after(normalize-space(COL5),' '),' '),'16')">

                      <xsl:value-of select="concat(substring-before(normalize-space(COL5),' '),' ',$varMonthCode,substring(substring-after(substring-after(normalize-space(COL5),' '),' '),2,1))"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="concat(substring(COL5,1,2),' ',substring(COL5,3,4))"/>
                    </xsl:otherwise>
                ...
                <xsl:when test="$Asset='FutureOption'">
                  <xsl:value-of select="concat(substring(substring-before(COL5,' '),1,2),' ',substring(substring-before(COL5,' '),3,4),substring(substring-after(COL5,' '),1,1),substring(substring-after(COL5,' '),2))"/>
                </xsl:when>
