...
{
    internal class SwapActivityGenerator : TradingActivityGenerator
    {
        /// <summary>
                ...
                switch (CashTransactionSource)
                {
                    ...
                    case CashTransactionType.TradeImport:
                        ...
                        if (t.LongOrShort.Equals(PositionTag.Long))
                        {
                            ...
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.CDSLongAddition.ToString());

                            }
                            else if (t.TransactionType == Convert.ToString(TradingTransactionType.LongWithdrawal))
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.CDSLongWithdrawal.ToString());
                            }
                            else
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.CDSLong.ToString());
                            }
                        }
                        else
                        {
                            ...
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.CDSShortAddition.ToString());
                            }
                            else if (t.TransactionType == Convert.ToString(TradingTransactionType.ShortWithdrawal))
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.CDSShortWithdrawal.ToString());
                            }
                            else
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.CDSShort.ToString());
                            }
                        }
