	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and contains(normalize-space(COL9),'MSILF PRIME')">
					...
					<PositionMaster>
						...
						<SideTagValue>							
							<xsl:choose>
								<xsl:when test="contains(normalize-space(COL9),'PURCHASE')">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL9),'REDEMPTION')">
						...
						<xsl:variable name="CostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="normalize-space(COL58) div $Position"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="PB_BROKER_NAME">
							<xsl:value-of select="normalize-space(COL43)"/>
						</xsl:variable>
