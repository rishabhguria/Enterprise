		...
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail">
				...
				<xsl:variable name ="PRANA_FUND_NAME">
					<xsl:value-of select ="document('../ReconMappingXml/FundList.xml')/FundMapping/PB[@Name='Wells Fargo']/FundData[@PranaFund=$PB_FUND_NAME]/@PranaFund"/>
				</xsl:variable>




					<xsl:if test="CounterParty='WEFA' or CounterParty='WCHV'">
...
<xsl:value-of  select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID]/Symbol"/>
		...
		<xsl:variable name="PB_COUNTERPARTY">
			<xsl:value-of select="document('../ReconMappingXml/ThirdParty_ExecBrokerMapping.xml')/BrokerMapping/PB[@Name = 'ML Broadcourt']/BrokerData[@PranaBroker = $PRANA_COUNTERPARTY]/@MLPBroker"/>
		</xsl:variable>
		...
		METHOD = "{$varMethod}" MARKET = "{$PRANA_EXCHANGE}" EXECUTING_BROKER = "WCHV" CLEARING_BROKER = "" TRADE_DATE = "{TradeDate}" SETTLEMENT_DATE = "{SettlementDate}"
		...
		ACCOUNT_TYPE = "" IS_ALLOCATION = "" TRAILER_CODE_1 = "" TRAILER_DESCRIPTION_1 = "" VERSUS_PURCHASE_DATE_1 = "" VERSUS_PURCHASE_QUANTITY_1 = "" RR= "" 
		  EntityID="{EntityID}" TaxLotState="{TaxLotState}" TaxLotState1="" FileHeader="TRUE" FileFooter="TRUE">
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<xsl:variable name ="PRANA_FUND_NAME">
					<xsl:value-of select ="document('../ReconMappingXml/FundList.xml')/FundMapping/PB[@Name='ML Broadcourt']/FundData[@PranaFund=$PB_FUND_NAME]/@PranaFund"/>
				</xsl:variable>-->
				...
				<xsl:variable name ="PB_FUND_CODE">
					<xsl:value-of select ="document('../ReconMappingXml/EOD_Mapping.xml')/FundMapping/PB[@Name='EOD']/FundData[@NirvanaAccountName=$PRANA_FUND_NAME]/@DVPWF"/>
				</xsl:variable>
