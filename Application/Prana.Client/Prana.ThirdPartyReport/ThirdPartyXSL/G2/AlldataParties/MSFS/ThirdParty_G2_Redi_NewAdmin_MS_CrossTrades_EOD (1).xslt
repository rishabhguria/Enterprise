...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:my="put-your-namespace-uri-here">
	<xsl:output method="xml" indent="yes" encoding="ISO-8859-1" />

	<msxsl:script language="C#" implements-prefix="my">
		public string Now(int year, int month)
		{
		DateTime firstWorkingDay= new DateTime(year, month, 1);
		...
		return firstWorkingDay.ToString();
		}
	</msxsl:script>
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[(CommissionCharged + SoftCommissionCharged + OtherBrokerFee + ClearingBrokerFee + StampDuty + TransactionLevy + ClearingFee + TaxOnCommissions + MiscFees + SecFee + OccFee + OrfFee)=0 and (CounterParty = 'Undefined')]">
				...
				<xsl:variable name="FirstWorkingDay">
					<xsl:choose>
						<xsl:when test="number($Month) and number($Year)">
							<xsl:value-of select="my:Now(number($Year),number($Month))"/>
						</xsl:when>
					</xsl:choose>
				</xsl:variable>

				<!--<xsl:if test="number(substring-before(substring-after($FirstWorkingDay,'/'),'/')) = number(substring-before(substring-after(TradeDate,'/'),'/'))">-->
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<Group 
				...
				ExecBkr="REBL" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{FullSecurityName}" TDate="{TradeDate}" SDate="{SettlementDate}"
				...
				Tax2="0"
				feesind="W" interest="0" interestindicator="" netamount="{format-number($GroupNetAmt,'###.00')}" hsyind="Y" custbkr="{$Custodian}" mmgr="" bookid=""
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<ThirdPartyFlatFileDetail
							...
							Taxfees="{$TaxFees}" 
							Tax2="0"
							feesind="W" interest="0" interestindicator="" netamount="{format-number(NetAmount,'###.00')}" hsyind="Y" custbkr="{$Custodian}" mmgr="" bookid=""
							dealid="" taxlotid="" taxdate="" taxprice="" closeoutmethod="" exrate="" acqdate="" comments="" fxind=""
							EntityID="{EntityID}" />
			</xsl:for-each>
		</Group>
