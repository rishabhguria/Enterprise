    ...
    <DocumentElement>
		...
		<xsl:variable name="maxDate">
			<xsl:for-each select="//PositionMaster">
				<xsl:sort select="substring(COL12, 7, 4)" data-type="number" order="descending"/>
				<xsl:sort select="substring(COL12, 1, 2)" data-type="number" order="descending"/>
				<xsl:sort select="substring(COL12, 4, 2)" data-type="number" order="descending"/>
				<xsl:if test="position()=1">
					<xsl:value-of select="COL12"/>
				</xsl:if>
			</xsl:for-each>
		</xsl:variable>
		<xsl:for-each select="//PositionMaster[COL12 = $maxDate]">
