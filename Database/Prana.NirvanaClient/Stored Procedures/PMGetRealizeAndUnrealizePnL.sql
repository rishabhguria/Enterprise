...
Modified Date: 6-Oct-2015                                                                        
Modified By : Pooja Porwal                                                                                                                                           
Description: Add Currency as bloomberg symbol and Sedol in case of asset class 'Cash'      
Jira : http://jira.nirvanasolutions.com:8080/browse/PRANA-10746      
      
      
Modified Date: 6-Oct-2015                                                                        
Modified By :  Pooja Porwal                                                                                                                                         
Description: Handle equity Swap as a saprate asst class      
Jira : http://jira.nirvanasolutions.com:8080/browse/PRANA-9557      
      
Modified Date: 6-Oct-2015                                                                        
Modified By :  Pooja Porwal                                                                                                                                         
Description:   Add open quantity Column      
Jira : http://jira.nirvanasolutions.com:8080/browse/PRANA-7217      
      
Modified Date: 30- Oct -2015                                                                                                        
Modified By : Pooja Porwal                                                                                                                                                                           
Description: set Settlement date as expiration date where trade is FX spot      
JIRA : http://jira.nirvanasolutions.com:8080/browse/PRANA-11515                  
...
---------------------------------Equity Swap--------------------------------      
--Check Asset class "EquitySwap" required not also equity       
...
IF((select count(AssetId) from @T_AssetIDs where AssetId NOT IN ( SELECT assetID from t_asset))=1 and  (select count(AssetId) from #T_Asset where AssetId=1)=0 )                                                 
BEGIN         
set @Flag=1        
INSERT INTO #T_Asset select * from t_asset where assetID=1      
END        
      
----Check only equity Required not EquitySwap      
IF((select count(AssetId) from @T_AssetIDs where AssetId NOT IN ( SELECT assetID from t_asset))=0  and  (select count(AssetId) from #T_Asset where AssetId=1)=1 )                                                 
BEGIN       
set @Flag=2       
END      
...
Create Table #SymbolAUEC (Symbol varchar(Max),AUECID int)                    
Insert Into #SymbolAUEC (Symbol,AUECID)                    
Select Distinct Symbol,AUECID From V_SymbolAuec Where Symbol In (Select Symbol From #Symbol)        
        
Alter Table #SymbolAUEC          
Add  AssetID Int NOT NULL CONSTRAINT [DF_AssetID]  DEFAULT ((0))          
          
Update Temp          
Set Temp.AssetID  =AUEC.AssetID          
From #SymbolAUEC Temp          
Inner Join T_AUEC AUEC On AUEC.AUECID = Temp.AUECID                  
        
INSERT INTO #AUECYesterDates          
 SELECT DISTINCT          
  #SymbolAUEC.AUECID,              
  dbo.AdjustBusinessDays(@StartDate, -1, #SymbolAUEC.AUECID)              
 FROM #SymbolAUEC              
      
--Added a check  AssetID Not In (5,11) for FX and FX Forward to not include holidays while picking Mark Price          
--To Do : We have to remove this check when PM_DayMarkPrice stops considering holidays for FX and FXforward       
...
INSERT INTO #MarkPriceForStartDate (Finalmarkprice, Symbol, FundID)          
 ...
 FROM PM_DayMarkPrice          
 INNER JOIN #SymbolAUEC  ON PM_DayMarkPrice.Symbol = #SymbolAUEC.Symbol AND #SymbolAUEC.AssetID Not In (5,11)             
 INNER JOIN #AUECYesterDates AUECYesterDates  ON AUECYesterDates.AUECID = #SymbolAUEC.AUECID              
...
INSERT INTO #MarkPriceForStartDate (Finalmarkprice, Symbol, FundID)              
 SELECT              
  ...
  PM_DayMarkPrice.FundID              
 FROM PM_DayMarkPrice              
 INNER JOIN #SymbolAUEC  ON PM_DayMarkPrice.Symbol = #SymbolAUEC.Symbol AND #SymbolAUEC.AssetID In (5,11)             
 INNER JOIN #AUECYesterDates AUECYesterDates  ON AUECYesterDates.AUECID = #SymbolAUEC.AUECID              
 WHERE DATEDIFF(D, PM_DayMarkPrice.Date,@DayPriorToStartDate) = 0           
              
INSERT INTO #AUECBusinessDatesForEndDate          
 SELECT DISTINCT          
  #SymbolAUEC.AUECID,              
  dbo.AdjustBusinessDays(DATEADD(D, 1, @EndDate), -1, #SymbolAUEC.AUECID)              
 FROM #SymbolAUEC              
...
INSERT INTO #MarkPriceForEndDate (Finalmarkprice, Symbol, FundID)          
 ...
 FROM PM_DayMarkPrice          
 INNER JOIN #SymbolAUEC ON PM_DayMarkPrice.Symbol = #SymbolAUEC.Symbol AND #SymbolAUEC.AssetID Not In (5,11)              
 INNER JOIN #AUECBusinessDatesForEndDate AUECBusinessDatesForEndDate ON AUECBusinessDatesForEndDate.AUECID = #SymbolAUEC.AUECID              
...
INSERT INTO #MarkPriceForEndDate (Finalmarkprice, Symbol, FundID)              
 ...
 INNER JOIN #SymbolAUEC ON PM_DayMarkPrice.Symbol = #SymbolAUEC.Symbol AND #SymbolAUEC.AssetID In (5,11)              
 INNER JOIN #AUECBusinessDatesForEndDate AUECBusinessDatesForEndDate ON AUECBusinessDatesForEndDate.AUECID = #SymbolAUEC.AUECID              
 WHERE DATEDIFF(D, PM_DayMarkPrice.Date, @EndDate) = 0          
...
INSERT INTO #MTMDataTable          
   ...
   **************************************************************************/          
   SELECT          
    ...
    ELSE (((ISNULL((PT.TaxLotOpenQty * SW.NotionalValue / G.CumQty),0) *          
...
INSERT INTO #MTMDataTable          
 SELECT          
  ...
  MIN(CashDiv.PayoutDate) AS SettlementDate,          
...
INSERT INTO #MTMDataTable          
 SELECT          
  ...
  MAX(DailyCash.DATE) AS SettlementDate,          
  '1800-01-01 00:00:00.000' AS ClosingDate,          
  0 AS Mark1,          
  0 AS Mark2,          
  0 AS IsSwapped,          
...
Min(          
 ...
 END          
  ...
  0 AS NetTotalCost_Base,          
  MIN(CLocal.CurrencySymbol) AS BloombergSymbol,      
  MIN(CLocal.CurrencySymbol) AS SedolSymbol,      
  '' AS ISINSymbol,          
  '' AS CusipSymbol,          
  '' AS OSISymbol,          
  '' AS IDCOSymbol,          
...
--AND DateDiff(d, dbo.AdjustBusinessDays(@StartDate, -1, @DefaultAUECID), FXDayRatesForStartDate.DATE) = 0           
  AND @DayPriorToStartDate = FXDayRatesForStartDate.DATE            
...
--AND DateDiff(d, dbo.AdjustBusinessDays(@StartDate, -1, @DefaultAUECID), ZeroFundFxRateStartDate.DATE) = 0          
  AND @DayPriorToStartDate = ZeroFundFxRateStartDate.DATE          
  AND ZeroFundFxRateStartDate.FundID = 0          
  )            
LEFT OUTER JOIN #FXConversionRates FXDayRatesForEndDate ON (            
...
--AND DateDiff(d, dbo.AdjustBusinessDays(DATEADD(D, 1, @EndDate), -1, @DefaultAUECID), FXDayRatesForEndDate.DATE) = 0           
  AND @EndDate = FXDayRatesForEndDate.DATE            
...
--AND DateDiff(d, dbo.AdjustBusinessDays(DATEADD(D, 1, @EndDate), -1, @DefaultAUECID), ZeroFundFxRateEndDate.DATE) = 0          
  AND @EndDate = ZeroFundFxRateEndDate.DATE          
  AND ZeroFundFxRateEndDate.FundID = 0          
  )           
...
----------------------------------        
UPDATE #MTMDataTable        
SET Asset='EquitySwap'      
,AssetID=@EquitySwapId      
where Asset='Equity' AND isswapped=1        
      
---IF only Swap required      
IF @flag=1        
delete #MTMDataTable where asset='Equity'      
      
---If only Equity filtered      
IF @Flag=2      
delete #MTMDataTable where asset='EquitySwap'  AND isswapped=1        
-------------------------------------------           
...
OpenQuantity float Not Null DEFAULT ((0))      
...
SET FundNav = ISNULL(PM_NAV.NAVValue, 0),          
 TotalPNL = (#MTMDataTable.RealizedPNLMTM_Base + #MTMDataTable.UnRealizedPNLMTM_Base + #MTMDataTable.Dividend),     PNLPercentByNAV =          
  CASE          
...
------------------------------------------------------------------------          
------------------------------------------------------------------------      
-----Updating Settlement date as expiration date for FXSpot only--------      
------------------------------------------------------------------------           
UPDATE #MTMDataTable      
SET ExpirationDate =  SettlementDate      
--from #MTMDataTable      
Where Asset in ('FX')        
------------------------------------------------------------------------      
------------------------------------------------------------------------      
...
DROP TABLE #SymbolAUEC,#MarkPriceForStartDate, #MarkPriceForEndDate, #SecMasterDataTempTable              
