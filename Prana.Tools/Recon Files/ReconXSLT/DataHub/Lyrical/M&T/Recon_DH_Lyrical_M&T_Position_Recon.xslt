			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name="Quantity">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="number(normalize-space(substring(COL1,127,16)))"/>
					</xsl:call-template>
				</xsl:variable>

				<xsl:choose>
					<xsl:when test="$PRANA_FUND_NAME!='' and number(normalize-space(substring(COL1,127,16))) and normalize-space(substring(COL1,221,11))!='WGOXX'">
						<PositionMaster>							
							...
							<xsl:variable name="AvgPrice">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="number(normalize-space(substring(COL1,179,14)))"/>
								</xsl:call-template>
							</xsl:variable>
							<AvgPX>
								<xsl:choose>
							...
							<xsl:variable name="MarketValue">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="number(substring(COL1,644,14))"/>
								</xsl:call-template>
							</xsl:variable>
							<MarketValue>
								<xsl:choose>
							...
							<xsl:variable name ="MarkPrice">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="number(substring(COL1,179,14))"/>
								</xsl:call-template>
							</xsl:variable>
