...
*/            
CREATE PROCEDURE [dbo].[PMGetFundPositionsWithMarketValue_KLDS_MS_POS] (            
...
,Symbol_PK BIGINT   
 ...
 SELECT #PositionTable.SideID            
  ,T_Side.Side            
  ,#PositionTable.Symbol  as TickerSymbol        
  ,            
  --TODO:                                          
  ...
  ,CASE             
   ...
   ELSE CASE             
     WHEN (            
      ...
      THEN IsNull((#PositionTable.Quantity * (IsNull(coalesce(MP.FinalMarkPrice, MP1.FinalMarkPrice), 0) - IsNull(#PositionTable.AvgPX, 0)) * #PositionTable.Multiplier * dbo.GetSideMultiplier(#PositionTable.SideID)), 0) * IsNull(coalesce(FX.RateValue, FX
1  
...
.RateValue), 0)            
  ...
  --ELSE 0                                  
  -- END                                                                               
  IsNull((#PositionTable.Quantity * IsNull(#PositionTable.AvgPx, 0) * #PositionTable.Multiplier * dbo.GetSideMultiplier(#PositionTable.SideID) + #PositionTable.TotalCommissionandFees), 0) AS NetNotionalValue            
  ,CASE             
   WHEN #PositionTable.CurrencyID = TC.LocalCurrency            
  ...
  ,#PositionTable.Bloomberg AS Symbol            
  ...
  --#PositionTable.AUECLocalDate as TradeDate,                                                            #PositionTable.ExpirationDate            
  ...
  ,#PositionTable.TotalCommissionandFees as Commission          
 FROM #PositionTable            
