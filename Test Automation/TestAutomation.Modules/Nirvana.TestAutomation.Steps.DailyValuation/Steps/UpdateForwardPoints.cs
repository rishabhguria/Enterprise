...
{
   public class UpdateForwardPoints : ForwardPointsUIMap, ITestStep
    {
       /// <summary>
       /// Run the test.
       /// </summary>
       ...
       public TestResult RunTest(System.Data.DataSet testData, Dictionary<int, string> sheetIndexToName)
       {
           ...
           try
           {
               ...
               UpdateForwardsPointsData(testData, sheetIndexToName);
        ...
        /// <summary>
        /// Updates the new forward points.
        /// </summary>
        /// <param name="testData">The test data.</param>
       private void UpdateForwardsPointsData(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            try
            {

                DataTable forwardPoints = CSVHelper.CSVAsDataTable(GrdPivotDisplay.InvokeMethod("GetAllVisibleDataFromTheGrid", null).ToString());
                ...
                {
                    DataRow[] foundRow = forwardPoints.Select(String.Format(@"[" + TestDataConstants.COL_SYMBOL + "]='{0}'", dataRow[TestDataConstants.COL_SYMBOL]));                    
                    if (foundRow.Length > 0)
                    {
                        int index = forwardPoints.Rows.IndexOf(foundRow[1]);
                        ...
                        {
                            columToMSAAIndexMapping = gridRow.GetColumnIndexMaping(forwardPoints);
                            SetValueInGrid(columToMSAAIndexMapping, dataRow, gridRow, "Forward Points");
                        }
                    }
                }
            }
