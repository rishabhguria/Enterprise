...
>
  ...
  <xsl:template name="spaces">
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-after(normalize-space(COL2),' '),7,1),'C') or contains(substring(substring-after(normalize-space(COL2),' '),7,1),'P')">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring-before(substring-after(substring-after(normalize-space(COL3),' '),' '),' '),'#.00')"/>
			</xsl:variable>
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_SYMBOL_NAME">
			  ...
			  <xsl:variable name="Symbol" >
				  <xsl:choose>
					  <xsl:when test="number(string-length(COL2)) = 3 and number(COL2)">
						  <xsl:value-of select="concat('0',COL6,$PRANA_SUFFIX_NAME)"/>
					  </xsl:when>

					  <xsl:when test="number(string-length(COL2)) = 4 and number(COL2)">
						  <xsl:value-of select="concat(COL6,$PRANA_SUFFIX_NAME)"/>
					  </xsl:when>
					  <xsl:otherwise>
						  <xsl:value-of select="concat(COL2,$PRANA_SUFFIX_NAME)"/>
					  </xsl:otherwise>
				  </xsl:choose>
			  </xsl:variable>
