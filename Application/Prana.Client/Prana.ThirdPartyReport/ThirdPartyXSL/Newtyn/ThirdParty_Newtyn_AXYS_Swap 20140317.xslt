...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name ="varSourceType">
			<xsl:choose>
		...
		<!--<xsl:variable name ="varSecurity">
			<xsl:choose>
				...
				<xsl:when test ="$varEqtSymbol='OGS-W'">
					<xsl:value-of select ="'ogs-w'"/>
				</xsl:when>						
				--><!-- http://jira.nirvanasolutions.com:8080/browse/SONOMA-250 --><!--
				--><!-- Date: 03-Feb -2014,Commented by Sandeep as per discussion with Dharendra --><!--
				--><!--<xsl:when test ="Asset = 'Equity' and $varCheckSymbolUnderlying != '' and SEDOL != ''">
		...
		<xsl:variable name ="varNetAmount">
			<xsl:choose>
				<xsl:when test ="Side = 'Buy' or Side = 'Buy to Open' or Side = 'Buy to Close'">
					<xsl:value-of select ="NetAmount - StampDuty"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select ="NetAmount + StampDuty"/>
				</xsl:otherwise>
		...
		<xsl:variable name ="varSourceSymbol">
			<xsl:choose>
				...
				<xsl:when test ="contains(AccountName, 'Swap') != false and (Side = 'Sell' or Side = 'Buy to Close' or Side = 'Sell to Close')">
					<xsl:choose>
						<xsl:when test ="AccountName = 'BNP Swap NP'">
							<xsl:value-of select ="'bnpswpnl'"/>
						</xsl:when>
						<xsl:when test ="AccountName = 'BNP Swap NTE'">
							<xsl:value-of select ="'bnpswpnl'"/>
						</xsl:when>
						<xsl:when test ="AccountName = 'Citi Swap NP'">
							<xsl:value-of select ="'gswappnl'"/>
						</xsl:when>
						<xsl:when test ="AccountName = 'Citi Swap NTE'">
							<xsl:value-of select ="'gswappnl'"/>
						</xsl:when>
						<xsl:when test ="AccountName = 'GS Swap NP'">
							<xsl:value-of select ="'citswpnl'"/>
						</xsl:when>
						<xsl:when test ="AccountName = 'GS Swap NTE'">
							<xsl:value-of select ="'citswpnl'"/>
						</xsl:when>
				...
				<xsl:otherwise>
					<xsl:value-of select ="$varSourceSymbol1"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<Group 
			...
			CloseMeth="{$varClosingMethod}" VersusDate="" SourceType="{$varSourceType}" SourceSymbol="{$varSourceSymbol}" TradeDateFXRate="" SettleDateFXRate=""
			OriginalFXRate="" MarkToMarket="{$varM2M}" TradeAmount="{$varNetAmount}" OriginalCost="" Comment1="" WithholdingTax="" Exchange=""
			ExchangeFee="{format-number(StampDuty, '###.00')}" commission="{format-number(CommissionCharged, '#.00')}" Broker="{$varBroker}" ImpliedComm="n"
		...
		<Group 
			...
			SecuritySymbol="{$varSecurity}" TradeDate="{translate(TradeDate,'/','')}" SettleDate="{translate(SettlementDate,'/','')}" OriginalCostDate="" Quantity="{AllocatedQty}"
			CloseMeth="{$varClosingMethod}" VersusDate="" SourceType="{$varSourceType}" SourceSymbol="{$varSourceSymbol}" TradeDateFXRate="" SettleDateFXRate=""
			OriginalFXRate="" MarkToMarket="{$varM2M}" TradeAmount="{NetAmount}" OriginalCost="" Comment1="" WithholdingTax="" Exchange=""
			ExchangeFee="{format-number(StampDuty, '###.00')}" commission="{format-number(CommissionCharged, '#.00')}" Broker="{$varBroker}" ImpliedComm="n"
			OtherFees="{format-number($varOthFee,'#.00')}" CommPurpose="" Pledge="n" LotLocation="253" DestPledge="" DestLotLocation="" OriginalFace="" YieldOnCost=""
			DurationOnCost="" UserDef1="" UserDef2="" UserDef3="" TranID="" IPCounter="" Repl="" Source="" Comment2="" OmniAcct="" Recon=""
			Post="y" LabelName="" LabelDefinition="" LabelDefinition_Date="" LabelDefinition_String="" Comment3="" RecordDate="" ReclaimAmount=""
