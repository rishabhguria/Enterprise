	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($Cash) and contains(COL3,'UVLCZ')  or contains(COL3,'BPTXX')">
					
				<!--<xsl:if test="number(COL13) and contains(COL5,'BNY MELLON USD') and (contains(COL1,'Clover Street') or contains(COL4,'VENBIO SELECT'))">-->
					<PositionMaster>
						...
						<xsl:variable name="PRANA_ACRONYM_NAME">
							<xsl:choose>
								...
								<xsl:when test="(normalize-space(COL3) = 'BPTXX') and ($Cash &lt; 0)">
