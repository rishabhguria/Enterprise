...
{
    class OperationonMTT : IUIAutomationTestStep
    {
        public TestResult RunUIAutomationTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                if (testData.Tables[0] != null && testData.Tables[0].Rows.Count > 0)
                {
                    ...
                    bool success = uiAutomationHelper.RightClickOnGridAndSelectItem(testData.Tables[0], "MultiTradingTicket");

                    if (!success)
                    {
                        throw new Exception("OperationonMTT  failed on clicking menuitem");
                    }
                }
            }
            catch (Exception ex)
