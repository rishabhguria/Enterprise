  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:if test="number($Cash) and contains(COL2,'Cash')='true' and not(contains(COL7,'Dividends'))">
          ...
          <PositionMaster>
            ...
            <!--<xsl:variable name="PB_Account_NAME" select="COL26"/>

            <xsl:variable name="PRANA_Account_NAME">
              <xsl:value-of select ="document('../ReconMappingXML/AccountMapping.xml')/AccountMapping/PB[@Name=$PB_NAME]/AccountData[@PBAccountName=$PB_Account_NAME]/@PranaAccount"/>
            </xsl:variable>-->
			  ...
			  <Symbol>
				  <xsl:choose>
					  ...
					  <xsl:when test="contains(COL6,'FADXX')">
						  <xsl:value-of select="'USD'"/>
					  </xsl:when>
					  <xsl:when test="contains(COL8,'_')">
						<xsl:value-of select="substring-before(COL8,'_')"/>
					</xsl:when>
					  <xsl:otherwise>
						  <xsl:value-of select="''"/>
					  </xsl:otherwise>
            ...
            <xsl:variable name ="Date" select="COL27"/>
