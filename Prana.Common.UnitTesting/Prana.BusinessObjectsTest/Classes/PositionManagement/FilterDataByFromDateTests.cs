...
{
    public class FilterDataByFromDateTests
    {
        ...
        [Fact]
        [Trait("Prana.BusinessObjects", "FilterDataByFromDate")]
        public void Filterdata_DatesAfterFromDate_ReturnsFilteredData()
        {
            // Arrange
            var filter = new FilterDataByFromDate
            {
                FromDate = new DateTime(2023, 1, 1)
            };
            object[] dataToFilter = {
                new MockFilterable(new DateTime(2023, 1, 1)),
                new MockFilterable(new DateTime(2023, 2, 1)),
            ...
            Assert.Contains(result, item => item.GetDate() == new DateTime(2023, 2, 1));
        }

        [Fact]
        [Trait("Prana.BusinessObjects", "FilterDataByFromDate")]
        public void Filterdata_DatesBeforeFromDate_ReturnsEmptyList()
        {
            // Arrange
            var filter = new FilterDataByFromDate
            {
            ...
            object[] dataToFilter = {
                new MockFilterable(new DateTime(2022, 12, 1)),
                new MockFilterable(new DateTime(2022, 12, 31))
            };
        ...
        [Fact]
        [Trait("Prana.BusinessObjects", "FilterDataByFromDate")]
        public void Filterdata_ExactFromDate_ReturnsMatchingData()
        {
            // Arrange
            var filter = new FilterDataByFromDate
            {
            ...
            Assert.Equal(2, dataToFilter.Length);
            Assert.Contains(result, item => item.GetDate() == new DateTime(2023, 1, 1));
            Assert.Contains(result, item => item.GetDate() == new DateTime(2023, 1, 2));
        }
