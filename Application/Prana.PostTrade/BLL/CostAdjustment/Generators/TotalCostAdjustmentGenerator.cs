...
// ***********************************************************************
// <copyright file="TotalCostAdjustmentGenerator.cs" company="Microsoft">
...
{
    /// <summary>
    /// Class TotalCostAdjustmentGenerator.
    /// </summary>
    internal class TotalCostAdjustmentGenerator : ICostAdjustmentGenerator
    {
        ...
        public CostAdjustmentResult AdjustCost(List<TaxLot> taxlotList, List<CostAdjustmentParameter> parameterList)
        {
            try
            {
                ...
                Parallel.ForEach(parameterList, parameter =>
                {
                    ...
                    //Getting total cost based on quantity
                    decimal totalCost = GetTotalCost(parameter.Taxlots, quantityToAdjust, taxlotList);

                    // If total cost to be adjusted is greater than the total cost of quantity to be adjusted, then it returns error
                    // http://jira.nirvanasolutions.com:8080/browse/PRANA-7359
                    if (parameter.AdjustCost < 0 && totalCost < Math.Abs(parameter.AdjustCost))
                    {
                        result.Error = " Adjusted cost should be less than total cost. \n Adjusted cost: " + Math.Abs(parameter.AdjustCost) + "\n Quantity to be adjusted: " + parameter.AdjustQty + "\n Total cost for adjusted quantity: " + totalCost;
                        return;
                    }
                    //Calculating average price based on total cost cost to adjust and quantity to adjust
                    averagePrice = (totalCost + parameter.AdjustCost) / quantityToAdjust;
        ...
        /// <param name="taxlotList">The taxlot list.</param>
        /// <returns>totalCost of taxlots</returns>
        private decimal GetTotalCost(List<CostAdjustmentTaxlot> taxlots, decimal quantity, List<TaxLot> taxlotList)
        {
            try
            {
                ...
                foreach (CostAdjustmentTaxlot taxlot in taxlots)
                {
                    if (quantity != 0)
                    {
                        decimal quantityAdjusted = this.GetQuantityAdjustedForTaxlot(quantity, taxlot);

                        TaxLot selectedTaxlot = (taxlotList.Single(a => a.TaxLotID == taxlot.TaxlotId));
                        totalCost += NotionalCalculator.GetNotional(selectedTaxlot, quantityAdjusted);
                        // Removed condition because now we are getting absolute data for Total Open Quantity to Adjust and Quantity to Adjust
                        // Now OrderSide Tag value is not required here
                        //quantity -= quantityAdjusted * Calculations.GetSideMultilpier(taxlot.OrderSideTagValue);
        ...
        {
            get { return CostAdjustmentType.Total; }
        }
