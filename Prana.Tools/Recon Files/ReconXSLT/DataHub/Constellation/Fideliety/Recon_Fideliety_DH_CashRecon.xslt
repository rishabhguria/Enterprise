      ...
      <xsl:for-each select ="//Comparision">
        <xsl:variable name="Cash">
          <xsl:call-template name="Translate">
            <xsl:with-param name="Number" select="substring(COL1,107,18)"/>
          </xsl:call-template>
        </xsl:variable>
          ...
          <xsl:when test="number($Cash)">
            <PositionMaster>
              ...
              <xsl:variable name = "PB_FUND_NAME">
                <xsl:value-of select="substring(COL1,5,6)"/>
              </xsl:variable>
              ...
              <!--<Currency>
								<xsl:choose>
								<xsl:when test ="COL2='U.S. Dollars'">
									<xsl:value-of select ="USD"/>
								</xsl:when>
									<xsl:when test ="COL2='Canadian Dollar'">
										<xsl:value-of select ="CAD"/>
									</xsl:when>
									<xsl:when test ="COL2='Euro'">
										<xsl:value-of select ="EUR"/>
									</xsl:when>
									<xsl:when test ="COL2='Hong Kong Dollar'">
										<xsl:value-of select ="HKD"/>
									</xsl:when>
									<xsl:when test ="COL2='Japanese Yen'">
										<xsl:value-of select ="JPY"/>
									</xsl:when>
									<xsl:when test ="COL2='Pound Sterling'">
										<xsl:value-of select ="GBP"/>
									</xsl:when>
									<xsl:when test ="COL2='Swedish Krona'">
										<xsl:value-of select ="SEK"/>
									</xsl:when>
									<xsl:when test ="COL2='Australian Dollar'">
										<xsl:value-of select ="AUD"/>
									</xsl:when>
									<xsl:when test ="COL2='Mexican Peso'">
										<xsl:value-of select ="MXN"/>
									</xsl:when>
									<xsl:otherwise>
              ...
              <xsl:variable name="CashValuelocal">
                <xsl:call-template name="Translate">
                  <xsl:with-param name="Number" select="$Cash div 100"/>
                </xsl:call-template>
              </xsl:variable>

              <OpeningBalanceDR>
                <xsl:choose>
                  <xsl:when test="substring(COL1,71,1)='-'">
                    <xsl:choose>
