        ...
        <xsl:if test="COL1 != 'Confirmed'">
          <PositionMaster>
            ...
            <xsl:variable name="varOptionYear">
			  ...
			  <xsl:variable name="varLMEOptionMonthNo">
				  <xsl:if test="COL8 = 'Option' and COL13 = 'LME' ">
						  <xsl:call-template name="MonthNo">
							  <xsl:with-param name="varMonth" select="substring(COL17,1,1)"/>
						  </xsl:call-template>
				  </xsl:if>
			  ...
			  <xsl:variable name="varLMEOptionYearno">
				  <xsl:if test="COL8= 'Option' and COL13 = 'LME'">
					  <xsl:value-of select="substring(COL17,2,2)"/>
				  </xsl:if>
			  </xsl:variable>

			  <!--substring-before(my:Now(number($varLMEOptionMonthNo), number($varLMEOptionYearno)),' ')-->
			  ...
			  <xsl:variable name ="thirdWednesday">
				  <xsl:choose>
					  <xsl:when test ="$varAssetCategory = 'Option' and $varExchange = 'LME'">
						  <xsl:value-of select ="substring-before(my:Now1(number($varLMEOptionYearno),number($varLMEOptionMonthNo)),' ')"/>
					  </xsl:when>
				  </xsl:choose>
			  </xsl:variable>
			  ...
			  <UnderlyingSymbol>
				  ...
				  <xsl:choose>					
					  <xsl:when test ="$varExchange = 'LME' and $varAssetCategory = 'Option'">
						  <xsl:value-of select =" substring-before(my:Now(concat(20,number($varLMEOptionYearno)),number($varLMEOptionMonthNo)),' ')"/>
					  </xsl:when>
					  <xsl:otherwise>
						  <xsl:value-of select ="''"/>
					  </xsl:otherwise>
