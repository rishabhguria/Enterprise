...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<Group 
				...
				EntityID="" TaxLotState="" IsCaptionChangeRequired="true" RowHeader="true"/>
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="QtySum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/AllocatedQty)"/>
		</xsl:variable>
		<xsl:variable name="GroupNetAmt">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/NetAmount)"/>
		</xsl:variable>
		<xsl:variable name="CommissionSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/CommissionCharged)"/>
		</xsl:variable>
		<xsl:variable name="TaxOnCommissionSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/TaxOnCommissions)"/>
		</xsl:variable>
		<xsl:variable name="SecFeeSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/SecFee)"/>
		</xsl:variable>
		<xsl:variable name="StampDutySum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/StampDuty)"/>
		</xsl:variable>
		<xsl:variable name="TransactionLevySum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/TransactionLevy)"/>
		</xsl:variable>
		<xsl:variable name="ClearingFeeSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/ClearingFee)"/>
		</xsl:variable>
		<xsl:variable name="MiscFeesSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/MiscFees)"/>
		</xsl:variable>
		<xsl:variable name="OrfFeeSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/OrfFee)"/>
		</xsl:variable>
		<xsl:variable name="OtherBrokerFeeSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/OtherBrokerFee)"/>
		</xsl:variable>
		<xsl:variable name="GrossAmountSum">
			<xsl:value-of  select="sum(/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/GrossAmount)"/>
		</xsl:variable>
		...
		<xsl:variable name="tempCPVar">
			<xsl:value-of  select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID][FromDeleted != 'Yes']/CounterParty"/>
		</xsl:variable>
		...
		<xsl:variable name="GroupNetAmt1">
			<xsl:choose>
				...
				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
					<xsl:value-of select="$GroupNetAmt * $varFXRate"/>
				</xsl:when>

				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
					<xsl:value-of select="$GroupNetAmt div $varFXRate"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<xsl:variable name="Commission">
			<xsl:choose>
				...
				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
					<xsl:value-of select="$COMM2 * $varFXRate"/>
				</xsl:when>

				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
					<xsl:value-of select="$COMM2 div $varFXRate"/>
				</xsl:when>
		...
		<xsl:variable name="GroupTotalFees1">
			<xsl:choose>
				...
				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
					<xsl:value-of select="$GroupTotalFees * $varFXRate"/>
				</xsl:when>

				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
					<xsl:value-of select="$GroupTotalFees div $varFXRate"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<Group 
				...
				EntityID="{EntityID}" TaxLotState="{TaxLotState}" IsCaptionChangeRequired="true" RowHeader="true">
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<xsl:variable name="TaxlotGrsAmt">
					<xsl:choose>
						...
						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
							<xsl:value-of select="$TaxlotGRS * $varFXRate"/>
						</xsl:when>

						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
							<xsl:value-of select="TaxlotGRS div $varFXRate"/>
						</xsl:when>
				...
				<xsl:variable name="TAXFEES">
					<xsl:choose>
						...
						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
							<xsl:value-of select="$TaxFees * $varFXRate"/>
						</xsl:when>

						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
							<xsl:value-of select="$TaxFees div $varFXRate"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="''"/>
						</xsl:otherwise>
				...
				<xsl:variable name="Commission1">
					<xsl:choose>
						...
						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
							<xsl:value-of select="format-number($COMM1 * $varFXRate,'##.00')"/>
						</xsl:when>

						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
							<xsl:value-of select="format-number($COMM1 div $varFXRate,'##.00')"/>
						</xsl:when>
				...
				<xsl:variable name="AMNT">
					<xsl:choose>
						...
						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
							<xsl:value-of select="format-number($NTANT * $varFXRate,'##.00')"/>
						</xsl:when>

						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
							<xsl:value-of select="format-number($NTANT div $varFXRate,'##.00')"/>
						</xsl:when>
				...
				<ThirdPartyFlatFileDetail
							...
							translevel="A" ClientRef="{concat(substring(EntityID,3),'C')}" Associated="{PBUniqueID}" ExecAccount="038082566" AccountID="{AccountMappedName}"
							ExecBkr="{$CPVar}" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{translate(FullSecurityName,',','')}" TDate="{TradeDate}" SDate="{SettlementDate}"
							...
							EntityID="{EntityID}" IsCaptionChangeRequired="true" RowHeader="true"/>
			</xsl:for-each>
		</Group>
	</xsl:template>
</xsl:stylesheet>