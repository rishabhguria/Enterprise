					...
					<PositionMaster>						
						...
						<xsl:variable name="OptionSymbol">
							<xsl:choose>
								...
								<xsl:when test="string-length($Symbol) &gt; 13">
						...
						<xsl:variable name="CommissionShare">
							<xsl:choose>
								
								<xsl:when test="number(translate(COL9,'$',''))">
									<xsl:value-of select="translate(COL9,'$','')"/>
								</xsl:when>

								<xsl:when test="contains(COL9,'e')">
									<xsl:value-of select="translate(substring-before(COL9,'e'),'$','')"/>
								</xsl:when>
						...
						<xsl:variable name="Commission">
							<xsl:value-of select="$NetPosition * $CommissionShare"/>
						</xsl:variable>
