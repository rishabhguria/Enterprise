  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($varMidPrice) and not(contains(COL5,'Cash'))">
          <PositionMaster>
            ...
            <Symbol>
              <xsl:choose>
                ...
                <xsl:when test ="substring-before(COL10,'_') != 'USD'">
                  <xsl:value-of select ="''"/>
                </xsl:when>
                
                <xsl:when test ="COL5= 'Short Fixed Income'">                  
                  <xsl:value-of select ="''"/>
                </xsl:when>
            ...
            <MarkPrice>
              <xsl:choose>
                <xsl:when test="$varMidPrice &gt; 0">
                  <xsl:value-of select="$varMidPrice"/>
                </xsl:when>
                <xsl:when test="$varMidPrice &lt; 0">
                  <xsl:value-of select="$varMidPrice * (-1)"/>
                </xsl:when>
                <xsl:otherwise>
