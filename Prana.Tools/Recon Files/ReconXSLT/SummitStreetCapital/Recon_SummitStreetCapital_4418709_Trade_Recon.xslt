	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Position) and (COL1='4418709') and (normalize-space(COL9) ='Equities') and
						(COL15 !='OPENING BALANCE' and COL15 !='CLOSING BALANCE') and not(contains(COL15,'INCOME')) and  not(contains(COL15,'Ex Date')) ">
					...
					<PositionMaster>
						...
						<xsl:variable name="CostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring-before(substring-after(substring-after(normalize-space(COL15),'PRICE OF'),'$'),' ')"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="Commission">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring-before(substring-after(substring-after(normalize-space(COL15),'COMMISSION OF'),'$'),' ')"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="varSecFees">
							<xsl:choose>
								<xsl:when test="contains(COL15,'OTHER CHARGES OF')">
									<xsl:call-template name="Translate">
										<xsl:with-param name="Number" select="substring-before(substring-after(substring-after(normalize-space(COL15),'OTHER CHARGES OF'),'$'),' ')"/>
									</xsl:call-template>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="''"/>
