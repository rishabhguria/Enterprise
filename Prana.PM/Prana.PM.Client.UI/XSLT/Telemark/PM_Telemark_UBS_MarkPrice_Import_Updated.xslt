...
<!--CALL AAPL US 24/JUN/21 C190-->
<!--SMCI 3 C1200-->

	<xsl:template name="Option">
		<xsl:param name="Symbol"/>
		<xsl:variable name="UnderlyingSymbol">
			<xsl:value-of select="substring-before(normalize-space(COL12),' ')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryDay">
			<xsl:value-of select="normalize-space(substring-before(substring-after(normalize-space(COL3),'US'),'/'))"/>
		</xsl:variable>
		...
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after(substring-after(normalize-space(COL12),' '),' '),1,1)"/>
		</xsl:variable>			
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring(substring-after(substring-after(normalize-space(COL12),' '),' '),2),'##.00')"/>
		</xsl:variable>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($MarkPrice)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="Asset" select="COL6"/>
            <Symbol>
              <xsl:choose>
