				...
				<xsl:if test="number($Position)">
					<PositionMaster>
						...
						<xsl:variable name="varSymbol">
							<xsl:when test ="string-length(normalize-space(COL3)) !=12">
						...
						<xsl:variable name="varCostBasis">
							<xsl:choose>
								<xsl:when  test="normalize-space(COL5) !='0'">
									<xsl:call-template name="Translate">
										<xsl:with-param name="Number" select="normalize-space(COL6) div normalize-space(COL5)"/>
									</xsl:call-template>
								</xsl:when>
								<xsl:otherwise>
