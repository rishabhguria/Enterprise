	...
	<xsl:template name="Option">
		...
		<xsl:if test="COL9='OPT'">
			...
			<xsl:variable name="VarStrikePrice">
				<xsl:value-of select="substring(substring-after(normalize-space(COL5),' '),8,8)"/>
			</xsl:variable>

			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(($VarStrikePrice div 1000),'#.00')"/>
			</xsl:variable>
				...
				<xsl:template match="/">
					<DocumentElement>
						<xsl:for-each select ="//Comparision">
							...
							<xsl:if test="number($Quantity)and COL15!='DIVACC'">
								<PositionMaster>
						...
						<xsl:variable name="StrikePrice">
							<xsl:value-of select="format-number($VarStrikePrice div 1000,'#.##')"/>
						</xsl:variable>

						<xsl:variable name="Optionsymbol">
							<xsl:value-of select="concat('O:',$UnderlyingSymbol,' ',$ExpiryYear,$ExpiryMonth,$StrikePrice,'D',$ExpiryDay)"/>
						</xsl:variable>
						...
						<xsl:variable name="NetNaotionalValue">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="format-number(COL22,'##.####')"/>
							</xsl:call-template>
						</xsl:variable>
						<NetNotionalValue>
							<xsl:choose>
						...
						<xsl:variable name="NetNaotionalValueBase">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="format-number(COL23,'##.####')"/>
							</xsl:call-template>
						</xsl:variable>
