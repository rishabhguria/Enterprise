Create PROCEDURE [dbo].[P_FFGetLevel2_MarkstonAdmin_TaxlotState_ExportOnly]                                                                                                                                                     
(                                                                                                                                                                    
 @thirdPartyID int,                                                                                                                                                                    
...
End as CounterPartyID,            
 ...
 1                                                   
  ...
  from                                                                                                                                                             
    V_TaxLots VT                                                                
    Left Outer Join T_PBWiseTaxlotState on T_PBWiseTaxlotState.TaxlotID=VT.TaxlotID where (@fileFormatID = FileFormatID or FileFormatID = 0)                                                                                     
...
case                                           
 ...
 When T_CounterParty.ShortName = 'GWEP' and VT.AssetID = 8                      
  then  Cast(Cast(((Round(VT.AvgPrice, 4) * VT.TaxlotQty * 0.01) + ((VT.Commission + VT.OtherBrokerFees + VT.TransactionLevy + VT.StampDuty + VT.ClearingFee + VT.TaxOnCommissions + VT.MiscFees) * 0.01)) As Decimal(32,2)) As Varchar(500))                 
  
    
      
  when T_CounterParty.ShortName = 'GWEP'              
  then   Cast(Cast(((Round(VT.AvgPrice, 4) * VT.TaxlotQty * SM.Multiplier) + ((VT.Commission + VT.OtherBrokerFees + VT.TransactionLevy + VT.StampDuty + VT.ClearingFee + VT.TaxOnCommissions + VT.MiscFees) * VT.SideMultiplier)) As Decimal(32,2)) As Varchar 
...
And CTPM.InternalFundNameID_FK in (select FundID from @Fund)                                                                             
  And ((VT.TaxLotState <> 1) OR (VT.TaxLotState = 1 AND                                    
  datediff(d,                           (                                     
 case                                     
  when @dateType=1                                     
 then                                     
  ...
  ,@inputdate)=0 ))                                                                                       
  And (VT.TaxLotState <> 4 OR (VT.TaxLotState = 4 AND                                    
  datediff(d,                                    
  (                                     
  case                                     
   when @dateType=1                                     
...
ORDER BY Symbol,  
CONVERT(INT, LEFT(FundName, PATINDEX('%[^0-9]%', FundName+'z')-1)),FundName  
  
-- CAST(CASE    
--WHEN FundName LIKE '[0-9][0-9][0-9][0-9]%' THEN LEFT(FundName ,4)    
--WHEN FundName LIKE '[0-9][0-9][0-9]%' THEN LEFT(FundName ,3)    
--ELSE NULL END AS INT), FundName    
--taxlotid                                                  
