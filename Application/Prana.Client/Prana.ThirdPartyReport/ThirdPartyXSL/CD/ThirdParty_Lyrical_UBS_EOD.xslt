		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail">
				<ThirdPartyFlatFileDetail>
					...
					<xsl:variable name="VendorID" select="'NMUN'"/>					
					...
					<xsl:variable name="AccountBLANK">
						<xsl:call-template name="Space">
							<xsl:with-param name="number" select="14-string-length(AccountNo)"/>
						</xsl:call-template>
					</xsl:variable>

					<!--Need to change-->
					<xsl:variable name="AccountNumber">
						<xsl:value-of select="concat(AccountNo,$AccountBLANK)"/>
					</xsl:variable>
					...
					<xsl:variable name="OffsetAccount">
						<xsl:call-template name="Space">
							<xsl:with-param name="number" select="14"/>
						</xsl:call-template>
					</xsl:variable>
					...
					<xsl:variable name="StepoutInd" select="'Y'"/>
