/*                
Modified By : Kuldeep Kumar        
Description : Market Value of 'EquitySwap' is its Unrealized P&L.        
JIRA  : https://jira.nirvanasolutions.com:8443/browse/ONB-9882     
*/ 
CREATE PROCEDURE [dbo].[PMGetFundPositionsWithMarketValue_MarvinPalmer] (  
 ...
 SELECT #PositionTable.SideID  
  ...
  , Case
   When #PositionTable.IsSwapped = 1     
	  Then (#PositionTable.Quantity * dbo.GetSideMultiplier(#PositionTable.SideID)) *(IsNull(coalesce(MP.FinalMarkPrice, MP1.FinalMarkPrice), 0) - (UnitCost.Val))
      Else MarketValue.Val 
    End AS MarketValue 
  --, MarketValue.Val * DayEndFxRate.Val AS MarketValueBase  
  ,Case
   When #PositionTable.IsSwapped = 1      
	  Then (#PositionTable.Quantity * dbo.GetSideMultiplier(#PositionTable.SideID)) *(DayEndFxRate.Val * IsNull(coalesce(MP.FinalMarkPrice, MP1.FinalMarkPrice), 0) - (SettlCurrFxRate1.Val * UnitCost.Val))
      Else MarketValue.Val * DayEndFxRate.Val
...
End AS Asset    
 ...
 CROSS APPLY (  
  SELECT CASE   
    WHEN (  
    --  (#PositionTable.IsSwapped = 1)  or
        (#PositionTable.AssetID IN (3, 5, 11))  
      )  
 ...
 DROP TABLE #PositionTable, #FXConversionRatesDayMark, #ZeroFundIDFXConversionRates 
