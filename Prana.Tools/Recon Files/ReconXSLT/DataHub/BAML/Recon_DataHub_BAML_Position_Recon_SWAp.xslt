	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity)and COL19='Swap Valuation Position Mark to Market'">
						<PositionMaster>
							...
							<xsl:variable name="Asset">
								<xsl:choose>
									<xsl:when test="COL19='Equity Option'">
										<xsl:value-of select="'EquityOption'"/>
									</xsl:when>
									<xsl:when test="contains(COL19,'Swap')">
							...
							<xsl:variable name="Symbol">
								<xsl:value-of select="substring-before(COL108,' ')"/>
							</xsl:variable>
							...
							<Symbol>
								<xsl:choose>
									...
									<xsl:when test="$Symbol!='*'">
										<xsl:value-of select="concat('EQS.',$Symbol)"/>
									</xsl:when>
