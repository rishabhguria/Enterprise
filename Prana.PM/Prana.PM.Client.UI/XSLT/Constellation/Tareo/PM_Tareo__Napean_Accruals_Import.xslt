      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:variable name="Cash">
          <xsl:call-template name="Translate">
            <xsl:with-param name="Number" select="normalize-space(COL82)"/>
          </xsl:call-template>
        </xsl:variable>
<xsl:variable name = "varSubAccountDesc">
              <xsl:value-of select="normalize-space(COL14)"/>
            </xsl:variable>
        <xsl:if test="number($Cash) and ($varSubAccountDesc ='Money Market' or $varSubAccountDesc ='Collateral' or $varSubAccountDesc ='Equity Swap Financing' or $varSubAccountDesc ='Swap Activity' or $varSubAccountDesc ='Wires' or $varSubAccountDesc ='Interest' or $varSubAccountDesc ='Transaction Billing')">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_PRE">
              <xsl:choose>
				  <xsl:when test="$varSubAccountDesc ='Collateral' and $Cash &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Collateral' and $Cash &lt; 0 ">
					  <xsl:value-of select="'CASHTRA'"/>
				  </xsl:when>
				 
				 <xsl:when test="$varSubAccountDesc ='Equity Swap Financing' and $Cash &lt; 0 ">
					  <xsl:value-of select="'CASHTRA'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Equity Swap Financing' and $Cash &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  ...
				  <xsl:when test="$varSubAccountDesc ='Money Market' and $Cash &lt; 0 ">
					  <xsl:value-of select="'MoneyMarketSecurity'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Money Market' and $Cash &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Transaction Billingt' and $Cash &lt; 0 ">
					  <xsl:value-of select="'Ticker FeesExpense'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Swap Activity' and $Cash &lt; 0 ">
					  <xsl:value-of select="'SwapInterestExpense'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Swap Activity' and $Cash &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				   <xsl:when test="$varSubAccountDesc ='Wires' and $Cash &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Wires' and $Cash &lt; 0 ">
					  <xsl:value-of select="'CASH_WDL'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Transaction Billing' and $Cash &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='Transaction Billing' and $Cash &lt; 0 ">
					  <xsl:value-of select="'Ticker FeesExpense'"/>
