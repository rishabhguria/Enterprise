...
Usage :                 
[P_GetAllocationTicketData] '2013-1-14','2013-7-18'                                  
                                  
*********************************************/                                         
CREATE proc [dbo].[P_GetAllocationTicketData]                                                                                                                            
(                                                                                                                                                                                                                
 @startDate datetime,                                                                                                                          
 @enddate datetime                                                                                                                      
)                                                                                                                          
...
SET @BaseCurrencyID = (Select Top 1 BaseCurrencyID from T_Company Where BaseCurrencyID <> -1)                                  
...
Case          
 When G.CumQty = 0          
 Then 0          
 Else ISNULL(G.Commission,0) * (Temp.Quantity / G.CumQty )           
End as CommissionLocal,                                       
0 as CommissionBase,                                                                                   
Case          
 When G.CumQty = 0          
 Then 0          
 Else ISNULL(G.OtherBrokerFees,0) * (Temp.Quantity / G.CumQty )          
End as OtherBrokerFeesLocal,                                     
0 as OtherBrokerFeesBase,                                                      
Case          
 When G.CumQty = 0          
 Then 0          
 Else ISNULL(G.StampDuty + G.TransactionLevy + G.ClearingFee + G.TaxOnCommissions + G.MiscFees,0) * (Temp.Quantity / G.CumQty )          
...
TotalCommissionAndFeesLocal = CommissionLocal + OtherBrokerFeesLocal + OtherFeesLocal                                        
...
DROP TABLE #FXConversionData,#SecMasterDataTempTable,#CustomFundName,#V_Taxlots_Temp,#OpenTaxlots   
