...
{
    class ThirdPartySend:ThirdPartyUIMap,ITestStep
    {
        /// <summary>
        /// RunTest method takes two parameter-
        /// 1.testData of DataSet type which gives .xlsx file sheet data.
        ...
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                Wait(3000);
                //Deselect preselected checkbox on datagrid
        ...
        /// <summary>
        /// matchedSelect method compare test case of Datagrid with xls sheet and perform Send operation.
        /// </summary>
        /// <param name="dr"></param>
        private void MatchedSelect(DataRow dr)
        {
            try
            {
                ...
                msaaObj.CachedChildren[0].FindDescendantByName("Generate", 3000).Click(MouseButtons.Left);

                Wait(3000);

                if (Confirmation1.IsVisible)
                    ButtonYes3.Click(MouseButtons.Left);

                Wait(3000);

                //Click on Send button
                msaaObj.CachedChildren[0].FindDescendantByName("Send", 3000).Click(MouseButtons.Left);

                Wait(7000);

                if (Confirmation.IsVisible)
                    ButtonYes2.DoubleClick(MouseButtons.Left);
               // Wait(7000);
