	...
	<xsl:template match="/NewDataSet">
		<ThirdPartyFlatFileDetailCollection>
			...
			<ThirdPartyFlatFileDetail>
				...
				<StrikePriceCPFandCRFOnly>
					<xsl:value-of select="'Strike Price (CPF and CRF Only)'"/>
				</StrikePriceCPFandCRFOnly>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[AccountName = 'Heptagon - Longs - BBH']">
				...
				<xsl:choose>
					<xsl:when test ="TaxLotState!='Amemded'">
						<ThirdPartyFlatFileDetail>
							...
							<xsl:variable name="PB_NAME" select="'Brown Brothers Harriman'"/>
							...
							<UnitPriceAmount>
								<xsl:choose>
									<xsl:when test="number(AvgPrice)">
										<xsl:value-of select="format-number(AvgPrice,'##.00000000')"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="0"/>
									</xsl:otherwise>
							...
							<xsl:variable name="THIRDPARTY_BROKER_NAME">
								<xsl:value-of select="document('../ReconMappingXml/ThirdParty_ExecBrokerMapping.xml')/BrokerMapping/PB[@Name='BBH']/BrokerData[@PranaBroker=$PRANA_BROKER_NAME]/@PranaBrokerCode"/>
							</xsl:variable>

							<xsl:variable name="THIRDPARTY_BROKER">
								<xsl:value-of select="document('../ReconMappingXml/ExecBrokerDTCMapping.xml')/BrokerMapping/PB[@Name='BBH']/BrokerData[@PranaBroker=$PRANA_BROKER_NAME]/@DTCCode"/>
							</xsl:variable>

							<TradingBrokerTypeID>
								<xsl:choose>
									<xsl:when test="CurrencySymbol='USD'">
										<xsl:value-of select="concat('DTCYID','/',$THIRDPARTY_BROKER)"/>
									</xsl:when>
									<xsl:when test="UDACountryName='CANADA'">
							...
							<OtherFeesAmount>
								<xsl:choose>
									<xsl:when test="number($OtherFees)">
										<xsl:value-of select="format-number($OtherFees,'0.00')"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="0"/>
									</xsl:otherwise>
							...
							<CommissionAmount>
								<xsl:value-of select="format-number($varCommission,'0.00')"/>
							</CommissionAmount>
						...
						<xsl:if test ="number(OldExecutedQuantity)">
							<ThirdPartyFlatFileDetail>
								...
								<UnitPriceAmount>
									<xsl:choose>
										<xsl:when test="number(OldAvgPrice)">
											<xsl:value-of select="format-number(OldAvgPrice,'0.00000000')"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="0"/>
										</xsl:otherwise>
								...
								<NetAmount>
									<xsl:choose>
										<xsl:when test="CurrencySymbol='JPY'">
											<xsl:value-of select="format-number($varOldNetAmount,'0.')"/>
										</xsl:when>
								...
								<xsl:variable name="varOldOtherFees">
									<xsl:value-of select="OldOtherBrokerFees + OldClearingBrokerFee + OldStampDuty + OldTransactionLevy + OldClearingFee + TaxOnCommissions + OldMiscFees + OldOccFee + OldOrfFee"/>
								</xsl:variable>
								<OtherFeesAmount>
									<xsl:choose>
										<xsl:when test="number($varOldOtherFees)">
											<xsl:value-of select="format-number($varOldOtherFees,'0.00')"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="0"/>
										</xsl:otherwise>
								...
								<CommissionAmount>
									<xsl:value-of select="format-number($varCommission2,'0.00')"/>
								</CommissionAmount>
