  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
        <xsl:if test="normalize-space(COL1) != 'Trader Acct' and number(COL8)">
          <PositionMaster>
            ...
            <xsl:variable name="MonthCode">
              <xsl:call-template name="GetMonth">
                <xsl:with-param name="varMonth" select="substring($varOptionSymbol,5,3)"/>
                <xsl:with-param name="varPutCall" select="substring($varOptionSymbol,10,1)"/>
              </xsl:call-template>
            </xsl:variable>
            <Symbol>
              <xsl:choose>
                <xsl:when test="normalize-space(COL5) = '' or COL5 = '*'">
                  <xsl:value-of select="substring-before(COL4,' ')"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:variable name="varThirdFriday">
                    <xsl:value-of select =" my:Now(substring($varOptionSymbol,1,4),substring(COL5,5,2))"/>
                  </xsl:variable>

                  <xsl:variable name="varIsFlex">
                    <xsl:choose>
                      <xsl:when test="(substring-before(substring-after($varThirdFriday,'/'),'/') + 1) = normalize-space(substring(COL5,7,2))">
                  ...
                  <xsl:variable name="varDate">
                    <xsl:choose>
                      <xsl:when test="substring(COL5,7,1)='0'">
                        <xsl:value-of select="substring(COL5,8,1)"/>
                      </xsl:when>
                      <xsl:otherwise>
                  ...
                  <xsl:choose>
                    <xsl:when test="$varIsFlex=1">
                      <xsl:value-of select="concat('O:',substring-before(COL3,'+'),' ',substring($varOptionSymbol,3,2),$MonthCode,format-number(substring($varOptionSymbol,11),'#.00'),'D',$varDate)"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="concat('O:',substring-before(COL3,'+'),' ',substring($varOptionSymbol,3,2),$MonthCode,format-number(substring($varOptionSymbol,11),'#.00'))"/>
