...
<xsl:variable name="PB_FUND_NAME">
			...
			<xsl:variable name="Fees1">
              <xsl:choose>
			  <xsl:when  test="$Position &gt; 0 and string-length(COL8) &gt; 10">
                  <xsl:value-of select=" ($COL7 -(($Position * $varCostBasis * 100) + $varCommission + $varSecFee + $varOrffees))"/>
                </xsl:when>
				  <xsl:when  test="$Position &lt; 0 and string-length(COL8) &gt; 10">
                  <xsl:value-of select=" ($COL7 -(($Position * $varCostBasis * 100) + $varCommission + $varSecFee + $varOrffees))"/>
                </xsl:when>
            ...
            <SideTagValue>
			 ...
			 <xsl:variable name="PB_CountnerParty" select="normalize-space(COL23)"/>
            ...
            <CounterPartyID>
              <xsl:choose>
                <xsl:when test="contains(COL23,'ARCA')">
                  <xsl:value-of select="'1'"/>
                </xsl:when>
                <xsl:when test="contains(COL23,'CHBC')">
                  <xsl:value-of select="'7'"/>
                </xsl:when>
				<xsl:when test="contains(COL23,'DEAN')">
                  <xsl:value-of select="'9'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'DFIN')">
                  <xsl:value-of select="'11'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'ETRS')">
                  <xsl:value-of select="'12'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'FOGS')">
                  <xsl:value-of select="'14'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'NON')">
                  <xsl:value-of select="'15'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'OCC')">
                  <xsl:value-of select="'16'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'PUMX')">
                  <xsl:value-of select="'18'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'QNTX')">
                  <xsl:value-of select="'19'"/>
                </xsl:when>
				<xsl:when test="contains(COL23,'WEXM')">
                  <xsl:value-of select="'20'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'RDBN')">
                  <xsl:value-of select="'13'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'ISIG')">
                  <xsl:value-of select="'17'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'WELX')">
                  <xsl:value-of select="'19'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'WEX')">
                  <xsl:value-of select="'64'"/>
                </xsl:when>
				
				<xsl:when test="contains(COL23,'MLCO')">
