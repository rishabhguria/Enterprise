				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name="varPrevMonth">
							<xsl:choose>
								<xsl:when test ="number(substring-before(COL7,'/')) = 1">
									<xsl:value-of select ="12"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select ="number(substring-before(COL7,'/'))-1"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						...
						<xsl:variable name ="varYear">
							<xsl:choose>
								<xsl:when test ="number(substring-before(COL7,'/')) = 1">
									<xsl:value-of select ="number(substring-after(substring-after(COL7,'/'),'/')) -1"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select ="number(substring-after(substring-after(COL7,'/'),'/'))"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
