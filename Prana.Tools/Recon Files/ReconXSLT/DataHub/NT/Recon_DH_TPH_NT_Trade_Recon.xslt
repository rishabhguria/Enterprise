				...
				<xsl:choose>
					<xsl:when test="number($Quantity)">
						<PositionMaster>
							...
							<!--<xsl:variable name ="Date1" select="COL8"/>
							<xsl:variable name="Year1" select="substring($Date1,7,4)"/>
							<xsl:variable name="Month" select="substring($Date1,4,2)"/>
							<xsl:variable name="Day" select="substring($Date1,1,2)"/>
							<TradeDate>
								<xsl:value-of select="concat($Month,'/',$Day,'/',$Year1)"/>
							</TradeDate>-->
