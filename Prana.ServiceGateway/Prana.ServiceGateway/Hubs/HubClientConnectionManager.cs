...
{
    public class HubClientConnectionManager : IHubManager
    {
        //private static readonly Dictionary<string, List<string>> _userConnections = new Dictionary<string, List<string>>();
        private readonly IConfiguration _configuration;
        private readonly ILogger<HubClientConnectionManager> _logger;
        private readonly IHubContext<ServiceGatewayHub> _hubContext;

        public HubClientConnectionManager(IConfiguration configuration,
            ILogger<HubClientConnectionManager> logger,
            IHubContext<ServiceGatewayHub> hubManager)
        {
        ...
        public static void RemoveUserFromRTPNLUpdatesList(int userId)
        {
            try
            {
                ...
                _userRtpnlUpdatesList.TryRemove(userId, out _);
                Log.Information($"User {userId} removed from the list.");
            }
            catch (Exception ex)
            {
        ...
        public async void SendUserBasedMessage(string topic, RequestResponseModel message)
        {
            try
            {
                ...
                if (!isForcefulBroadcast)
                {
                    ...
                    var connections = HubClientConnectionManager.GetConnections(userId.ToString());
