	...
	<xsl:template match="/">
		<DocumentElement>
					...
					<PositionMaster>						
						...
						<xsl:variable name = "varCurrencyID" >
							<xsl:call-template name="tempCurrencyCode">
								<xsl:with-param name="paramCurrencySymbol" select="COL8" />
							</xsl:call-template>
						</xsl:variable>

						<xsl:variable name = "varCouponFrequencyID" >
							<xsl:call-template name="tempCouponFrequencyID">
								<xsl:with-param name="paramCouponFrequencyID" select="COL10" />
							</xsl:call-template>
						</xsl:variable>
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <FirstCouponDate>
                  <xsl:value-of select="'1/1/1800 12:00:00 PM'"/>
                </FirstCouponDate>
              </xsl:otherwise>
            </xsl:choose>
