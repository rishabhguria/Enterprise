				...
				<xsl:if test="number($Quantity)">
					<PositionMaster>
						...
						<xsl:variable name="Date">
							<xsl:choose>
								<xsl:when test="string-length(substring-before(substring-after(COL14,'/'),'/'))='1'">
									<xsl:value-of select="concat(0,substring-before(substring-after(COL14,'/'),'/'))"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<xsl:variable name="Date1">
							<xsl:choose>
								<xsl:when test="string-length(substring-before(substring-after(COL15,'/'),'/'))='1'">
									<xsl:value-of select="concat(0,substring-before(substring-after(COL15,'/'),'/'))"/>
								</xsl:when>
								<xsl:otherwise>
