...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test ="contains(COL23,'Option')">
			...
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring-after(substring-before(normalize-space($Symbol),'/'),' ')"/>
			</xsl:variable>
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="substring(substring-after(substring-after($Symbol,' '),' '),2 div 1000)"/>
			</xsl:variable>
				...
				<xsl:if test="number($NetPosition)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test ="contains(COL23,'Option')">
