				...
				<xsl:if test="number($Position)">
					<PositionMaster>
						...
						<xsl:variable name ="Asset">
							<xsl:choose>
								<xsl:when test="COL6='Equity Option' and string-length(COL7 &gt; 20)">
									<xsl:value-of select="'EquityOption'"/>
								</xsl:when>
								<xsl:when test="COL6='Equity Swap'">
						...
						<xsl:variable name="Commission">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="translate(COL15,'$','')"/>
							</xsl:call-template>
						</xsl:variable>
						<Commission>
							<xsl:choose>
							...
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:choose>
										...
										<xsl:when test="$Side='BUY TO COVER' or $Side='SELL TO CLOSE'">
						...
						<xsl:variable name="varStampDuty">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="translate(COL13,'$','')"/>
							</xsl:call-template>
						</xsl:variable>
						<SecFee>
							<xsl:choose>
						...
						<OtherBrokerFees>
							<xsl:choose>
								<xsl:when test ="number($OtherBrokerFee) and $OtherBrokerFee &gt; 0">
