/****** Object:  StoredProcedure [dbo].[P_W_GetRealTimeChartsData_RealtimeBase]    Script Date: 02/27/2014 10:05:32 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
-- =============================================
CREATE PROCEDURE [dbo].[P_W_GetRealTimeChartsData_RealtimeBase] 
...
(GroupBy varchar(Max),Client varchar(Max),Entity varchar(Max),[Group] varchar(Max),Symbol varchar(Max),[Exposure Base] float,[Day P&L Base] float)

If @SymbolLevel = 'Symbol'
Begin
	If @Groups = 'Entity'
	Begin
		Insert Into #AllPositions(GroupBy,Client,Entity,[Group],Symbol,[Exposure Base],[Day P&L Base])
		Select GroupBy,Client,Entity,Entity,Symbol,[Exposure Base],[Day P&L Base] From #EntitySymbolValue Where Symbol Is Not Null
	End
	Else If @Groups = 'UdaSector'
	Begin
		Insert Into #AllPositions(GroupBy,Client,Entity,[Group],Symbol,[Exposure Base],[Day P&L Base])
		Select GroupBy,Client,Entity,Sector,Symbol,[Exposure Base],[Day P&L Base] From #EntitySymbolValue Where Symbol Is Not Null
	End
End
...
Begin
	...
	Begin
		Insert Into #AllPositions(GroupBy,Client,Entity,[Group],Symbol,[Exposure Base],[Day P&L Base])
		Select GroupBy,Client,Entity,Entity,[Underlying Symbol],Sum([Exposure Base]),Sum([Day P&L Base]) From #EntitySymbolValue Where [Underlying Symbol] Is Not Null Group By GroupBy,Client,Entity,[Underlying Symbol]
	End
	Else If @Groups = 'UdaSector'
	Begin
		Insert Into #AllPositions(GroupBy,Client,Entity,[Group],Symbol,[Exposure Base],[Day P&L Base])
		Select GroupBy,Client,Entity,Sector,[Underlying Symbol],Sum([Exposure Base]),Sum([Day P&L Base]) From #EntitySymbolValue Where [Underlying Symbol] Is Not Null Group By GroupBy,Client,Entity,Sector,[Underlying Symbol]
	End
End

If @Content = 'Exposure Base'
Begin	
If @Group = ''
Begin 
	If @Groups = 'Entity'
	Begin
		Select GroupBy,Client,Entity,[Group],Sum([Exposure Base]) As [Exposure Base] From #AllPositions Group By GroupBy,Client,Entity,[Group] Order By GroupBy,Client,Entity,[Group]
	End
	Else If @Groups = 'UdaSector'
	Begin 
		Select GroupBy,Client,Entity,[Group],Sum([Exposure Base]) As [Exposure Base] From #AllPositions Where GroupBy = @GroupBy And Client = @Client And Entity = @Entity Group By GroupBy,Client,Entity,[Group] Order By GroupBy,Client,Entity,[Group]
	End 
End
Else If @Group <> ''
Begin
	Select [Symbol],Sum([Exposure Base]) As [Exposure Base],Sum([Day P&L Base]) As [Day P&L Base] From #AllPositions where GroupBy = @GroupBy And Client = @Client And Entity = @Entity And [Group] = @Group Group By [Symbol] Order by [Symbol] Asc
End
End

If @Content = 'Day P&L Base'
Begin	
If @Group = ''
Begin 
	If @Groups = 'Entity'
	Begin
		Select GroupBy,Client,Entity,[Group],Sum([Day P&L Base]) As [Day P&L Base] From #AllPositions Group By GroupBy,Client,Entity,[Group] Order By GroupBy,Client,Entity,[Group]
	End
	Else If @Groups = 'UdaSector'
	Begin 
		Select GroupBy,Client,Entity,[Group],Sum([Day P&L Base]) As [Day P&L Base] From #AllPositions Where GroupBy = @GroupBy And Client = @Client And Entity = @Entity Group By GroupBy,Client,Entity,[Group] Order By GroupBy,Client,Entity,[Group]
	End 
End
Else If @Group <> ''
Begin
