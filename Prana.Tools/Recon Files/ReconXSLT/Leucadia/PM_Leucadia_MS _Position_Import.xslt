	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL50,'PUTL') or contains(COL50,'CALLL') or contains(COL45,'USD') ">
			...
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-before(substring-after(substring-after(normalize-space(COL84),' '),' '),'/'),2)"/>
			...
			<xsl:variable name="StrikePrice">
				<!--<xsl:value-of select =" substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL84),' '),' '),' '),' '),2)"/>-->
		...
		<xsl:template match="/">
			<DocumentElement>
				...
				<xsl:for-each select="//PositionMaster">
					<xsl:if test="number(COL27) and COL8!='USD'">
						<PositionMaster>
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="(contains(COL50, 'PUTL') or contains(COL50, 'CALLL'))  and COL45='USD'">
