...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="varPutCall">
			<xsl:choose>
				<xsl:when test="(contains(COL50, 'PUT') or contains(COL50, 'CALL'))  and COL45='USD'">
					<xsl:value-of select ="substring(COL51,1,1)"/>
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select="//Comparision">
				...
				<xsl:if test ="number(COL27) and COL50!='CASH'  and COL51!='FX FORWARDS'">
					...
					<PositionMaster>
						...
						<xsl:variable name="varMarketValue">
							<xsl:choose>
								<xsl:when test="(contains(COL51, 'SWAP') and COL50!='FUTURE')">
										<xsl:value-of select="COL40 - COL34"/>
						...
						<xsl:variable name="Symbol">
							<xsl:choose>
								<xsl:when test="string-length(substring-before(COL14,' '))='3' and COL44='HKD'">
									<xsl:value-of select="concat(0,substring-before(COL14,' '))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL14,' '))='2' and COL44='HKD'">
									<xsl:value-of select="concat(0,0,substring-before(COL14,' '))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL14,' '))='1' and COL44='HKD'">
									<xsl:value-of select="concat(0,0,0,substring-before(COL14,' '))"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="substring-before(COL14,' ')"/>
								</xsl:otherwise>
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="(contains(COL51, 'SWAP') and COL50!='FUTURE')">
									<xsl:value-of select="concat($Symbol,$PRANA_Exchange,'/SWAP')"/>
								</xsl:when>
