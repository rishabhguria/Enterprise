	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				<xsl:if test="number(COL1) and COL3!='Cash and Equivalent'">
					<PositionMaster>
						...
						<xsl:choose>
							<xsl:when test="COL3  ='Option - Call'">
								<IDCOOptionSymbol>
									<xsl:value-of select="translate(concat(concat(substring-before(COL2,' '),'  ',substring-after(COL2,' ')),'U'),$vLowercaseChars_CONST,$vUppercaseChars_CONST)"/>
								</IDCOOptionSymbol>
								<Symbol>
									<xsl:value-of select ="''"/>
								</Symbol>
