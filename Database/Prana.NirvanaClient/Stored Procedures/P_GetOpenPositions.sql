      
CREATE Procedure [dbo].[P_GetOpenPositions]                                
(                                         
@ThirdPartyID int,                                                    
@CompanyFundIDs varchar(max),                                                                                                                                                                                  
@InputDate DateTime,                                                                                                                                                                              
@CompanyID Int,                                                                                                                                              
@AUECIDs varchar(max),                                                                                    
@TypeID int,  -- 0 means for Company Third Party, 1 means for Executing broker or All Data Parties                                                    
@DateType int, -- 0 means for Process Date and 1 means Trade Date.                                                                                                                                                                                    
@FileFormatID int                                          
)                                          
AS        
      
--Declare @ThirdPartyID int                                                    
--Declare @CompanyFundIDs varchar(max)      
--Declare @InputDate DateTime                                                                                                                                                                         
--Declare @CompanyID int      
--Declare @AUECIDs varchar(max)      
--Declare @TypeID int      
--Declare @DateType int      
--Declare @FileFormatID int        
      
      

--Set @thirdPartyID=37
--Set @companyFundIDs=N'9,13,12,11,3,8,2,10,1,14,4,5,6'
--Set @inputDate='2019-07-19 15:23:24'
--Set @companyID=7
--Set @auecIDs=N'20,30,63,182,44,34,43,56,53,59,31,54,45,21,60,18,180,1,15,62,73,32,81'
--Set @TypeID=0
--Set @dateType=0
--Set @fileFormatID=102     
      
    
      
Select Distinct Symbol  
Into #Temp
From PM_Taxlots With (NoLock)
Where TaxLotOpenQty > 0 And  
Taxlot_PK in                                                                                            
(                                                                                                   
   Select max(Taxlot_PK) from PM_Taxlots With (NoLock)
   where DateDiff(d,PM_Taxlots.AUECModifiedDate,@InputDate) >= 0                                                                      
   group by taxlotid                                                   
 )  


 Select 
 VSCD.BloombergSYmbol As BBCode  
 From #Temp Temp  
 Inner Join V_SecMasterData_WithUnderlying VSCD With (NoLock) On Temp.Symbol= VSCD.TickerSymbol
      

Drop Table #Temp