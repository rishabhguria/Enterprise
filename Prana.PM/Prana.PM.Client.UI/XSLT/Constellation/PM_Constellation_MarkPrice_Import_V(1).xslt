      ...
      <xsl:for-each select ="//PositionMaster">
		  ...
		  <xsl:variable name="MarkPrice">
			  <xsl:choose>
				  ...
				  <xsl:when test="contains(substring-before(substring-after(COL1,' '),' '),'SJ') or contains(substring-before(substring-after(COL1,' '),' '),'LN')">
					  <xsl:value-of select="$COL8 div 100"/>
				  </xsl:when>
		  ...
		  <xsl:if test="number($MarkPrice) and not(contains(COL1,'CURNCY')) ">
