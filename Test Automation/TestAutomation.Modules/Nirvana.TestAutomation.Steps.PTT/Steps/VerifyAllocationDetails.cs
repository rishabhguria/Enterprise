...
{
    class VerifyAllocationDetails : PTTUIMap, IUIAutomationTestStep
    {
        public TestResult RunUIAutomationTest(System.Data.DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                try
                {
                    ...
                    DataTable UIData = gridDataProvider.GetWPFGridData("WindowAllocationDetails");
                    List<String> errors = Recon.RunRecon(UIData, testData.Tables[0], new List<string>(), 0.01);
                    if (errors.Count > 0)
                ...
                finally
                {
                    try
                    {
                        UIAutomationHelper.DetectAndCloseWindow("Window");
