...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<Group 
				...
				SettlementCCY="{SettlCurrency}"	ExchangeCode="{$varEXCODE}" Quantity="{$QtySum}" 	PriceAmount="{format-number(AveragePrice,'###.0000')}"	PriceIndicator="N" Principal="{format-number($GroupGrsAmt,'###.00')}" 
				ExecutingBrokerCommission="" ExecutingBrokerCommissionIndicator="F" 
				MSFees="" 
				NotRequired=""
				MSFeesIndicator="F" DividendEntitlementPercent="" Spread="" NetAmount="{format-number($NetAmountSum,'###.00')}" HearsayInd="N" Custodian="MSCO" MoneyManager="" Bookid=""
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID and TaxLotState!='Sent']">
				...
				<ThirdPartyFlatFileDetail		
							...
							Quantity="{AllocatedQty}" PriceAmount="{format-number(AveragePrice,'###.0000')}" PriceIndicator="N"	
							Principal="{format-number($TaxlotGrsAmt,'###.00')}" ExecutingBrokerCommission="" 
							ExecutingBrokerCommissionIndicator="F" MSFees="" 
							NotRequired=""
							MSFeesIndicator="F" DividendEntitlementPercent="" Spread="" NetAmount="{format-number(NetAmount,'###.00')}" HearsayInd="N" Custodian="MSCO" 
							...
							AcquitionDate="" Comments="" SwapReferenceNo="" BasketId="" PriceCurrency="{CurrencySymbol}" ResetIndicator="" EntityID="{EntityID}" TaxLotState="{TaxLotState}"/>
			</xsl:for-each>
		</Group>
	</xsl:template>
</xsl:stylesheet>
