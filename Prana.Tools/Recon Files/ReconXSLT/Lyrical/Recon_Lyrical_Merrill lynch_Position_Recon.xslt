	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Position) and (not(contains($SymbolChk,'SD1')) and not(contains($SymbolChk,'SD2')) and not(contains($SymbolChk,'SD3')))">
						...
						<MarketValue>
							<xsl:choose>
								...
								<xsl:when test="number($MarketValue)">
									<xsl:value-of select="number($MarketValue)"/>
								</xsl:when>
