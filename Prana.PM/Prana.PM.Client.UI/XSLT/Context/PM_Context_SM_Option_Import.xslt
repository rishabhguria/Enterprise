				...
				<xsl:if test="$varAssetType='EquityOption'">
					...
					<PositionMaster>
						...
						<xsl:variable name="varExpiryYear">
							<xsl:value-of select="concat('20',substring(substring-after(normalize-space(COL10),' '),1,2))"/>
						</xsl:variable>
						...
						<xsl:variable name="StrikePrice">
							<xsl:value-of select="substring(substring-after(normalize-space(COL10),' '),8)"/>
						</xsl:variable>
