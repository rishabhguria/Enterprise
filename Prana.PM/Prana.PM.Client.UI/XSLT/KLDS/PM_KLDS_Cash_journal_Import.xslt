      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($varCash)">
          ...
          <PositionMaster>
            ...
            <JournalEntries>
              <xsl:choose>
                <xsl:when test="$varTransactionType = 'Withdrawal'">
                  <xsl:value-of select="concat('CASH_WDL',':', $varAmount , '|','Cash', ':' , $varAmount)"/>
                </xsl:when>
                <xsl:when test="$varTransactionType = 'Deposit'">
                  <xsl:value-of select="concat('Cash',':', $varAmount , '|','CASH_DEP', ':' , $varAmount)"/>
                </xsl:when>
                <xsl:when test="$varTransactionType = 'Transfer Out'">
                  <xsl:value-of select="concat('CashTransferOut',':', $varAmount , '|','Cash', ':' , $varAmount)"/>
                </xsl:when>
                <xsl:when test="$varTransactionType = 'Transfer In'">
                  <xsl:value-of select="concat('Cash',':', $varAmount , '|','CashTransferIn', ':' , $varAmount)"/>
                </xsl:when>
              </xsl:choose>
            </JournalEntries>
