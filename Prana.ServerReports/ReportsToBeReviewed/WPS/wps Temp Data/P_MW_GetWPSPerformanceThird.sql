...
Alter Procedure [dbo].[P_MW_GetWPSPerformance]                                                          
(                                                          
@StartDate datetime,                                                      
@EndDate datetime,                                            
@Funds Varchar(max),                             
...
------for asked funds so that it is not required to be queried again and again

select * into #tempT_MW_GenericPNL  from T_MW_GenericPNL             
where Rundate between @StartDate and @EndDate and fund in (@Funds)                   
...
SELECT EndingPriceBase,Symbol from #tempT_MW_GenericPNL where DATEDIFF(D,Rundate,@StartDate)=0                      
                      
--------------------------------------------------------Closing Mark Price of Start Date------------------------------------                         
                     
-----------------------------------------------------NET PNL/ACB For Group 1------------------------------------------------------                                     
            
select symbol into #T_distinctSymbol from #tempT_MW_GenericPNL Where                    
...
select underlyingSymbol into #T_distinctUnderlyingSymbol from #tempT_MW_GenericPNL Where                    
datediff (day , @Startdate , Rundate) >= 0                 
and datediff(day , Rundate , @EndDate) >= 0                  
and Open_CloseTag <> 'Accruals'                  
...
ELSE IF @paramGroupByLevel1='MasterFund'
begin            
...
ELSE IF @paramGroupByLevel1='Symbol'
begin            
...
ELSE IF @paramGroupByLevel1='UnderlyingSymbol'
begin            
...
Else IF @paramGroupByLevel2='UnderlyingSymbol'                      
begin            
...
Else IF @paramGroupByLevel1='Fund' 
Begin                      
		update #UnrealizedPNLTable                  
		Set NETACBPNLBasedOnGrouping1=ACB.result              
...
#ACBGrouping1,#ClosingMarkPriceForStartDate,#T_distinctSymbol,#T_distinctUnderlyingSymbol,#ACBGrouping2