...
{
    class VerifyUnAssignedAccounts : IUIAutomationTestStep
    {
        public TestResult RunUIAutomationTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("VerifyUnAssignedAccounts failed as DataSet is empty.");
                }
                ...
                bool result = gridDataobj.CheckEmptyGridData(testData.Tables[0], ds.Tables["UnAssignedAccounts"]);
                if (!result)
                {

                    errors = Recon.RunRecon(ds.Tables["UnAssignedAccounts"], testData.Tables[0], columns, 0.01);
                    if (errors.Count > 0)
