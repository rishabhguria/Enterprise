			...
			<xsl:for-each select="ThirdPartyFlatFileDetail">
				...
				<ThirdPartyFlatFileDetail>
					...
					<qty>
						<xsl:choose>
							<xsl:when test="number(AllocatedQty)">
								<xsl:value-of select ="format-number(AllocatedQty,'#.00')"/>
							</xsl:when>
							<xsl:otherwise>
					...
					<xsl:variable name ="varNetNotional">
						...
						<xsl:choose>
							<xsl:when test ="Asset='EquityOption'">
								<xsl:value-of select="(NetAmount div AllocatedQty)div 100"/>
							</xsl:when>
					...
					<PRICE>
						<xsl:choose>
							<xsl:when test="number(AveragePrice)">
								<xsl:value-of select ="format-number($varNetNotional,'#.00000000')"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="0"/>
							</xsl:otherwise>
					...
					<CLRBRKRACCT>
						<xsl:value-of select ="concat('19Capital LP',' ','-',' ',$varBROKER)"/>
					</CLRBRKRACCT>
