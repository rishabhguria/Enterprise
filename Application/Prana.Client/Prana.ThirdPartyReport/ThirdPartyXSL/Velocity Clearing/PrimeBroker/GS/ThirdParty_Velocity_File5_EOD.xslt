...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
  ...
  <xsl:template match="/ThirdPartyFlatFileDetailCollection">
    <ThirdPartyFlatFileDetailCollection>
      ...
      <xsl:for-each select="ThirdPartyFlatFileDetail[(Asset='Equity') and CurrencySymbol='USD' and CounterParty!='VCGO'and CounterParty!='VONE' and CounterParty!='VTFX']">
        <ThirdPartyFlatFileDetail>
          ...
          <BROKER>
            <xsl:choose>
              <xsl:when test="CounterParty='VCGO'">
                <xsl:value-of select="'0005'"/>
              </xsl:when>
              <xsl:when test="CounterParty='VALR'">
                <xsl:value-of select="'0023'"/>
              </xsl:when>
              <xsl:when test="CounterParty='QAMM'">
                <xsl:value-of select="'0024'"/>
              </xsl:when>
              <!-- <xsl:when test="CounterParty='PERS'"> -->
              <!-- <xsl:value-of select="'0443'"/> -->
              <!-- </xsl:when> -->
              <xsl:when test="CounterParty='RJFI'">
                <xsl:value-of select="'0725'"/>
              </xsl:when>
              <xsl:otherwise>
