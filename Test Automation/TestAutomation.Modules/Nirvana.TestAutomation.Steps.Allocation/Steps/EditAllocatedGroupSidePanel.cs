...
{
    class EditAllocatedGroupSidePanel : AllocationUIMap, ITestStep
    {
        /// <summary>
        ...
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                string Applybtn = "";
                string closebtn = "";
                if (!testData.Tables[0].Columns.Contains("Apply") && !testData.Tables[0].Columns.Contains("Cancel"))
                {
                    AllocationGrids1.Click(MouseButtons.Left);
                }
                else
                {
                     Applybtn = string.IsNullOrEmpty(testData.Tables[0].Rows[0]["Apply"].ToString()) ? null : testData.Tables[0].Rows[0]["Apply"].ToString();
                     closebtn = string.IsNullOrEmpty(testData.Tables[0].Rows[0]["Cancel"].ToString()) ? null : testData.Tables[0].Rows[0]["Cancel"].ToString();
                     if (!string.IsNullOrEmpty(testData.Tables[0].Rows[0]["Apply"].ToString()))
                     {
                         testData.Tables[0].Rows[0]["Apply"] = "";  
                     }
                     if (!string.IsNullOrEmpty(testData.Tables[0].Rows[0]["Cancel"].ToString()))
                     {
                         testData.Tables[0].Rows[0]["Cancel"] = "";  
                     }
                }
                ...
                {
                    ButtonCancel.DoubleClick(MouseButtons.Left);
                }
            ...
            {
                SamsaraHelperClass.CaptureMyScreen("", ApplicationArguments.TestCaseToBeRun, "EditAllocatedGroupSidePanel");
                _res.IsPassed = false;
            ...
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    foreach (DataColumn col in row.Table.Columns)
                    {
                        ...
                        {
                            if (col.ColumnName.ToString().Contains("date") || col.ColumnName.ToString().Contains("DATE") || col.ColumnName.ToString().Contains("Date"))
                            {
                                try
                                {
                                   string tempDate = DataUtilities.DateHandler(row[col].ToString());
                                   string fromdate = String.Format(ExcelStructureConstants.COMMON_DATE_FORMAT, DateTime.Parse(tempDate));
                                    row[col] = fromdate;
                                }
                                catch{}
                            }
