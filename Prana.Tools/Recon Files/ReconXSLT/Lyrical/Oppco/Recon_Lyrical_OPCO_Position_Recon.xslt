	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($varQuantity) and not(contains(substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL1),'|'),'|'),'|'),'|'),'|'),'|'),'USD') ='true') ">
					...
					<PositionMaster>
						...
						<xsl:variable name="MarkPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$varMarketValue div $varQuantity"/>
							</xsl:call-template>
						</xsl:variable>
						<MarkPrice>
							<xsl:choose>
