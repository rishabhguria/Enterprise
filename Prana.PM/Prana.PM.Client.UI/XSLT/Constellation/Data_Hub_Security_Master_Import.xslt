...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="Translate">
	...
	<xsl:template match="/">
        ...
        <xsl:if test="number($varMultiplier)">		
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								...
								<xsl:when test="contains(COL1,'FUTURE') or contains(COL1,'Futuer')">
						...
						<PutOrCall>
							<xsl:choose>
								<xsl:when test="contains(substring(COL15,1,1),'C') and contains(COL5,'O:')">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="contains(substring(COL15,1,1),'P') and contains(COL5,'O:')">
									<xsl:value-of select="'0'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="'-1'"/>
