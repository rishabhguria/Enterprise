  ...
  <xsl:template name="Option">
    ...
    <xsl:param name="Suffix"/>    
      ...
      <xsl:variable name="PutORCall">
        <xsl:value-of select="substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL8),'/'),'/'),' '),' '),1,1)"/>
      </xsl:variable>
      <xsl:variable name="StrikePrice">
        <xsl:value-of select="format-number(substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL8),'/'),'/'),' '),' '),2),'##.00')"/>
      </xsl:variable>
        ...
        <xsl:if test="number($Position)">
          <PositionMaster>
            ...
            <xsl:variable name="varBloomberg">
              <xsl:value-of select="translate(normalize-space(COL9),$vLowercaseChars_CONST,$vUppercaseChars_CONST)"/>
            </xsl:variable>
            <Symbol>
              <xsl:choose>
