...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[AccountName!='Heptagon - Shorts - Morgan Stanley' and IsSwapped!='true' and not(contains(Asset,'FX'))]">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="COMM">
			<xsl:choose>
				...
				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
					<xsl:value-of select="($CommissionSum + $SoftCommissionSum) * $varFXRate"/>
				</xsl:when>

				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
					<xsl:value-of select="($CommissionSum + $SoftCommissionSum) div $varFXRate"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<xsl:variable name="GrossAmnt">
			<xsl:choose>
				...
				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
					<xsl:value-of select="$GroupGrsAmt * $varFXRate"/>
				</xsl:when>

				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
					<xsl:value-of select="$GroupGrsAmt div $varFXRate"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<Group 
				...
				SCCY="{SettlCurrency}"	ExCode="" qty="{$QtySum}" 	Price="{format-number($AveragePrice,'#.0000000')}"	type="G" prin="{format-number($GrossAmnt,'#.0000000')}" 
				comm="{format-number($COMM,'#.0000000')}" comtype="F" 
				Taxfees="{format-number($GroupTotalFees1,'#.0000000')}" 
				Tax2="0"
				feesind="F" interest="0" interestindicator="" netamount="{format-number($GroupNetAmt1,'#.0000000')}" hsyind="N" custbkr="MSCO" mmgr="" bookid=""
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<xsl:variable name="TaxlotNetAmt">
					<xsl:value-of select="($TaxlotGrsAmt + (SecFee + CommissionCharged + SoftCommissionCharged + StampDuty + TransactionLevy + ClearingFee + MiscFees + OtherBrokerFee + TaxOnCommissions) * $TaxlotSideMultiplier)"/>
				</xsl:variable>
				...
				<xsl:variable name="Commission1">
					<xsl:choose>
						...
						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
							<xsl:value-of select="$COMM1 * $varFXRate"/>
						</xsl:when>

						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
							<xsl:value-of select="$COMM1 div $varFXRate"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="''"/>
						</xsl:otherwise>
				...
				<xsl:variable name="AMNT">
					<xsl:choose>
						...
						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
							<xsl:value-of select="$NTANT * $varFXRate"/>
						</xsl:when>

						<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
							<xsl:value-of select="$NTANT div $varFXRate"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="''"/>
						</xsl:otherwise>
				...
				<ThirdPartyFlatFileDetail
							...
							ExecBkr="{$CPVar}" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{translate(FullSecurityName,',','')}" TDate="{TradeDate}" SDate="{SettlementDate}"
							SCCY="{SettlCurrency}"	ExCode="" qty="{AllocatedQty}" Price="{format-number($AveragePrice,'#.0000000')}" type="G" prin="{format-number($TaxlotGrsAmt,'#.0000000')}"
							comm="{format-number($Commission1,'#.0000000')}" comtype="F"
							Taxfees="{format-number($TAXFEES,'#.0000000')}" 
							Tax2="0"
							feesind="F" interest="0" interestindicator="" netamount="{format-number($AMNT,'#.0000000')}" hsyind="N" custbkr="MSCO" mmgr="" bookid=""
