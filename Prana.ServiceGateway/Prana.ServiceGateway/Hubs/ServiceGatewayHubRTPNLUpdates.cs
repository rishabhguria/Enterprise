...
{
    [SignalRHub]
    public class ServiceGatewayHubRTPNLUpdates : Hub
    {
        ...
        public override Task OnConnectedAsync()
        {
            try
            {
                ...
                {
                    HubClientConnectionManagerRTPNLUpdates.AddConnection(userId, Context.ConnectionId);
                }
            }
            catch (Exception ex)
            {
        ...
        public override Task OnDisconnectedAsync(Exception exception)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(userId))
                {
                    HubClientConnectionManagerRTPNLUpdates.RemoveConnection(userId, Context.ConnectionId);
                }
            }
            catch (Exception ex)
            {
        ...
        public ServiceGatewayHubRTPNLUpdates(ILogger<ServiceGatewayHub> logger)
        {
            this.logger = logger;
        }
