...
{
    public class CommissionBulkChangeOnGroups : AllocationUIMap, ITestStep
    {
        /// <summary>
        /// Runs the test.
        /// </summary>
        ...
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                OpenCommissionAtributeBulkChangeOnGroups();
                InputChanges(testData, sheetIndexToName);
                AllocationGrids.Click(MouseButtons.Left);
            }
            catch (Exception ex)
            {
                SamsaraHelperClass.CaptureMyScreen("", ApplicationArguments.TestCaseToBeRun, "CommissionBulkChangeOnGroups");
                _res.IsPassed = false;
        ...
        /// </summary>
        private void OpenCommissionAtributeBulkChangeOnGroups()
        {
            try
            {
                OpenAllocation();
            ...
            try
            {
                ...
                else if (dr[TestDataConstants.COL_USE_SELECT_SPECIFY].ToString().Equals("Specify"))
                {
                    ...
                    foreach (DataColumn col in test.Columns)
                    {
                            ...
                            if (counter < 1 || counter % 2 != 0)    //For cliking on check box except value
                            {
                                counter++;
                                continue;
                            }
                            else if (!dr[counter].ToString().Equals(String.Empty) && !dr[counter + 1].ToString().Equals(String.Empty))
                            {
                             ...
                             //   Wait(2000);
                                ...
                                ExtentionMethods.UpdateCellValueConditions(dr[counter + 1].ToString(), string.Empty);
                                matchedindex++;
                                comboindex++;
                                counter++;
                            }
