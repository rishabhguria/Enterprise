...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-after(substring-after(COL2,' '),' '),1,1),P) or contains(substring(substring-after(substring-after(COL2,' '),' '),1,1),C)">
			...
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring-after(substring-before(COL2,'/'),' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring-before(substring-after(substring-after(COL2,'/'),'/'),' ')"/>
			</xsl:variable>

			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(substring-after(COL2,'/'),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after(COL2,'/'),' '),2) ,'#.00')"/>
			</xsl:variable>
                ...
                <xsl:with-param name="varMonth" select="number(substring-before(COL26,'/'))"/>
				  ...
				  <xsl:value-of select="document('../ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name = $PB_NAME]/SymbolData[@PBCode = $varPBCode and @YellowFlag = $varYellowFlag]/@UnderlyingCode"/>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="contains(COL23,'Option') and (contains(COL29,'') or contains(COL29,'*'))">
									<xsl:value-of select="'EquityOptions'"/>
								</xsl:when>
								<!--<xsl:when test="string-length(COL29)&gt;20">
						...
						<Symbol>
							...
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOptions'">
                 ...
                 c
						...
						<FundName>
							<xsl:choose>
								...
								<xsl:when test="contains(COL23,'Option') and contains(COL16,'FIM2')">
						...
						<xsl:variable name="PB_STRATEGY" select="concat(COL6,',',COL7)"/>
