	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Cash) and (normalize-space(COL1)='wd' or normalize-space(COL1)='dp')">
					...
					<PositionMaster>
						...
						<JournalEntries>
							<xsl:choose>
								<xsl:when test="normalize-space(COL1)='wd'">
									<xsl:value-of select="concat('CASH_WDL', ':' , $AbsCash , '|' , 'Cash', ':' , $AbsCash)"/>
								</xsl:when>
								<xsl:when test="normalize-space(COL1)='dp'">
