...
{
    internal class FutureOptionActivityGenerator : TradingActivityGenerator
    {
        /// <summary>
                ...
                switch (CashTransactionSource)
                {
                    ...
                    case CashTransactionType.TradeImport:
                        ...
                        if (t.LongOrShort.Equals(PositionTag.Long))
                        {
                            ...
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.FutureOptionLongAddition.ToString());
                            }
                            else if (t.TransactionType == Convert.ToString(TradingTransactionType.LongWithdrawal))
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.FutureOptionLongWithdrawal.ToString());
                            }
                            else
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.FutureOptionL.ToString());
                            }
                        }
                        else
                        {
                            ...
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.FutureOptionShortAddition.ToString());
                            }
                            else if (t.TransactionType == Convert.ToString(TradingTransactionType.ShortWithdrawal))
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.FutureOptionShortWithdrawal.ToString());
                            }
                            else
                            {
                                cashActivity.ActivityType = CachedDataManager.GetActivityTypeWithAcronym(Activities.FutureOptionS.ToString());
                            }
                        }
