	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and COL7!='Cash'">
					<PositionMaster>
            ...
            <xsl:variable name="varCOLP">
              <xsl:call-template name="Translate">
                <xsl:with-param name="Number" select="translate(COL16,'$','')"/>
              </xsl:call-template>
            </xsl:variable>
            <xsl:variable name="varCOLQ">
              <xsl:call-template name="Translate">
                <xsl:with-param name="Number" select="translate(COL17,'$','')"/>
              </xsl:call-template>
            </xsl:variable>
            <xsl:variable name="varCOLR">
              <xsl:call-template name="Translate">
                <xsl:with-param name="Number" select="translate(COL18,'$','')"/>
              </xsl:call-template>
            </xsl:variable>
            <xsl:variable name="varCOLS">
              <xsl:call-template name="Translate">
                <xsl:with-param name="Number" select="translate(COL19,'$','')"/>
              </xsl:call-template>
            </xsl:variable>
            <xsl:variable name="varCOLT">
              <xsl:call-template name="Translate">
                <xsl:with-param name="Number" select="translate(COL20,'$','')"/>
              </xsl:call-template>
            </xsl:variable>
            ...
            <ClearingBrokerFees>
              <xsl:choose>
                <xsl:when test="$Asset='EquityOption'">
                  <xsl:value-of select ="$varCOLP*$varCOLQ*100 - $varCOLR - $varCOLS + $varCOLT"/>
                </xsl:when>
                <xsl:when test ="$Asset='Equity'">
                  <xsl:value-of select ="$varCOLP*$varCOLQ - $varCOLR - $varCOLS + $varCOLT"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select ="0"/>
                </xsl:otherwise>
