...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="varMonthCode">
	...
	<!--LIN    MAR 15 2024   465.000 C-->
	<xsl:template name="Option">
		<xsl:param name="Symbol"/>
		<xsl:variable name="UnderlyingSymbol">
			<xsl:value-of select="substring-before(normalize-space(COL66),' ')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryDay">
			<xsl:value-of select="substring-before(substring-after(substring-after(normalize-space(COL66),' '),' '),' ')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring-before(substring-after(normalize-space(COL66),' '),' ')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL66),' '),' '),' '),3,2)"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL66),' '),' '),' '),' '),' ')"/>
		</xsl:variable>
		<xsl:variable name="StrikePricebeforeDecimal">
			<xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after(normalize-space(COL66),' '),' '),' '),' '),'.')"/>
		</xsl:variable>
		<xsl:variable name="StrikePriceafterDecimal">
			<xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL66),' '),' '),' '),' '),'.'),' ')"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(concat($StrikePricebeforeDecimal,'.',$StrikePriceafterDecimal),'##.00')"/>
		</xsl:variable>
					...
					<PositionMaster>
						...
						<xsl:variable name="varOption">
							<xsl:call-template name="Option">
								<xsl:with-param name="Symbol" select="COL66"/>
							</xsl:call-template>
						</xsl:variable>
		                ...
		                <xsl:variable name="varExpiryMon">
			                <xsl:call-template name="varMonthCode">
			                	<xsl:with-param name="Month" select="substring-before(substring-after(normalize-space(COL66),' '),' ')"/>
			                </xsl:call-template>
		                </xsl:variable>
						
						<xsl:variable name="varExpiryMonth">
			        		<xsl:choose>
			        			<xsl:when test="string-length($varExpiryMon)=2">
			        				<xsl:value-of select="$varExpiryMon"/>
			        			</xsl:when>
			        			<xsl:otherwise>
			        				<xsl:value-of select="concat('0',$varExpiryMon)"/>
			        			</xsl:otherwise>
			        		</xsl:choose>
			        	</xsl:variable>
						
		                <xsl:variable name="varExpiryYear">
		                	<xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL66),' '),' '),' '),1,4)"/>
		                </xsl:variable>
