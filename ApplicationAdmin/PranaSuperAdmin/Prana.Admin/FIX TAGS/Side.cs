...
namespace Nirvana.Admin
{
	...
	public class Side : System.Windows.Forms.Form
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(Side));
			...
			Infragistics.Win.UltraWinGrid.UltraGridColumn ultraGridColumn6 = new Infragistics.Win.UltraWinGrid.UltraGridColumn("Name", 5, null, 0, Infragistics.Win.UltraWinGrid.SortIndicator.Ascending, false);
		...
		{	
			...
			Nirvana.Admin.BLL.Side side = new Nirvana.Admin.BLL.Side();
			...
			if(newSideID == -1)
			{
				errorProvider1.SetError(btnSave, "Side Already Exists");
			}
			...
			private void btnDelete_Click(object sender, System.EventArgs e)
			{
				try
				{
					...
					if(grdSide.Rows.Count>0)
					{
						...
						if(MessageBox.Show(this, "Do you want to delete this Side?", "Nirvana Alert", MessageBoxButtons.YesNo) == DialogResult.Yes)
						{
							...
							bool chkVarraible = OrderManager.DeleteSide(sideID, false);
							if(!(chkVarraible))
							{
								MessageBox.Show(this, "Side is referenced in CounterpartyVenue/Compliance.\n Please remove references first to delete it.", "Nirvana Alert");
							}
							else
							{
								BindSideGrid();
					...
					{
					string formattedInfo = ex.StackTrace.ToString();
					Logger.Write(formattedInfo, Common.LOG_CATEGORY_EXCEPTION, 1, 1, Severity.Error, FORM_NAME);
					AppMessageExceptionHandler appMessageExceptionHandler = new AppMessageExceptionHandler();
				appMessageExceptionHandler.HandleException(new Exception(Common.ERROR_STATEMENT), Common.POLICY_GLOBAL, System.Guid.NewGuid());
					}
				#endregion
