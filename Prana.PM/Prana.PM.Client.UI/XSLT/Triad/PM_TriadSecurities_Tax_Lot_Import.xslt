	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and not(contains(COL10,'$Cash'))">
					...
					<PositionMaster>
						...
						<xsl:variable name="Costbasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="($COL18 div $COL14) div $COL17"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="MarkPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="($COL23 div $COL14) div $COL17"/>
							</xsl:call-template>
						</xsl:variable>
						<MarkPrice>
							<xsl:choose>
