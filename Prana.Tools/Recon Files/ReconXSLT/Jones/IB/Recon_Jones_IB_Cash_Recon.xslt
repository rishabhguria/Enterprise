			...
			<xsl:for-each select ="//Comparision">
				<xsl:variable name="Cash">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="COL172"/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:if test="number($Cash)">
					<PositionMaster>
						...
						<xsl:variable name="PB_FUND_NAME" select="COL169"/>
						...
						<CashValueLocal>
							<xsl:choose>
								<xsl:when test="$CashValueLocal &gt; 0">
									<xsl:value-of select="$CashValueLocal"/>
								</xsl:when>
								<xsl:when test="$CashValueLocal &lt; 0">
									<xsl:value-of select="$CashValueLocal * (1)"/>
								</xsl:when>
								<xsl:otherwise>
