...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail">				
				<!--<xsl:sort select="FundName" data-type="number" order="ascending"/>-->
				<!--<xsl:sort select="FundName" data-type="text" order="ascending"/>-->
	...
	<xsl:template name="TaxLotIDBuilderSwap">
		...
		<!--<Group 
			PortfolioCode="{$varClientAccount1}" TranCode="{$varTransactionCode}" Comment="" SecType="{$varSecType}"
			...
			CloseMeth="{$varClosingMethod}" VersusDate="" SourceType="{$varSourceType1}" SourceSymbol="{$varSourceSymbol1}" TradeDateFXRate="" SettleDateFXRate=""
	...
	<xsl:template name="TaxLotIDBuilderNonSwaps">
		...
		<xsl:variable name="varClientAccount">
			<xsl:choose>
				<xsl:when test ="AccountName = '1109'">
		...
		<xsl:variable name ="varSybmolLower">
			<xsl:value-of select="translate(Symbol,$vUppercaseChars_CONST,$vLowercaseChars_CONST)"/>
		</xsl:variable>

		<xsl:variable name ="varSecurity">			
			<xsl:choose>
				<xsl:when test="$varSybmolLower = 'amp' or $varSybmolLower = 't' or $varSybmolLower = 'adp' or $varSybmolLower = 'bk' or $varSybmolLower = 'lmca' or $varSybmolLower = 'msg' or $varSybmolLower = 'cc' or $varSybmolLower = 'yum' or $varSybmolLower = 'wm'">
					<xsl:value-of select="concat($varSybmolLower,'_')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select ="$varSybmolLower"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name ="varBroker">
			<xsl:choose>
				...
				<xsl:when test ="string-length(CounterParty) &lt; 4">
					<xsl:value-of select="concat(translate(CounterParty,$vUppercaseChars_CONST,$vLowercaseChars_CONST),'')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
		...
		<xsl:variable name="varLotLocation">
			<xsl:choose>
				<xsl:when test="Side='Buy' or Side='Buy to Open'">
		...
		<xsl:variable name="PB_NAME" select="'AXYS'"/>
		...
		<!--ExchangeFee="{format-number(StampDuty + TransactionLevy, '###.00')}"-->
		<Group 
			PortfolioCode="{$Fund}" TranCode="{$varTransactionCode}" Comment="" SecType="{$varSecType}"
			SecuritySymbol="{$varSecurity}" TradeDate="{translate(TradeDate,'/','')}" SettleDate="{translate(SettlementDate,'/','')}" OriginalCostDate="" Quantity="{AllocatedQty}"
			CloseMeth="{$varClosingMethod}" VersusDate="" SourceType="caus" SourceSymbol="cash" TradeDateFXRate="" SettleDateFXRate=""
			OriginalFXRate="" MarkToMarket="{$varM2M}" TradeAmount="{$varNetAmount2}" OriginalCost="" Comment1="" WithholdingTax="" Exchange="{$varExchange}"
			ExchangeFee="{$varExchangeFee}" commission="{format-number(CommissionCharged, '0.####')}" Broker="{$varBroker}" ImpliedComm="n"
			OtherFees="{format-number($varOthFee,'#.00')}" CommPurpose="" Pledge="n" LotLocation="{$varLotLocation}" DestPledge="" DestLotLocation="" OriginalFace="" YieldOnCost=""
			DurationOnCost="" UserDef1="" UserDef2="" UserDef3="" TranID="" IPCounter="" Repl="" Source="1" Comment2="" OmniAcct="" Recon="n"
			Post="y" LabelName="" LabelDefinition="" LabelDefinition_Date="" LabelDefinition_String="" Comment3="" RecordDate="" ReclaimAmount=""
			Strategy="" Comment4="" IncomeAccount="" AccrualAccount="" DivAccrualMethod="" PerfContributionOrWithdrawal=""
