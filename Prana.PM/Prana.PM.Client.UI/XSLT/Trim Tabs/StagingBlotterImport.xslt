	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="$varSide != 'Side' and translate(normalize-space(COL1),'&quot;','')!='USD' and translate(normalize-space(COL1),'&quot;','')!='CASH'">
					<PositionMaster>
						<OrderSideTagValue>
							<xsl:choose>
								<xsl:when test="$varSide='Buy' or $varSide='BUY'">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								
								<xsl:when test="$varSide='Sell' or $varSide='SELL'">
						...
						<CounterPartyName>
							<xsl:choose>
								...
								<xsl:when test="$varCounterPartyName ='CANT-F'">
									<xsl:value-of select="'CANT-F'"/>
								</xsl:when>		
								<xsl:when test="$varCounterPartyName ='BBG'">
									<xsl:value-of select="'BBG'"/>
								</xsl:when>
							</xsl:choose>
						...
						<xsl:variable name ="varInternational">
							<xsl:value-of select ="translate(COL10,'&quot;','')"/>
						</xsl:variable>
						
						<TIF>
							<xsl:choose>
								<xsl:when test ="$varInternational = 'GTC'">
										<xsl:value-of select="1"/>
								</xsl:when>
						...
						<Level1ID>
							<xsl:choose>
								...
								<xsl:when test ="$varFund = 'TTAC'">
									<xsl:value-of select="1"/>
								</xsl:when>

								<xsl:when test ="$varFund = 'JP'">
									<xsl:value-of select="2"/>
								</xsl:when>

								<xsl:when test ="$varFund = 'TTAI'">
									<xsl:value-of select="3"/>
								</xsl:when>
