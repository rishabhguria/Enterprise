			...
			<xsl:for-each select="//Comparision">
				...
				<xsl:variable name="varDividend">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="normalize-space(COL60) - normalize-space(COL63)"/>
					</xsl:call-template>
				</xsl:variable>
				...
				<xsl:choose>
					...
					<xsl:when test="number($varDividend)">
						...
						<PositionMaster>
							...
							<xsl:variable name="varWithHolding">
								<xsl:call-template name="Translate">
							...
							<DividendBase>
								<xsl:choose>
									<xsl:when test="$varWithHolding !=0">
										<xsl:value-of select="$varWithHolding"/>
									</xsl:when>										
									<xsl:otherwise>
