...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
 ...
 <BrokerCode>
   ...
   <xsl:for-each select="ThirdPartyFlatFileDetail[(Asset='Equity') and CurrencySymbol!='USD' and CounterParty!='VCGO' and CounterParty!='VCLT']"> 
          ...
          <COMMISSION>
           ...
           <BrokerCode>
            <xsl:choose>
			  ...
			  <xsl:when test="CounterParty='CSFB' and MEMOFIELD='AL@C' and AccountName='Multi Strat Leucadia Kathmandu AVML'">
                <xsl:value-of select="'AVML1209'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='UBSB' and MEMOFIELD='AL@C' and AccountName='Multi Strat Leucadia Kathmandu AVML'">
                <xsl:value-of select="'AVML1209'"/>
              </xsl:when>
              <xsl:when test="CounterParty='UBS' and MEMOFIELD='AL@C' and AccountName='Multi Strat Leucadia Kathmandu AVML'">
                <xsl:value-of select="'AVML1209'"/>
              </xsl:when>			  
			  <xsl:when test="CounterParty='CSFB' and MEMOFIELD='C@N@' and AccountName='A45C1209'">
                <xsl:value-of select="'039143037'"/>
              </xsl:when>
			   <xsl:when test="CounterParty='UBSB' and MEMOFIELD='C@N@' and AccountName='A45C1209'">
                <xsl:value-of select="'039159686'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='UBS' and MEMOFIELD='C@N@' and AccountName='A45C1209'">
                <xsl:value-of select="'039159686'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='BERN' and MEMOFIELD='C@N@' and AccountName='A45C1209'">
                <xsl:value-of select="'BETG'"/>
              </xsl:when>
			 ...
			 <xsl:when test="MEMOFIELD!='AL@C' and CounterParty='MZHO'">
	          ...
	          <xsl:when test="MEMOFIELD!='AL@C' and CounterParty='RBCM'">
                <xsl:value-of select="'039810999'"/>
              </xsl:when>
			  ...
			  <xsl:when test="MEMOFIELD!='AL@C' and CounterParty='PIPR'">
			   ...
			   <xsl:when test="MEMOFIELD!='AL@C' and CounterParty='OGRU'">
                <xsl:value-of select="'039152012'"/>
              </xsl:when>
			  
			  <xsl:when test="CounterParty='JEFF'">
                <xsl:value-of select="'JCAA'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='JPMS'">
                <xsl:value-of select="'JMAB'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='NEDO'">
                <xsl:value-of select="'NEED'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='COWN' and AccountName='AVKP1209'">
                <xsl:value-of select="'COWR'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='COWN' and AccountName='AVKR1209'">
                <xsl:value-of select="'COWR'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='COWN' and AccountName='AVKT1209'">
                <xsl:value-of select="'COWR'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='COWN' and AccountName='AVKN1209'">
                <xsl:value-of select="'COWR'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='COWN' and AccountName='AVKV1209'">
                <xsl:value-of select="'COWR'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='COWN' and AccountName='AVJ71209'">
                <xsl:value-of select="'COWR'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='MLCO'">
                <xsl:value-of select="'CMAD'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='Canaccord'">
                <xsl:value-of select="'CMAD'"/>
              </xsl:when>
