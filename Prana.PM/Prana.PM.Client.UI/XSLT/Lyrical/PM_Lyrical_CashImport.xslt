			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:variable name="varCashValueLocal">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="substring(COL1,79,15)"/>
					</xsl:call-template>
				</xsl:variable>
				
				<xsl:if test ="number($varCashValueLocal) and contains(substring(COL1,136,4),'JPMC')='true'">
					...
					<PositionMaster>
						...
						<xsl:variable name = "PB_FUND_NAME">
							<xsl:value-of select="substring(COL1,1,12)"/>
						</xsl:variable>
						...
						<xsl:variable name="CashValueLocal">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="format-number(substring(COL1,79,15),'#.##')"/>
							</xsl:call-template>
						</xsl:variable>
