...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs"
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<!--<xsl:for-each select="ThirdPartyFlatFileDetail[AccountName='Bristol County Retirement System' or AccountName='DCGT FBO CCIPS LYRICAL ASSET: PRQD' or AccountName='The Riverside Church: 01AV' or AccountName='SSCSIL SIG LYRICAL FUND']">-->
			<xsl:for-each select="ThirdPartyFlatFileDetail">
				<ThirdPartyFlatFileDetail>
					...
					<xsl:variable name ="varQty">
						<xsl:value-of select="my:RoundOff(AveragePrice * AllocatedQty)"/>
					</xsl:variable>					
					<GROSS>
						<xsl:value-of select="format-number(AveragePrice * AllocatedQty,'0.##')"/>
					</GROSS>
					...
					<AccountNumber>
						<!--<xsl:choose>
							<xsl:when test="FundName='Bristol County Retirement System'">
								--><!--<xsl:value-of select="'12570901'"/>--><!--
								<xsl:value-of select="concat(concat('=&quot;','ERM1'),'&quot;')"/>
							</xsl:when>

							<xsl:when test="FundName='DCGT FBO CCIPS LYRICAL ASSET: PRQD'">
								--><!--<xsl:value-of select="'14462208'"/>--><!--
								<xsl:value-of select="concat(concat('=&quot;','PRQD'),'&quot;')"/>
							</xsl:when>
