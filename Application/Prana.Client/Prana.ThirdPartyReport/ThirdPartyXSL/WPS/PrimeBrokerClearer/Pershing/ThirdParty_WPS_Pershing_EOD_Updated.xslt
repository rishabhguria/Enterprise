...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
  ...
  <xsl:template name="TaxLotIDBuilder">
    ...
    <!--<xsl:variable name="LOCALREF">
        <xsl:value-of select="concat('T8A',substring(substring-before(TradeDate,'/'),2,1),substring(PBUniqueID,string-length(PBUniqueID)-3),substring(CompanyFundID,string-length(CompanyFundID)-1))"/>
    </xsl:variable>-->
	  <xsl:variable name="LOCALREF">
		  <!--<xsl:value-of select="concat('T8A',substring(EntityID,string-length(EntityID)-6,string-length(EntityID)))"/>-->
		  <xsl:value-of select ="concat('T8A','0',PBUniqueID)"/>
	  </xsl:variable>
    ...
    <xsl:variable name="DESCRIPTION1">
		<xsl:choose>
			<xsl:when test="CounterParty = 'JPGA'">
				<xsl:value-of select="'PB JPMS'"/>
			</xsl:when>
			<xsl:when test="CounterParty = 'CGAU'">
				<xsl:value-of select="'PB SBSH'"/>
			</xsl:when>
			<xsl:otherwise>
    ...
    <xsl:variable name="SECACCOUNT">
          ...
          <xsl:value-of select="concat('3DT','904191','0')"/>
    ...
    <xsl:variable name="INSTTYP">
	  ...
	  <xsl:variable name="CLRAGNTCD">
		  <xsl:choose>
			  <xsl:when test="CounterParty = 'JPGA'">
				  <xsl:value-of select="'JPMS'"/>
			  </xsl:when>
			  <xsl:when test="CounterParty = 'CGAU'">
				  <xsl:value-of select="'SBSH'"/>
			  </xsl:when>
			  <xsl:otherwise>
	  ...
	  <Group
		  ...
		  TIRSEQ="" SECIDTYPE="{$SECIDTYPE}" SECURITYID="{$SECURITYID}" DESCRIPTION1="{$DESCRIPTION1}" DESCRIPTION2=""
		  DESCRIPTION3="" DESCRIPTION4="" TRADEDATE="{$TRADEDATE}" SETLDATE="{$SETLDATE}" QUANTITY="{$QtySum}" QUANTITYDESC=""
		  NETMONEY=""	CASHACCOUNT="{$CashAccount}" SECACCOUNT="{$SECACCOUNT}" 	TRADECURRID="USD"	SETLCURRID="USD" BSIND="{$Side}"
		  INSTTYP="{$INSTTYP}" PRICE="{format-number(AveragePrice,'####.#######')}"
		  COMMISSION="{format-number($CommissionSum,'#.######')}" STAMPTAX=""
		  LOCALCHGS="" INTEREST="" PRINCIPAL="" SECFEE="" EXECBROKER="" BROKEROS="" TRAILERCD1="" TRAILERCD2=""
		  TRAILERCD3="" BLOTTERCD="49" CLRNGHSE="Y" CLRAGNTCD="{$CLRAGNTCD}" CLRAGNT1="" CLRAGNT2="" CLRAGNT3="" CLRAGNT4="" CNTRPRTYCD="" CNTRPTY1="" CNTRPTY2="" CNTRPTY3="" CNTRPTY4=""
		  ...
		  EntityID="{EntityID}" TaxLotState="{TaxLotState}" RowHeader="false" FileHeader="true" FileFooter="true">
		  ...
		  <xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
			  ...
			  <xsl:variable name="LOCALREF_Alloc">
				  <!--<xsl:value-of select="concat('T8A',substring(substring-after(substring-after(TradeDate,'/'),'/'),4,1),substring-before(TradeDate,'/'),substring-before(substring-after(TradeDate,'/'),'/'),substring(CompanyFundID,string-length(CompanyFundID)-1))"/>-->
				  <xsl:value-of select="concat('T8A',substring(substring-before(TradeDate,'/'),2,1),substring(PBUniqueID,string-length(PBUniqueID)-3),substring(CompanyAccountID,string-length(CompanyAccountID)-1))"/>
			  </xsl:variable>

			  <xsl:variable name="varTIRORDERID">
				  <xsl:value-of select="concat('BLK',PBUniqueID)"/>
			  </xsl:variable>
			  ...
			  <xsl:variable name="Description3">
				  <xsl:choose>
					  <xsl:when test="Side='Sell short'">
						  <xsl:value-of select="concat('S',$CD,PBUniqueID)"/>
					  </xsl:when>
					  <xsl:otherwise>
						  <xsl:value-of select="''"/>
					  </xsl:otherwise>
        ...
        RowHeader="false" LOCALREF = "{$LOCALREF_Alloc}" CFID="{$LOCALREF_Alloc}" ROUTECD="PSHG" TIRORDERID="{$varTIRORDERID}" TIRPIECE=""
        TIRSEQ="" SECIDTYPE="{$SECIDTYPE}" SECURITYID="{$SECURITYID}" DESCRIPTION1="{$DESCRIPTION1}" DESCRIPTION2=""
        DESCRIPTION3="{$Description3}" DESCRIPTION4="" TRADEDATE="{$TRADEDATE}" SETLDATE="{$SETLDATE}" QUANTITY="{AllocatedQty}" QUANTITYDESC=""
        NETMONEY=""	CASHACCOUNT="{$CashAccount}" SECACCOUNT="{$SecAccount}" 	TRADECURRID="USD"	SETLCURRID="USD" BSIND="{$BSIND}"
        INSTTYP="{$INSTTYP}" PRICE="{format-number(AveragePrice,'####.#######')}"
        COMMISSION="{format-number(CommissionCharged,'#.######')}" STAMPTAX=""
        LOCALCHGS="" INTEREST="" PRINCIPAL="" SECFEE="" EXECBROKER="" BROKEROS="" TRAILERCD1="" TRAILERCD2=""
        TRAILERCD3="" BLOTTERCD="49" CLRNGHSE="Y" CLRAGNTCD="{$CLRAGNTCD}" CLRAGNT1="" CLRAGNT2="" CLRAGNT3="" CLRAGNT4="" CNTRPRTYCD="" CNTRPTY1="" CNTRPTY2="" CNTRPTY3="" CNTRPTY4=""
        INSTRUCT="" CEDELAKV="" ORIGLOCALREF="{$LOCALREF_Alloc}" NOTES="" FILLER1="" FILLER2="" RR="" SETLCOUNTRYCD="US" INSTRUMENTTYPE="" COMMISSIONRATE="" COMPANYNO="" Filler3="" Filler4=""
        Filler5="" Filler6="" Filler7="" GPF2IDCode="" GPF2Amount="" GPF2CurrencyCode="" GPF2AddSubtract="" GPF3IDCode="" GPF3Amount="" GPF3CurrencyCode="" GPF3AddSubtract=""
        GPF4IDCode="" GPF4Amount="" GPF4CurrencyCode="" GPF4AddSubtract="" GPF5IDCode="" GPF5Amount="" GPF5CurrencyCode="" GPF5AddSubtract=""
