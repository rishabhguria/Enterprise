	...
	<xsl:template name="MonthCode">
		...
		<xsl:if test="$PutOrCall='C'">
			<xsl:choose>
				...
				<xsl:when test="$Month='10' or $Month='10'">
					<xsl:value-of select="'J'"/>
				</xsl:when>
				<xsl:when test="$Month='11' or $Month='11'">
					<xsl:value-of select="'K'"/>
				</xsl:when>
				<xsl:when test="$Month='12' or $Month='12'">
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($varMarkPrice) and COL10 = 'USD'">
					...
					<PositionMaster>
						...
						<xsl:variable name = "varDay">
							<xsl:value-of select ="substring(normalize-space(COL22),7,2)"/>
						</xsl:variable>

						<xsl:variable name = "varMonth">
							<xsl:value-of select ="substring(normalize-space(COL22),5,2)"/>
						</xsl:variable>

						<xsl:variable name = "varYear">
							<xsl:value-of select ="substring(normalize-space(COL22),1,4)"/>
						</xsl:variable>
