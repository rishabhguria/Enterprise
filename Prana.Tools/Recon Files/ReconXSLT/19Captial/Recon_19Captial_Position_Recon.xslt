...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="(contains(COL42,'PUT') and contains(COL40,'Option')) or (contains(COL42,'CAL') and contains(COL40,'Option'))">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:choose>
					<xsl:when test="contains(COL66,' ')">
	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and not(contains(COL40,'Futures')) and not(contains(COL40,'Currency'))">
					<PositionMaster>

						<xsl:variable name="PB_NAME">
							<xsl:value-of select="'19Captial'"/>
						</xsl:variable>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="(contains(COL42,'PUT') and contains(COL40,'Option')) or (contains(COL42,'CAL') and contains(COL40,'Option'))">
									<xsl:value-of select ="'EquityOption'"/>
								</xsl:when>
								<xsl:when test="contains(COL40,'Equities')">
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='Equities'">
									<xsl:value-of select="substring-before(COL66,' ')"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<xsl:variable name="Side" select="COL85"/>
						...
						<xsl:variable name="Marketvalue">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL111"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name ="Date" select="COL87"/>
						<TradeDate>
