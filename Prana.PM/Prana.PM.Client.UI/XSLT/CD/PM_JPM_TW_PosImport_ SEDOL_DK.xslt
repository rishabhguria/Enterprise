...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
  ...
  <xsl:template name="Symbol">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$varCurrency = 'HKD' or $varCurrency = 'NOK'">
        ...
        <xsl:if test ="number(COL21)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_Symbol_NAME">
              <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='DB']/SymbolData[@CompanyName=$PB_Symbol_NAME]/@PranaSymbol"/>
            </xsl:variable>
                ...
                <Symbol>
                  <xsl:choose>
                    <xsl:when test="$PRANA_Symbol_NAME = ''">
                      <xsl:choose>
                        <xsl:when test="string-length(normalize-space(COL12))=4">
                          <xsl:value-of select="concat(COL12,$varSuffix)"/>
                        </xsl:when>
                        <xsl:when test="string-length(normalize-space(COL12))=3">
                          <xsl:value-of select="concat('0',COL12,$varSuffix)"/>
                        </xsl:when>
                        <xsl:when test="string-length(normalize-space(COL12))=2">
