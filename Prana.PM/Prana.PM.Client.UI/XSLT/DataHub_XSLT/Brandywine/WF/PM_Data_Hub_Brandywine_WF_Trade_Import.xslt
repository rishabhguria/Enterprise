	...
	<xsl:template match="/">
        ...
        <xsl:variable name="varAssetSpot">
          <xsl:value-of select="substring(COL16,1,4)"/>
				...
				<xsl:choose>
				<xsl:when test="number($Position) and not(contains(COL30,'CASH EQUIVALENTS'))and COL62!='DIVIDEND' and not(contains(COL19,'STATE STREET BANK &amp; TRUST CO.'))">
                ...
                <xsl:choose>
                  <xsl:when test="$Asset='Spot'">
					...
					<xsl:when test="contains(COL33,'FOREIGN EQUITIES')">
						<xsl:value-of select ="substring-before(substring-after(COL51,'SHARES AT '),' ')"/>
					</xsl:when>
                  <xsl:otherwise>
                    <xsl:choose>
                      ...
                      <xsl:otherwise>
                        <xsl:value-of select ="0"/>
							...
							<xsl:variable name="varCommissionAndFees">
								<xsl:choose>
									<xsl:when test="contains(COL49,'FEES')">
										<xsl:value-of select="substring-before(substring-after(substring-after(COL49,' '),' '),' ')"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="substring-after(COL49,'$')"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:variable>
              ...
              <xsl:variable name="varCurrency">
                <xsl:value-of select="substring-before(substring-before(substring-after(COL51,'SHARES AT '),' '),' ')"/>
              </xsl:variable>
              ...
              <xsl:variable name="varCommissionNonUSD">
                <xsl:choose>
                  <xsl:when test="$varCurrency='EUR' or $varCurrency='AUD' or $varCurrency='NZD' or $varCurrency='GBP'">
                    <xsl:value-of select="$Commission div $varPriceNonUSD"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="$Commission * $varPriceNonUSD"/>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:variable>
                ...
                <xsl:choose>
                  <xsl:when test="contains(COL33,'FOREIGN EQUITIES')">
                    <xsl:choose>
                      <xsl:when test="$varCommissionNonUSD &gt; 0">
                        <xsl:value-of select="$varCommissionNonUSD"/>
                      </xsl:when>
                      <xsl:when test="$varCommissionNonUSD &lt; 0">
                        <xsl:value-of select="$varCommissionNonUSD * (-1)"/>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="0"/>
                      </xsl:otherwise>
