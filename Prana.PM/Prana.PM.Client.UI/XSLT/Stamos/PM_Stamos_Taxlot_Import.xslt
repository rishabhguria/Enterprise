				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="COL7='Government Bonds' or COL7='Open End Funds' or COL7='Corporate Bonds'">
						...
						<xsl:variable name="Costbasis">
							<xsl:choose>
								<xsl:when test="$Asset='FixedIncom'">
									<xsl:value-of select="($COL20 div $COL14) div 100"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="($COL20 div $COL14)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						...
						<xsl:variable name="FXRate">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$COL29 div $COL20"/>
							</xsl:call-template>
