	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="($varTransaction_type='dv') and ($varSecurity_Type='cs' or $varSecurity_Type='mb' or $varSecurity_Type='mf'or $varSecurity_Type='eu')">
					...
					<PositionMaster>
						...
						<!--Code to know about Option call or put-->
						...
						<!--Code to set no of zeroes in int and decimal part for OSF (IDCOsymbol)-->
						...
						<xsl:choose>
							...
							<xsl:choose>
								<xsl:when  test="number(normalize-space(substring(COL1,37,11)))">
									<Dividend>
										<xsl:value-of select="number(normalize-space(substring(COL1,37,11)))"/>
									</Dividend>
								</xsl:when >
								<xsl:otherwise>
									<Dividend>
						...
						<ExDate>
							<xsl:value-of select="concat('20',substring(COL1,91,2),'/',substring(COL1,87,2),'/',substring(COL1,89,2))"/>
						</ExDate>
