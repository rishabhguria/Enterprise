        ...
        <xsl:if test="number($Cash)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_PRE">
              <xsl:choose>
			   <xsl:when test="contains($varSubAccountDes,'INTEREST') and $Cash &gt; 0">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="contains($varSubAccountDes,'Transfer') and $Cash &gt; 0">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="contains($varSubAccountDes,'INTEREST') and $Cash &lt; 0">
                  <xsl:value-of select="'Interest_Expense'"/>
                </xsl:when>
				<xsl:when test="contains($varSubAccountDes,'WIRE') and $Cash &gt; 0">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>			  
                <xsl:when test="contains($varSubAccountDes,'WIRE') and $Cash &lt; 0">
                  <xsl:value-of select="'Cash_Withdrawal'"/>
                </xsl:when>
				  <xsl:when test="contains($varSubAccountDes,'Fees') and $Cash &lt; 0">
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_POST">
              <xsl:choose>
			   ...
			   <xsl:when test="contains($varSubAccountDes,'INTEREST') and $Cash &gt; 0">
				  ...
				  <xsl:when test="contains($varSubAccountDes,'WIRE') and $Cash &gt; 0">
					  <xsl:value-of select="'Cash_Deposit'"/>
				  </xsl:when>
				   <xsl:when test="contains($varSubAccountDes,'WIRE') and $Cash &lt; 0">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
