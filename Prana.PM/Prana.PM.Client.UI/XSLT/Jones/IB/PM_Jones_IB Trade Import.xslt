			...
			<xsl:for-each select ="//PositionMaster">
				...
				<PositionMaster>
						...
						<xsl:variable name="Commission">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="translate(COL36,'$','')"/>
							</xsl:call-template>
						</xsl:variable>
						<Commission>
							<xsl:choose>
						...
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:choose>
										<xsl:when test="($Position &gt; 0) and COL40='O'">
											<xsl:value-of select="'A'"/>
										</xsl:when>
										<xsl:when test="($Position &lt; 0) and COL40='C'">
					...
					<ClearingBrokerFee>
						...
						<xsl:variable name="Date1">
							<xsl:value-of select ="concat(substring(COL28,5,2),'/',substring(COL28,7,2),'/',substring(COL28,1,4))"/>
						</xsl:variable>
