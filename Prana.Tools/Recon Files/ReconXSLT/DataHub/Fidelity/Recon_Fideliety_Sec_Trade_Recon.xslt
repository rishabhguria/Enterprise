  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:choose>
          <xsl:when test="number($Quantity) and normalize-space(COL119) !='CASH'">
            <PositionMaster>
              ...
              <xsl:variable name="AvgPrice">
                <xsl:call-template name="Translate">
                  <xsl:with-param name="Number" select="COL199 div 100000000"/>
                </xsl:call-template>
              </xsl:variable>

				<xsl:variable name="varAvgPrice">
					<xsl:choose>
						<xsl:when test="COL194 !='USD'">
              ...
              <xsl:variable name="Commission">
				...
				<xsl:variable name="varCommission">
					<xsl:choose>
						<xsl:when test="COL194 !='USD'">
							<xsl:value-of select="COL198 div 10000"/>
						</xsl:when>
						<xsl:otherwise>
              ...
              <xsl:variable name="StampDuty">
                <xsl:call-template name="Translate">
                  <xsl:with-param name="Number" select="COL196"/>
                </xsl:call-template>
              </xsl:variable>
				<xsl:variable name="varStampDuty">
					<xsl:choose>
						<xsl:when test="COL194 !='USD'">
							<xsl:value-of select="COL196 div 10000"/>
						</xsl:when>
						<xsl:otherwise>
              ...
              <xsl:variable name="GrossNotionalValue">
                <xsl:call-template name="Translate">
                  <xsl:with-param name="Number" select="COL201 div 10000"/>
                </xsl:call-template>
              </xsl:variable>
              <GrossNotionalValue>
                <xsl:choose>
              ...
              <xsl:variable name="TotalCommissionandFees">
				...
				<xsl:variable name="varTotalCommissionandFees">
					<xsl:choose>
						<xsl:when test="COL194 !='USD'">
							<xsl:value-of select="(COL196 + COL198) div 10000"/>
						</xsl:when>
						<xsl:otherwise>
