	...
	<xsl:template name="Option">
		...
		<xsl:if test="(contains(substring(substring-before(COL19,' '),1,1),'P') or contains(substring(substring-before(COL19,' '),1,1),'C')) and (COL4='48604' or COL4='48601')">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="substring-after(substring-after(substring-after(substring-after(normalize-space(COL19),' '),' '),' '),' ')"/>
			</xsl:variable>
  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($MarkPrice) and COL19 != 'DEC 15 ICUS MSCI EAFE'">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_SYMBOL_NAME">
			  ...
			  <xsl:variable name ="PRANA_ROOT_NAME">
				  <xsl:choose>
					  <xsl:when test="$PRANA_ECHANGE_NAME!=''">
						  <xsl:value-of select="document('../ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name=$PB_NAME]/SymbolData[@PBCode = $PB_ROOT_NAME and @ExchangeName=$Exchange_Name]/@UnderlyingCode"/>
					  </xsl:when>
					  <xsl:otherwise>
			  ...
			  <xsl:variable name ="FUTURE_EXCHANGE_CODE_EXCH">
				  <xsl:value-of select="document('../ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name=$PB_NAME]/SymbolData[@PBCode=$PB_ROOT_NAME and @ExchangeName = $Exchange_Name]/@ExchangeCode"/>
			  </xsl:variable>
			  ...
			  <Symbol>
				  ...
				  <xsl:choose>
					  ...
					  <xsl:when test="normalize-space(COL8)='F'">
						  <xsl:choose>
							  <xsl:when test="$FUTURE_EXCHANGE_CODE_EXCH != ''">
								  <xsl:value-of select="normalize-space(concat($Underlying,' ',$MonthYearCode,$FUTURE_EXCHANGE_CODE_EXCH))"/>
							  </xsl:when>
							  <xsl:otherwise>
