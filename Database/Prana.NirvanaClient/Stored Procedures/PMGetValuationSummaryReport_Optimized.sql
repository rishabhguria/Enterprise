...
Execution StateMent:                                                                                                                                                                                           
   EXEC [PMGetValuationSummaryReport_Optimized] 6, '04-19-2012','2,7,23','1', 'SedolSymbol',''                                                                             
****************************************************************************/                                                                                                                                                                                  
    
CREATE PROCEDURE [dbo].[PMGetValuationSummaryReport_Optimized]                                                         
(                                                                                                                                                                                          
 @companyID int,                                
 @date datetime,    
 @Fund varchar(max),     
...
--Set @Asset = '1'    
--Set @SearchBy = 'Symbol'    
--Set @SearchText = '0013-HKG'--'B5VJH76'     
    
-- Get a date < = end date where cash is non zero         
Declare @RecentDateForNonZeroCash DateTime        
Select @RecentDateForNonZeroCash = (Select dbo.[GetRecentDateForNonZeroCash](@date))      
...
Set @startingDate = (select min(AuecmodifiedDate) from PM_Taxlots        
      where taxlotopenqty <> 0 and Taxlot_pk in ( select max (Taxlot_pk) from PM_taxlots        
      where datediff (d,Auecmodifieddate,@date)>=0 group by Taxlotid))    
 ...
 G.GroupID,                                                                
 PT.PositionTag,                                                          
 Isnull(PT.FXRate,G.FXRate),                                                                  
 Isnull(PT.FXConversionMethodOperator,G.FXConversionMethodOperator),  
Currency.CurrencySymbol As TradeCurrency    
                                                                                      
from PM_Taxlots PT                                                                                               
Inner join  T_Group G on G.GroupID=PT.GroupID     
INNER JOIN #T_CompanyFunds FundIDs ON PT.FundID = FundIDs.CompanyFundID     
INNER JOIN #Temp_Asset AssetIDs ON G.AssetID = AssetIDs.AssetID    
Inner join T_Currency Currency On Currency.CurrencyID = G.CurrencyID                                                                                      
...
Create Table #Temp_VSR    
(    
Symbol Varchar(100),    
Quantity Float,    
CostPrice Float,    
TotalCommissionandFees Float,     
TotalCostPerShare Float,    
TotalCost Float,                                  
MarketValue Float,               
Gain Float,         
FundName Varchar(200),                                              
[Position Type] Varchar(20),    
CurrencySymbol Varchar(10),    
LanguageName Varchar(100),    
TotalCostInBaseCurrency   Float,                  
BaseCurrencyID Int,    
BaseCurrencyLanguageName Varchar(100),    
GroupParamaeter1 Varchar(10),    
GroupParamaeter2 Varchar(10),    
GroupParamaeter3 Varchar(10),    
MarketValueInBaseCurrency Float,    
GainInBaseCurrency Float,                         
AggregateOption Float,    
YesterdayMarketValue  Float,                      
YesterdayMarketValueInBaseCurrency Float,         
TradeDate   DateTime,                
Multiplier  Float,      
AUECID  int,        
AssetID     int,    
...
GroupParamaeter4 Varchar(10),    
I1 Float,                        
DC Float,             
RRMarkPriceCost Float,            
FXRRMarkPriceCost  Float,        
TDConversionFactorCost Float,     
TDConversionMethodCost Varchar(10),     
FXTDConversionFactorCost Float,    
FXTDConversionMethodCost Varchar(10),    
RRConversionFactorCost Float,    
RRConversionMethodCost Varchar(10),    
FXRRConversionFactorCost Float,    
FXRRConversionMethodCost Varchar(10),    
SideMultiplier Float,    
NotionalValue Float,      
SymbolCompanyName  Varchar(200),                                                                                                                                                                                                                      
  
                    
TaxlotID Varchar(50),                                        
YesterdayBusinessDate DateTime,    
FXYesterdayBusinessDate DateTime,    
CompanyFundID int,   
AssetName   Varchar(50),                                           
StrategyID  int,    
StrategyName    Varchar(200),                                       
UDATickerSymbol   Varchar(200),                                                                                                                                                                                           
UDAAssetName Varchar(200),                                                                                            
UDASecurityTypeName Varchar(200),                                                                                      
UDASectorName  Varchar(200),                                                                                          
UDASubSectorName Varchar(200),                                                                                        
UDACountryName  Varchar(200),                                                                                         
PutOrCall  Varchar(10),    
MasterStrategyName Varchar(200),                                    
...
Insert InTo #Temp_VSR                                                                      
 ...
 ISNULL(DayMark.TodayMarkPrice, 0) AS FXRRMarkPriceCost,                                 
...
CASE ISNULL(TFPVR.FXRate, 0)                                                                    
   ...
   ELSE                                                                    
  CASE                                          
   WHEN TFPVR.FXConversionMethodOperator ='M'                                      
   THEN  0                                  
   WHEN TFPVR.FXConversionMethodOperator ='D'                                      
   THEN  1                                   
...
ISNULL(RRConvFactor.ConversionMethod, 0) AS RRConversionMethodCost,                                                                    
 ...
 ISNULL(SM.UDASectorName, 'Undefined') AS UDASectorName,                                                                                  
 ISNULL(SM.UDASubSectorName, 'Undefined') AS UDASubSectorName,         
...
INNER JOIN #Temp_Asset A ON AUEC.AssetID = A.AssetID                                                                                                                    
...
--For FX spot and Forward, VsCurrencyID is same as G.CurrencyID,   
--so no need to get FX Rate on the basis of VsCurrencyIDs   
...
'' as IDCOSymbol,        
'' as IsSwapped,    
Min(C.CurrencySymbol) As TradeCurrency                         
                                                                      
   From T_DayEndBalances CFCC   
   --From PM_CompanyFundCashCurrencyValue CFCC                                                                 
   INNER JOIN T_Currency C ON CFCC.LocalCurrencyID = C.CurrencyID                                                          
   ...
   WHERE DateDiff(Day,Date,@RecentDateForNonZeroCash)=0 --And #Temp_Asset.AssetName in ('Cash')                                                                        
...
If(@SearchString <> '')    
 Begin    
  Exec  ('Select * from #Temp_VSR Where ' + @SearchBy  + ' in ' + '(' + '''' + @SearchString +  '''' + ')' )     
 End    
Else    
 Begin    
  Select * from #Temp_VSR     
 End    
...
DROP TABLE #Temp_Asset,#T_CompanyFunds ,#Temp_VSR                                                       
END