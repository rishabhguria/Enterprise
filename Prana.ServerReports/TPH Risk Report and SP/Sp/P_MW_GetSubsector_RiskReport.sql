...
Description : This Sp returns dataset of SubSectorWiseGMV for TPH Risk Management Report PRANA-10756               
Execution Statement:                           
        
Exec P_MW_GetSubsector_RiskReport '2015-10-10','1182,1183,1184','MTM_V0'        
        
*************************************************/         
CREATE Procedure [dbo].[P_MW_GetSubsector_RiskReport]              
(              
@EndDate datetime,              
@Fund varchar(max),              
...
----------------------------------------------------------------------------        
--Creating Temp Table for Storing SubSector Wise GMV        
----------------------------------------------------------------------------        
        
Create Table #TempSubSectorWiseGMV            
(              
...
Insert Into #TempSubSectorWiseGMV(UDASubSector, Side,EndingMarketValue)        
Select          
...
PNL.Side As Side,        
 ...
 Group By PNL.UDASubSector ,PNL.Side              
...
--Select *  from #TempSubSectorWiseGMV          
        
-------------calculation of PercGMV and NetPercGMV----------------------------        
        
Select         
UDASubSector,        
IsNull(SUM(ABS(EndingMarketValue))/NULLIF(@GrossMarketValue,0),0)*100 AS PercGMV,        
IsNull(SUM(EndingMarketValue)/NULLIF(@GrossMarketValue,0),0)*100 AS NetPercGMV         
From #TempSubSectorWiseGMV       
Group By UDASubSector        
        
------------Drop All temp tables        
Drop table #PNL,#ParameterPreferences,#TempSubSectorWiseGMV,#Funds,#T_CompanyFunds