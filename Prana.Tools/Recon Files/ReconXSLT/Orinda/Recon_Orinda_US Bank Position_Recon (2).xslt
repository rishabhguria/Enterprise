...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL2,'CALL') or contains(COL2,'PUT')">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after($Symbol,$UnderlyingSymbol),6)  ,'#.00')"/>
			</xsl:variable>
			...
			<!--<xsl:choose>
				...
				<xsl:otherwise>-->
			<xsl:value-of select="concat('O:',$UnderlyingSymbol,$Suffix,' ',$ExpiryYear,substring(substring-after($Symbol,$UnderlyingSymbol),5,1),$StrikePrice,'D',$Day)"/>
			<!--</xsl:otherwise>-->
			<!--
	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and contains(COL23,'CASH')!='true'">
					...
					<PositionMaster>
						...
						<xsl:variable name="Symbol" select="normalize-space(translate(COL23,'+',''))"/>
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="string-length(COL23) &gt; 20">
									<xsl:value-of select="''"/>
								</xsl:when>

								<xsl:when test="string-length(COL23) &gt; 8">
									<xsl:value-of select="''"/>
								</xsl:when>
						...
						<IDCOOptionSymbol>
							...
							<xsl:choose>
								...
								<xsl:when test="string-length(COL23) &gt; 20">
									<xsl:value-of select="concat(COL23,'U')"/>
								</xsl:when>
						...
						<xsl:variable name="Marketvalue">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL116"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name ="Date" select="COL61"/>
