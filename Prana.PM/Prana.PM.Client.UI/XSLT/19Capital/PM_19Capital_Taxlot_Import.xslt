				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name ="Asset">
							<xsl:choose>
								<xsl:when test="contains(COL5,'PUT') or contains(COL5,'CALL')">
						...
						<xsl:variable name="StrikePrice">
							<xsl:value-of select="format-number(substring(substring-after($Symbol,$UnderlyingSymbol),10) div 1000,'#.00')"/>
						...
						<xsl:variable name="Costbasis">
							<xsl:choose>
								<xsl:when test="$Asset='Option'">
									<xsl:value-of select="($COL16 div $Position ) div 100"/>
								</xsl:when>

								<xsl:otherwise>
									<xsl:value-of select="($COL16 div $Position )"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
