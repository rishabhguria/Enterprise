    ...
    <xsl:variable name="varClosingMethod">
	  ...
	  <xsl:variable name ="varAvgPrice">
		  <xsl:choose>
			  ...
			  <xsl:when test ="CounterParty = 'ENSK'">
	  ...
	  <xsl:variable name="NetAmount_Swap">
		  <xsl:choose>
			  <xsl:when test="contains(AccountName, 'Swap') != false">
				  <xsl:choose>
					  <xsl:when test="contains(Side,'Buy')">
						  <xsl:value-of select="GrossAmount + $varCommission"/>
					  </xsl:when>
	  ...
	  <Group 
        ...
        TradeCurrency="{CurrencySymbol}" SettleCurrency="{$PB_SettleCurrency}" EffectiveYield="" Commission="{$varCommission}" SECFee="{$varStampDuty}"
        OtherFees="{$varOtherBrokerFee}" AccruedInterest="{AccruedInterest}" NetTradeCash="{$NetAmount_Swap}" Counterparty="{CounterParty}"
        ...
        SpecifiedLot1OpenDate10="{$varClosedAgainstDate10}" SpecifiedLot1Price10 ="{$varClosedPrice10}"  SpecifiedLot1Qty10="{$varQ10}"
         UDASecurityType="{UDASecurityTypeName}" UDAAsset="{UDAAssetName}"
