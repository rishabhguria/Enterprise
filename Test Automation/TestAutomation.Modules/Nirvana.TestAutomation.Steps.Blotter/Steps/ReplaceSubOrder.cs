...
{
    public class ReplaceSubOrder : BlotterUIMap, ITestStep
    {
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            {
                SamsaraHelperClass.CaptureMyScreen("", ApplicationArguments.TestCaseToBeRun, "ReplaceSubOrder");
                _res.IsPassed = false;
            ...
            try
            {
                ...
                if (!DataUtilities.checkList)
                {
                    if (index < 0)
                    {
                        ...
                        throw new Exception("Trade not found during ReplaceSubOrder step. [Symbol= " + dr["Symbol"] + "], Quantity = [" + dr["Quantity"] + "] Side = [" + dr["Side"] + "]\nRecon Error: " + String.Join("\n\r", errors));
                    }
                }
                DgBlotter2.InvokeMethod("ScrollToRow", index);
                Wait(2000);
                Console.WriteLine("Click on the sub order grid row");
                
                var rowIndex = index + 1;
                //msaaObj.CachedChildren[0].CachedChildren[index + 1].CachedChildren[index].Click(MouseButtons.Right);
                try
                {
                    maingrid = msaaObj.FindDescendantByName("OrderBindingList row " + rowIndex, 4000);
                    
                }
                catch { maingrid = msaaObj.FindDescendantByName("OrderBindingList row 1", 4000);
                }
