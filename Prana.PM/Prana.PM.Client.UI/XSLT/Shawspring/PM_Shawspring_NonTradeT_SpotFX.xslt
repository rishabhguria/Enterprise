	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($varAmount) and contains(COL10,'FOREIGN CURRENCY') and COL5 !='USD'">
						...
						<JournalEntries>
							<xsl:choose>								
								<xsl:when test="$varLeadingCurrency = '1'">
									<xsl:value-of select="concat($varCurrencyName,':' , $varCash , '|','USD' ,':', $varCash)"/>
								</xsl:when>
								<xsl:when test="$varLeadingCurrency = '0'">
									<xsl:value-of select="concat('USD',':' , $varCash , '|',$varCurrencyName ,':', $varCash)"/>
								</xsl:when>
							</xsl:choose>
						</JournalEntries>
