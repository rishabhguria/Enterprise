  ...
  <xsl:template match="/">
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($NetPosition) and COL3 !='T' and COL3 !='C' and (COL2 !='1LD51129' and COL2 !='1TR40168')" >
          <PositionMaster>
            ...
            <SideTagValue>
				<xsl:choose>
					...
					<!--For option trades-->
					<xsl:when test="COL3 = 'P' and COL40 = 'BO' and (number(COL38)) ">
						<xsl:value-of select="'A'"/>
					</xsl:when>
					<xsl:when test="COL3 = 'P' and COL40 = 'BC' and (number(COL38)) ">
						<xsl:value-of select="'B'"/>
					</xsl:when>
					<xsl:when test="COL3 = 'S' and COL40 = 'SC' and (number(COL38)) ">
						<xsl:value-of select="'D'"/>
					</xsl:when>
					<xsl:when test="COL3 = 'S' and COL40 = 'SO' and (number(COL38)) ">
						<xsl:value-of select="'C'"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="''"/>
