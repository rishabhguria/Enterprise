...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
    ...
    <DocumentElement>
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:choose>
          <xsl:when test="number($Quantity) and contains(COL7,'Cash Disbursement')!='true'">
            <PositionMaster>
              ...
              <Symbol>
                <xsl:choose>
                  ...
                  <xsl:when test="string-length(normalize-space(COL4))">
                    <xsl:value-of select="''"/>
                  </xsl:when>
              ...
              <CUSIP>
                <xsl:choose>
                  ...
                  <xsl:when test="string-length(normalize-space(COL4))&gt; 7">
              ...
              <xsl:variable name="PRANA_BROKER_ID">
                <xsl:value-of select="document('../ReconMappingXml/ExecBrokerMapping.xml')/BrokerMapping/PB[@Name=$PB_NAME]/BrokerData[@MLPBroker=$PB_BROKER_NAME]/@PranaBrokerCode"/>
              </xsl:variable>
              <CounterPartyID>
                <xsl:choose>
              ...
              <xsl:variable name="AvgPrice">
                <xsl:choose>
                  <xsl:when test="COL5 =''">
                    <xsl:value-of select="$varNetNotional div $Quantity "/>
                  </xsl:when>
                  <xsl:when test="COL5 !='' or COL5 !='*'">
                    <xsl:value-of select="$varAvgPrice div $Quantity "/>
                  </xsl:when>
                </xsl:choose>
              </xsl:variable>
              ...
              <xsl:variable name="PB_COUNTER_PARTY" select="''"/>
              ...
              <Side>
                <xsl:choose>
                  ...
                  <xsl:when test ="$Side='Free Receipt (Rcpt in Kind)' or $Side='Receipt in Kind' ">
                    <xsl:value-of select ="'Buy'"/>
                  </xsl:when>
                  <xsl:when test ="$Side='Sell an Asset'">
                    <xsl:value-of select ="'Sell'"/>
                  </xsl:when>
                  <xsl:when test ="$Side='Free Delivery (Distr in kind)' ">
                    <xsl:value-of select ="'Sell'"/>
                  </xsl:when>
                  <xsl:when test ="$Side='Purchase an Asset'">
                    <xsl:value-of select ="'Buy'"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="0"/>
