	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">
				<xsl:if test="number(COL9) and contains(COL3,'%')">
					<PositionMaster>						
						...
						<xsl:variable name="varSymbol" select="COL29"/>
						...
						<xsl:variable name="varSedol" select="COL30"/>
						...
						<MaturityDate>
							<xsl:value-of select="substring-after(substring-after(normalize-space(COL3),'%'),' ')"/>
						</MaturityDate>
