...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[(AccountName = 'Sheffield Partners LP-MS'  or AccountName='Sheffield Partners LP-MS SWAP') and CounterParty!='CorpAction' and CounterParty!='Transfer' and CounterParty!='Washsale' and CounterParty!='BOXcollapse' and CounterParty!='SwapReset']">
				<!--<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[IsSwapped!='true' and AccountName='Sheffield Partners LP-MS']">-->
	...
	<xsl:template name="TaxLotIDBuilder">
			...
			<xsl:variable name="varNetamountSettl">
				<xsl:choose>
					<xsl:when test="$varFXRate=0">
						<xsl:value-of select="$GroupNetAmt1"/>
					</xsl:when>

					<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
						<xsl:value-of select="format-number($GroupNetAmt1 * $varFXRate,'##.00')"/>
					</xsl:when>

					<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
						<xsl:value-of select="format-number($GroupNetAmt1 div $varFXRate,'##.00')"/>
					</xsl:when>
		...
		<xsl:variable name="varGroupTotalFees">
			<xsl:choose>
				<xsl:when test="$varFXRate=0">
				...
				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='M'">
					<xsl:value-of select="format-number($GroupTotalFees * $varFXRate,'##.00')"/>
				</xsl:when>

				<xsl:when test="$varFXRate!=0 and FXConversionMethodOperator_Trade='D'">
					<xsl:value-of select="format-number($GroupTotalFees div $varFXRate,'##.00')"/>
				</xsl:when>
		...
		<xsl:variable name="varExachangeRate">
			<xsl:choose>
				<xsl:when test="SettlCurrency != CurrencySymbol">
					<xsl:value-of select="'1'"/>
				</xsl:when>
		...
		<Group
				TransactionType = "{$TransactionType}" TransactionStatus="{$varTaxlotStateTx}" BuySelIndicator="{$varBuySelIndicator}" LongShortIndicator="{$varLongShortIndicator}" PositionType="{$Sidevar}"
				translevel="B" ClientTradeRefNo="{PBUniqueID}" AssociatedTradeId="{PBUniqueID}" ExecutionAccount="038169108" AccountID="038169108"
		...
		SettlementCCY ="{SettlCurrency}"	ExchangeCode="" Quantity="{$QtySum}" 	Price="{format-number($varAveragePriceSettl,'###.000000')}"	PriceIndicator="G" Principal="{format-number($varGroupGrsAmtSettl,'###.00')}"
		CommissionAmount="{format-number($varCOMM,'0.##')}" CommissionIndicator="F" Taxorfees="{format-number($varGroupTotalFees,'0.##')}" Tax2="" Taxorfeesindicator="F"
		Interest="{$varAccruedInterest}"	Interestindicator="{$varInterestindicator}" NetAmount="{format-number($varNetamountSettl,'0.##')}" HearsayInd="{$varHearsayInd}" Custodian="MSCO" MoneyManager=""  Bookid=""
		DealId="" TaxLotId="" Originaltaxlottransactiondate="" OriginalTaxlottransactionprice="" TaxLotCloseouMethod="" ExachangeRate="{$varExachangeRate}" AcquisitionDate="" Comments=""	
		...
		EntityID="{EntityID}" TaxLotState="{TaxLotState}" >
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID and CounterParty!='CorpAction' and CounterParty!='Transfer' and CounterParty!='Washsale' and CounterParty!='BOXcollapse' and CounterParty!='SwapReset']">
				...
				<xsl:variable name="TaxlotGrsAmt">
					<xsl:value-of select="AllocatedQty * format-number(AveragePrice,'###.000000') * AssetMultiplier"/>
				</xsl:variable>
				...
				<ThirdPartyFlatFileDetail
							TaxLotState="{TaxLotState}" TransactionType = "{$TransactionType}" TransactionStatus="{$varTaxlotStateTx}" BuySelIndicator="{$varBuySelIndicator}" LongShortIndicator="{$varLongShortIndicator}" PositionType="{$Sidevar}"
				translevel="A" ClientTradeRefNo="{concat(substring(EntityID,3),'C')}" AssociatedTradeId="{PBUniqueID}" ExecutionAccount="038169108" AccountID="038C69013"
				BrokerCounterparty="{$CPVar}" SecurityIdentifierType="{$varSecType}" SecurityIdentifier="{$varSymbol}" SecurityDescription="{translate(FullSecurityName,',','')}" TradeDate="{TradeDate}" SettlementDate="{SettlementDate}"
		SettlementCCY ="{SettlCurrency}"	ExchangeCode="" Quantity="{$QtySum}" 	Price="{format-number($varAveragePriceSettl,'###.000000')}"	PriceIndicator="G" Principal="{format-number($varGroupGrsAmtSettl,'###.00')}"
		CommissionAmount="{format-number($varCOMM,'0.##')}" CommissionIndicator="F" Taxorfees="{format-number($varGroupTotalFees,'0.##')}" Tax2="" Taxorfeesindicator="F"
		Interest="{$varAccruedInterest}"	Interestindicator="{$varInterestindicator}" NetAmount="{format-number($varNetamountSettl,'0.##')}" HearsayInd="{$varHearsayInd}" Custodian="MSCO" MoneyManager=""  Bookid=""
