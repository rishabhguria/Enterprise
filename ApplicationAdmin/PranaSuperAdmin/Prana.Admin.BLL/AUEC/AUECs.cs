...
namespace Prana.Admin.BLL
{
    ...
    public class AUECs : IList
    {
        ...
        /// <summary>
        /// Checcking Auec is exists
        /// </summary>
        /// <param name="auecId"></param>
        /// <returns></returns>
        internal bool ContainsAUEC(int auecId)
        {
            try
            {
                IEnumerable<AUEC> _auecList = _auecs.Cast<AUEC>();
                return _auecList.Any(x => x.AUECID == auecId);
            }
            catch (Exception)
            {
        ...
        /// <summary>
        /// Get AUEC Detail by Id
        /// </summary>
        /// <param name="auecId"></param>
        /// <returns></returns>
        internal string GetAUECDetail(int auecId)
        {
            try
            {
                IEnumerable<AUEC> _auecList = _auecs.Cast<AUEC>();
                var auec = _auecList.FirstOrDefault(x => x.AUECID == auecId);
                if (auec != null)
                    return auec.DisplayName;
                else
                    return string.Empty;
        ...
        /// Get AUEC Id by Name
        /// </summary>
        /// <param name="auecName"></param>
        /// <returns></returns>
        internal int GetAUECId(string auecName)
        {
            try
            {
                IEnumerable<AUEC> _auecList = _auecs.Cast<AUEC>();
                var auec = _auecList.FirstOrDefault(x => x.DisplayName.Equals(auecName));
                if (auec != null)
                    return auec.AUECID;
                else
                    return -1;
