...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-after(normalize-space(COL4),' '),9,1),'C') or contains(substring(substring-after(normalize-space(COL4),' '),9,1),'P')">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL4),' '),10),'00.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="contains(substring(substring-after(normalize-space(COL4),' '),9,1),'C') or contains(substring(substring-after(normalize-space(COL4),' '),9,1),'P')">
						...
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:choose>
										<xsl:when test="contains($Side,'BTO')">
											<xsl:value-of select="'A'"/>
										</xsl:when>
										<xsl:when test="contains($Side,'STC')">
											<xsl:value-of select="'D'"/>
										</xsl:when>
									</xsl:choose>
								</xsl:when>
