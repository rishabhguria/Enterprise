...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="varTradeId">
			<xsl:value-of select="concat('DL',EntityID)"/>
		</xsl:variable>
		...
		<Group
				  TRADE_ID ="{$varTradeId}" ACTION ="{$varTaxlotStateTx}" SECURITY ="{$varSymbol}" WI ="" OPTION_TYPE ="" UNDERLYER ="" STRIKE = "" EXPIRY_DATE = ""
			...
			SIDE = "{$Sidevar}" QUANTITY = "{AllocatedQty}" PRICE = "{format-number($varPrice,'##.####')}"
			COMMISSION_TYPE = "TOTAL" COMMISSION_AMOUNT = "{format-number($varCommissionAmount,'##.####')}" IMPACT_SETTLEMENT_MONEY = "{$varImpactSettlement}" SETTLEMENT_CURRENCY = "{$varSettleCurrency}" PREFIGURED_PRINCIPAL = ""
			INTEREST = "" SERVICE_FEES = "" POSTAGE ="" STAMP_TAX = "" LEVY_TAX = "" ACCOUNT ="{AccountNo}"
			ACCOUNT_TYPE = "" IS_ALLOCATION = "" TRAILER_CODE_1 = "" TRAILER_DESCRIPTION_1 = "" VERSUS_PURCHASE_DATE_1 = "" VERSUS_PURCHASE_QUANTITY_1 = "" RR= ""
				  EntityID="{EntityID}" TaxLotState="{TaxLotState}" RowHeader="true" FileHeader="true" FileFooter="true">
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<xsl:variable name="varTradeid">
					<xsl:value-of select="concat('CL',substring(EntityID,string-length(EntityID)-7,string-length(EntityID)))"/>
				</xsl:variable>

				<!--<ThirdPartyFlatFileDetail
					   TaxLotState="{TaxLotState}" TRADE_ID ="{$varTradeid}" ACTION ="{$varTaxlotStateTx}" SECURITY ="{$varSymbol}" WI ="" OPTION_TYPE ="" UNDERLYER ="" STRIKE = "" EXPIRY_DATE = "" METHOD = "{$varMethod}" MARKET = "" EXECUTING_BROKER = "{$varCounterParty}"
					  CLEARING_BROKER = "" TRADE_DATE = "{TradeDate}" SETTLEMENT_DATE = "{SettlementDate}" SIDE = "{$Sidevar}" QUANTITY = "{AllocatedQty}" PRICE = "{format-number($varPrice,'##.##')}" COMMISSION_TYPE = "TOTAL"
					  COMMISSION_AMOUNT = "{format-number($varCommissionAmount,'##.##')}" IMPACT_SETTLEMENT_MONEY = "{$varImpactSettlement}" SETTLEMENT_CURRENCY = "{$varSettleCurrency}" PREFIGURED_PRINCIPAL = "" INTEREST = ""
					  SERVICE_FEES = "" POSTAGE ="" STAMP_TAX = "" LEVY_TAX = "" ACCOUNT ="{AccountNo}" ACCOUNT_TYPE = "" IS_ALLOCATION = "Y"
					  TRAILER_CODE_1 = "" TRAILER_DESCRIPTION_1 = "" VERSUS_PURCHASE_DATE_1 = "" VERSUS_PURCHASE_QUANTITY_1 = "" RR= ""
								EntityID="{EntityID}"  RowHeader="true" FileHeader="true" FileFooter="true"/>-->
			</xsl:for-each>
		</Group>
	</xsl:template>
</xsl:stylesheet>
