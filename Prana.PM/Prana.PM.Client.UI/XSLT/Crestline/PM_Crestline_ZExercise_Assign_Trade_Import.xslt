...
>
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL7),' '),' '),'/'),'/'),' '),1,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring-before(substring(substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL7),' '),' '),'/'),'/'),' '),2),' '),'##.00')"/>
		</xsl:variable>
	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($NetPosition) and ( COL38 = 'Z Exercise / Assign' and COL55 ='Equity')">
					<PositionMaster>
						...
						<xsl:variable name="varAssetType">
							<xsl:choose>
								<xsl:when test="string-length(COL7) &gt; 18">
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$varAssetType = 'Equity'">
									<xsl:value-of select="substring-before(COL7,' US')"/>
								</xsl:when>
	...
	<xsl:variable name="varCommission">
						...
						<xsl:variable name="CostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL17 div $NetPosition"/>
							</xsl:call-template>
						</xsl:variable>
...
<xsl:variable name ="Side" select="COL11"/>
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="$Side='ASSNOPT'">
									<xsl:value-of select="'B'"/>
								</xsl:when>
								<xsl:when test="contains($Side,'EXRBUY')">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="contains($Side,'ASSNSELL') or contains($Side,'EXRSELL')"> 
									<xsl:value-of select="'2'"/>
								</xsl:when>
								<xsl:when test="contains($Side,'EXRBUYCOV')">
									<xsl:value-of select="'B'"/>
								</xsl:when>
								<xsl:when test="$Side='EXRSHS'">
