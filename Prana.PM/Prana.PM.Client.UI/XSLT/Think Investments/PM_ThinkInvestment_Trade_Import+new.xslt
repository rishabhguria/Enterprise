...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL24,OPTION)">
			...
			<xsl:variable name="ExpiryMonth">
				<xsl:choose>
					<xsl:when test="not(contains(COL5,'US'))">
						<xsl:value-of select="substring-before(substring-after(normalize-space(COL5),' '),'/')"/>
					</xsl:when>
					<xsl:otherwise>
			...
			<xsl:variable name="StrikePrice">
				...
				<xsl:choose>
					<xsl:when test="not(contains(COL5,'US'))">
						<xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(normalize-space(COL5),'/'),'/'),' '),2),'#.00')"/>
					</xsl:when>
					<xsl:otherwise>
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								...
								<xsl:when test="contains(COL24,'OPTION')">
								...
								<xsl:when test="contains(COL24,'FX FWD')">
									<xsl:value-of select="'FxForward'"/>
								</xsl:when>
								<xsl:when test="contains(COL24,'FX SPOT')">
						...
						<!--<xsl:variable name="varFXForward">
							<xsl:choose>
								<xsl:when test="COL17='EUR' or COL17='GBP' or COL17='NZD'">
						...
						<xsl:variable name="varFXForward">
							<xsl:value-of select="concat($varLCurrency,' ',$varBCurrency,' ','SPOT')"/>
						</xsl:variable>
						...
						<xsl:variable name="Secfee">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$varScefee"/>
							</xsl:call-template>
						</xsl:variable>
						<SecFee>
							<xsl:choose>
						...
						<xsl:if test="$Asset='EquitySwap'">
							...
							<xsl:variable name="varPrevMonth">
								<xsl:choose>
									<xsl:when test ="number(substring-before(COL18,'/')) = 1">
										<xsl:value-of select ="12"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select ="number(substring-before(COL18,'/'))-1"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:variable>
							...
							<xsl:variable name ="varSYear">
								<xsl:choose>
									<xsl:when test ="number(substring-before(COL18,'/')) = 1">
										<xsl:value-of select ="number(substring-after(substring-after(COL18,'/'),'/')) -1"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select ="number(substring-after(substring-after(COL18,'/'),'/'))"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:variable>
