  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//Comparision">
        ...
        <xsl:if test="number(COL4) and normalize-space(COL1)='71211520'">
          <PositionMaster>
			...
			<xsl:variable name="varMarketValue" select ="number(COL4) * number(COL5)"/>
			<MarketValue>
			<xsl:choose>
			<xsl:when test="$varCallPut = 1">
			...
			<xsl:value-of select ="$varMarketValue div 100"/>
			</xsl:when>
			<xsl:otherwise>
