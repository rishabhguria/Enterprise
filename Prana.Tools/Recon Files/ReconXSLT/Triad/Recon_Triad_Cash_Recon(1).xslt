			...
			<xsl:for-each select ="//Comparision">
				<xsl:variable name="Cash">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="substring(COL1,72,17)"/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:if test="number($Cash)">
					<PositionMaster>
						...
						<xsl:variable name="CashValueLocal">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring(COL1,72,17) div 100"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="Year">
							...
							<!--<xsl:value-of select="concat(20,substring(COL1,11,2))"/>-->
