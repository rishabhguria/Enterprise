	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test ="number($Position) and (substring(COL1,236,4)='JPMC' and number(substring(COL1,195,7))='0.00')">
					<PositionMaster>
						...
						<xsl:variable name="Costbasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="format-number(substring(COL1,103,16),'#.##') div $Position"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
