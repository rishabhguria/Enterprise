...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[AccountName='Iguana Healthcare-Jefferies' or AccountName='OCA Iguana']">
				...
				<ThirdPartyFlatFileDetail>
					...
					<AccountId>
						<xsl:choose>
							<xsl:when test="AccountName='Iguana Healthcare-Jefferies'">
								<xsl:value-of select="'43300266'"/>
							</xsl:when>
							<xsl:when test="AccountName='OCA Iguana'">
								<xsl:value-of select="'43101165'"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="''"/>
							</xsl:otherwise>
					...
					<xsl:variable name ="varCounterParty">
						<xsl:choose>
							<xsl:when test="CounterParty ='JEFFI' or CounterParty ='JEFFO'">
								<xsl:value-of select="'JEFF'"/>
							</xsl:when>
							<xsl:when test="CounterParty ='JEFFR'">
								<xsl:value-of select="'JDMA'"/>
							</xsl:when>
							<xsl:otherwise>
