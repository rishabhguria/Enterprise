...
-- Description: Nirvana Data access webservice proedure for webmethod SymbolList
-- EXEC [P_DA_SymbolList] 1,'',0
-- Modified By: Sandeep Singh
-- Modification Date: OCT 11, 2019
-- Description: Input parameters changed
-- =============================================   */   
CREATE Procedure [dbo].[P_DA_SymbolList]
(
@AddOnId INT,
...
Set @SpecificSymbolsList = (SELECT SpecificSymbols FROM T_DA_AddOns WHERE SpecificSymbols Is Not Null And AddOnId = @AddOnId)
	...
	Insert Into @T_AUECIDs 
	Select Cast(RTRIM(LTRIM(Items)) As Int) From dbo.Split(@SymbolsAUECIDsList, ',')  
	...
	INNER JOIN T_SMReuters ON T_SMSymbolLookUpTable.Symbol_PK = T_SMReuters.Symbol_PK
	WHERE T_SMSymbolLookUpTable.AUECID In 
	(
		Select AUECID from @T_AUECIDs
	)  OR 
	TickerSymbol In 
	(
		Select SpecificSymbol from @T_SpecificSymbolsList
	)
		
		Union

	Select 
	SpecificSymbol As BloombergSymbol, 
	'' As CUSIPSymbol, 
	'' As SEDOLSymbol, 
	'' As TickerSymbol, 
	'' As ISINSymbol, 
	'' As ReutersSymbol		
	FROM @T_SpecificSymbolsList
