...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[TaxLotState!='Sent'][Asset='Equity' and IsSwapped='true']">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="varPBUniqueIDAndDatePart">
			<xsl:value-of select="concat('A',PBUniqueID,substring(TradeDate,4,2))"/>
		</xsl:variable>
		...
		<Group 
				...
				transactionlevel="B" ClientRefNo="{$varPBUniqueIDAndDatePart}" AssociatedTradeId="{$varPBUniqueIDAndDatePart}" ExecutionAccount="38224911" AccountID="38224911"
				...
				SettlementCCY="{SettlCurrency}"	ExchangeCode="{$varEXCODE}" Quantity="{$QtySum}" 	PriceAmount="{format-number(AveragePrice,'###.000000')}"	PriceIndicator="G" Principal="{format-number($GroupGrsAmt,'###.00')}" 
				ExecutingBrokerCommission="{format-number($CommissionSum + $SoftCommissionSum + $TaxOnCommissionSum,'###.0000')}" ExecutingBrokerCommissionIndicator="F" 
				MSFees="" 
				NotRequired=""
				MSFeesIndicator="F" DividendEntitlementPercent="" Spread="" NetAmount="{format-number($varGroupNetAmnt,'###.00')}" HearsayInd="Y" Custodian="MSSW" MoneyManager="" Bookid=""
				Dealid="" Taxlotid="" Originaltaxlottransactiondate="" OriginalTaxlottransactionprice="" TaxLotCloseoutMethod="" PriceFXRate="" AcquitionDate="" Comments="" SwapReferenceNo="" BasketId="" PriceCurrency="{CurrencySymbol}" ResetIndicator=""
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID and TaxLotState!='Sent' and AccountName!='Strategy Test']">
				...
				<ThirdPartyFlatFileDetail		
							...
							LongShortIndicator="" PositionType="{$Sidevar}" transactionlevel="A" ClientRefNo="{concat('B',substring(EntityID,3))}" 
							AssociatedTradeId="{$varPBUniqueIDAndDatePart}" ExecutionAccount="38224911" AccountID="038QLCXA2"
							...
							Quantity="{AllocatedQty}" PriceAmount="{format-number(AveragePrice,'###.000000')}" PriceIndicator="G"	
							Principal="{format-number($TaxlotGrsAmt,'###.00')}" ExecutingBrokerCommission="{format-number(CommissionCharged + SoftCommissionCharged + TaxOnCommissions,'###.00')}" 
							ExecutingBrokerCommissionIndicator="F" MSFees="" 
							NotRequired=""
							MSFeesIndicator="F" DividendEntitlementPercent="" Spread="" NetAmount="{format-number($varAllocatinNetAmnt,'###.00')}" HearsayInd="Y" Custodian="MSSW" 
							MoneyManager="" Bookid="" Dealid="" Taxlotid="" Originaltaxlottransactiondate="" OriginalTaxlottransactionprice="" TaxLotCloseoutMethod="" PriceFXRate=""
