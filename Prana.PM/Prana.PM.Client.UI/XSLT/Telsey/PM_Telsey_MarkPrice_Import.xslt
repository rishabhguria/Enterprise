			...
			<xsl:for-each select ="//PositionMaster">
				<xsl:variable name="MarkPrice">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="substring(COL1,85,6)"/>
					</xsl:call-template>
				</xsl:variable>
				...
				<xsl:if test="number($MarkPrice)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Symbol" select="substring(COL1,15,9)"/>

						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="substring(COL1,49,1) ='C' or (substring(COL1,49,1)='P')">
						...
						<IDCOOptionSymbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:value-of select="concat(substring(COL1,37,21),'U')"/>
								</xsl:when>
