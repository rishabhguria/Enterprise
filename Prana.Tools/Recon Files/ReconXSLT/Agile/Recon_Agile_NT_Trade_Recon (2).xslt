	...
	<xsl:template name="Option">
		...
		<xsl:if test="COL2='Option'">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL42,' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring-before(substring-after(COL42,'/'),'/')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring-before(substring-after(COL42,' '),'/')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring-before(substring-after(substring-after(COL42,'/'),'/'),' ')"/>
			</xsl:variable>
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring-before(substring-after(substring-after(COL42,' '),' '),' ')"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="substring-after(substring-after(substring-after(COL42,' '),' '),' ')"/>
			</xsl:variable>
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								...
								<xsl:when test="COL2='Future Option'">
						...
						<xsl:variable name="varFutStrickPrice">
							<xsl:value-of select="substring-before(substring-after(COL77,' '),' ')"/>
						</xsl:variable>
						...
						<xsl:variable name="varFUTUnderlyng">
							<xsl:choose>
								<xsl:when test="string-length(substring-before(COL77,' '))= 5">
									<xsl:value-of select="substring(substring-before(COL77,' '),1,2)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="''"/>
								</xsl:otherwise>
						...
						<xsl:variable name="varFutCode">
							<xsl:choose>
								<xsl:when test="string-length(substring-before(COL77,' '))= 5">
									<xsl:value-of select="substring(substring-before(COL77,' '),3)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="''"/>
								</xsl:otherwise>
						...
						<IDCOOptionSymbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:value-of select="concat(COL83,'U')"/>
								</xsl:when>
								<xsl:when test="$Asset='FutureOption'">
									<xsl:value-of select="''"/>
								</xsl:when>
						...
						<xsl:variable name="Side" select="COL90"/>
						<Side>
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:choose>										
						...
						<xsl:variable name="FEES">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL10 + COL16"/>
							</xsl:call-template>
						</xsl:variable>
						<MiscFees>
							<xsl:choose>
