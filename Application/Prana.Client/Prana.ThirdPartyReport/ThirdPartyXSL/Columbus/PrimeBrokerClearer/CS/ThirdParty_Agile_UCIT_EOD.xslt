...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[AccountName='UCIT']">
				<ThirdPartyFlatFileDetail>
					<RowHeader>
						<xsl:value-of select ="'false'"/>
					</RowHeader>
					...
					<TradeID>
						<!--<xsl:value-of select="concat(substring(EntityID,1,6),substring(EntityID,11))"/>-->
						<xsl:value-of select="substring(EntityID,10)"/>
					</TradeID>

					<TradeDirection>
						<xsl:choose>
							<xsl:when test="Side='Buy' or Side='Buy to Close' or Side='Buy to Open' or Side='Buy to Cover'">
								<xsl:value-of select ="'B'"/>
							</xsl:when>
							<xsl:when test="Side='Sell' or Side='Sell short' or Side='Sell to Close' or Side='Sell to Open'">
					...
					<NetPrice>
						<xsl:choose>
							<xsl:when test="IsSwapped='true'">
								<xsl:value-of select="format-number(NetAmount div AllocatedQty,'##.######')"/>
							</xsl:when>
