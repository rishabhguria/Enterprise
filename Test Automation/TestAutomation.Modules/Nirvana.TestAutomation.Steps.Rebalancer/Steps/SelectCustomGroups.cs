...
{
    class SelectCustomGroups : IUIAutomationTestStep
    {
        public TestResult RunUIAutomationTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("SelectCustomGroups failed as DataSet is empty.");
                }
                ...
                bool selectionSucceeded = uiAutomationHelper.CustomGroupDataSelectItem("RebalancerWindow", "RebalancerWindow", testData.Tables[0], "SelectCustomGroups", "ListItems");

                if (!selectionSucceeded)
                {
                    throw new Exception("SelectCustomGroups" +selectionSucceeded);
                }
            }
            catch (Exception ex)
            {
