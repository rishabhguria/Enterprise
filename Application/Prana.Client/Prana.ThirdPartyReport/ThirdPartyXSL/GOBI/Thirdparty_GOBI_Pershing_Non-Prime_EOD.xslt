...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="Blottermapping">
			<xsl:value-of select="document('../ReconMappingXml/ThirdParty_BlotterCodeMapping.xml')/BrokerMapping/PB[@Name=$PB_NAME]/BrokerData[@Exchange=$Exchange]/@BrokerCode"/>
		</xsl:variable>

		<xsl:variable name="BlotterCodeValues">
			<xsl:choose>
		...
		<xsl:variable name="Blottercode">
			<xsl:value-of select="concat($Blottermapping,$BlotterCodeValues)"/>
		</xsl:variable>
		...
		<Group
			...
			TRAILERCD3="" BLOTTERCD="{$Blottercode}" CLRNGHSE="Y" CLRAGNTCD="{$DESCRIPTION1}" CLRAGNT1="" CLRAGNT2="" CLRAGNT3="" CLRAGNT4="" CNTRPRTYCD="" CNTRPTY1="" CNTRPTY2="" CNTRPTY3="" CNTRPTY4=""
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				TaxLotState="{TaxLotState}" LOCALREF = "{$LOCALREF_Allo}" CFID="{$CFID}" ROUTECD="PSHG" TIRORDERID="{$TIRORDERID}" TIRPIECE=""
				TIRSEQ="" SECIDTYPE="{$SECIDTYPE}" SECURITYID="{$SECURITYID}" DESCRIPTION1="{$DESCRIPTION}" DESCRIPTION2=""
