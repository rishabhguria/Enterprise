	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name="varEntryCondition">
					<xsl:choose>
						...
						<xsl:when test="COL2='Currency'">
							<xsl:choose>
								<xsl:when test="contains(COL5,'Payables')">
									<xsl:value-of select="1"/>
								</xsl:when>
								<xsl:when test="contains(COL5,'Receivables')">
									<xsl:value-of select="1"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
				...
				<!--Handling for picking up all position with 0 PNL except FX FWD/20181119-->
        <xsl:choose>
          ...
          <xsl:when test="number($Quantity)and ($varEntryCondition='1') ">
            ...
            <PositionMaster>
              ...
              <xsl:variable name = "varSymbol" >
                <xsl:choose>
                  <xsl:when test="contains(COL5,'Payables') or contains(COL5,'Receivables')">
                    <xsl:value-of select="concat(COL4,'Fwd')"/>
                  </xsl:when>
              ...
              <xsl:variable name="PB_FUND_NAME" select="'K2DEF_MS'"/>
