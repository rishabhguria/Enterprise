			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($varAmount)">
					...
					<PositionMaster>
						...
						<xsl:variable name="varDayP">
							<xsl:value-of select="substring(COL15,7,2)"/>
						</xsl:variable>
						<xsl:variable name="varYearP">
							<xsl:value-of select="substring(COL15,1,4)"/>
						</xsl:variable>

						<xsl:variable name="varMonthP">
							<xsl:value-of select="substring(COL15,5,2)"/>
						</xsl:variable>
						<xsl:variable name="varCurrentDateP">
							<xsl:value-of select="concat($varMonthP,'/',$varDayP, '/', $varYearP)"/>
						</xsl:variable>
						<PayoutDate>
							<xsl:value-of select="$varCurrentDateP" />
						</PayoutDate>
