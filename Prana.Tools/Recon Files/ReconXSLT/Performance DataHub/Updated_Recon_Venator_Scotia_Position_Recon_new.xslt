			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Position)">
					<PositionMaster>
						...
						<xsl:variable name="PB_SUFFIX_NAME" select="normalize-space(COL19)"/>

						<!--<xsl:variable name="PB_SUFFIX_NAME" select="$Currency"/>-->
						...
						<xsl:variable name="varSymbol">
							<!--<xsl:choose>
								<xsl:when test="normalize-space(COL30)='OPTION'">
						...
						<xsl:variable name="AssetType">
							<xsl:choose>
								<xsl:when test="normalize-space(COL7)='OPTION'">
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$AssetType='Rights'">
									<xsl:value-of select="'TRC/R'"/>
								</xsl:when>
								<xsl:when test="contains(COL7,'CONV BOND') or contains(COL7,'CORP')">
								...
								<xsl:when test="$AssetType='Equity'">
									<xsl:choose>
										...
										<xsl:when test="COL19!='CAD' and contains($varSymbol,'/') or contains($varSymbol,'-')">
											<xsl:choose>
												<xsl:when test="contains($varSymbol,'/')">
													<xsl:value-of select="concat(translate($varSymbol,'/','.'),'-TC')"/>
												</xsl:when>
												<xsl:when test="contains($varSymbol,'-')">
													<xsl:value-of select="concat(translate($varSymbol,'-','.'),'N','-TC')"/>
												</xsl:when>
												<xsl:otherwise>
													<xsl:choose>
														<xsl:when test="contains(substring($varSymbol,string-length($varSymbol)-1),'CN') or contains(substring($varSymbol,string-length($varSymbol)-1),'CT')">
														...
														<xsl:otherwise>
															<!--<xsl:value-of select ="normalize-space($varSymbol)"/>-->
															<xsl:value-of select="concat($varSymbol,'-TC')"/>
														</xsl:otherwise>
													</xsl:choose>
													<!--<xsl:value-of select="concat($varSymbol,$PRANA_SUFFIX_NAME)"/>-->
												</xsl:otherwise>
						...
						<xsl:variable name="Side" select="normalize-space(COL24)"/>
						...
						<Commission>
							<xsl:choose>
								<xsl:when test="normalize-space(COL9)='CANCEL'">
									<xsl:choose>
