...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-before(COL12,' '),1,1),C) or contains(substring(substring-before(COL12,' '),1,1),P)">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring-before(substring-after(substring-after(substring-after(normalize-space(COL12),' '),' '),' '),' ') ,'#.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="(string-length(COL29) &gt; 20) and contains(COL7,'B')">
									<xsl:value-of select="'EquityOption'"/>
								</xsl:when>
								<xsl:when test="COL7='0'">
									<xsl:value-of select="'Equity'"/>
								</xsl:when>
								<xsl:when test="COL7='4'">
						...
						<xsl:variable name ="BlankAvrage">
							<xsl:choose>
								<xsl:when test="COL21='*' or COL21=''">
									<xsl:value-of select="0"/>
								</xsl:when>
								<xsl:when test="COL21!='*' or COL21!=''">
						...
						<xsl:variable name="varAagprice">
							<xsl:choose>
								<xsl:when test="COL13='B'">
									<xsl:value-of select="$COL17 - $COL18 - $BlankAvrage"/>
								</xsl:when>
								<xsl:when test="COL13='SS' or COL13='S'">
									<xsl:value-of select="$COL17 + $COL18 + $BlankAvrage"/>
								</xsl:when>
							</xsl:choose>
						</xsl:variable>
						...
						<xsl:variable name="AvgPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$varAagprice div $Quantity"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
