  ...
  <xsl:template name="FormatDate">
	...
	<xsl:template match="/">
		...
		<DocumentElement>
            ...
            <xsl:variable name="PositionStartDate">
             <xsl:choose>
            ...
            <xsl:variable name="varPositionStartDate">
              <xsl:call-template name="FormatDate">
                <xsl:with-param name="DateTime" select="$PositionStartDate"/>
              </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:variable name="varOriginalPurchaseDate">
              <xsl:call-template name="FormatDate">
                <xsl:with-param name="DateTime" select="$OriginalPurchaseDate"/>
              </xsl:call-template>
            </xsl:variable>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="COL19='EquitySwap'">
						...
						<OrigTransDate>
							...
							<xsl:variable name="varYear">
								<xsl:choose>
									<xsl:when test="contains($varOriginalPurchaseDate,'/')">
										<xsl:value-of select="substring-after(substring-after($varOriginalPurchaseDate,'/'),'/')"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="substring-after(substring-after($varOriginalPurchaseDate,'-'),'-')"/>
									</xsl:otherwise>
							...
							<xsl:variable name="Day">
								<xsl:choose>
									<xsl:when test="contains($varOriginalPurchaseDate,'/')">
										<xsl:choose>
											<xsl:when test="string-length(number(substring-before(substring-after($varOriginalPurchaseDate,'/'),'/'))) = 1">
												<xsl:value-of select="concat(0,substring-before(substring-after($varOriginalPurchaseDate,'/'),'/'))"/>
											</xsl:when>
											<xsl:otherwise>
												<xsl:value-of select="substring-before(substring-after($varOriginalPurchaseDate,'/'),'/')"/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
									<xsl:otherwise>
							...
							<xsl:variable name="Month">
								<xsl:choose>
									<xsl:when test="contains($varOriginalPurchaseDate,'/')">
										<xsl:choose>
											<xsl:when test="string-length(number(substring-before($varOriginalPurchaseDate,'/'))) = 1">
												<xsl:value-of select="concat(0,substring-before($varOriginalPurchaseDate,'/'))"/>
											</xsl:when>
											<xsl:otherwise>
												<xsl:value-of select="substring-before($varOriginalPurchaseDate,'/')"/>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
									<xsl:otherwise>
