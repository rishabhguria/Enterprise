PRINT 'Started Executing: ClientDB_1939_PMGetRealizeAndUnrealizePnL_NAV.sql'
GO
IF EXISTS (
		SELECT *
		FROM sysobjects
		WHERE id = object_id(N'[dbo].PMGetRealizeAndUnrealizePnL_NAV')
			AND OBJECTPROPERTY(id, N'IsProcedure') = 1
		)
BEGIN
	DROP PROCEDURE [dbo].[PMGetRealizeAndUnrealizePnL_NAV]
	PRINT 'Deleted existing Stored Procedure PMGetRealizeAndUnrealizePnL_NAV'
END
GO
/*
...
exec [PMGetRealizeAndUnrealizePnL_NAV] '10/1/2013','4/07/2015' ,'1182,1183,1184','1,2,3,4,5,6,7,8,9','MTM_V0',1,'Symbol','DELL'                                                                                        

*/  
                       
CREATE PROCEDURE  [dbo].[PMGetRealizeAndUnrealizePnL_NAV]                                                                         
(                                                                                                                                                                                                                                              
...
--Set @EndDate='2015-03-12'              
--Set @Fund = '1182,1183'            
--Set @Asset= '1,2,3,4,5,6,7,8,9'            
--Set @ReportId = 'MTM_V0'                 
...
Set @DefaultAUECID=(select TOP 1 DefaultAUECID from T_Company where CompanyID > 0)                                                      
    
Create Table #FundsInMP                                
  (                                  
   FundID INT    
  )      
Insert into #FundsInMP    
Select distinct FundID from PM_DayMarkPrice   
                                                                                                                               
Create Table #FundsInFXMP                                
  (                                  
   FundID INT    
  )      
Insert into #FundsInFXMP    
Select distinct FundID from T_CurrencyConversionRate   
...
Insert into #FXConversionRates Exec P_GetAllFXConversionRatesFundWiseForGivenDateRange @MinTradeDate,@EndDate                                                                      
...
Select               
  ...
  PM_DayMarkPrice.FundID           
      From PM_DayMarkPrice                 
       INNER JOIN V_SymbolAuec ON PM_DayMarkPrice.Symbol = V_SymbolAuec.Symbol                
       INNER JOIN #AUECBusinessDatesForEndDate AUECBusinessDatesForEndDate ON AUECBusinessDatesForEndDate.AUECID = V_SymbolAuec.AUECID                 
...
Left Outer Join T_CompanyFunds TC ON PT.FundID= TC.CompanyFundID                      
...
And DateDiff(d,G.ProcessDate,FXDayRatesForTradeDate.Date)=0 And (FXDayRatesForTradeDate.FundID = PT.FundID And FXDayRatesForTradeDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForTradeDate.FromCurrencyID = G.CurrencyID    
AND FXDayRatesForTradeDate.ToCurrencyID = TC.LocalCurrency                                                                                                     
 And DateDiff(d,G.ProcessDate,FXDayRatesForTradeDate.Date)=0 And (FXDayRatesForTradeDate.FundID=0 Or FXDayRatesForTradeDate.FundID is NUll And  PT.FundID  Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
AND FXDayRatesForStartDate.ToCurrencyID = TC.LocalCurrency                                                                                                     
 And DateDiff(d,AUECYesterDates.YESTERDAYBIZDATE,FXDayRatesForStartDate.Date)=0 And (FXDayRatesForStartDate.FundID = PT.FundID And FXDayRatesForStartDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForStartDate.FromCurrencyID = G.CurrencyID  
AND FXDayRatesForStartDate.ToCurrencyID = TC.LocalCurrency                                                                                                       
 And DateDiff(d,AUECYesterDates.YESTERDAYBIZDATE,FXDayRatesForStartDate.Date)=0 And (FXDayRatesForStartDate.FundID=0 Or FXDayRatesForStartDate.FundID is NUll And PT.FundID Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
AND FXDayRatesForClosingDate.ToCurrencyID = TC.LocalCurrency                                                                                                     
 And DateDiff(d,G1.ProcessDate,FXDayRatesForClosingDate.Date)=0 And (FXDayRatesForClosingDate.FundID = PT.FundID And FXDayRatesForClosingDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForClosingDate.FromCurrencyID = G.CurrencyID  
AND FXDayRatesForClosingDate.ToCurrencyID = TC.LocalCurrency                                                                                                      
 And DateDiff(d,G1.ProcessDate,FXDayRatesForClosingDate.Date)=0 And (FXDayRatesForClosingDate.FundID=0 Or FXDayRatesForClosingDate.FundID is NUll And PT.FundID Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
IsNull(PT.OpenTotalCommissionAndFees,0) As TotalOpenCommission_Local,                
 ...
 Case                                                                                                                                                                                              
  When G.CurrencyID =  TC.LocalCurrency                      
...
Case G.IsSwapped                                                                                                                                                                                                                                     
 ...
 Then                                                                                                                                                                
  Case                                                                                                   
   When G.CurrencyID = TC.LocalCurrency                                                           
   Then (((isnull((PT.TaxLotOpenQty * SW.NotionalValue / G.CumQty) ,0) * (isnull(SW.BenchMarkRate,0) + isnull(SW.Differential,0)) * DateDiff(d,SW.OrigTransDate,@EndDate))/100)/SW.DayCount)* dbo.GetSideMultiplier(PT.OrderSideTagValue)                      
   Else (((isnull((PT.TaxLotOpenQty * SW.NotionalValue / G.CumQty) ,0) * (isnull(SW.BenchMarkRate,0) + isnull(SW.Differential,0)) * DateDiff(d,SW.OrigTransDate,@EndDate))/100)/SW.DayCount)* dbo.GetSideMultiplier(PT.OrderSideTagValue)                   * IsNull(FXDayRatesForEndDate.RateValue,0)                                                                  
  End                                                                                                                                                                       
 Else 0                                                                                       
End as SwapData,                               
  ...
  Then                                       
   Case                                       
    ...
    Then                                      
     Case                                                                                                                                      
      When Isnull(PT.FXRate,G.FXRate) > 0 And IsNull(PT.FXConversionMethodOperator,G.FXConversionMethodOperator)='M'                                                                                                                                
						Then IsNull(PT.AvgPrice * Isnull(PT.FXRate,G.FXRate) * TaxlotOpenQty * IsNUll(SM.Multiplier,0) * dbo.GetSideMultiplier(PT.OrderSideTagValue),0)                                                                
      When Isnull(PT.FXRate,G.FXRate) > 0 And IsNull(PT.FXConversionMethodOperator,G.FXConversionMethodOperator)='D'                                                                                                                                           
						Then IsNull((PT.AvgPrice * 1/Isnull(PT.FXRate,G.FXRate)) * TaxlotOpenQty * IsNUll(SM.Multiplier,0) * dbo.GetSideMultiplier(PT.OrderSideTagValue),0)                                                                                                       
...
(G.AssetID = 4 and TC.LocalCurrency <> G.CurrencyID and @InternationalFutureOptionPNLWithBothOrEndFXRate=1)                            
)                             
 Then                     
  Case                                                                
   When G.CurrencyID = TC.LocalCurrency                                                                                                                                                                     
...
(G.AssetID = 4 and TC.LocalCurrency <> G.CurrencyID and @InternationalFutureOptionPNLWithBothOrEndFXRate = 0)                            
)      
 Then                                                                                                            
  Case                                                        
   When G.CurrencyID = TC.LocalCurrency                                                                                                                                                            
...
'' as ClosedTradeAttribute6 ,    
 ...
 From #PM_Taxlots PT                                                                                                                                                                           
  Inner Join #T_CompanyFunds TC ON  PT.FundID= TC.CompanyFundID                               
  Inner Join T_Group G on G.GroupID = PT.GroupID             
  Inner Join #T_Asset On #T_Asset.AssetId=G.AssetID   
  
Left Outer Join #MarkPriceForStartDate  MPS on     
   (MPS.Symbol=PT.Symbol And (MPS.FundID = PT.FundID And PT.FundID in (Select FundID from #FundsInMP Where FundID is Not Null)))    
   Or    
   (PT.Symbol = MPS.Symbol And (MPS.FundID=0 Or MPS.FundID is NUll And PT.FundID Not in (Select FundID from #FundsInMP Where FundID is Not Null)))    
    
  Left Outer Join #MarkPriceForEndDate MPE on     
   (MPE.Symbol=PT.Symbol And (MPE.FundID = PT.FundID And PT.FundID in (Select FundID from #FundsInMP Where FundID is Not Null)))    
   Or    
   (PT.Symbol = MPE.Symbol And (MPE.FundID=0 Or MPE.FundID is NUll And PT.FundID Not in (Select FundID from #FundsInMP Where FundID is Not Null)))    
 ...
 on (FXDayRatesForTradeDate.FromCurrencyID = G.CurrencyID    
AND FXDayRatesForTradeDate.ToCurrencyID = TC.LocalCurrency                                                                                                    
 And DateDiff(d,G.AUECLocalDate,FXDayRatesForTradeDate.Date)=0 And (FXDayRatesForTradeDate.FundID = PT.FundID And FXDayRatesForTradeDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForTradeDate.FromCurrencyID = G.CurrencyID   
AND FXDayRatesForTradeDate.ToCurrencyID = TC.LocalCurrency                                                                                                     
 And DateDiff(d,G.AUECLocalDate,FXDayRatesForTradeDate.Date)=0 And (FXDayRatesForTradeDate.FundID=0 Or FXDayRatesForTradeDate.FundID is NUll And PT.FundID Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
AND FXDayRatesForEndDate.ToCurrencyID = TC.LocalCurrency                                                                                                    
 And DateDiff(d,AUECBusinessDatesForEndDate.YESTERDAYBIZDATE,FXDayRatesForEndDate.Date)=0 And (FXDayRatesForEndDate.FundID = PT.FundID And FXDayRatesForEndDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForEndDate.FromCurrencyID = G.CurrencyID    
AND FXDayRatesForEndDate.ToCurrencyID = TC.LocalCurrency                                                                                                    
 And DateDiff(d,AUECBusinessDatesForEndDate.YESTERDAYBIZDATE,FXDayRatesForEndDate.Date)=0 And (FXDayRatesForEndDate.FundID=0 Or FXDayRatesForEndDate.FundID is NUll And PT.FundID Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
IsNull(PTClosedTotalCommissionandFees,0) As  TotalOpenCommission_Local,              
   ...
   Case                                                                                                                                                                                     
 When GCurrencyID =  TC.LocalCurrency                                                                                                                                                                             
...
from #TempClosingData                                        
  Inner Join #T_CompanyFunds TC ON  #TempClosingData.PTFundID= TC.CompanyFundID                               
  Inner Join #T_Asset On #T_Asset.AssetId=#TempClosingData.GAssetID                                                                                                                                                            
  --Inner Join T_AUEC AUEC on G.AUECID = AUEC.AUECID   
  
Left Outer Join #MarkPriceForStartDate  MPS on     
   (MPS.Symbol=#TempClosingData.PTSymbol And (MPS.FundID = #TempClosingData.PTFundID And #TempClosingData.PTFundID in (Select FundID from #FundsInMP Where FundID is Not Null)))    
   Or    
   (#TempClosingData.PTSymbol = MPS.Symbol And (MPS.FundID=0 Or MPS.FundID is NUll And #TempClosingData.PTFundID Not in (Select FundID from #FundsInMP Where FundID is Not Null)))    
    
  Left Outer Join #MarkPriceForEndDate MPE on     
   (MPE.Symbol=#TempClosingData.PTSymbol And (MPE.FundID = #TempClosingData.PTFundID And #TempClosingData.PTFundID in (Select FundID from #FundsInMP Where FundID is Not Null)))    
   Or    
   (#TempClosingData.PTSymbol = MPE.Symbol And (MPE.FundID=0 Or MPE.FundID is NUll And #TempClosingData.PTFundID Not in (Select FundID from #FundsInMP Where FundID is Not Null)))    
...
Case                                                                                                                                      
  When Min(CashDiv.CurrencyID) =  Min(TC.LocalCurrency)                                                                               
  Then Sum(CashDiv.Amount)                                                        
...
Min(TC.LocalCurrency) as BaseCurrencyID   
  ...
  from T_CashTransactions CashDiv 
   ...
   Inner Join #T_CompanyFunds TC ON  CashDiv.FundID= TC.CompanyFundID                              
...
Left outer join #FXConversionRates FXDayRatesForDiviDate  
 on (FXDayRatesForDiviDate.FromCurrencyID = CashDiv.CurrencyID   
AND FXDayRatesForDiviDate.ToCurrencyID = TC.LocalCurrency                                                                                                   
 And DateDiff(d,CashDiv.ExDate,FXDayRatesForDiviDate.Date)=0 And (FXDayRatesForDiviDate.FundID = CashDiv.FundID And FXDayRatesForDiviDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForDiviDate.FromCurrencyID = CashDiv.CurrencyID   
AND FXDayRatesForDiviDate.ToCurrencyID = TC.LocalCurrency                                                                                                    
 And DateDiff(d,CashDiv.ExDate,FXDayRatesForDiviDate.Date)=0 And (FXDayRatesForDiviDate.FundID=0 Or FXDayRatesForDiviDate.FundID is NUll And CashDiv.FundID Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
AND FXDayRatesForStartDate.ToCurrencyID = CF.LocalCurrency                                                                                                   
 And DateDiff(d,dbo.AdjustBusinessDays(@StartDate,-1,@DefaultAUECID),FXDayRatesForStartDate.Date)=0 And (FXDayRatesForStartDate.FundID = DailyCash.FundID And FXDayRatesForStartDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForStartDate.FromCurrencyID = DailyCash.LocalCurrencyID   
AND FXDayRatesForStartDate.ToCurrencyID = CF.LocalCurrency                                                                                                    
 And DateDiff(d,dbo.AdjustBusinessDays(@StartDate,-1,@DefaultAUECID),FXDayRatesForStartDate.Date)=0 And (FXDayRatesForStartDate.FundID=0 Or FXDayRatesForStartDate.FundID is NUll And DailyCash.FundID Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
AND FXDayRatesForEndDate.ToCurrencyID = CF.LocalCurrency                                                                                                   
 And DateDiff(d,dbo.AdjustBusinessDays(DateAdd(d,1,@EndDate),-1,@DefaultAUECID),FXDayRatesForEndDate.Date)=0 And (FXDayRatesForEndDate.FundID = DailyCash.FundID And FXDayRatesForStartDate.FundID in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
Or    
(FXDayRatesForEndDate.FromCurrencyID = DailyCash.LocalCurrencyID   
AND FXDayRatesForEndDate.ToCurrencyID = CF.LocalCurrency                                                                                                    
 And DateDiff(d,dbo.AdjustBusinessDays(DateAdd(d,1,@EndDate),-1,@DefaultAUECID),FXDayRatesForEndDate.Date)=0 And (FXDayRatesForEndDate.FundID=0 Or FXDayRatesForEndDate.FundID is NUll And DailyCash.FundID Not in (Select FundID from #FundsInFXMP Where FundID is Not Null)))    
...
----updated by Sandeep Singh on 08-29-2014                  
--- @NAVOfCurrentOrPriorDay = 1 for currecnt date and 2 for prior date                  
--select @StartDate = dbo.F_getAdjustedDateForGivenDate(@StartDate,1)                         
select @StartDate =                   
Case                  
When @NAVOfCurrentOrPriorDay = 1                  
Then @StartDate                         
...
UnderlyingSymbolCompanyName nvarchar(500),
BaseCurrency varchar(10)      
    
UPDATE  #MTMDataTable  
SET BaseCurrency=C.CurrencySymbol  
From T_Currency C INNER JOIN #MTMDataTable MTM ON MTM.BaseCurrencyID=C.CurrencyID  
    
Update  #MTMDataTable        
Set UnderlyingSymbolCompanyName = SM.CompanyName        
From  #MTMDataTable MTM        
Inner Join V_SecMasterData_WithUnderlying  SM on SM.TickerSymbol = MTM.UnderlyingSymbol                
...
,PNLPercentByNAV= case                      
 when (ISNULL(PM_NAV.NAVValue,0) <> 0 )                    
 then (((#MTMDataTable.RealizedPNLMTM_Base+#MTMDataTable.UnRealizedPNLMTM_Base+#MTMDataTable.Dividend)*100)/PM_NAV.NAVValue)                     
 else 0                    
 END                     
from                     
#MTMDataTable                      
left OUTER JOIN PM_NAVValue PM_NAV ON(#MTMDataTable.FundID = PM_NAV.FundID and datediff(d,date,@StartDate)=0)            
            
If(@SearchString <> '')                             
 Begin                           
  If (@searchby='Symbol')                  
	  Begin                  
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.Symbol                  
		  Order by symbol                  
	  End                  
  Else if (@searchby='underlyingSymbol')                  
	  Begin              
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.underlyingSymbol                  
		  Order by symbol                  
	  End                    
  Else if (@searchby='BloombergSymbol')                  
	  Begin                  
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.BloombergSymbol                  
		  Order by symbol                  
	  End                      
  Else if (@searchby='SedolSymbol')                  
	  Begin                  
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.SedolSymbol                  
		  Order by symbol                  
	  End                      
  Else if (@searchby='OSISymbol')                  
	  Begin                  
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.OSISymbol                  
		  Order by symbol                  
	  End                      
  Else if (@searchby='IDCOSymbol')                  
	  Begin                  
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.IDCOSymbol                  
		  Order by symbol                  
	  End                      
  Else if (@searchby='ISINSymbol')                  
	  Begin                  
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.ISINSymbol                  
		  Order by symbol                  
	  End                     
  Else if (@searchby='CUSIPSymbol')                  
	  Begin                  
		  SELECT * FROM #MTMDataTable                  
		  Inner Join #Symbol on #Symbol.items = #MTMDataTable.CUSIPSymbol                  
		  Order by symbol                  
	  End                               
 End                              
Else                              
 Begin                              
  Select * from #MTMDataTable Order By symbol                    
 End                                                    
-------------------------------------------------------                                 
...
DROP table #FXConversionRates, #AUECYesterDates, #AUECBusinessDatesForEndDate,#MTMDataTable,#FundsInMP,#FundsInFXMP,#TempSymbol           
END     
    
 
go
PRINT 'Done executing ClientDB_1939_PMGetRealizeAndUnrealizePnL_NAV.sql'