PRINT N'Started executing ClientDB_1588_PMGetRealizeAndUnrealizePnL.sql'
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PMGetRealizeAndUnrealizePnL]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PMGetRealizeAndUnrealizePnL]
PRINT N'Deleted PMGetRealizeAndUnrealizePnL EXISTING sp'
GO
...
DROP table #FXConversionRates, #AUECYesterDates, #AUECBusinessDatesForEndDate,#MTMDataTable,#Symbol,#TempSymbol                    
END   

go
PRINT 'Done! ClientDB_1588_PMGetRealizeAndUnrealizePnL.sql'
