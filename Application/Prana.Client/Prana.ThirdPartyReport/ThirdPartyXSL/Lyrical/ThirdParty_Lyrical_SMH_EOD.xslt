		...
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail">
				...
				<xsl:variable name ="PRANA_FUND_NAME">
					<xsl:value-of select ="document('../ReconMappingXml/FundList.xml')/FundMapping/PB[@Name='SMHI']/FundData[@PranaFund=$PB_FUND_NAME]/@PranaFund"/>
				</xsl:variable>




					<xsl:if test="CounterParty='SMHI'">
...
<xsl:value-of  select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID=$I_GroupID]/Symbol"/>
		...
		METHOD = "{$varMethod}" MARKET = "{$PRANA_EXCHANGE}" EXECUTING_BROKER = "SMH" CLEARING_BROKER = "" TRADE_DATE = "{TradeDate}" SETTLEMENT_DATE = "{SettlementDate}"
		  ...
		  EntityID="{EntityID}" TaxLotState="{TaxLotState}" TaxLotState1="" FileHeader="TRUE" FileFooter="TRUE">
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<xsl:variable name ="PB_FUND_CODE">
					<xsl:value-of select ="document('../ReconMappingXml/EOD_Mapping.xml')/FundMapping/PB[@Name='EOD']/FundData[@NirvanaAccountName=$PRANA_FUND_NAME]/@DVPSMH"/>
				</xsl:variable>
