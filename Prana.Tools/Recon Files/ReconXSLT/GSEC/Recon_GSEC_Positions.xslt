		...
		<DocumentElement>
			<xsl:for-each select="//Comparision">
				<PositionMaster>
					...
					<xsl:choose>
						<xsl:when test="boolean(number(normalize-space(substring(COL1,843,18))))">
							<AvgPX>
								<xsl:value-of select="number(normalize-space(substring(COL1,843,18)))"/>
							</AvgPX>
						</xsl:when>
						<xsl:otherwise>
							<AvgPX>
					...
					<xsl:choose>
						<xsl:when test="boolean(number(normalize-space(substring(COL1,797,19))))">
							<Quantity>
								<xsl:value-of select="number(normalize-space(substring(COL1,797,19)))"/>
							</Quantity>
						</xsl:when>
						<xsl:otherwise>
							<Quantity>
