	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($varMarkPrice) and COL10 ='Pricing'">
					<PositionMaster>
						...
						<xsl:variable name="PB_SYMBOL_NAME">
							<xsl:value-of select = "normalize-space(COL104)"/>
						</xsl:variable>
							...
							<xsl:choose>
								...
								<xsl:when test="$varAsset='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="varSymbol" select="normalize-space(COL103)" />
									</xsl:call-template>
								</xsl:when>
						...
						<xsl:variable name="varCostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$varPrice2"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
