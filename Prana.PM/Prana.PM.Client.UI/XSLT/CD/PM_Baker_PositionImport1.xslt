...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
  ...
  <xsl:template name="GetSuffix">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$Suffix = 'JT'">
        <xsl:value-of select="'-TSE'"/>
      </xsl:when>

      <xsl:when test="$Suffix = 'JO'">
        <xsl:value-of select="'-OSE'"/>
      </xsl:when>

      <xsl:when test="$Suffix = 'JQ'">
        <xsl:value-of select="'-JAQ'"/>
      </xsl:when>

      <xsl:when test="$Suffix = 'TB'">
        <xsl:value-of select="'-SET'"/>
      </xsl:when>

      <xsl:when test="$Suffix = 'SP'">
      ...
      <xsl:when test="$Suffix = 'LI'">
        <xsl:value-of select="'-LSIN'"/>
      </xsl:when>


      <xsl:when test="$Suffix = 'FH'">
      ...
      <xsl:when test="$Suffix = 'SJ'">
        <xsl:value-of select="'-SJE '"/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="ConvertBBCodetoTicker">
    ...
    <xsl:variable name="varExYear">
      <xsl:value-of select="substring(substring-after($varBBCode,' '),3,2)"/>
    </xsl:variable>

    <xsl:variable name="varPutCall">
      <xsl:value-of select="substring(substring-after(substring-after(substring-after(substring-after($varBBCode,' '),' '),' '),' '),1,1)"/>
    </xsl:variable>

    <xsl:variable name="varStrike">
      <xsl:value-of select="format-number(substring-after($varBBCode,'@ '),'#.00')"/>
    </xsl:variable>
    ...
    <xsl:variable name="varExDay">
      <xsl:value-of select="substring(substring-after(substring-after($varBBCode,' '),' '),5,2)"/>
    </xsl:variable>
    ...
    <xsl:variable name="varExMonth">
      <xsl:call-template name="GetMonth">
        <xsl:with-param name="varMonth" select="substring(substring-after($varBBCode,' '),6,3)"/>
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template match="/">
    <DocumentElement>
      ...
      <xsl:for-each select="//PositionMaster">
        ...
        <xsl:if test="number(COL12) and $PRANA_FUND_NAME != ''">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PB_Symbol" select="COL11"/>
            ...
            <xsl:variable name="varSuffix">
              <xsl:choose>
                <xsl:when test ="substring-after(COL6,' ') = 'US'">
                  <xsl:value-of select ="''"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:call-template name="GetSuffix">
                    <xsl:with-param name="Suffix" select="substring-after(COL6,' ')"/>
                  </xsl:call-template>
                </xsl:otherwise>
              </xsl:choose>
            ...
            <!--<Bloomberg>
              <xsl:value-of select="concat(normalize-space(COL6),' EQUITY')"/>
            </Bloomberg>-->
