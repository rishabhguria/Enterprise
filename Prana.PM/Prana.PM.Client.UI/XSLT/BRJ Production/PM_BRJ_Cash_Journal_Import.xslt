  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($varCash) and (COL8='Charges' or COL8='Deposit' or COL8='Withdraw' or COL8='Interest')">
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME">
              <xsl:choose>
                <xsl:when test="contains(COL8,'Interest') and $varCash &gt; 0">
                ...
                <xsl:when test="contains(COL8,'Interest') and $varCash &lt; 0">
				  ...
				  <!--<xsl:when test="contains(COL8,'Charges') and $varCash &gt; 0">
					  <xsl:value-of select="'Interest Income'"/>
				  </xsl:when>-->
                ...
                <xsl:when test="contains(COL8,'Deposit') and $varCash &gt; 0">
                  <xsl:value-of select="'CASH_DEP'"/>
                </xsl:when>

                <xsl:when test="contains(COL8,'Withdraw') and $varCash &lt; 0">
            ...
            <xsl:variable name="varFMonthName">
              <xsl:call-template name="FDate">
                <xsl:with-param name="Month" select="$varMonthName"/>
              </xsl:call-template>
            </xsl:variable>
            
            <xsl:variable name="varYearName">
              <xsl:choose>
                <xsl:when test="string-length(substring-after(substring-after(COL4,'-'),'-'))=2">
                  <xsl:value-of select="concat('20',substring-after(substring-after(COL4,'-'),'-'))"/>
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:variable name="varDateName">
              <xsl:value-of select="concat($varFMonthName,'/',$varDayName,'/',$varYearName)"/>
            </xsl:variable>
