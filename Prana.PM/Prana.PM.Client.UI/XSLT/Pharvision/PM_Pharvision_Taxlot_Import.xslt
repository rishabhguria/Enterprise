    ...
    <xsl:variable name="UnderlyingSymbol">
      <xsl:choose>
        <xsl:when test="contains(COL4,'_')">
          <xsl:value-of select="substring-before(normalize-space(COL4),'_')"/>
        </xsl:when>
        <xsl:otherwise>
        ...
        <xsl:if test="number($varNetPosition)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="varAssetType">
              <xsl:choose>
                <xsl:when test="contains($varAsset,'Stock Options')">
            ...
            <xsl:variable name="varTradeDate">
              <xsl:value-of select="substring-after(substring-after(substring-before(normalize-space(COL2),' '),'-'),'-')"/>
            </xsl:variable>
			...
			<xsl:variable name="varTradeYear">
              <xsl:value-of select="substring(substring-before(normalize-space(COL2),'-'),3,2)"/>
            </xsl:variable>
			...
			<xsl:variable name="varTradeSetDate">
              <xsl:value-of select="substring-after(substring-after(substring-before(normalize-space(COL1),' '),'-'),'-')"/>
            </xsl:variable>
			
			<xsl:variable name="varTradeSetMonth">
              <xsl:value-of select="substring(substring-after(normalize-space(COL1),'-'),1,2)"/>
            </xsl:variable>
			
			<xsl:variable name="varTradeSetYear">
              <xsl:value-of select="substring(substring-before(normalize-space(COL1),'-'),3,2)"/>
            </xsl:variable>
			
            <PositionStartDate>
              <xsl:value-of select="concat($varTradeSetMonth,'/',$varTradeSetDate,'/',$varTradeSetYear)"/>
            </PositionStartDate>
