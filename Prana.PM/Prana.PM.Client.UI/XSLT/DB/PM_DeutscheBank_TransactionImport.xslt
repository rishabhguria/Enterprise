			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test ="COL17!='' and COL17!=0 ">
					...
					<PositionMaster>
						...
						<xsl:variable name="varSymbol" select="substring-before(COL14,'.')"/>
						<xsl:variable name="varSecurityType" select="COL11"/>
						...
						<xsl:variable name ="varCallPutCode">
							<xsl:choose>
								<xsl:when test ="contains(COL13,' CALL')!=False">
									<xsl:value-of select ="'1'"/>
								</xsl:when>
								<xsl:when test ="contains(COL13,' PUT')!=False">
						...
						<xsl:choose>
							<xsl:when test ="$varSecurityType='Equity Option'">
								<Symbol>
									<xsl:value-of select ="concat('O:',$varOption_Underlying,' ',$varOption_Year,$varOption_MonthCode,$varOption_StrikePrice)"/>
								</Symbol>
							</xsl:when>
							<xsl:when test ="$varSecurityType !='Equity Option'">
								<Symbol>
						...
						<xsl:choose>
							<xsl:when  test="number(COL17) &lt; 0 and $varSecurityType !='Equity Option'">
								<SideTagValue>
