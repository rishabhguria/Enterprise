	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity) and (COL13='PURCHASE' or COL13='DIVIDENDS RECEIVED')and COL3!='VPWF8'">
						<PositionMaster>
							...
							<xsl:variable name="AvgPrice">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="format-number(COL7,'##.###') "/>
								</xsl:call-template>
							</xsl:variable>
							<AvgPX>
								<xsl:choose>
							...
							<xsl:variable name="Commission">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="format-number(COL8,'##.###') "/>
								</xsl:call-template>
							</xsl:variable>
							<Commission>
								<xsl:choose>
							...
							<xsl:variable name="varSecFee">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="format-number(COL9,'##.###') "/>
								</xsl:call-template>
							</xsl:variable>
							<SecFee>
								<xsl:choose>
						...
						<OrfFeeBase>
							...
							<Side>
								<xsl:choose>
									...
									<xsl:when test="$varSide ='SECURITY DELIVERIES'">
