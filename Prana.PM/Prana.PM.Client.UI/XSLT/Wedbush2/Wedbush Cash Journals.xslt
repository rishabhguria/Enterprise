  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
		  ...
		  <xsl:if test="number(COL6) and not(contains(COL4,'DIV')) and (normalize-space(normalize-space(COL4)) != 'PDST' and normalize-space(normalize-space(COL4)) != 'MGNADV') and $PRANA_FUND_NAME!=''">
			  <PositionMaster>
				  ...
				  <xsl:variable name="PRANA_ACRONYM_NAME">
					  <xsl:choose>
						  ...
						  <xsl:when test="(normalize-space(COL5) = 'INTEREST' or contains(COL3,'INT') or contains(COL4,'INTR')) and (COL6 &gt; 0)">
						  ...
						  <xsl:when test="(contains(COL3,'CIP') or contains(COL3,'/17') or contains(COL3,'ACH')or normalize-space(COL4) = 'STAX' or normalize-space(COL5) = 'OTHER INC/EXP') and (COL6 &gt; 0)">
							  <xsl:value-of select="'MISC_INC'"/>
						  </xsl:when>
						  <xsl:when test="(contains(COL3,'CIP') or  contains(COL3,'/17') or contains(COL3,'FEE')or  contains(COL3,'NRA')or contains(COL3,'STERLING') or contains(COL3,'ACATS') or normalize-space(COL5) = 'OTHER INC/EXP') and (COL6 &lt; 0)">
							  <xsl:value-of select="'MISC_EXP'"/>
						  </xsl:when>
						  						 					  
						  <xsl:when test="((normalize-space(COL4) = 'CSHMVE') or (normalize-space(COL4) = 'JRL' or normalize-space(COL5) = 'WITHDRAWALS')) and (COL6 &lt; 0)  ">
							  <xsl:value-of select="'CASH_WDL'"/>
						  </xsl:when>
						  
						  <xsl:when test="((normalize-space(COL4) = 'ACTMVT' ) or (normalize-space(COL4) = 'JRL' or normalize-space(COL5) = 'CONTRIBUTIONS')) and (COL6 &gt; 0) ">
