	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		<ThirdPartyFlatFileDetailCollection>
			...
			<ThirdPartyFlatFileDetail>
					...
					<xsl:variable name="PB_NAME" select="'Walleye'"/>
				<Status>
					<xsl:choose>
				...
				<SecFee>
					<xsl:choose>
						...
						<xsl:when test="SettlCurrFxRate!=0 and SettlCurrFxRateCalc='M'">
							<xsl:value-of select="format-number($varSecFee * SettlCurrFxRate,'##.00')"/>
						</xsl:when>

						<xsl:when test="SettlCurrFxRate!=0 and SettlCurrFxRateCalc='D'">
							<xsl:value-of select="format-number($varSecFee div SettlCurrFxRate,'##.00')"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="''"/>
						</xsl:otherwise>
