	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($varAmount) and (contains(normalize-space(COL2),'GCL'))">
            ...
            <xsl:variable name="PRANA_PRE_ACRONYM_NAME">
              <xsl:choose>
                <xsl:when test="$varAmount &gt;0 and (contains(normalize-space(COL2),'GCL'))">
           ...
           <xsl:variable name="PRANA_POST_ACRONYM_NAME">
             <xsl:choose>
			  ...
			  <xsl:when test="$varAmount &gt; 0">
                ...
                <xsl:when test="$varAmount &lt; 0">
                  <xsl:value-of select="concat($PRANA_POST_ACRONYM_NAME, ':' , $varCash , '|' , 'Cash', ':' , $varCash)"/>
                </xsl:when>
