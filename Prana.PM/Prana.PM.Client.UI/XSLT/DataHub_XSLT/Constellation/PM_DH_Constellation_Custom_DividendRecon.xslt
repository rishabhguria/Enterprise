        ...
        <xsl:choose>
          <xsl:when test="number($varDividend) and contains($varDividendIdentify,'Dividend')">
            <PositionMaster>
              ...
              <xsl:variable name="varEXDateMonth">
                <xsl:call-template name="MonthName">
                  <xsl:with-param name="Month" select="substring-before(substring-after(COL8,'-'),'-')"/>
                </xsl:call-template>
              </xsl:variable>
              <xsl:variable name="varEXDateDay" select="substring-before(COL8,'-')"/>
              <xsl:variable name="varEXDateYear" select="substring-after(substring-after(COL8,'-'),'-')"/>
                  
              <xsl:variable name="varEXDate" select="concat($varEXDateMonth,'/',$varEXDateDay,'/','20',$varEXDateYear)"/>
              
              <xsl:variable name="varPDateMonth">
                <xsl:call-template name="MonthName">
                  <xsl:with-param name="Month" select="substring-before(substring-after(COL10,'-'),'-')"/>
                </xsl:call-template>
              </xsl:variable>

              <xsl:variable name="varPDateDay" select="substring-before(COL10,'-')"/>
              <xsl:variable name="varPDateYear" select="substring-after(substring-after(COL10,'-'),'-')"/>

              <xsl:variable name="varPDate" select="concat($varPDateMonth,'/',$varPDateDay,'/','20',$varPDateYear)"/>
