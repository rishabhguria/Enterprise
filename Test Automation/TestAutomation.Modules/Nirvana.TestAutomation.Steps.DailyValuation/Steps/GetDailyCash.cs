...
{
   public class GetDailyCash : DailyCashUIMap, ITestStep
    {
       /// <summary>
       /// Run the test
       /// </summary>
       ...
       /// <returns></returns>
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
               // Wait(5000);
                ...
                {
                    DailyCashData(inputData, selectDate, copyDate);
                }
                else
                {
                    MonthlyCash(inputData);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="copyDate">Copy Date.</param>
        private void DailyCashData(DataTable inputData, String selectDate, String copyDate)
        {
            try
            {
        ...
        /// </summary>
        /// <param name="inputData">The test Data.</param>
        private void MonthlyCash(DataTable inputData)
        {
            try
            {
                String date = string.Empty;
