	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity)and (not(contains(COL7,'Equity Swap')) and not(contains(COL18,'FUTURES')) and not(contains(COL6,'SPOT')) and  not(contains(COL6,'CDX.')) and not(contains(COL6,'USD'))) ">
						<PositionMaster>
							...
							<xsl:variable name="Symbol">
								<xsl:choose>
									...
									<xsl:when test="COL4='USD' and COL7='Equity Swap'">
										<xsl:value-of select="concat(COL8,' ','US')"/>
									</xsl:when>
