  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//Comparision">
        ...
        <xsl:choose>
          <xsl:when test="number($Quantity) and contains(COL26,'Trade')">
            <PositionMaster>
              ...
              <xsl:variable name="varSymbol" select="normalize-space(COL19)"/>
              ...
              <xsl:variable name="PB_FUND_NAME" select="'Tebuah Capital LP'"/>
              ...
              <xsl:variable name="varSide" select="normalize-space(COL9)"/>
              <Side>
                <xsl:choose>
                  <xsl:when test="contains($varSide,'BUY LONG')">
                    <xsl:value-of select="'Buy'"/>
                  </xsl:when>
                  <xsl:when test="contains($varSide,'BUY TO COVER')">
                    <xsl:value-of select="'Buy to Close'"/>
                  </xsl:when>
                  <xsl:when test="contains($varSide,'SELL SHORT')">
                    <xsl:value-of select="'Sell short'"/>
                  </xsl:when>
                  <xsl:when test="$varSide='SELL' or $varSide='SELL LONG'">
