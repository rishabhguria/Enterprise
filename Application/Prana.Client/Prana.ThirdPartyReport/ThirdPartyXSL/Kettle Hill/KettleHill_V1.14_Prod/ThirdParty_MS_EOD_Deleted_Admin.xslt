...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetailThirdPartyFlatFileDetail[CounterParty ='MSCO' or CounterParty ='MSCOS' or (contains(AccountName, 'Morgan Stanley'))]">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="PB_NAME" select="'MS Passport'"/>
		...
		<xsl:variable name="COMM">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of select="$CommissionSum * $varFXRate"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
