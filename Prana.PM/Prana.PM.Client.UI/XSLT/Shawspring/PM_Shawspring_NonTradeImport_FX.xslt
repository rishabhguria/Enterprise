	...
	<xsl:template match="/">
		...
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($varAmount1) and COL7='FX'">
					...
					<PositionMaster>
						...
						<JournalEntries>
							<xsl:value-of select="concat('Cash', ':' , $varAmount1 , '|' , 'Cash', ':' , $varAmount2s)"/>
						</JournalEntries>

						<xsl:variable name="FXRate">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="normalize-space(COL12) div normalize-space(COL14)"/>
							</xsl:call-template>
						</xsl:variable>
