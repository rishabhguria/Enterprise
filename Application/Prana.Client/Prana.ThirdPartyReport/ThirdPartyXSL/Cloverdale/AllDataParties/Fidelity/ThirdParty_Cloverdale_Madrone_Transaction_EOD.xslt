...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[not(contains(AccountName,'AAAAAAAAAAAAAAAA'))]">
				<ThirdPartyFlatFileDetail>
					...
					<Side>
						<xsl:choose>
							<xsl:when test="Asset='EquityOption'">
								<xsl:choose>
									<xsl:when test="(contains(Side,'Buy') and contains(PutOrCall,'CALL')) or (contains(Side,'Sell') and contains(PutOrCall,'PUT'))">
										<xsl:value-of select="'Long'"/>
									</xsl:when>
									<xsl:when test="(contains(Side,'Buy') and contains(PutOrCall,'PUT')) or (contains(Side,'Sell') and contains(PutOrCall,'CALL'))">
