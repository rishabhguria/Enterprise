			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test ="number($Position)">
					<PositionMaster>
						...
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="$Asset='Equity' or $Asset='PrivateEquity' and $varside='Sell'">
									<xsl:value-of select="'2'"/>
								</xsl:when>
								<xsl:when test="$Asset='Equity' or $Asset='PrivateEquity' and $varside='Buy'">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="$Asset='Equity' and $varside='Buy to Close'">
									<xsl:value-of select="'B'"/>
								</xsl:when>

								<xsl:when test="$Asset='FixedIncome' and $varside='Sell'">
									<xsl:value-of select="'2'"/>
								</xsl:when>
								<xsl:when test="$Asset='FixedIncome' and $varside='Buy'">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="$Asset='FixedIncome' and $varside='Buy to Close'">
									<xsl:value-of select="'B'"/>
								</xsl:when>
