	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity) and COL13!='CURRENCY' and COL13!='LOANS'">
						<PositionMaster>
							<xsl:variable name="PB_NAME">
								<xsl:value-of select="'Gemini'"/>
							</xsl:variable>

							<xsl:variable name = "PB_SYMBOL_NAME" >
								<xsl:choose>
									
									<xsl:when test="COL13 ='CURRENCY CONTRACTS'">
							...
							<xsl:variable name="Asset">
								<xsl:choose>
									<xsl:when test="COL13 ='OPTION'">
										<xsl:value-of select="'EquityOption'"/>
									</xsl:when>
									<xsl:when test="COL13 ='CURRENCY CONTRACTS'">
										<xsl:value-of select="'FX'"/>
									</xsl:when>
									<xsl:when test="contains(COL13,'SWAP')">
							...
							<xsl:variable name="varBStrikeAndPutCal">
								<xsl:value-of select="substring-after(substring-after(substring-after(COL4,' '),' '),' ')"/>
							</xsl:variable>

							
							<xsl:variable name="varUnderlyingIndex">
								<xsl:value-of select="substring-before(substring-after(substring-after(substring-after(COL6,' '),' '),' '),' ')"/>
							</xsl:variable>
							<xsl:variable name="varBDateIndex">
								<xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(COL6,' '),' '),' '),' '),' '),' ')"/>
							</xsl:variable>
							<xsl:variable name="varBStrikeAndPutCalIndex">
								<xsl:value-of select="substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(COL6,' '),' '),' '),' '),' '),' ')"/>
							</xsl:variable>


							<xsl:variable name="varSymbol">
								<xsl:choose>
									<xsl:when test="$Asset = 'EquityOption' and contains(COL4,'INDEX')">
										<xsl:value-of select="concat($varUnderlyingIndex,' ',$varBDateIndex,' ',$varBStrikeAndPutCalIndex)"/>
									</xsl:when>
									<xsl:when test="$Asset = 'EquityOption'">
										<xsl:value-of select="concat($varUnderlying,' ',$varBDate,' ',$varBStrikeAndPutCal)"/>
									</xsl:when>
									<xsl:when test="$Asset = 'FX'">
										<xsl:value-of select="concat(COL5,'Fwd')"/>
									</xsl:when>
									<xsl:when test="$Asset = 'Equity' and $Suffix =''">
										<xsl:value-of select="concat(COL5,' US')"/>
									</xsl:when>
									<xsl:when test="$Asset = 'EquitySwap' and (COL5='' or COL5='*')">
										<xsl:value-of select="COL6"/>
									</xsl:when>
									<xsl:otherwise>
