			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:variable name="Position">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="normalize-space(COL106)"/>
					</xsl:call-template>
				</xsl:variable>
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<SideTagValue>							
							<xsl:choose>
								<xsl:when test="contains(normalize-space(COL85),'Buy')">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL85),'Sell')">
						...
						<xsl:variable name="CostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="normalize-space(COL108)"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="SecFee">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="normalize-space(COL115)"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="OrfFee">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="normalize-space(COL116)"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="varPositionStartDate">
                            <xsl:value-of select="normalize-space(COL100)"/>						
						</xsl:variable>
						...
						<xsl:variable name="varPositionSettlementDate">
                            <xsl:value-of select="normalize-space(COL101)"/>											
						</xsl:variable>
