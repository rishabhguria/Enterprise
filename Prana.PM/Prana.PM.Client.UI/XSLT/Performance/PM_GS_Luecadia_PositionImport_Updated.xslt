...
>
  ...
  <xsl:template name="Option">
    ...
    <xsl:if test="string-length(COL5) &gt; 20">
      ...
      <xsl:variable name="StrikePrice">
        <xsl:value-of select="format-number(concat(substring(substring-after($Symbol,$UnderlyingSymbol),9,5),'.',substring(substring-after($Symbol,$UnderlyingSymbol),14)),'#.00')"/>
      </xsl:variable>
        ...
        <xsl:if test="normalize-space(COL2)!='Fund' and number(COL7)and not(contains(COL4,'RTS/'))">
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_SymbolTicker_NAME">
              <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='GS']/SymbolData[@CompanyName=$PB_SymbolTicker_NAME]/@PranaSymbol"/>
            </xsl:variable>
