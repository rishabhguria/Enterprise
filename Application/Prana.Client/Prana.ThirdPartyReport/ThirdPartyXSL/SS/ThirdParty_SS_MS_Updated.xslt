...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[contains(FundName , 'FCM') != 'true']">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="CPVar">
			<xsl:choose>
				...
				<xsl:when test="$tempCPVar='BTIGALGO'">BTIG</xsl:when>
				<xsl:otherwise>
		...
		<Group 
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<xsl:variable name="varPBundCode">
					<xsl:value-of select="document('../ReconMappingXml/FundMapping.xml')/FundMapping/PB[@Name='MS']/FundData [@PranaFund = $varFundName]/@PBFundCode"/>
				</xsl:variable>

				<xsl:variable name="CPVara">
					<xsl:choose>
						<xsl:when test="CounterParty='BTIGALGO'">BTIG</xsl:when>
						<xsl:otherwise>
				...
				<ThirdPartyFlatFileDetail
							...
							Associated="{PBUniqueID}" ExecAccount="038238804" CustAccount="{$varPBundCode}"
							ExecBkr="{$CPVara}" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{FullSecurityName}"
