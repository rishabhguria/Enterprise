			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="$varSec_Type!='CA'">
					...
					<xsl:choose>
						...
						<xsl:variable name ="varCallPutCode">
							<xsl:choose>
								...
								<xsl:when test ="normalize-space(substring(COL1,25,2))='pl'">
									<xsl:value-of select ="'0'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select ="''"/>
					...
					<xsl:choose>
						<xsl:when test ="$varCallPutCode!=''">
							<PBSymbol>
								<xsl:value-of select ="substring(COL1,127,30)"/>
							</PBSymbol>
						</xsl:when>
						<xsl:otherwise>
							<PBSymbol>
						...
						<xsl:choose>
							<xsl:when  test="substring(COL1,18,6)!=''">
								<PositionStartDate>
