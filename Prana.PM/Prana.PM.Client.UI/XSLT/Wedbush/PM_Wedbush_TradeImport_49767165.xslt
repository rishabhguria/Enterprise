  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
			...
			<xsl:if test="number(COL4) and (COL2='49767165')">			
          <PositionMaster>
			  <xsl:variable name = "PB_COMPANY" >
				  <xsl:value-of select="COL18"/>
			  </xsl:variable>
			  <xsl:variable name="PRANA_SYMBOL">
				  <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='WedbushP']/SymbolData[@PBCompanyName=$PB_COMPANY]/@PranaSymbol"/>
			  </xsl:variable>
			  ...
			  <xsl:variable name="PRANA_FUND_NAME">
				  <xsl:value-of select="document('../ReconMappingXml/AccountMapping.xml')/FundMapping/PB[@Name='WedbushP']/FundData[@PBFundCode=$PB_FUND_NAME]/@PranaFund"/>
			  </xsl:variable>

			  <AccountName>
				  <xsl:choose>
					  <xsl:when test="$PB_FUND_NAME ='49767165'">
						  <xsl:value-of select="'AXELTECHNOLOGIES LLC-49767165'"/>
					  </xsl:when>
					  <xsl:otherwise>
            ...
            <xsl:variable name="varSymbol2" select="substring-after(COL5,$varSingleQuote)"/>
			  <xsl:variable name="varSymbol1" select="substring-before(COL5,$varSingleQuote)"/>
