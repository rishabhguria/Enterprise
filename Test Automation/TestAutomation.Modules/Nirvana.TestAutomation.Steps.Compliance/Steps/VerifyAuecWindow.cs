...
{
    class VerifyAuecWindow : PopUpUIMap, ITestStep
    {
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                PopUpUIMap.DumpWindow("AuecWindow");
                StringBuilder errorBuilder = new StringBuilder();
                string DevPath = Path.GetFullPath(Path.Combine(Environment.CurrentDirectory, @"..\..\..\.."));
                DataTable xmltoDataTable = CommonMethods.ReadXML(DevPath + TestDataConstants.DUMP_PATH + "AuecWindow.xml");
                List<string> keyColumns = new List<string>() { "auecId"};
