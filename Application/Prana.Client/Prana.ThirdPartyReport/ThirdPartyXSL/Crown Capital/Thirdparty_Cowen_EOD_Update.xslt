...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:my="put-your-namespace-uri-here">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[CounterParty ='CCMB' or CounterParty ='CONC' or (contains(AccountName, 'KH'))]">
				...
				<ThirdPartyFlatFileDetail>
					...
					<xsl:variable name="PB_NAME" select="'PershingThirdParty'"/>
					...
					<StrikePrice>
						<xsl:choose>
							<xsl:when test ="number(StrikePrice)">
								<xsl:value-of select="format-number(StrikePrice,'0.####')"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="''"/>
							</xsl:otherwise>
					...
					<BookingCategory>
						<xsl:choose>
							<xsl:when test="CounterParty='CCMB' and contains($PRANA_FUND_NAME, 'KH')">
								<xsl:value-of select="'CPS-House'"/>
							</xsl:when>
							<xsl:when test="CounterParty='CONC' and contains($PRANA_FUND_NAME, 'KH')">
								<xsl:value-of select="'CPS-House'"/>
							</xsl:when>
							<xsl:when test="CounterParty='CCMB' and not(contains($PRANA_FUND_NAME, 'KH'))">
								<xsl:value-of select="'CPS-DVP'"/>
							</xsl:when>
							<xsl:when test="CounterParty='CONC' and not(contains($PRANA_FUND_NAME, 'KH'))">
								<xsl:value-of select="'CPS-DVP'"/>
							</xsl:when>
							<xsl:otherwise>
					...
					<xsl:variable name="Commission8" select="($VarComm div AllocatedQty) * 100"/>


					<CommissionCentsPerLot>
						<xsl:value-of select="format-number($Commission8,'0.####')"/>
					</CommissionCentsPerLot>
					...
					<xsl:variable name = "OthFees">
						<xsl:value-of select="(SecFee + StampDuty + TaxOnCommissions + ClearingFee + OtherBrokerFee + MiscFees + TransactionLevy + OrfFee)"/>
					...
					<CommissionFlatFee>
						<xsl:value-of select="format-number($VarfEES,'0.####')"/>
					</CommissionFlatFee>
					...
					<xsl:variable name="DateResult">
						<xsl:value-of select="my:Now(TradeDate)"/>
					</xsl:variable>

					<ExecutionState>
						<!--<xsl:value-of select="$varTaxlotState"/>-->
						<xsl:choose>
							<xsl:when test="$DateResult = 0 and TaxLotState='Deleted'">
								<xsl:value-of select ="'Cancel'"/>
							</xsl:when>
							<xsl:when test ="$DateResult &lt; 0 and TaxLotState='Deleted'">
								<xsl:value-of select="'PriorDayCancel'"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$varTaxlotState"/>
							</xsl:otherwise>
					...
					<ClientID>
						<xsl:value-of select="'Kettle Hill'"/>
					</ClientID>
