  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
        <xsl:if test="normalize-space(COL10) = '0' and  normalize-space(COL16) != 'DIV' and   normalize-space(COL16) != 'TFR'">
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_FUND_NAME">
			  ...
			  <xsl:variable name = "PRANA_ACRONYM_NAME">
				  <xsl:choose>
					  <xsl:when test ="COL14 &gt; 0 and (normalize-space($varDescription) = 'WIR' or normalize-space($varDescription) = 'TFR') ">
						  <xsl:value-of select="'CASH-WDL'"/>
					  </xsl:when>
					  <xsl:when test ="COL14 &lt; 0 and (normalize-space($varDescription) = 'WIR' or normalize-space($varDescription) = 'TFR')">
						  <xsl:value-of select="'CASH-DEP'"/>
					  </xsl:when>
					  <xsl:when test ="COL14 &gt; 0 and normalize-space($varDescription) = 'INT'">
						  <xsl:value-of select="'Interest_Expense'"/>
					  </xsl:when>
					  <xsl:when test ="COL14 &lt; 0 and normalize-space($varDescription) = 'INT'">
						  <xsl:value-of select="'Interest_Income'"/>
					  </xsl:when>
					  <xsl:when test ="COL14 &gt; 0 and normalize-space($varDescription) = 'FEE'">
