...
{
    class OperationsOnNIC : IUIAutomationTestStep
    {
        ...
        public TestResult RunUIAutomationTest(DataSet testData, Dictionary<int, string> sheetIndexToName) 
        {
            ...
            try
            {
                if (!testData.Tables[0].Columns.Contains("Market Value")) {
                    return _result;
                }
                ...
                uiAutomationHelper.FindAndClickElement(ApplicationArguments.mapdictionary["RebalancerTab"].AutomationUniqueValue);
                ...
                IUIAutomationCondition condition = automation.CreatePropertyCondition(
                    ...
                    ApplicationArguments.mapdictionary["NAV Impacting Components Groups"].AutomationUniqueValue
                );
                IUIAutomationElement element = gridElement.FindFirst(TreeScope.TreeScope_Descendants, condition);

                if (element != null)
                {
                    var legacyPatternObj = element.GetCurrentPattern(UIA_PatternIds.UIA_LegacyIAccessiblePatternId) as IUIAutomationLegacyIAccessiblePattern;
                    ...
                    if (legacyPatternObj != null)
                    {
                        ...
                        if(legacyState.ToString().Contains("Expand"))
                        {
                            uiAutomationHelper.FindAndClickElement(ApplicationArguments.mapdictionary["NAV Impacting Components Groups"].AutomationUniqueValue);
                            System.Threading.Thread.Sleep(2000);
                        }
                    }
                }
                ...
                gridElement = appWindow.FindFirst(
                        TreeScope.TreeScope_Descendants,
                        automation.CreatePropertyCondition(UIA_PropertyIds.UIA_AutomationIdPropertyId, ApplicationArguments.mapdictionary["NAVGridGridName"].AutomationUniqueValue));
                 ...
                 for (int i = 0; i < rawChildren.Length; i++)
                 {
                     ...
                     if (child.CurrentControlType == UIA_ControlTypeIds.UIA_DataItemControlTypeId)
                     {
                         ...
                         for (int j = 1; j < dataItemChildren.Length; j++)
                         {
                             ...
                             if(dr[j - 1].ToString().Contains("ToggleState"))
                             {
                                 ...
                                 if(!dr[j - 1].ToString().Equals(togglePattern.CurrentToggleState.ToString()))
                                 {
                                     togglePattern.Toggle();
                                 }
                                 var navChild = checkBoxElement.FindAll(TreeScope.TreeScope_Subtree, automation.CreateTrueCondition());
                                 for (int k = 0; k < navChild.Length; k++) 
                                 {
                                     if (navChild.GetElement(k).CurrentControlType == UIA_ControlTypeIds.UIA_EditControlTypeId) {
                                         GridDataProvider.click(navChild.GetElement(k));
                                         System.Threading.Thread.Sleep(1000);
                                         DataUtilities.clearTextData(0);
                                         Keyboard.SendKeys(dr["User Adjusted NAV"].ToString());
                                         System.Threading.Thread.Sleep(2000);
                                     }
                                 }
                             }
