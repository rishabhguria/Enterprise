...
--P_AL_GetAllocationStateNew '1800-01-01'
CREATE Procedure [dbo].[P_AL_GetAllocationStateNew]
(                          
	@FromDate DateTime 
	--,@Symbol Varchar(200),  
	--@Level2ID int                         
...
Select T.TaxlotID, T.GroupID, T.Symbol,T.TaxlotOpenQty * dbo.GetSideMultiplier(T.OrderSideTagValue) as TaxlotOpenQty, 
T.FundID,T.Level2ID, Ltrim(rtrim(T.OrderSideTagValue)) as OrderSideTagValue, T.AvgPrice, T.OpenTotalCommissionandFees,G.IsSwapped
...
--and T.symbol ='CHS' and T.Level2ID=1
--and T.Level2ID =@Level2ID
--Where Taxlot_PK in
--(
...
SELECT 
	...
	TAX.FundID,
	TAX.Level2ID,
	TAX.OrderSideTagValue,
	SUM(TAX.TaxlotOpenQty ) AS CumQuantity,
	--,SUM(TAX.TaxlotOpenQty * dbo.GetSideMultiplier(TAX.OrderSideTagValue)) AS CumQuantity,
	SUM(
		CASE
			WHEN VSEC.AssetId in (8,13) THEN
				(TAX.TaxlotOpenQty * IsNull(TAX.AvgPrice,0) * VSEC.Multiplier)/100 
			ELSE
				TAX.TaxlotOpenQty * IsNull(TAX.AvgPrice,0) * VSEC.Multiplier 
		END
		) as NetNotional
	into #FinalData
from 
...
select Symbol,FundID,Level2ID,OrderSideTagValue, CumQuantity,NetNotional from #FinalData
where CumQuantity<>0

--Select * from PM_TAXLOTS

DROP TABLE #TEMPAllocationState,#TEMPVsecData,#TempTaxlot_PK,#FinalData
COMMIT
