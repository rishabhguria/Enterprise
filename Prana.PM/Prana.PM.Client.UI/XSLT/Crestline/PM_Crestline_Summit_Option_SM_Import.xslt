...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL8),'/'),'/'),' '),1,1)"/>
		</xsl:variable>		
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(normalize-space(COL8),'/'),'/'),' '),2),'##.00')"/>
		</xsl:variable>
				...
				<xsl:if test="$varAssetType='EquityOption'">
					...
					<PositionMaster>
						...
						<xsl:variable name="varExpiryYear">
							<xsl:value-of select="concat('20',substring-before(substring-after(substring-after(normalize-space(COL8),'/'),'/'),' '))"/>
						</xsl:variable>
