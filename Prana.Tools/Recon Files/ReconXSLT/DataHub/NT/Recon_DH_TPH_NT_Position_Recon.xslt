	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<!--<xsl:when test="number($Quantity) and COL33!='Cash and Cash Equivalents' ">-->
						<xsl:when test="number($Quantity) and COL33!='Cash and Cash Equivalents' and not(contains(COL17,'OTC DERIVATIVE CASH'))">
						<PositionMaster>
							...
							<Symbol>
								<xsl:choose>
									...
									<xsl:when test="contains(COL36,'TICKER')">
										<xsl:value-of select="normalize-space(COL40)"/>
									</xsl:when>
									<!--<xsl:when test="contains(COL36,'SEDOL')">
										<xsl:value-of select="''"/>
									</xsl:when>-->
									<xsl:otherwise>
