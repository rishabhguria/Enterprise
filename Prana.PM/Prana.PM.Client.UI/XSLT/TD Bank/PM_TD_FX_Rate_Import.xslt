  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="normalize-space(COL4)='Cash' and normalize-space(COL8)!='USD'">
            ...
            <xsl:variable name="FXRate">
              <xsl:choose>
                <xsl:when test ="$PB_CURRENCY_NAME='GBP' or $PB_CURRENCY_NAME='EUR' or $PB_CURRENCY_NAME='AUD' or $PB_CURRENCY_NAME='NZD'">
                  <xsl:value-of select="number(COL12) div number(COL11)"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="number(COL11) div number(COL12)"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:variable>
