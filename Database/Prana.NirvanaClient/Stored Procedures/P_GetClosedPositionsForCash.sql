CREATE PROCEDURE [dbo].[P_GetClosedPositionsForCash] (              
...
BEGIN              
 ...
 IF (              
  ...
  FROM dbo.Split(@Local_Symbols, ',')          
              
 SELECT             
   @YesterDayDATE as RunDate            
...
ON (PTC.ClosingTaxlotID = PT1.TaxlotID AND PTC.TaxLotClosingId = PT1.TaxLotClosingId_Fk)          
...
on PT.FundID = CP.FundID             
 ...
 WHERE (              
  ...
  AND Datediff(d, @YesterDayDATE, PTC.AUECLocalDate) >= 0 AND Datediff(d, PTC.AUECLocalDate, @YesterDayDATE) >= 0           
and G.AssetID in (8,13) and G1.AssetID in (8,13)          
and CP.IsAccruedTillSettlement = 1    
