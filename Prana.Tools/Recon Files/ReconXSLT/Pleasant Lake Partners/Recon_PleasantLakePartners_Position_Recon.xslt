	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="normalize-space(COL2) !='CASH'">
					...
					<PositionMaster>
						...
						<xsl:variable name="varAssetType">
						    <xsl:choose>
						    	<xsl:when test="normalize-space(COL2)='Call Option' or normalize-space(COL2)='Put Option'">
						...
						<xsl:variable name="MarkPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="number(COL8)"/>
							</xsl:call-template>
						</xsl:variable>
