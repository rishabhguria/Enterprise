		...
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name="Quantity">
					<xsl:choose>
						<xsl:when test="COL30='1'">
							<xsl:value-of select="COL32"/>
						</xsl:when>
						<xsl:when test="COL30='2'">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity) and COL21='F'">
						<PositionMaster>
							...
							<xsl:variable name="TotalCommissionandFees">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="COL51 + COL55"/>
								</xsl:call-template>
							</xsl:variable>
							...
							<xsl:variable name="varMontH">
								<xsl:value-of select="substring(COL24,5,2)"/>
							</xsl:variable>

							<xsl:variable name="varDay">
								<xsl:value-of select="substring(COL24,7,2)"/>
							</xsl:variable>

							<xsl:variable name="varYear">
								<xsl:value-of select="substring(COL24,1,4)"/>
							</xsl:variable>
