...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="varThirdFriday">
			...
			<xsl:choose>
				<xsl:when test="contains(COL14,'Option') and number($varYear) and number($varMonth)">
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test ="number($MarkPrice)">
					...
					<PositionMaster>
						...
						<xsl:variable name="varSymbol" select="COL6"/>


						<xsl:variable name="PB_SUFFIX_NAME">
							<xsl:choose>
								<xsl:when test="COL14='Equity'">
										...
										<xsl:when test="COL14='Equity'">
											<xsl:choose>
												<xsl:when test="$PRANA_SUFFIX_NAME!=''">
													<xsl:value-of select="normalize-space(concat(substring-before(COL6,' '),$PRANA_SUFFIX_NAME))"/>
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of select="$varSymbol"/>
												</xsl:otherwise>
