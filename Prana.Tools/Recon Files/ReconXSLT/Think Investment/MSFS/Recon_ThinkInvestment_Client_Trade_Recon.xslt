	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL19,'OPTION')">
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL),'/'),'/'),' '),' '),1,1)"/>
			</xsl:variable>
				...
				<xsl:if test="number($Quantity) ">
					<PositionMaster>
						...
						<xsl:variable name="varFuture">
							<xsl:choose>
								<xsl:when test="contains(COL19,'FUTURE') and contains(COL4,'IS')">
