				...
				<xsl:if test="number($varNetPosition)">
					...
					<PositionMaster>
						...
						<xsl:variable name="varFullDate">
							<xsl:value-of select="concat($varPMonth,'/',$varPDate,'/',$varPYear)"/>
						</xsl:variable>
						...
						<xsl:variable name="varSMonth">
							<xsl:value-of select="concat(0,substring-before(normalize-space(COL2),'/'))"/>
						</xsl:variable>
						...
						<xsl:variable name="varSFullDate">
							<xsl:value-of select="concat($varSMonth,'/',$varSDate,'/',$varSYear)"/>
						</xsl:variable>

						<PositionSettlementDate>
							<xsl:value-of select="$varSFullDate"/>
						</PositionSettlementDate>
