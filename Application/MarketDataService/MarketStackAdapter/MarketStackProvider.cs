...
{
    public class MarketStackProvider : ILiveFeedProvider
    {
        /// <summary>
        ...
        public MarketStackProvider()
        {
            Initialize();
        }
        /// <summary>
        /// Initializes a new instance of the <see cref="MarketStackProvider"/> class.
        /// </summary>
        private void Initialize()
        {
            #region Configs
            //Read MarketStack specific config file
            ...
            Type type = typeof(MarketDataParameter);
            ...
            pricingSourceCol.DefaultValue = "MarketStack";
        ...
        private DataTable GetLiveFeedData(Dictionary<string, string> openSymbols)
        {
            ...
            Dictionary<string, MarketDataParameter> results = new Dictionary<string, MarketDataParameter>();
            string URL = GetURL();
            foreach (KeyValuePair<string, string> kvp in openSymbols)
            {
                try
                {
                    string json = new WebClient().DownloadString(URL + kvp.Value);
                    var ab = JsonConvert.DeserializeObject<MarketDataRoot>(json);
                    if (ab == null)
                    {
                        MarketDataParameter res = new MarketDataParameter();
                        res.open = 0;
                        res.high = 0;
                        res.low = 0;
                        res.last = 0;
                    ...
                    {
                        MarketDataParameter res = ab.data[0];
                        //ResponseModification(res);
                        results.Add(kvp.Key, res);
                    }
                }
        ...
        {
            string URL = "http://api.marketstack.com/v1/intraday/latest?access_key=" + GetApiKey() + "&symbols=";
            return URL;
        }
        ...
        private void ResponseModification(MarketDataParameter res)
        {
        ...
        /// <returns></returns>
        private DataTable CreateDataTable(Dictionary<string, MarketDataParameter> dict)
        {
            try
            {
