      ...
      <xsl:for-each select="//PositionMaster">
        <xsl:if test="number(COL4)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PB_CountnerParty" select="normalize-space(COL5)"/>
            <xsl:variable name="PRANA_CounterPartyID">
              <xsl:value-of select="document('../ReconMappingXml/ExecBrokerMapping.xml')/BrokerMapping/PB[@Name= $varPBName]/BrokerData[@GSBroker=$PB_CountnerParty]/@PranaBrokerCode"/>
            </xsl:variable>
