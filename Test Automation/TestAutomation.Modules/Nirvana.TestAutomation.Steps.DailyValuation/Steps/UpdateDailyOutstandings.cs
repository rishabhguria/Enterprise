...
{
  public class UpdateDailyOutstandings : DailyOutstandingsUIMap, ITestStep
    {
      /// <summary>
      /// Run the test.
      /// </summary>
      ...
      /// <returns></returns>
        public TestResult RunTest(System.Data.DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                UpdateDailyOutstandingsData(testData, sheetIndexToName);
      ...
      /// <param name="testData">The test data.</param>
      /// <param name="sheetIndexToName">The sheet no.</param>
        private void UpdateDailyOutstandingsData(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            try
            {

                DataTable dtDailyOutstandings = CSVHelper.CSVAsDataTable(GrdPivotDisplay.InvokeMethod("GetAllVisibleDataFromTheGrid", null).ToString());
                ...
                {
                    DataRow[] foundRow = dtDailyOutstandings.Select(String.Format(@"[" + TestDataConstants.COL_SYMBOL + "] ='{0}'", dataRow[TestDataConstants.COL_SYMBOL]));
                    if (foundRow.Length > 0)
                    {
                        int index = dtDailyOutstandings.Rows.IndexOf(foundRow[0]);
                        ...
                        {
                            columToMSAAIndexMapping = gridRow.GetColumnIndexMaping(dtDailyOutstandings);
