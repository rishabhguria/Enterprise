...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[contains(Asset,'FX')='true']">
				...
				<xsl:if test="(1=position()) or(preceding-sibling::*[1]/PBUniqueID != PBUniqueID)">
					<xsl:if test="CounterParty != 'Undefined' and AccountName!='Quantum Partners LP'">
    ...
    <!--<xsl:variable name="PB_NAME" select="'G2'"/>-->
		...
		<Group
					...
					TransactionType = "FX002" TransactionStatus="{$varTransactionStatus}" TransactionLevel="B" Product_Intrumenttype="{$varIntrumenttype}" ClientTraqdeRefNo="{PBUniqueID}"
						AssociatedTrade_RequestId="{PBUniqueID}" ExecutionAccount="038306460" AccountId_Fund="038306460" ExecutingBroker="{$CPVar}" TradeDate="{TradeDate}"
						ValueDate="{SettlementDate}" BuyQuantity="{$varBuyQuantity}" SellQuantity="{$varSellQuantity}" BuyCCY="{$varBuyCurrency}" SellCCY="{$varSellCurrency}" DealtCurrency="{$varDealtCurr}"
...
<xsl:value-of select="AllocatedQty div $FXRate"/>
				...
				<ThirdPartyFlatFileDetail
						...
						AssociatedTrade_RequestId="{PBUniqueID}" ExecutionAccount="038306460" AccountId_Fund="{$AccountId_Alloc}" ExecutingBroker="{$CPVar}" TradeDate="{TradeDate}"
						ValueDate="{SettlementDate}" BuyQuantity="{$varBuyQuantity_Alloc}" SellQuantity="{$varSellQuantity_Alloc}" BuyCCY="{$varBuyCurrency}" SellCCY="{$varSellCurrency}" DealtCurrency="{$varDealtCurr}"
						Rate="{$VarAvgPrice}"	NdfFlag="FALSE" NdfFixingDate="" 	NdfLinkedTrade=""	PB="MSCO" FarValueDate=""
						FarValueRate="" ClientBaseEquivalent=""
						HedgeorSpeculative=""
						TaxIndicator=""
