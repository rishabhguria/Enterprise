...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		...
		<Groups>
		   ...
		   EntityID="{EntityID}" TaxLotState="{TaxLotState}" TaxLotState1="" FileHeader="FALSE" FileFooter="FALSE" RowHeader="false"/>
			...
			<xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail[AccountName='UCIT-GS']">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name ="varCommission">
			<xsl:choose>
				<xsl:when test ="IsSwapped='true'and CurrencySymbol!='USD'">
				...
				<xsl:when test ="IsSwapped='true'and CurrencySymbol='USD'">
					<xsl:value-of  select="format-number(CommissionCharged,'0.##')"/>
					<!--<xsl:value-of  select="format-number((CommissionCharged div (AllocatedQty * $varAvgPrice )),'0.########')"/>-->
				</xsl:when>
				<xsl:when test ="Asset = 'EquityOption'">
					<xsl:value-of  select="format-number(CommissionCharged div AllocatedQty ,'0.######')"/>
				</xsl:when>
		...
		<xsl:variable name ="varAccount">
			<xsl:choose>
				<xsl:when test ="IsSwapped='true'">
					<!--<xsl:value-of select="concat(concat('=&quot;','050033786'),'&quot;')"/>-->
					<xsl:value-of  select="'050521715'"/>
				</xsl:when>
				<xsl:otherwise>
					<!--<xsl:value-of select="concat(concat('=&quot;','002674034'),'&quot;')"/>-->
					<xsl:value-of  select="''"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
