	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:variable name="varDividendUSD">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="COL11"/>
					</xsl:call-template>
				</xsl:variable>
							
			   <xsl:variable name="varDividendNonUSD">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="COL9"/>
					</xsl:call-template>
				</xsl:variable>

				<xsl:if test="($varDividendUSD &gt; 0.1 or $varDividendUSD &lt; -0.1) ">
					...
					<PositionMaster>
						...
						<Amount>
							<xsl:choose>								
								<xsl:when test="number($varDividendUSD) and normalize-space(COL8) = 'USD'">
									<xsl:value-of select="-1 * $varDividendUSD"/>
								</xsl:when>								
								<xsl:otherwise>
									<xsl:value-of select="-1 * $varDividendNonUSD"/>
								</xsl:otherwise>
							</xsl:choose>
						</Amount>
						...
						<xsl:choose>								
							   <xsl:when test ="normalize-space(COL8) != 'USD'">
									<xsl:value-of select ="'Dividend cancelled'"/>
								</xsl:when>
								 <xsl:when test ="normalize-space(COL8) = 'USD'">
									<xsl:value-of select ="'Dividend correction'"/>
								</xsl:when>					      	
								<xsl:otherwise>
									<xsl:value-of select="''"/>
								</xsl:otherwise>
						...
						<ActivityType>
							<xsl:choose>
								<xsl:when test="$varDividendUSD &gt; 0">
