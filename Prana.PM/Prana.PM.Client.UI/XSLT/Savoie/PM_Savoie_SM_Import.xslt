	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and (contains(substring-before(COL7,' '),'CALL') or contains(substring-before(COL7,' '),'PUT'))">
					<PositionMaster>
						...
						<xsl:variable name ="Asset">
							<xsl:choose>
								<xsl:when test="contains(substring-before(COL7,' '),'CALL') or contains(substring-before(COL7,' '),'PUT')">
						...
						<PutOrCall>
							<xsl:choose>
								<xsl:when test="contains(COL7,'PUT')">
									<xsl:value-of select="'0'"/>
								</xsl:when>
								<xsl:when test="contains(COL7,'CALL')">
