...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL5,'Options')">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before($Symbol,1)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(normalize-space(COL8),$UnderlyingSymbol),5,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(normalize-space(COL8),$UnderlyingSymbol),3,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-after(normalize-space(COL8),$UnderlyingSymbol),1,2)"/>
			</xsl:variable>

			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(normalize-space(COL8),$UnderlyingSymbol),7,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL8),$UnderlyingSymbol),8),'#.000')"/>
			</xsl:variable>
	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and COL4!='Cash'">
					<PositionMaster>
                ...
                <xsl:variable name="Symbol" select="normalize-space(COL8)"/>
							...
							<!-- <xsl:choose>
								...
								<xsl:otherwise>
									<xsl:choose>
										...
										<xsl:otherwise>
											<xsl:value-of select ="normalize-space($PB_COMPANY_NAME)"/>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:otherwise>
						...
						<xsl:variable name="TotalCostBasis">
							<xsl:call-template name="Translate">
						...
						<xsl:variable name="CostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$TotalCostBasis div $Position div 100"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
						...
						<xsl:variable name="Side">
							<xsl:value-of select= ("normalize-sapce(COL4)"/>)
						</xsl:variable>
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
						...
						<!--<xsl:variable name="varCOL40">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="translate(COL40,'$','')"/>
							</xsl:call-template>
						</xsl:variable>-->

						<!--<xsl:variable name="varFXR">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="translate(COL39,'$','')"/>
							</xsl:call-template>
						</xsl:variable>-->

						<!--<xsl:variable name="varFaxRate">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$varCOL40 div $varFXR "/>
							</xsl:call-template>
						</xsl:variable>-->
						...
						<!--<FXRate>
							<xsl:choose>
								<xsl:when test="number($varFaxRate)">
									<xsl:value-of select="$varFaxRate"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
