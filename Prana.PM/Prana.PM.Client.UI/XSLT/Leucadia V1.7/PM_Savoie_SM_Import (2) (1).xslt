	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and string-length(COL6) &gt; 20">
					...
					<PositionMaster>
						...
						<PutOrCall>
							<xsl:choose>
								<xsl:when test="contains(substring(substring-after(normalize-space(COL6),' '),7,1),'P')">
									<xsl:value-of select="'0'"/>
								</xsl:when>
								<xsl:when test="contains(substring(substring-after(normalize-space(COL6),' '),7,1),'C')">
						...
						<xsl:variable name="OsiOption">
							<xsl:choose>
								...
								<xsl:when test="string-length(substring-before(COL6,' '))= 4">
									<xsl:value-of select="concat(substring-before(COL6,' '),' ',' ',substring-after(COL6,' '))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL6,' '))= 3">
									<xsl:value-of select="concat(substring-before(COL6,' '),' ',' ',' ',substring-after(COL6,' '))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL6,' '))= 2">
									<xsl:value-of select="concat(substring-before(COL6,' '),' ',' ',' ',' ',substring-after(COL6,' '))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL6,' '))= 1">
									<xsl:value-of select="concat(substring-before(COL6,' '),' ',' ',' ',' ',' ',substring-after(COL6,' '))"/>
								</xsl:when>
							...
							<xsl:choose>
								<!--<xsl:when test="number($varStrikePrice)">-->
									...
									<!--<xsl:value-of select="$varStrikePrice"/>-->
										<xsl:value-of select="concat($varStrikePrice,'.',$varDecimal)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
