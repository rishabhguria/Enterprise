...
namespace ExcelLibrary
{
    ...
    public sealed class DataSetHelper
    {
        ...
        public static void CreateWorkbook(String filePath, DataSet dataset,DataSet dsSummary)
        {
            ...
            foreach (DataTable dt in dataset.Tables)
            {
                ...
                //if (dtsummary != null)
                //{
                //    int offset = dt.Rows.Count + 1;
                //    for (int i = 0; i < dtsummary.Columns.Count ; i++)
                //    {
                //        for (int j = offset; j < dtsummary.Rows.Count + offset; j++)
                //            worksheet.Cells[j + 1, i] = new Cell(dtsummary.Rows[j - offset][i].ToString());
                //    }
              //  }
