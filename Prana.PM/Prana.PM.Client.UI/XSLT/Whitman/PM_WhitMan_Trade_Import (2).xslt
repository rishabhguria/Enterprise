	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($NetPosition) and COL1='T'">
						...
						<xsl:variable name ="PRANA_ROOT_NAME">
							<xsl:value-of select="document('../ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name=$PB_NAME]/SymbolData[@InstrmentCode = $PB_ROOT_NAME]/@UnderlyingCode"/>
						</xsl:variable>
						...
						<Symbol>
							<xsl:choose>
								...
								<!--<xsl:when test="$Asset='FUTOP'">
									<xsl:value-of select="concat($UnderlyingSymbol,' ',$Future,COL17,(COL16 * 100))"/>
								</xsl:when>-->

								<xsl:when test="COL6!='*'">
									<xsl:value-of select="normalize-space(concat($UnderlyingSymbol,' ',$Future,$PRANA_SUFFIX_NAME))"/>
								</xsl:when>
						...
						<xsl:variable name="PB_FUND_NAME" select="concat(substring(COL3,1,2),COL4)"/>
						...
						<NetPosition>
							...
							<xsl:variable name="Underlyer" select="COL12"/>
						...
						<Commission>
							...
							<xsl:variable name="Fee">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="COL37 + COL39 + COL41 + COL43 + COL45"/>
								</xsl:call-template>
							</xsl:variable>
							<Fees>
								<xsl:choose>
