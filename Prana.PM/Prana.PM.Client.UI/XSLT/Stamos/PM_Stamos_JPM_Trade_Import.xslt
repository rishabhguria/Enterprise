	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Quantity) and (normalize-space(COL15)='B' or normalize-space(COL15)='S')">
					...
					<PositionMaster>
						...
						<xsl:variable name="PB_SUFFIX_NAME">
							<xsl:value-of select="substring-after(COL13,' ')"/>
						</xsl:variable>
                  ...
                  <xsl:call-template name="Option">
                    ...
                    <xsl:with-param name="Suffix" select="''"/>
						...
						<xsl:variable name="varAvgPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL17 div 100000000"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							...
							<xsl:choose>
								<xsl:when test="normalize-space(COL6)='PVOXX'">
									<xsl:choose>
						...
						<xsl:variable name="varCONTRACT" select="COL56"/>

						<SideTagValue>
              <xsl:choose>
                <xsl:when test="$varAsset='EquityOption'">
                  <xsl:choose>
                    <xsl:when test="$Side='B' and COL45='OC'">
                      <xsl:value-of select="'A'"/>
                    </xsl:when>
                    <xsl:when test="$Side='S' and COL45='OC'">
                      <xsl:value-of select="'C'"/>
                    </xsl:when>
                    <xsl:when test="$Side='B' and COL45='CG'">
                      <xsl:value-of select="'B'"/>
                    </xsl:when>
                    <xsl:when test="$Side='S' and COL45='CG'">
                      <xsl:value-of select="'D'"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="''"/>
