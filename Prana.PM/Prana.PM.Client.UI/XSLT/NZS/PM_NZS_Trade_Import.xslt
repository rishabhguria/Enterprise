...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL4),' '),8,8) div 1000,'#.00')"/>
		</xsl:variable>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<SideTagValue>							
							<xsl:choose>
								<xsl:when test="contains(normalize-space(COL5),'Buy')">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL5),'Sell')">
