...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-after(substring-after(substring-after(COL1,' '),' '),' '),1,1),c) or contains(substring(substring-after(substring-after(substring-after(COL1,' '),' '),' '),1,1),c) ">
			...
			<!--<xsl:choose>
				...
				<xsl:otherwise>-->
			<xsl:value-of select="concat('O:',translate($UnderlyingSymbol,$lower_CONST,$upper_CONST),' ',$ExpiryYear,$MonthCodeVar,$StrikePrice,'D',$Day)"/>
			<!--</xsl:otherwise>-->
			<!--
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($NetPosition)">
					...
					<PositionMaster>
							...
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:call-template name ="Option">
										<xsl:with-param name="Symbol" select="translate(COL1,$lower_CONST,$upper_CONST)"/>
						...
						<xsl:variable name="PB_BROKER_NAME" select="COL11"/>
