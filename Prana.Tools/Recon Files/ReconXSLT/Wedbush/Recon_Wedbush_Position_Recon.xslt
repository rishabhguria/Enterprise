...
>
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="ExpiryDay">
			<xsl:value-of select="substring(substring-after(normalize-space(COL14),' '),5,2)"/>
		</xsl:variable>
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring(substring-after(normalize-space(COL14),' '),3,2)"/>
		</xsl:variable>
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring(substring-after(normalize-space(COL14),' '),1,2)"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after(normalize-space(COL14),' '),7,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL14),' '),8,8) div 1000,'#.00')"/>
			...
			<xsl:for-each select="//Comparision">
				...
				<xsl:if test="number($varQuantity)">
					...
					<PositionMaster>
							...
							<xsl:choose>
								...
								<xsl:when test="$varAssetType='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="Symbol" select="normalize-space(COL14)"/>
									</xsl:call-template>
								</xsl:when>
						...
						<xsl:variable name="varMarketValueForEquity">
						   <xsl:choose>
								<xsl:when test="$varQuantity &lt;0">
									<xsl:value-of select="$varQuantity * $varMarkPrice *(-1)"/>
								</xsl:when>
								<xsl:when test="$varQuantity &gt;0">
									<xsl:value-of select="$varQuantity * $varMarkPrice"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
						...
						<xsl:variable name="varMarketValueForOptions">
						    <xsl:choose>
								<xsl:when test="$varQuantity &lt;0">
									<xsl:value-of select="$varQuantity * $varMarkPrice * 100 *(-1)"/>
								</xsl:when>
								<xsl:when test="$varQuantity &gt;0">
									<xsl:value-of select="$varQuantity * $varMarkPrice * 100"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
						...
						<MarketValue>
							<xsl:choose>
								<xsl:when test="$varOption !=''">
									<xsl:value-of select="$varMarketValueForOptions"/>
								</xsl:when>
								<xsl:when test="$varOption =''">
									<xsl:value-of select="$varMarketValueForEquity"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
