...
{
    class VerifyMasterFundSymbolDivisorWindow : PopUpUIMap, ITestStep
    {
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                PopUpUIMap.DumpWindow("MasterFundSymbolDivisorWindow");
                StringBuilder errorBuilder = new StringBuilder();
                string DevPath = Path.GetFullPath(Path.Combine(Environment.CurrentDirectory, @"..\..\..\.."));
                DataTable xmltoDataTable = CommonMethods.ReadXML(DevPath + TestDataConstants.DUMP_PATH + "MasterFundSymbolDivisorWindow.xml");
                List<string> keyColumns = new List<string>() { "symbol", "masterFundId" };
