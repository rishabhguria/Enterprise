				...
				<xsl:if test="number($varQuantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="varSedol">
							<xsl:value-of select="substring(normalize-space(COL8),2)"/>
						</xsl:variable>
						...
						<xsl:variable name="NetNotionalValueBase">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="normalize-space(COL72)"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<Side>
							<xsl:choose>
								<xsl:when test="normalize-space(COL16)='BUY'">
									<xsl:value-of select="'Buy'"/>
								</xsl:when>
								<xsl:when test="normalize-space(COL16)='SELL'">
						...
						<xsl:variable name="varPositionSettlementDate">
							<xsl:call-template name="FormatDate">
								<xsl:with-param name="DateTime" select="normalize-space(COL106)"/>
							</xsl:call-template>
						</xsl:variable>
