...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template match="/">
		<DocumentElement>
			  ...
			  <Symbol>
				  <xsl:choose>
					  <xsl:when test ="$PRANA_Symbol_NAME = ''">
						  <xsl:choose>
							  <xsl:when test="$varAssetType = '2' or $varAssetType = '7' or $varAssetType = '8' or $varAssetType = '11' or $varAssetType = '13'">
								  <xsl:value-of select="''"/>
							  </xsl:when>
							  <xsl:otherwise>
								  <xsl:choose>
									  ...
									  <xsl:otherwise>
										  <xsl:choose>
											  <xsl:when test ="$varEquitySymbol = '*' or $varEquitySymbol = ' '">
            ...
            <CUSIP>
              <xsl:choose>
                <xsl:when test="$varAssetType = '7' or $varAssetType = '8' or $varAssetType = '11' or $varAssetType = '13'">
            ...
            <SMRequest>
              <xsl:value-of select="$varSMRequest"/>
						...
						<Quantity>
							<xsl:if test="contains($varside, 'Sell')">
								<xsl:value-of select="number(COL7)*(-1)"/>
							</xsl:if>
							<xsl:if test="not(contains($varside, 'Sell'))">
