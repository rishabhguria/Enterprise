...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="UnderlyingSymbol">
			<xsl:value-of select="substring-before(normalize-space(substring-after(normalize-space(COL4),'-')),' ')"/>
		</xsl:variable>
		
		<xsl:variable name="ExpiryDay">
			<xsl:value-of select="substring-before(substring-after(substring-after(normalize-space(COL4),'EXP'),'/'),'/')"/>
		</xsl:variable>
		
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring-before(normalize-space(substring-after(normalize-space(COL4),'EXP')),'/')"/>
		</xsl:variable>
		
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL4),'EXP'),'/'),'/'),3,2)"/>
		</xsl:variable>
		...
		<xsl:variable name="StrikePrice1">
			<xsl:value-of select="normalize-space(substring-before(normalize-space(substring-after(normalize-space(COL4),'@')),'EXP'))"/>
		</xsl:variable>	
				...
				<xsl:if test="$varAssetType='EquityOption'">
					...
					<PositionMaster>			
	                	...
	                	<xsl:variable name="varExpiryYear">
	                		<xsl:value-of select="substring-after(substring-after(substring-after(normalize-space(COL4),'EXP'),'/'),'/')"/>
	                	</xsl:variable>
						...
						<xsl:variable name="varDecimalAfter">
	                    	 <xsl:value-of select="format-number($varPreDecimal + $varDecimal,'##.00')"/>
	                    </xsl:variable>
	                    ...
	                    <xsl:variable name="StrikePrice">
	                       <xsl:choose>
	                    		<xsl:when test="contains($StrikePrice1,'/')">
	                    			<xsl:value-of select="($varDecimalAfter)"/>
	                    		</xsl:when>
	                    		<xsl:otherwise>
						...
						<xsl:variable name="varStrikePrice1">
	                       <xsl:choose>
	                    		<xsl:when test="contains($StrikePrice1,'/')">
	                    			<xsl:value-of select="($varDecimalAfter) * 1000"/>
	                    		</xsl:when>
	                    		<xsl:otherwise>
	                    			<xsl:value-of select="($StrikePrice1) * 1000"/>
	                    		</xsl:otherwise>
	                    	</xsl:choose>
	                    </xsl:variable>
						...
						<xsl:variable name="OsiOption">
							<xsl:choose>
								<xsl:when test="string-length($UnderlyingSymbol)=1">
									<xsl:value-of select="concat($UnderlyingSymbol,' ',' ',' ',' ',' ',$varExpiryYearOSI,$varExpiryMonth,$varExpiryDay,$varPutORCall,$varStrikePrice2)"/>
								</xsl:when>
