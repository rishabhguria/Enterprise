...
namespace Prana.PostTrade
{
    class LIFO : AlgoBase
    {
        ...
        {
            taxlots.Sort(delegate (TaxLot t1, TaxLot t2) { return (t1.ClosingDate.Date.CompareTo(t2.ClosingDate.Date) * (-1)); });


            //Once the taxlots are sorted in DSC order on closing date then secondary sorting will be applied on the same date taxlots by secondary criteria either AveragePrice or TaxlotID....
            switch (sortCriteria)
            {
                case PostTradeEnums.SecondarySortCriteria.AvgPxASC:
                    taxlots.Sort(delegate (TaxLot t1, TaxLot t2) { int result = t1.ClosingDate.Date.CompareTo(t2.ClosingDate.Date); return (result != 0) ? result * (-1) : (t1.AvgPrice.CompareTo(t2.AvgPrice)); });
                    break;

                case PostTradeEnums.SecondarySortCriteria.AvgPxDESC:
                    taxlots.Sort(delegate (TaxLot t1, TaxLot t2) { int result = t1.ClosingDate.Date.CompareTo(t2.ClosingDate.Date); return (result != 0) ? result * (-1) : (t1.AvgPrice.CompareTo(t2.AvgPrice) * (-1)); });
                    break;
                ...
                case PostTradeEnums.SecondarySortCriteria.OrderSequenceASC:
                    taxlots.Sort(delegate (TaxLot t1, TaxLot t2) { int result = t1.ClosingDate.Date.CompareTo(t2.ClosingDate.Date); return (result != 0) ? result * (-1) : (t1.TaxLotID.CompareTo(t2.TaxLotID)); });
                    break;

                case PostTradeEnums.SecondarySortCriteria.OrderSequenceDESC:
                    taxlots.Sort(delegate (TaxLot t1, TaxLot t2) { int result = t1.ClosingDate.Date.CompareTo(t2.ClosingDate.Date); return (result != 0) ? result * (-1) : (t1.TaxLotID.CompareTo(t2.TaxLotID) * (-1)); });
                    break;
        ...
        private List<TaxLot> SortSellList(List<TaxLot> taxlots, PostTradeEnums.SecondarySortCriteria sortCriteria)
        {
            ...
            //Once the taxlots are sorted in DESC order on closing date then secondary sorting will be applied on the same date taxlots by secondary criteria either AveragePrice or TaxlotID....
            switch (sortCriteria)
            {
                case PostTradeEnums.SecondarySortCriteria.AvgPxASC:
                    taxlots.Sort(delegate (TaxLot t1, TaxLot t2) { int result = t1.ClosingDate.Date.CompareTo(t2.ClosingDate.Date); return (result != 0) ? result * (-1) : (t1.AvgPrice.CompareTo(t2.AvgPrice)); });
