	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and COL12='T'">
					<PositionMaster>
						...
						<xsl:variable name="varAsset">
							<xsl:choose>
								<xsl:when test="substring-before(COL9,' ')='CALL' or substring-before(COL9,' ')='PUT'">
                  ...
                  <xsl:call-template name="Option">
                    ...
                    <xsl:with-param name="Suffix" select="''"/>
						...
						<xsl:variable name="varPosition">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL18 div 100000"/>
							</xsl:call-template>
						</xsl:variable>
						<Quantity>
							<xsl:choose>
						...
						<xsl:variable name="AvgPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL19 div 100000000"/>
							</xsl:call-template>
						</xsl:variable>
						<AvgPX>
							<xsl:choose>
						...
						<xsl:variable name="SecFee">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL21 div 100"/>
							</xsl:call-template>
						</xsl:variable>
						<SecFee>
							<xsl:choose>
						...
						<xsl:variable name="Fees">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL36 div 100"/>
							</xsl:call-template>
						</xsl:variable>
						<OtherBrokerFees>
							<xsl:choose>
						...
						<xsl:variable name="varOpenAndClose">
							<xsl:value-of select="normalize-space(COL56)"/>
						</xsl:variable>
						<Side>
							<xsl:choose>
								<xsl:when test="$varAsset='EquityOption'">
									<xsl:choose>
										<xsl:when test="$Side='B' and $varOpenAndClose='OPEN CONTRACT'">
											<xsl:value-of select="'Buy to Open'"/>
										</xsl:when>

										<xsl:when test="$Side='B' and $varOpenAndClose='CLOSE CONTRACT'">
											<xsl:value-of select="'Buy to Close'"/>
										</xsl:when>

										<xsl:when test="$Side='S' and $varOpenAndClose='OPEN CONTRACT'">
											<xsl:value-of select="'Sell to Open'"/>
										</xsl:when>

										<xsl:when test="$Side='S' and $varOpenAndClose='CLOSE CONTRACT'">
						...
						<xsl:variable name="NetNotionalValue">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL34 div 100"/>
							</xsl:call-template>
						</xsl:variable>
						<NetNotionalValue>
							<xsl:choose>
						...
						<xsl:variable name="PB_BROKER_NAME">
							<xsl:value-of select="normalize-space(COL81)"/>
						</xsl:variable>
						...
						<xsl:variable name="Year">
							<xsl:choose>
								<xsl:when test="string-length(COL10)=7">
									<xsl:value-of select="substring(COL10,4,4)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="substring(COL10,5,4)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						<xsl:variable name="Month">
							<xsl:choose>
								<xsl:when test="string-length(COL10)=7">
									<xsl:value-of select="concat('0',substring(COL10,1,1))"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<xsl:variable name="Day">
							<xsl:choose>
								<xsl:when test="string-length(COL10)=7">
									<xsl:value-of select="substring(COL10,2,2)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="substring(COL10,3,2)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						<TradeDate>
						...
						<xsl:variable name="SYear">
							<xsl:choose>
								<xsl:when test="string-length(COL11)=7">
									<xsl:value-of select="substring(COL11,4,4)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="substring(COL11,5,4)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						<xsl:variable name="SMonth">
							<xsl:choose>
								<xsl:when test="string-length(COL11)=7">
									<xsl:value-of select="concat('0',substring(COL11,1,1))"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="substring(COL11,1,2)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						...
						<xsl:variable name="SDay">	
							<xsl:choose>
								<xsl:when test="string-length(COL11)=7">
									<xsl:value-of select="substring(COL11,2,2)"/>
								</xsl:when>
								<xsl:otherwise>
