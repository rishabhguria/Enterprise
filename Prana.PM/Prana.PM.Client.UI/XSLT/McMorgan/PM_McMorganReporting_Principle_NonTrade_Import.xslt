        ...
        <xsl:if test="number($Cash)">
          ...
          <PositionMaster>
             ...
             <xsl:variable name="PB_NAME">
              <xsl:value-of select="'Principal Financial'"/>
            </xsl:variable>
              ...
              <xsl:choose>
			   <xsl:when test="($varSubAccountDesc ='INTEREST') and $Cash &gt; 0">
                ...
                <xsl:when test="$varSubAccountDesc ='INTEREST' and $Cash &lt; 0">
				 ...
				 <xsl:when test="($varSubAccountDesc ='PURCHASE') and $Cash &gt; 0">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				 <xsl:when test="$varSubAccountDesc ='PURCHASE' and $Cash &lt; 0">
					  <xsl:value-of select="'VP4530003 Money Fund'"/>
				  </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="''"/>
                </xsl:otherwise>
            ...
            <xsl:variable name="Month" select="substring(COL6,5,2)"/>
            <xsl:variable name="DD" select="substring(COL6,7,2)"/>
