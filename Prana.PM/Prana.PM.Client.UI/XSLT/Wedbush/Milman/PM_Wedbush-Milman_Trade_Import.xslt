...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL29,OPTN)">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(normalize-space(COL20),' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(normalize-space(COL20),' '),7,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(normalize-space(COL20),' '),5,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-after(normalize-space(COL20),' '),3,2)"/>
			</xsl:variable>

			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(normalize-space(COL20),' '),9,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL20),' '),10),'#.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($NetPosition)">
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="contains(COL29,'OPTN')">
						...
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="$Asset ='EquityOption'">
									<xsl:choose>
										<xsl:when test="$Side='B' and COL3 ='3'">
											<xsl:value-of select="'B'"/>
										</xsl:when>
										...
										<xsl:when test="$Side='S' and COL3 ='3'">
											<xsl:value-of select="'C'"/>
										</xsl:when>
									</xsl:choose>
								</xsl:when>
