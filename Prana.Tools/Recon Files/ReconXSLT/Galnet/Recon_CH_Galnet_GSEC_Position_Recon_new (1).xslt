			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name="Asset">
					<xsl:choose>
						...
						<!--<xsl:when test="COL3!='' and COL13=''">-->
						<xsl:when test="COL3!='0' and COL12='' and COL14='0'">
							<xsl:value-of select="'FutureOption'"/>
						</xsl:when>
						<xsl:when test="COL12=''">
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name="StrikePrice">
							<xsl:value-of select="number(substring-before(substring-after($Symbol,' '),' '))"/>
						</xsl:variable>
						...
						<xsl:variable name="FutureOption1">
							<xsl:choose>
								<xsl:when test="COL3!='0' and COL13='*' or COL13='' and COL14='0'">
									<xsl:value-of select="concat($Week,'E',' ',$Month,$Year,$PUTCALL,$StrikePrice1)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="concat(substring(COL13,1,2),' ',substring(substring-before(COL13,' '),3),substring-before(substring-after(COL13,' '),' '))"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>

						<xsl:variable name="FUTOPT">
							<xsl:choose>
								<xsl:when test="COL13!=''">
									<xsl:value-of select="concat(substring(COL13,1,2),' ',substring(substring-before(COL13,' '),3),substring-before(substring-after(COL13,' '),' '))"/>
								</xsl:when>
						...
						<Symbol>
							<xsl:choose>
								...
								<!--<xsl:when test="$FUTOPT!=''">
									<xsl:value-of select="$FUTOPT"/>
								</xsl:when>-->
								<xsl:when test="$Asset='FutureOption'">
									<xsl:value-of select="FutureOption1"/>
								</xsl:when>
								<xsl:otherwise>
