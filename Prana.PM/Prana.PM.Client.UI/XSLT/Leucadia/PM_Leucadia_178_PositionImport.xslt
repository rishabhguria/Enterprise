...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL10,'PUT') or contains(COL10,'C ALL')">
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position)  and COL20!='CASH'">
            ...
            <xsl:variable name="PRANA_SUFFIX_NAME">
              <xsl:value-of select="document('../ReconMappingXML/SymbolSuffixMapping.xml')/SymbolMapping/PB[@Name=$PB_NAME]/SymbolData[@PBSuffixCode=$PB_SUFFIX_NAME]/@PranaSuffixCode"/>
						...
						<xsl:variable name="Asset">
                          <xsl:choose>
                                  <xsl:when test="contains(COL10,'PUT') or contains(COL10,'CALL')">
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="contains(COL4,'FIXED INCOME')!=''">
									<xsl:value-of select="''"/>
								</xsl:when>
						...
						<xsl:variable name="Costbasis">
							<xsl:choose>
								<xsl:when test="COL4='FIXED INCOME' or $Asset='EquityOption'">
									<xsl:value-of select="COL54"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="COL45 div COL21"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						<CostBasis>
