  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//Comparision">
        ...
        <xsl:if test="number($varLCash) and COL7 ='TRADE DATE CASH'">
          <PositionMaster>
            ...
            <xsl:variable name="varMonth" select="substring-before(substring-after(COL31,'-'),'-')"/>
            <xsl:variable name="varDay" select="substring-before(COL31,'-')"/>
            <xsl:variable name="varYear" select="substring-after(substring-after(COL31,'-'),'-')"/>
            <Date>
