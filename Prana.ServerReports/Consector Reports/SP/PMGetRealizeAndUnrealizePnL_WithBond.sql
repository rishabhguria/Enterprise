...
[PMGetRealizeAndUnrealizePnL_WithBond] '2014/04/01','2014/04/30' ,'1213,1214,1233,1234,1235,1236,1237,1239,1240,1241,1242,1243',              
'1,2,3,4,5,6,7,8,9','MTM_V0','30_360'                
                                                                                                    
******************************************************************/                  
                                   
CREATE PROCEDURE  [dbo].[PMGetRealizeAndUnrealizePnL_WithBond]                                                                                       
(                                                                                                                                                              
 @StartDate datetime,                   
 @EndDate datetime,                  
 @Fund Varchar(max),                  
...
Create Table #MarkPriceForEndDate                                                 (                                                                                                       
                                                     ...
                                                     Finalmarkprice float ,                                                                                                                       Symbol varchar(50)                                    
...
Create Table #SecMasterDataTempTable                                                                                                                      (                                                                                                   
                                                       ...
                                                       AUECID int,                                                                      
  TickerSymbol Varchar(100),                                                                                                                                CompanyName  VarChar(500),                                                                 
        
          
            
                                                       AssetID int,                                      AssetName Varchar(100),               
            
                                                                                                      
  SecurityTypeName Varchar(200),                                                                                                                            SectorName Varchar(100),                              
                                                       SubSectorName Varchar(100),                                                                                                                               CountryName  Varchar(100),                    
...
CompanyName varchar(200),                                                                                                                                 FundName varchar(100),                                                                              
...
G1.TradeAttribute5 as G1TradeAttribute5,              G1.TradeAttribute6 as G1TradeAttribute6                                              
...
**************************************************************************/                                                                                                  
 Select                                                                                                                              
  PT.FundID As FundID,                                                                                                                                      PT.Symbol As Symbol,                                 
  PT.TaxLotOpenQty As TaxLotOpenQty ,                                                                                                                       PT.AvgPrice As AvgPrice ,                                                                         
  
    
                                       
  0 as ClosingPrice,                          
  G.AssetID As AssetID,                                                                                                                                     G.CurrencyID AS CurrencyID,                  
...
--  And DateDiff(d,AUECYesterDates.YESTERDAYBIZDATE,FXDayRatesForStartDate.Date)=0)                                                
--                                                                                                               
--  -- Forex Price for Closing Date                                                                                                                                                 --  Left outer  join #FXConversionRates FXDayRatesForClosingDate           
...
Insert into #MTMDataTable               
 ...
 Select                                                                                                                                                      IsNull(DailyCash.FundId,0) as FundID,                                                             
...
-------------------------------------------------------            
--Daily Bond Intrest Calculations            
-------------------------------------------------------            
...
select         
  ...
  case CouponFrequencyID          
   ...
   when 3 then 12.0        
   when 4 then 0        
  end as CouponFrequencyMonth,        
  FixedIncomeData.* into #BondCalculations         
from V_Secmasterdata          
inner join (select * from #MTMDataTable  where Asset = 'FIXEDINCOME') FixedIncomeData        
on V_Secmasterdata.TickerSymbol = FixedIncomeData.symbol        
        
alter table #BondCalculations        
add LatestCouponDate datetime,NumberOfCoupons int,NextCouponDate datetime        
       
alter table #MTMDataTable      
add CouponInterest float       
      
update #BondCalculations        
set NumberOfCoupons =        
 CASE        
  When (@EndDate) < (FirstCouponDate)          
  Then 0      
  When month(@EndDate) = month(FirstCouponDate)      
  Then 1      
  WHEN  datediff(m,FirstCouponDate,@EndDate)>0       
  Then      
 Case       
  when datediff(m,FirstCouponDate,@EndDate)/CouponFrequencyMonth  > cast(datediff(m,FirstCouponDate,@EndDate)/CouponFrequencyMonth as int)       
  then (datediff(m,FirstCouponDate,@EndDate)/CouponFrequencyMonth) + 1      
  else cast(datediff(m,FirstCouponDate,@EndDate)/CouponFrequencyMonth as int)      
 End         
 End        
        
update #BondCalculations set LatestCouponDate      
=         
(case                        
 when NumberOfCoupons=0      
 Then FirstCouponDate      
 when NumberOfCoupons>0      
 then Dateadd(m,CouponFrequencyMonth*(NumberOfCoupons-1),FirstCouponDate)       
End)      
      
                          
update #BondCalculations set NextCouponDate=                           
(case                        
 when NumberOfCoupons =0      
 Then FirstCouponDate      
 when NumberOfCoupons>0      
 then Dateadd(m,CouponFrequencyMonth,LatestCouponDate)         
End)      
        
      
update #MTMDataTable      
set CouponInterest = Coupons.Interest      
from       
#MTMDataTable inner join       
(      
 select         
   symbol        
  ,TaxlotOpenQty     
  ...
  ,SUM(  
    case        
   when datediff(d,settlementDate,@EndDate)>=0 and datediff(d,LatestCouponDate,firstCouponDate)=0         
   and datediff(d,firstCouponDate,NextCouponDate)=0 --and datediff(d,@EndDate,firstCouponDate)>0        
   then ((TaxlotOpenQty*AssetMultiplier*CouponRate*dbo.F_GetTotalDaysInDateRange(settlementDate,@EndDate,@DayCountConvention))/100 )        
   when  datediff(d,LatestCouponDate,NextCouponDate)<>0 and datediff(d,@EndDate,NextCouponDate)>0 and datediff(d,LatestCouponDate,@EndDate)>=0        
   then ((TaxlotOpenQty*AssetMultiplier*CouponRate*dbo.F_GetTotalDaysInDateRange(LatestCouponDate,@EndDate,@DayCountConvention))/100 )        
  End        
  ) as Interest    
 from #BondCalculations     
 where datediff(d,settlementdate,@EndDate)>=0    
group by Symbol,TaxlotOpenQty,AssetMultiplier,settlementdate          
) Coupons      
 on Coupons.Symbol = #MTMDataTable.Symbol and Coupons.TaxlotOpenQty = #MTMDataTable.TaxlotOpenQty        
 and #MTMDataTable.Asset ='FixedIncome' and Coupons.AssetMultiplier = #MTMDataTable.AssetMultiplier and      
 Coupons.settlementdate = #MTMDataTable.settlementdate       
    
-- select         
--   symbol        
--  ,TaxlotOpenQty      
--  ,SUM(((TaxlotOpenQty*AssetMultiplier*CouponRate*dbo.F_GetTotalDaysInDateRange(settlementDate,@EndDate,@DayCountConvention))/100 ))    
--   +MAX(AccruedInterest)    
--   as Interest        
-- from  #BondCalculations     
-- where datediff(d,settlementdate,@EndDate)>=0    
--group by Symbol,TaxlotOpenQty,settlementdate    
---- daily interest       
----    
---- select         
----   symbol        
...
--------Daily Bond Intrest Calculations            
-------------------------------------------------------      
  -- RowIdentifier to distinguish shortrebate and bankfees  
alter table #MTMDataTable  
add RowIdentifier varchar(100)  
  
insert into #MTMDataTable    
 Select                                                                                                                                                      IsNull(Journal.FundId,0) as FundID,                                                              
   ...
   0  as AvgPrice ,                                                                                                         
   0  as ClosingPrice,                                                                                               
   6  as AssetID,                                                                                                                                            MIN(Journal.CurrencyID) as CurrencyID,                   
   MIN(CLocal.CurrencySymbol) as CurrencySymbol,                                                                                     
   0  as AUECID ,                                                                                                                                            0 As TotalOpenCommission_Local,          
   0  as TotalOpenCommission_Base,                          
   0  as TotalClosedCommission_Local,                                                                   
   0  as TotalClosedCommission_Base,                                                                                                       
   1  as AssetMultiplier,                                                   
    Max('1800-01-01') as TradeDate,                                                                                                          
   '1800-01-01 00:00:00.000' as ClosingDate,                                                                                                               0  as Mark1,                                                                                        
                                                        0  as Mark2,                                                           
   0  as IsSwapped,                                                                                                   
   0  as NotionalValue,                  
   0  as BenchMarkRate,                                                                   
   0  as Differential,                                                                                                                                       0  as OrigCostBasis,                      
   0  as DayCount,                                                                                                                                            
   '1800-01-01 00:00:00.000' as FirstResetDate,                                                                                                                                     
   '1800-01-01 00:00:00.000' as OrigTransDate,                                                                                                                
   0  as SwapData,                                                                                                                                                                                      
   ...
   0  as  BeginningMarketValue_Base,                          
   0  as  EndingMarketValue_Local ,                                                                                                       
0 as  EndingMarketValue_Base ,                                                                                                                            'CASH' as Open_CloseTag,                                              
   0 as ConversionRateTrade,                                                                                                                          
   0 as ConversionRateStart,                                                        
   0 as ConversionRateEnd,                                                                   
   'Undefined' as CompanyName,                                                                                                                               Min(CF.FundName) as FundName,                                                                    
  
                                                                                                     
   'Strategy Unallocated' as StrategyName,                                                                       
  Case                                                                     
  When Sum(Journal.DR-Journal.CR) >= 0                                                                    
 Then 'Long'                  
 Else 'Short'                                                                    
  End as Side,                                                                                                                                     
...
Case                                                                   
 ...
 Then  Sum(Journal.DR-Journal.CR)                                                                  
 Else 0                                                                  
End as CashFXUnrealizedPNL,                                      
0 As NetTotalCost_Base,                                              
'' as BloombergSymbol,                                         
...
0 As RealizedPNLMTM_Local,                        
0 As UnrealizedPNLMTM_Base,                        
Sum(Journal.DR-Journal.CR) As RealizedPNLMTM_Base   ,                
 '1800-01-01'  as ExpirationDate,                
'' as OpenTradeAttribute1,                
'' as OpenTradeAttribute2,                
'' as OpenTradeAttribute3,                
...
0  ,  
case   
 When  Journal.SubAccountID = 63  
 Then 'ShortRebate'  
 When  Journal.SubAccountID = 64  
 Then 'BankFees'  
End as RowIdentifier          
 ...
 INNER JOIN #T_CompanyFunds CF ON Journal.FundID = CF.CompanyFundID                                                                                
   INNER JOIN T_Currency CLocal ON Journal.CurrencyID = CLocal.CurrencyID                       
   INNER JOIN T_Currency CBase ON @BaseCurrencyID = CBase.CurrencyID                                                                                          
   LEFT OUTER JOIN T_CompanyMasterFundSubAccountAssociation CMFSSAA ON CF.CompanyFundID = CMFSSAA.CompanyFundID                                    
   LEFT OUTER JOIN T_CompanyMasterFunds CMF ON CMFSSAA.CompanyMasterFundID = CMF.CompanyMasterFundID                                                                   
WHERE DateDiff(d,'2014/01/01',Journal.TransactionDate) >=0                                           
 AND DateDiff(d,Journal.TransactionDate,'2014/10/21') >=0                     
 AND (Journal.TaxLotID IS NULL or Journal.TaxLotID = '')      
 and  SubAccounts.SubAccountID in (63,64)       
 GROUP BY CMF.MasterFundName,Journal.FundID,Journal.CurrencyID,Journal.SubAccountID      
    
      
-------------------------------------------------------            
Select * from #MTMDataTable     
where   Asset <>'FixedIncome' OR (Asset ='FixedIncome' and datediff(d,settlementdate,@EndDate)>=0)    
Order By Symbol                         
-------------------------------------------------------                                             
      
DROP TABLE #T_Asset, #PM_Taxlots, #T_CompanyFunds,#T_CorpActionData , #TempClosingData                                                                    DROP table #MarkPriceForStartDate, #MarkPriceForEndDate ,#SecMasterDataTempTable                    
  
    
DROP TABLE #TempSplitFactorForClosed_2,#TempSplitFactorForClosed_1,#TempSplitFactorForOpen,#TempSplitFactorForClosed                                      DROP table #FXConversionRates, #AUECYesterDates, #AUECBusinessDatesForEndDate,#MTMDataTable          
