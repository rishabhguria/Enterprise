			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($varCash)">
					...
					<PositionMaster>
						...
						<Description>
							<xsl:value-of select="'Daily Accrued Interest'"/>
						</Description>
						...
						<JournalEntries>
							<xsl:choose>
								<xsl:when test="$varCash &lt; 0">
									<xsl:value-of select="concat('Interest_Expense', ':' , $varCash , '|' , 'Interest_Payable', ':' , $varCash)"/>
								</xsl:when>	
							    <xsl:when test="$varCash &gt; 0">
									<xsl:value-of select="concat('Interest_Receivable', ':' , $varCash , '|' , 'Interest_Income', ':' , $varCash)"/>
								</xsl:when>	
							</xsl:choose>
						</JournalEntries>
