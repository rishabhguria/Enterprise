        ...
        <xsl:if test="number($Cash)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_PRE">
              <xsl:choose>
			   <xsl:when test="(contains($varSubAccountDes,'ACCT_XFER') or $varSubAccountDes ='WIRE_INCOMING')  and $Cash &gt; 0">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  
                <xsl:when test="(contains($varSubAccountDes,'ACCT_XFER') or $varSubAccountDes ='WIRE_INCOMING' or  $varSubAccountDes ='WIRE_OUTGOING')  and $Cash &lt; 0">
                  <xsl:value-of select="'CashTransferOut'"/>
                </xsl:when>
				
				  <xsl:when test="contains($varSubAccountDes,'FEE_TRANSACTION') and $Cash &lt; 0">
                  <xsl:value-of select="'AnalysisFee'"/>
                </xsl:when>
            ...
            <AccountName>
              <!--<xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:value-of select ="'JP Morgan 80003070299'"/>
                </xsl:otherwise>
              </xsl:choose>-->
				<xsl:value-of select ="'JP Morgan 80003070299'"/>
            </AccountName>

            <xsl:variable name="varCurrency" select="''"/>
            <CurrencyName>
