...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="ScientificToNumber">
		...
		<xsl:variable name="vExponent" select="substring-after($ScientificN,'+')"/>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) ">
					<PositionMaster>
						...
						<xsl:variable name="TotalCommissionandFeesSci">
							<xsl:call-template name="ScientificToNumber">
								<xsl:with-param name="ScientificN" select="COL31"/>
							</xsl:call-template>
						</xsl:variable>
						
						<xsl:variable name="TotalCommissionandFees">
							<xsl:choose>
								<xsl:when test="contains(COL31,'e')">
									<xsl:value-of select="$TotalCommissionandFeesSci"/>
								</xsl:when>
								<xsl:otherwise>
