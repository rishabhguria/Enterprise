	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test ="number($varCurrencyLocal) and ($varAsset ='CASH')">
					...
					<PositionMaster>
						...
						<xsl:variable name="varCurrencyBase">
							<xsl:call-template name="Translate">
						...
						<CashValueBase>
							<xsl:choose>
								<xsl:when test="number($varCurrencyBase)">
									<xsl:value-of select="$varCurrencyBase"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
