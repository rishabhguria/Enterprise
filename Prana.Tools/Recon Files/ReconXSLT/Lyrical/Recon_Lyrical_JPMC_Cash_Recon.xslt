			...
			<xsl:for-each select ="//Comparision">
				<xsl:variable name="Cash">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="substring(COL1,25,9)"/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:if test ="number($Cash) and contains(substring(COL1,79,4),'JPMC')='true'">
					<PositionMaster>
						...
						<xsl:variable name = "PB_FUND_NAME">
							<xsl:value-of select="substring(COL1,7,9)"/>
						</xsl:variable>
