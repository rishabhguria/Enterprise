	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				<xsl:if test ="COL1='0O100' and COL21!='SETTLEMENT'">
					<xsl:if test ="COL21 !='JOURNAL'">
							...
							<xsl:variable name = "PB_SYMBOL_NAME">
								<xsl:choose>
									<xsl:when test ="COL6='S9'">
										<xsl:value-of select="concat(COL8,COL9,' ','CURNCY')" />
									</xsl:when>
									<xsl:otherwise>
						...
						<xsl:variable name ="PRANA_SUFFIX_NAME">
							...
							<xsl:variable name="Year2" select="substring(COL25,1,4)" />
							<xsl:variable name="Month1">
								<xsl:value-of select="substring(COL25,5,2)" />
							</xsl:variable>
							<xsl:variable name="Day1">
								<xsl:value-of select="substring(COL25,7,2)" />
							</xsl:variable>
							...
							<xsl:variable name ="NetPosition">
								<xsl:choose>
									<xsl:when test ="COL6='S9'">
										...
										<!--<xsl:choose>
											<xsl:when test="COL8='AUD' or COL8='GBP' or COL8='EUR' or COL8='NZD'">
												<xsl:value-of select="number(COL11)" />
											</xsl:when>
											<xsl:otherwise>
							...
							<xsl:variable name="PB_Contract" select="concat(COL5,COL6)" />
							...
							<xsl:variable name="varAvgPrice">
								<xsl:choose>
									<xsl:when test="contains(COL6,'S9')">
										<xsl:choose>
											<xsl:when test="COL7='BUY'">
							...
							<!--<Quantity2>
								<xsl:choose>
									<xsl:when test="$varAvgPrice &gt; 0">
										<xsl:value-of select="$varAvgPrice * $NetPosition" />
									</xsl:when>
									<xsl:when test="$varAvgPrice &lt; 0">
										<xsl:value-of select="$varAvgPrice * (-1) * $NetPosition" />
									</xsl:when>
              ...
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:value-of select ="0" />
							...
							<xsl:variable name="Multiplier" select="(COL23)" />
