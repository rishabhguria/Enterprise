	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Position) and $Asset!='Cash' and COL7!='Dividend'">
					...
					<PositionMaster>
						...
						<Symbol>
							...
							<xsl:choose>
								...
								<xsl:when test="normalize-space(COL4)='Option'">
									<xsl:value-of select="''"/>

								</xsl:when>
								
								<xsl:when test="normalize-space(COL4)='Equity'">
						...
						<IDCOOptionSymbol>
							<xsl:choose>
								...
								<xsl:when test="normalize-space(COL5) = 'Option'">
