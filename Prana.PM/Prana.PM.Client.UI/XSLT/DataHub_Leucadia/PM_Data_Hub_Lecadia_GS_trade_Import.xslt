	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and COL5!='Dividends'">
					...
					<PositionMaster>
						...
						<xsl:variable name="PB_SUFFIX_NAME" select="substring-after(COL12,'.')"/>
						...
						<xsl:variable name="varExFactor">
							<xsl:value-of select="substring(COL12,string-length(substring-before(COL12,'.')),1)"/>
						</xsl:variable>
								...
								<xsl:when test="COL12!=''">
									<xsl:choose>
										...
										<xsl:when test="$varExFactor='a'">
											<xsl:value-of select="concat(substring(COL12,1,string-length(substring-before(COL12,'.'))-1),'_a',$PRANA_SUFFIX_NAME,'/SWAP')"/>
										</xsl:when>

										<xsl:when test="$varExFactor='b'">
											<xsl:value-of select="concat(substring(COL12,1,string-length(substring-before(COL12,'.'))-1),'_b',$PRANA_SUFFIX_NAME,'/SWAP')"/>
										</xsl:when>

										<xsl:when test="$PRANA_SUFFIX_NAME!=''">
											<xsl:value-of select="concat(substring-before(COL12,'.'),$PRANA_SUFFIX_NAME,'/SWAP')"/>
										</xsl:when>

										<xsl:otherwise>
											<xsl:value-of select="concat(COL12,'/SWAP')"/>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:when>
						...
						<SideTagValue>
							<xsl:choose>
								...
								<xsl:when test="COL6='Cancel'">
									
									<xsl:choose>
										<xsl:when test="COL3='Buy' and COL4='Open'">
											<xsl:value-of select="'2'"/>
										</xsl:when>

										<xsl:when test="COL3='Buy' and COL4='Close'">
											<xsl:value-of select="'5'"/>
										</xsl:when>

										<xsl:when test="COL3='Sell' and COL4='Open'">
											<xsl:value-of select="'B'"/>
										</xsl:when>

										<xsl:when test="COL3='Sell' and COL4='Close'">
						...
						<xsl:variable name="CostBasis">
									<xsl:value-of select="$COL25 div $Position"/>
						  </xsl:variable>
