...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="ConvertBBCodetoTicker">
		...
		<xsl:variable name="varExYear">
			<xsl:value-of select="substring(substring-after(substring-after(substring-after($varBBCode,' '),' '),' '),3, 2)"/>
		</xsl:variable>

		<xsl:variable name="varStrike">
			<xsl:value-of select="format-number(substring-before(substring-after(substring-after(substring-after(substring-after($varBBCode,' '),' '),' '),' '), ' '),'#.00')"/>
		</xsl:variable>

		<xsl:variable name="varExDay">
			<xsl:value-of select="substring-before(substring-after(substring-after($varBBCode, ' '),' '),' ')"/>
		</xsl:variable>
		...
		<xsl:variable name="varExMonth">
			<xsl:call-template name ="MonthNo">
				<xsl:with-param name ="varMonth" select ="substring-before(substring-after($varBBCode,' '),' ')"/>
			</xsl:call-template>
		</xsl:variable>
					...
					<PositionMaster>
							...
							<xsl:choose>
								...
								<xsl:when test ="COL5 = 'OPTION'">
									<xsl:call-template name ="ConvertBBCodetoTicker">
										<xsl:with-param name ="varBBCode" select ="normalize-space(COL4)"/>
									</xsl:call-template>
								</xsl:when>
								<xsl:when test=" COL5='BOND'">
									<xsl:value-of select="''"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<CUSIP>
							...
							<xsl:when test=" COL5='BOND'">
								...
								<xsl:when test="COL4='DBV TECHNOLOGIES SA' or COL4='CELLECTIS SA'">
						...
						<xsl:variable name="varShortSell" select ="normalize-space(COL13)"/>

						<xsl:variable name ="OpenClose" select ="COL15"/>
						...
						<Side>
							<xsl:choose>
								...
								<xsl:when test ="COL5 = 'OPTION'">
									<xsl:choose>
										<xsl:when test="$varSide ='1' and $OpenClose='C'">
											<xsl:value-of select="'Buy to Close'"/>
										</xsl:when>
										<xsl:when test="$varSide ='2' and $OpenClose='O'">
											<xsl:value-of select="'Sell to Open'"/>
										</xsl:when>
										<xsl:when test="$varSide ='1' and $OpenClose='O'">
											<xsl:value-of select="'Buy to Open'"/>
										</xsl:when>
										<xsl:when test="$varSide ='2' and $OpenClose='C'">
								...
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="$varSide ='1' and $varShortSell='0'">
											<xsl:value-of select="'Buy'"/>
										</xsl:when>
										<xsl:when test="$varSide ='2' and $varShortSell='0'">
											<xsl:value-of select="'Sell'"/>
										</xsl:when>
										<xsl:when test="$varSide ='1' and $varShortSell='3'">
											<xsl:value-of select="'Buy to Close'"/>
										</xsl:when>
										<xsl:when test="$varSide ='2' and $varShortSell='1'">
						...
						<xsl:variable name="TotalCommission">
							<xsl:choose>
								<xsl:when test ="COL5 = 'OPTION'">
									<xsl:value-of select ="COL12 + (COL9 * COL10 * 100)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="COL12 + (COL9 * COL10)"/>
								</xsl:otherwise>
