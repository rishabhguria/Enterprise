  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//Comparision">
        <xsl:if test="number(COL3) and COL2 != 'Cash and Equivalents'">
          <PositionMaster>
            <!--   Fund -->
            ...
            <xsl:variable name="PRANA_FUND_NAME">
              <xsl:value-of select="document('../ReconMappingXml/FundMapping.xml')/FundMapping/PB[@Name='Morcom']/FundData[@PBFundCode=$PB_FUND_NAME]/@PranaFund"/>
            </xsl:variable>

            <xsl:variable name="PB_Symbol" select="COL7"/>
            <xsl:variable name="PRANA_SYMBOL_NAME">
              <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name= 'Morcom']/SymbolData[@PBCompanyName=$PB_Symbol]/@PranaSymbol"/>
            </xsl:variable>
            ...
            <xsl:variable name="varNetPosition">
			  ...
			  <xsl:variable name="varMarkPrice">
				  <xsl:choose>
					  <xsl:when test="string-length(COL4) &gt; 12">
						  <xsl:value-of select="(COL16 div COL3) div 100"/>
					  </xsl:when>
					  <xsl:otherwise>
						  <xsl:value-of select="COL16 div COL3"/>
					  </xsl:otherwise>
				  </xsl:choose>
			  </xsl:variable>
            ...
            <xsl:variable name="varPutCall">
              <xsl:value-of select="substring(substring-after(normalize-space(COL8),' '),9,1)"/>
            </xsl:variable>

            <xsl:variable name="varStrike">
              <xsl:value-of select="format-number(substring-after(substring-after(normalize-space(COL8),' '),' '),'#.00')"/>
            </xsl:variable>

            <xsl:variable name="varExDay">
              <xsl:value-of select="substring(substring-after(normalize-space(COL8),' '),7,2)"/>
            </xsl:variable>
            ...
            <AccountName>
              <xsl:value-of select="'Iguana Healthcare-JPM'"/>
            </AccountName>
