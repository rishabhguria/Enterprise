...
{
    public class EditStageOrder : BlotterUIMap, ITestStep
    {
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            {
                SamsaraHelperClass.CaptureMyScreen("", ApplicationArguments.TestCaseToBeRun, "EditStageOrder");
                _res.IsPassed = false;
            ...
            try
            {
                ...
                if (!DataUtilities.checkList)
                {
                    if (index < 0)
                    {
                        ...
                        throw new Exception("Trade not found during EditStageOrder step. [Symbol= " + dr["Symbol"] + "], Quantity = [" + dr["Target Qty"] + "] Side = [" + dr["Side"] + "]\nRecon Error: " + String.Join("\n\r", errors));
                    }
                }
                ...
                {
                    msaaObj.CachedChildren[0].CachedChildren[index + 1].Click(MouseButtons.Right);
                    if (EditOrders.IsVisible)
                    {
                        EditOrders.Click(MouseButtons.Left);
                    }
                    else
                    {
                        Console.WriteLine("Menu Item {0} is not visible", EditOrders.MsaaName);
                    }
                }
