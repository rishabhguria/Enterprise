	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="($varTransaction_type='by' or $varTransaction_type='cs' or $varTransaction_type='sl' or $varTransaction_type='ss') and ($varSecurity_Type='cl' or $varSecurity_Type='cs' or $varSecurity_Type='pt'or $varSecurity_Type='cb' or $varSecurity_Type='gb' or $varSecurity_Type='mb'or $varSecurity_Type='mm' or  $varSecurity_Type='mf' or $varSecurity_Type='ps' or $varSecurity_Type='rw'or $varSecurity_Type='wi')">
					...
					<PositionMaster>
						...
						<xsl:variable name="varOptionString" select="translate(normalize-space(substring(COL1,94,30)),$vLowercaseChars_CONST,$vUppercaseChars_CONST)"/>
						...
						<xsl:choose>
							<xsl:when test ="$varCallPutCode!=''">
								<PBSymbol>
									<xsl:value-of select ="translate(normalize-space(substring(COL1,94,30)),$vLowercaseChars_CONST,$vUppercaseChars_CONST)"/>
								</PBSymbol>
							</xsl:when>
							<xsl:otherwise>
								<PBSymbol>
						...
						<xsl:variable name="varAmount" select="number(normalize-space(substring(COL1,37,11)))"/>
						<xsl:variable name="varQuantity" select="number(normalize-space(substring(COL1,57,11)))"/>
						<xsl:choose>
							<xsl:when  test="number(normalize-space(substring(COL1,57,11)))&gt; 0">
								<CostBasis>
									<xsl:value-of select="$varAmount div $varQuantity"/>
								</CostBasis>
							</xsl:when >
							<xsl:otherwise>
								<CostBasis>
						...
						<xsl:choose>
							...
							<xsl:when test="$varSide = 'cs'  and $varCallPutCode=''">
								<SideTagValue>
						...
						<xsl:variable name="PB_CounterParty" select="normalize-space(substring(COL1,69,7))"/>
