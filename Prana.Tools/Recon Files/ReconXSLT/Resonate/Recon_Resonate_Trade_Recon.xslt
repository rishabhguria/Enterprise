...
>
	...
	<xsl:template name="MonthCode">
		...
		<!--COL 7 - > QQQ US 03/31/23 P290 EQUITY-->
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="ExpiryDay">
			<xsl:value-of select="substring-after(substring-before(normalize-space(COL7),'/'),'/')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring-after(substring-after(substring-before(normalize-space(COL7),'/'),' '),' ')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring-after(substring-after(substring-before(normalize-space(COL7),'/'),'/'),' ')"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL7),'/'),'/'),' '),1,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring-before(substring(substring-after(substring-after(substring-after(normalize-space(COL7),'/'),'/'),' '),2),' '),'#.00')"/>
		</xsl:variable>
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($varQuantity) and COL4 != 'Cash'">
					...
					<PositionMaster>
						...
						<xsl:variable name="subTicker">
		                	<xsl:value-of select="substring-before(normalize-space(COL7),' ')"/>
		                </xsl:variable>
		                <xsl:variable name="subCK">
		                	<xsl:value-of select="substring-after(substring-before(normalize-space(COL7),' '),' ')"/>
		                </xsl:variable>
		                <xsl:variable name="subEquity">
		                	<xsl:value-of select="substring-after(substring-after(substring-before(normalize-space(COL7),' '),' '),' ')"/>
		                </xsl:variable>
						
					    <xsl:variable name="varBloomberg">
							<xsl:choose>
								<xsl:when test="contains(normalize-space(COL7),'(')">
									<xsl:value-of select="concat($subTicker,' ',$subCK,' ',$subEquity)"/>
								</xsl:when>													
								<xsl:otherwise>
							...
							<xsl:choose>
								...
								<xsl:when test="$AssetType='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="Symbol" select="normalize-space(COL7)"/>
									</xsl:call-template>
								</xsl:when>							
								<xsl:otherwise>
							...
							<xsl:choose>
								<xsl:when test="$AssetType='EquityOption'">
									<xsl:choose>
										...
										<xsl:when test="normalize-space(COL11) = 'BuyCover'">
						...
						<SettlementDate>
							<xsl:call-template name="FormatDate">
								<xsl:with-param name="DateTime" select="normalize-space(COL10)"/>
							</xsl:call-template>						
						</SettlementDate>
