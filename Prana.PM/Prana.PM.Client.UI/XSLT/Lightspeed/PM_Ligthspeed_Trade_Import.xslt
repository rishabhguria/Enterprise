...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<!--<xsl:if test="contains(substring(substring-after(COL2,' '),9,1),'C') or contains(substring(substring-after(COL2,' '),9,1),'P')">-->
		<xsl:if test="contains(COL2,'.')">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="substring(substring-after(normalize-space(COL2),' '),10)"/>
			</xsl:variable>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Side" select="normalize-space(COL1)"/>
						...
						<xsl:variable name="Fees">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL10 + COL11"/>
							</xsl:call-template>
						</xsl:variable>
						<Fees>
							<xsl:choose>
