      ...
      <xsl:for-each select="//PositionMaster">
		  <xsl:if test="COL1 != 'Report_Date'">
            ...
            <xsl:variable name="varMarkPrice">
				<xsl:choose>
					<xsl:when test ="COL17 = '7' or COL17 = '8' or COL17 = '11'">
            ...
            <xsl:variable name="varSuffix">
              <xsl:call-template name="GetSuffix">
                <xsl:with-param name="Suffix" select="$varCurrency"/>
							  ...
							  <xsl:choose>
								  <xsl:when test ="$PRANA_Symbol_NAME = ''">
									  <xsl:choose>
										  ...
										  <xsl:otherwise>
											  <xsl:choose>
												  <xsl:when test ="$varCurrency != 'USD' or ($varAssetType = '6' or $varAssetType = '9' or $varAssetType = '7' or $varAssetType = '11')">
													  <xsl:value-of select="''"/>
												  </xsl:when>
												  <xsl:otherwise>
				  ...
				  <CUSIP>
					  <xsl:choose>
						  <xsl:when test ="($varCurrency != 'USD' or ($varAssetType = '6'  or $varAssetType = '9' or $varAssetType = '7' or $varAssetType = '11')) and $varCUSIP != ' '">
