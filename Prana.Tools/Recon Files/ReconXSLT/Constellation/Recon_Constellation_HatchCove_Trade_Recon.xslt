      ...
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:if test="number($Quantity)">
          <PositionMaster>
            ...
            <AccountName>
              <xsl:choose>
				 <xsl:when test ="COL28 ='NTRS'">
                  <xsl:value-of select ="'Verger Capital Management LLC'"/>
                </xsl:when>
				   <xsl:when test ="COL28 ='USBK'">
                  <xsl:value-of select ="'State of Utah'"/>
                </xsl:when>
		...
		<xsl:variable name="PB_COUNTER_PARTY" select="COL15"/>
