	...
	<xsl:template name="TaxLotIDBuilder">
      ...
      <xsl:choose>
		...
		<!--<xsl:variable name ="FundAccountNol">
			<xsl:choose>
				<xsl:when test="FundAccountNo ='420-04649'">
		...
		<Group 
				  TRADE_ID ="{$I_GroupID}" ACTION ="NEW" TRADE_DATE = "{TradeDate}" SETTLEMENT_DATE = "{SettlementDate}" 
			ACCOUNT ="{$varAccountNo}" METHOD = "{$varMethod}" SIDE = "{$Sidevar}" SECURITY ="{$varSymbol}" 
			QUANTITY = "{$varQuantity}" PRICE = "{format-number(AveragePrice,'00000000.00000000')}" COMMISSION_TYPE = "TOTAL" 
			COMMISSION_AMOUNT = "{format-number($varCommissionAmount,'00000000.00')}" EXECUTING_BROKER = "{$varCounterParty}"   
			IS_ALLOCATION = ""
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<ThirdPartyFlatFileDetail
					  Group_Id="" TRADE_ID ="" ACTION ="" TRADE_DATE = "" SETTLEMENT_DATE = "" ACCOUNT ="{AccountNo}"  
					  METHOD = "" SIDE = "" SECURITY ="" QUANTITY = "{AllocatedQty}" PRICE = "" COMMISSION_TYPE = ""
					  COMMISSION_AMOUNT = "" EXECUTING_BROKER = "" IS_ALLOCATION = "Y"
