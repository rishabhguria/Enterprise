...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="Option">
       ...
       <xsl:variable name="ExpiryYear">
           <xsl:value-of select="substring-before(substring-after(substring-after(COL27,'/'),'/'),' ')"/>  
       </xsl:variable>
       ...
       <xsl:variable name="PutORCall">
            <xsl:value-of select="substring(substring-after(substring-after(substring-after(COL27,'/'),'/'),' '),1,1)"/>		 
       </xsl:variable>
       <xsl:variable name="StrikePrice">          
	      <xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(COL27,'/'),'/'),' '),2),'##.00')"/>	  
       </xsl:variable>
				...
				<xsl:if test="$varAsset='OPTION'">	
					...
					<PositionMaster>
						...
						<xsl:variable name="varExpiryYear">
							<xsl:value-of select="concat('20',substring-before(substring-after(substring-after(COL27,'/'),'/'),' '))"/>
						</xsl:variable>
						
						<xsl:variable name="varExpiryFullDay">
							<xsl:value-of select="concat($varExpiryMonth,'/',$varExpiryDay,'/',$varExpiryYear)"/>
						</xsl:variable>
						
						<ExpirationDate>
							<xsl:value-of select="$varExpiryFullDay"/>
						</ExpirationDate>
