...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL83,'CALLL') or contains(COL83,'PUTL')">
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(substring-after(normalize-space(COL25),' '),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="COL93"/>
			</xsl:variable>
	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and COL26='Trade'">
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								...
								<xsl:when test="COL83='STOCK'">
									<xsl:value-of select="'EquitySwap'"/>
								</xsl:when>
								
								<xsl:when test="COL83='EQTY'">
						...
						<xsl:variable name="Root_Code">
							<xsl:choose>
								<!--<xsl:when test="string-length(COL25)= 5 and $Asset='FutureOption'">
									<xsl:value-of select="substring(COL25,1,2)"/>
								</xsl:when>-->
								<xsl:when test="string-length(COL25)= 5 and $Asset='Future'">
									<xsl:value-of select="substring(COL25,1,3)"/>
								</xsl:when>
								<xsl:when test="contains(COL25,' ')">
									<xsl:value-of select="substring-before(COL25,' ')"/>
								</xsl:when>
								<xsl:when test="string-length(COL25)= 4">
									<xsl:value-of select="substring(COL25,1,2)"/>
								</xsl:when>
							</xsl:choose>
						</xsl:variable>
						<xsl:variable name="MONTH_CODE">
							<xsl:choose>
								<xsl:when test="string-length(COL25)= 5">
								...
								<xsl:when test="contains(COL25,' ')">
									<xsl:value-of select="substring(substring-after(COL25,' '),1,1)"/>
								</xsl:when>
							</xsl:choose>
						</xsl:variable>
						<xsl:variable name ="Year">
							<xsl:choose>
								<xsl:when test="string-length(normalize-space(COL25))= 5">
									<xsl:value-of select="substring(COL25,5,1)"/>
								</xsl:when>
								<xsl:when test="string-length(normalize-space(COL25))= 4">
									<xsl:value-of select="substring(COL25,4,1)"/>
								</xsl:when>
								<xsl:when test="contains(COL25,' ')">
									<xsl:value-of select="substring(substring-after(COL25,' '),2,1)"/>
								</xsl:when>
							</xsl:choose>
						</xsl:variable>
						...
						<Symbol>
							<xsl:choose>
								...
								<!--<xsl:when test="$Asset='FutureOption'">
									<xsl:value-of select="concat($varUnderlying,' ',$MONTH_CODE,$varFutOptStrikePrice)"/>
								</xsl:when>-->
								<xsl:otherwise>
						...
						<Side>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="$Side='BUY' or $Side='B'">
											<xsl:value-of select="'Buy'"/>
										</xsl:when>
										<xsl:when test="$Side='SELL' or $Side='B'">
						...
						<xsl:variable name="varMonth">
							<xsl:value-of select="substring(COL95,5,2)"/>
						</xsl:variable>
						<xsl:variable name="varDay">
							<xsl:value-of select="substring(COL95,7,2)"/>
						</xsl:variable>
						<xsl:variable name="varYear">
							<xsl:value-of select="substring(COL95,1,4)"/>
						</xsl:variable>
						<TradeDate>
							<xsl:value-of select="COL36"/>
						</TradeDate>

						<xsl:variable name="COL51">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL51"/>
							</xsl:call-template>
						</xsl:variable>

						<xsl:variable name="COL52">
							<xsl:call-template name="Translate">
						...
						<xsl:variable name="Commission">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="($COL51 + $COL52 + $COL54 + $COL56)"/>
							</xsl:call-template>
						</xsl:variable>
						<Commission>
							<xsl:choose>
