					...
					<xsl:when test ="TaxLotState!='Amemded'">
						<ThirdPartyFlatFileDetail>
							...
							<SettlementTransactionConditionIndicator>
								<xsl:choose>
									<xsl:when test="CurrencySymbol='JPY' and (Side='Buy' or Side='Sell')">
										<xsl:value-of select="'SPST'"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:choose>
						...
						<xsl:if test ="number(OldExecutedQuantity)">
							<ThirdPartyFlatFileDetail>
								...
								<xsl:variable name = "varNETAMNT">
									<xsl:choose>
										...
										<xsl:when test="$varOldFXRate!=0 and OldFXConversionMethodOperator='D'">
											<xsl:value-of select="$varNetAmmount div $varOldFXRate"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="''"/>
										</xsl:otherwise>
								...
								<SettlementTransactionConditionIndicator>
									...
									<xsl:choose>
										<xsl:when test="CurrencySymbol='JPY' and (OldSide='Buy' or OldSide='Sell')">
											<xsl:value-of select="'SPST'"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:choose>
