			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name="Quantity">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="COL106"/>
					</xsl:call-template>
				</xsl:variable>

				<xsl:choose>
					<!--<xsl:when test="number($Quantity)">-->
						<xsl:when test="number($Quantity) and COL75!='CXL' and not(contains(COL81,'Expiration')or contains(COL81,'Exercise') or contains(COL81,'Corporate Actions'))">
						<PositionMaster>
							...
							<xsl:variable name="Commission1">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="COL112 + COL116 + COL118"/>
								</xsl:call-template>
							</xsl:variable>
							...
							<xsl:variable name="NetNotionalValue">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="COL109"/>
								</xsl:call-template>
							</xsl:variable>
							<NetNotionalValue>
								<xsl:choose>
							...
							<xsl:variable name ="Date1" select="COL100"/>
							...
							<TradeDate>
								<xsl:choose>
									...
									<xsl:when test="string-length($var3)=2">
										<xsl:value-of select="concat($var3,'/',$var2,'/',$var1)"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="''"/>
									</xsl:otherwise>
