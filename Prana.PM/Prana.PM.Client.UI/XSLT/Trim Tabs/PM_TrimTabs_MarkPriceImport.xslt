...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
 ...
 <xsl:template name="Option">
    ...
    <xsl:variable name="PutORCall">
      <xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL2),'/'),'/'),' '),1,1)"/>
    </xsl:variable>
    <xsl:variable name="StrikePrice">
      <xsl:value-of select="format-number(substring-before(substring(substring-after(substring-after(substring-after(normalize-space(COL2),'/'),'/'),' '),2),'EQUITY'),'##.00')"/>
    </xsl:variable>
  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($MarkPrice) and normalize-space(COL35)='Common Stock'">
