    ...
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
		  ...
		  <xsl:variable name="PRANA_FUND_NAME">
			  <xsl:value-of select="document('../ReconMappingXml/AccountMapping.xml')/FundMapping/PB[@Name='LIME']/FundData[@PBFundName=$PB_FUND_NAME]/@PranaFund"/>
		  </xsl:variable>
		  ...
		  <xsl:if test="number(COL6) and normalize-space(normalize-space(COL4)) != 'DIVPAY'and normalize-space(normalize-space(COL4)) != 'PDST' and $PRANA_FUND_NAME!=''">
			  <PositionMaster>
				  ...
				  <xsl:variable name="PRANA_ACRONYM_NAME">
					  <xsl:choose>
						  ...
						  <!--<xsl:when test="contains(COL3,'FEE') and (COL6 &lt; 0)">
						  ...
						  <xsl:when test="(normalize-space(COL5) = 'INTEREST' or normalize-space(COL4)='CINTR' or normalize-space(COL4)='INT') and (COL6 &gt; 0)">
							  <xsl:value-of select="'Interest_Income'"/>
						  </xsl:when>
						  <xsl:when test="(normalize-space(COL5) = 'INTEREST' or normalize-space(COL4)='INTR' or normalize-space(COL4)='CRIPAY') and (COL6 &lt; 0)">
							  <xsl:value-of select="'Interest_Expense'"/>
						  </xsl:when>

						  <xsl:when test="(normalize-space(COL4) = 'T/A' and contains(COL3,'TRANSFER CASH'))  and (COL6 &gt; 0) ">
							  <xsl:value-of select="'CASH_DEP'"/>
						  </xsl:when>

						  <xsl:when test="(contains(COL3,'CIP') or contains(COL3,'FEE') or contains(COL3,'/16') or normalize-space(COL5) = 'OTHER INC/EXP') and (COL6 &gt; 0)">
							  <xsl:value-of select="'MISC_INC'"/>
						  </xsl:when>
						  <xsl:when test="(contains(COL3,'CIP') or contains(COL3,'FEE')or contains(COL3,'STERLING') or contains(COL3,'NIRVANA') or normalize-space(COL5) = 'OTHER INC/EXP') and (COL6 &lt; 0)">
							  <xsl:value-of select="'MISC_EXP'"/>
						  </xsl:when>

						  <xsl:when test="((normalize-space(COL4) = 'FEDWT') or(normalize-space(COL4) = 'JNL' or normalize-space(COL5) = 'WITHDRAWALS')) or (contains(COL3,'WIRE')) and (COL6 &lt; 0)  ">
							  <xsl:value-of select="'CASH_WDL'"/>
						  </xsl:when>

						  <xsl:when test="((normalize-space(COL4) = 'FEDWT') or(normalize-space(COL4) = 'JRL' or normalize-space(COL5) = 'CONTRIBUTIONS')) or (contains(COL3,'WIRE'))  and (COL6 &gt; 0) ">
							  <xsl:value-of select="'CASH_DEP'"/>
						  </xsl:when>
