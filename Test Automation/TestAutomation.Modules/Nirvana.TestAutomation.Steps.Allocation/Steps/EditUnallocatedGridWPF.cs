...
{
    internal class EditUnallocatedGridWPF : AllocationUIMap, ITestStep
    {
        /// <summary>
        ...
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (gridColumnMapping.ContainsKey(TestDataConstants.COL_SYMBOL) && gridColumnMapping.ContainsKey(TestDataConstants.COL_QUANTITY) && gridColumnMapping.ContainsKey(TestDataConstants.COL_AVERAGE_PRICE_BASE))
                    {
                        ...
                        try
                        {
                            string StepName = "EditUnallocatedGridWPF";
            ...
            {
                SamsaraHelperClass.CaptureMyScreen("", ApplicationArguments.TestCaseToBeRun, "EditUnallocatedGridWPF");
                _res.IsPassed = false;
        ...
        {
            aeGrid = null;
            string errorMessage = "Could not find the Unallocated grid";
            try
            {
                ...
                if (aeDesktop != null)
                {
                    ...
                    if (aePrana != null)
                    {
                        AutomationElement aeAlGrid = aePrana.FindFirst(TreeScope.Descendants, new PropertyCondition(AutomationElement.AutomationIdProperty, "GridUnallocated"));
        ...
        private string EnterValuesInGrid(TablePattern aeGrid, Dictionary<string, string> columnValues, Dictionary<string, int> gridColumnMapping)
        {
            ...
            try
            {
                ...
                int executedQuantityIndex = gridColumnMapping["Executed Qty"];
                ...
                for (i = 0; i < rowCount; i++)
                {
                    ...
                    ValuePattern excqty = (ValuePattern)aeGrid.GetItem(i, executedQuantityIndex).GetCurrentPattern(ValuePattern.Pattern);
                    qty = Convert.ToInt32(excqty.Current.Value); 
                }
                if (i < rowCount)
                {
                    foreach (string columnName in columnValues.Keys)
                    {
                        if (!(columnName.Equals(TestDataConstants.COL_SYMBOL) || columnName.Equals(TestDataConstants.COL_QUANTITY) || columnName.Equals(TestDataConstants.COL_AVERAGE_PRICE_BASE)))
                        {
                            ...
                            // Need custom Handling for Executed Qty should be less than or equal to Qty Popup
                            if (columnValues.ContainsKey("Executed Qty"))
                            {
                                int quantity = Convert.ToInt32(columnValues["Executed Qty"]);
                                if (columnName.Equals("Executed Qty"))
                                {
                                    if (quantity > qty)
                                    {
                            ...
                            //Need custom handling for order side due to transaction type pop-up
                            if (columnName.Equals("Side"))
                            {
                    ...
                    errorMsg = "Row not found in Unallocated Grid";
