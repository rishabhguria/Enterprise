    ...
    <xsl:variable name="varFeeValue2">
 	...
 	<xsl:variable name ="varAllocationState">
	  ...
	  <xsl:variable name="VarComm">
		  <xsl:choose>
			  <xsl:when test="SettlCurrFxRate=0">
				  <xsl:value-of select="format-number($varCommissions,'##.00')"/>
			  </xsl:when>
			  <xsl:when test="SettlCurrFxRate!=0 and SettlCurrFxRateCalc='M'">
				  <xsl:value-of select="format-number($varCommissions * SettlCurrFxRate,'##.00')"/>
			  </xsl:when>

			  <xsl:when test="SettlCurrFxRate!=0 and SettlCurrFxRateCalc='D'">
				  <xsl:value-of select="format-number($varCommissions div SettlCurrFxRate,'##.00')"/>
			  </xsl:when>
			  <xsl:otherwise>
				  <xsl:value-of select="''"/>
			  </xsl:otherwise>
	  ...
	  <xsl:variable name="PB_COUNTERPARTY_NAME_A">
		  <xsl:value-of select="document('../ReconMappingXml/ThirdParty_ExecBrokerMapping.xml')/BrokerMapping/PB[@Name=$PB_NAME]/BrokerData[@PranaBroker=$PRANA_COUNTERPARTY_NAME]/@BrokerMappingName"/>
	  </xsl:variable>
	  ...
	  <xsl:variable name="TradeAttribute2">

		  <xsl:choose>
			  <xsl:when test ="TradeAttribute2!='*' and TradeAttribute2!='' ">
				  <xsl:value-of select="translate(TradeAttribute2,$vLowercaseChars_CONST,$vUppercaseChars_CONST)"/>
			  </xsl:when>
			  <xsl:otherwise>
	  ...
	  <xsl:variable name="SecFees">
		  <xsl:choose>
			  <xsl:when test="contains(BBCode,'US Equity')">
	  ...
	  <Group 
			  ...
			  Broker="{$TradeAttribute2}" Custodian="NT" Exchange="{$varEXCODE}" TransactionType="{$varTransactionType}" CurrencyCode="{SettlCurrency}"
			  ...
			  TradeTax="{$varTradeTax}" OtherBrokerFees="{format-number(OtherBrokerFee+ClearingBrokerFee,'#.000000')}" NetAmount="{$varNetAmount}" Principal="{$varGrossAmount}" Description="FX"
			  SecurityType="{$varSecurityName}" CountrySettlementCode="" ClearingAgent="" SECFee="{format-number(SecFee,'#.00')}" RepoOpenSettleDate="" RepoMaturityDate=""
