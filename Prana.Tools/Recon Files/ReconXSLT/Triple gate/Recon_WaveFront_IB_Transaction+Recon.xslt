...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="string-length(COL7) &gt; 15 and contains(COL7,'_')!='true'">
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) ">
					...
					<PositionMaster>
						...
						<xsl:variable name="OpenClose" select="normalize-space(COL31)"/>
							...
							<xsl:choose>
								<xsl:when test="$Asset='Equity'">
									<xsl:choose>
										...
										<xsl:when test="($Side='SELL' or $Side='Delivered') and $OpenClose = 'O'">
						...
						<xsl:variable name="varCommission">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$COL18 * $COL19 - $COL25"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="Commission">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL22 * ( -1) "/>
							</xsl:call-template>
						</xsl:variable>
						<TotalCommissionandFeesBase>
							<xsl:choose>
								<xsl:when test="COL24='Delivered'">
									<xsl:choose>
