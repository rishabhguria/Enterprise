				...
				<xsl:if test="number(COL34) and COL26='Trade' and COL7!='FX'">
					<PositionMaster>		
						...
						<xsl:variable name="PRANA_EXCHANGE_NAME">
							<xsl:value-of select="document('../../../MappingFiles/ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name=$varPB_Name]/SymbolData[@PBCode=$BB_Code]/@ExchangeName"/>
						</xsl:variable>
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="COL83='FUTURE'">
									<xsl:value-of select="normalize-space(concat($varUnderlying,' ',$MONTH_No,$Year,$PRANA_EXCHANGE_NAME))"/>
								</xsl:when>
								<xsl:when test="contains(COL84,'SWAP')">
									<xsl:value-of select="concat($Symbol,$PRANA_Exchange,'/SWAP')"/>
								</xsl:when>
								<xsl:when test="contains(COL84,'CROPE')">
									<xsl:value-of select="''"/>
								</xsl:when>
