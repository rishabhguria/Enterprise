	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">			
				...
				<xsl:if test="number($varQuantity) and (COL12 ='TICKET CHARGE' or COL12='SHORT INTEREST REBATE')">
					...
					<PositionMaster>
						...
						<xsl:variable name="PRANA_ACRONYM_NAME">
							<xsl:choose>
								<xsl:when test="normalize-space(COL12)='TICKET CHARGE'">
									<xsl:value-of select="'Ticket fees Paid'"/>
								</xsl:when>
								<xsl:when test="COL12='SHORT INTEREST REBATE' and $varQuantity &lt;0">
