...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[IsSwapped='false' or (contains(Asset,'Equity'))]">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="Custodian">
			<xsl:choose>
				<xsl:when test="contains(FundName,'BAML')">
		...
		<Group 
				...
				translevel="B" ClientRef="{PBUniqueID}" Associated="{PBUniqueID}" ExecAccount="038200770" AccountID="038200770"
				ExecBkr="{$CPVar}" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{FullSecurityName}" TDate="{TradeDate}" SDate="{SettlementDate}"
				SCCY="{CurrencySymbol}"	ExCode="" qty="{$QtySum}" 	Price="{format-number(AveragePrice,'0.####')}"	type="G" prin="{format-number($GroupGrsAmt,'###.00')}" 
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
				...
				<xsl:variable name="THIRDPARTY_FUND_NAME_T">
					<xsl:value-of select="document('../ReconMappingXml/FundMapping.xml')/FundMapping/PB[@Name=$PB_NAME]/FundData[@PranaFund=$PRANA_FUND_NAME_T]/@PBFundCode"/>
				</xsl:variable>
				...
				<ThirdPartyFlatFileDetail
							...
							translevel="A" ClientRef="{concat(substring(EntityID,3),'C')}" Associated="{PBUniqueID}" ExecAccount="038200770" AccountID="{FundAccountNo}"
							ExecBkr="{$CPVar}" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{FullSecurityName}" TDate="{TradeDate}" SDate="{SettlementDate}"
							SCCY="{CurrencySymbol}"	ExCode="" qty="{AllocatedQty}" Price="{format-number(AveragePrice,'0.####')}" type="G" prin="{format-number($TaxlotGrsAmt,'###.00')}"
