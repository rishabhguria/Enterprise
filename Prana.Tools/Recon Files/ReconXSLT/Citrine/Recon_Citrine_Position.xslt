					...
					<PositionMaster>
						...
						<xsl:variable name="varContractCode">
							<xsl:choose>
								...
								<xsl:when test="COL15 = 'BP'">
									<xsl:value-of select="'6B'"/>
								</xsl:when>
								<xsl:when test="COL15 = 'HG'">
									<xsl:value-of select="'HG'"/>
								</xsl:when>
								<xsl:when test="COL15 = 'EC'">
									<xsl:value-of select="'6E'"/>
								</xsl:when>
								<xsl:when test="COL15 = 'ES'">
						...
						<xsl:variable name="PRANA_Multiplier">
							<xsl:value-of select="document('../ReconMappingXml/PriceMulMapping.xml')/PriceMulMapping/PB[@Name='JPM']/MultiplierData[@PranaRoot=$varContractCode]/@Multiplier"/>
						</xsl:variable>
						...
						<xsl:variable name="varMarkPrice">
							<xsl:choose>
								<xsl:when test ="normalize-space($PRANA_Multiplier) != ''">
									<xsl:value-of select ="COL7*$PRANA_Multiplier"/>
								</xsl:when>
								<xsl:otherwise>
            ...
            <xsl:variable name="PB_Suffix">
              <xsl:value-of select="document('../ReconMappingXml/SymbolSuffixMapping.xml')/SymbolMapping/PB[@Name = $varPBName]/SymbolData[@TickerSuffixCode = $PB_Currency_Name]/@PBSuffixCode"/>
						...
						<xsl:variable name="varStrike">
							<xsl:choose>
								<xsl:when test="contains(COL16,'EURO')">
									<xsl:value-of select="format-number(COL38*1000,'#')"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<xsl:variable name="ThirdWednesday">
							<xsl:choose>
								<xsl:when test="$varAssetType = 'FUT' and $varExchange = 'LME'">
									<xsl:value-of select ="my:Now2(number($varExYear),number($varExMonth))"/>
								</xsl:when>
							</xsl:choose>
						</xsl:variable>
						...
						<Symbol>
							<xsl:choose>
								<xsl:when test="$varAssetType = 'FUT' and $varExchange = 'LME'">
									<xsl:choose>
										<xsl:when test="number(substring-before(substring-after($ThirdWednesday,'/'),'/')) = number($varFutExDayMod)">
											<xsl:value-of select="concat($varInitial,' ',substring($varExYear,4,1), $varMonthCode, '-LME')"/>
										</xsl:when>
										<xsl:otherwise>
								...
								<xsl:when test="$varAssetType = 'OPT' and $varExchange != 'LME'">
									<xsl:value-of select="concat($varContractCode,' ',$varMonthCode,substring($varExYear,4,1), $varPutCall, $varStrike)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="''"/>
								</xsl:otherwise>
						...
						<ExpirationDate>
							<xsl:choose>
								<xsl:when test="$varAssetType = 'OPT' and $varExchange = 'LME'">
									<xsl:value-of select="substring-before(my:Now1(number($varExYear),$varExMonth),' ')"/>
								</xsl:when>
								<xsl:otherwise>
