CREATE PROCEDURE [dbo].[P_CalculateFXPNLOnSymbolLevelAccruals_AuditTrail]
	@StartDate DATETIME
	,@EndDate DATETIME
	,@FundIDs VARCHAR(max)
	,@userID INT = NULL 
...
--EXEC P_GetAllFXConversionRatesFundWiseForGivenDateRangeGetCashPnL @DayPriorToStartDate,@EndDate,@CurrencyID

INSERT INTO #FXConversionRates
EXEC P_GetAllFXConversionRatesFundWiseForGivenDateRangeGetCashPnL @DayPriorToStartDate,@EndDate,@CurrencyID,@Fund_Ids
...
WHERE DateDiff(Day, SubAccountBalances.TransactionDate, @DayPriorToStartDate) = 0
	...
	AND SubAccountBalances.CurrencyID = @CurrencyID
...
-------------------------------Rounding all basic field to Six decimal for the symbol wise accruals ------------------------------------------                                                                                       
UPDATE #PNLTable
SET BeginningMarketValueLocal = Round(BeginningMarketValueLocal, 6)
	,EndingMarketValueLocal = Round(EndingMarketValueLocal, 6)
...
AND journal.CurrencyID = @CurrencyID
