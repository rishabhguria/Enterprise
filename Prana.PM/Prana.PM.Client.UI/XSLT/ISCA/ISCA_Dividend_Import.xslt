			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:variable name="varDividend">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="substring(COL1,21,8)"/>
					</xsl:call-template>
				</xsl:variable>
				...
				<xsl:if test="number($varDividend)">
					...
					<PositionMaster>
						...
						<xsl:variable name = "PB_FUND_NAME" >
							<xsl:value-of select="substring(COL1,1,18)"/>
						</xsl:variable>
						...
						<xsl:variable name="PB_Symbol" select="normalize-space(substring(COL1,93,38))"/>
						...
						<!--<xsl:variable name="Symbol" select="substring(substring-after(substring-after(substring(COL1,44,24),'{'),'{'),1,5)"/>-->
						<xsl:variable name="Symbol" select="substring(COL1,44,24)"/>
						...
						<PayoutDate>
							...
							<!--<xsl:value-of select="substring(COL1,106,9)"/>-->
						</PayoutDate>

						<ExDate>
							<xsl:value-of select="''"/>

							<!--<xsl:value-of select="substring(COL1,98,8)"/>-->
						</ExDate>
