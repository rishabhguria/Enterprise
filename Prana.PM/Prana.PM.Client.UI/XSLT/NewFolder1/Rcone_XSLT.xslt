			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name ="PB_COMPANY"> <!--< Put description ="here">-->
							<xsl:value-of select ="COL20"/>
						</xsl:variable>
						<xsl:variable name="PRANA_SYMBOL"> <!--<This mapping >-->
						...
						<xsl:variable name ="Month">
							<xsl:value-of select ="substring(COL35,5,2)"/>
						</xsl:variable>
						<xsl:variable name ="Date">
							<xsl:value-of select ="substring(COL35,7,2)"/>
						</xsl:variable>
						<xsl:variable name ="Year">
							<xsl:value-of select ="substring(COL35,1,4)"/>
						</xsl:variable>
						<PositionStartDate>
							<xsl:value-of select ="concat($Month,'/',$Date,'/',$Year)"/>
							<!--<xsl:value-of select="concat(substring(COL35,5,2),'/',substring(COL35,7,2),'/',substring(COL35,1,4))"/>-->
						</PositionStartDate>
						...
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="COL33='BUY'">
									<xsl:value-of select="'1'"/>
								</xsl:when>

								<xsl:when test="COL33='SELL'">
