<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:my="put-your-namespace-uri-here" version="1.0">
  ...
  <msxsl:script language="C#" implements-prefix="my">
    public string Now(int year, int month) { DateTime thirdFriday= new DateTime(year, month, 15); while (thirdFriday.DayOfWeek != DayOfWeek.Friday) { thirdFriday = thirdFriday.AddDays(1); } return thirdFriday.ToString(); }
  </msxsl:script>
  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
        ...
        <xsl:if test="number($NetPosition) and COL4='NEW'">
          <PositionMaster>
            <xsl:variable name="PB_NAME">
              <xsl:value-of select="''"/>
            </xsl:variable>
            
            <xsl:variable name="MappedPBName" select="''"/>
            ...
            <xsl:variable name="Asste">
              <xsl:choose>
                <xsl:when test="COL25='OPTIONS' and (substring-before(COL13,' ')='PUT' or substring-before(COL13,' ')='CAL')">
            ...
            <xsl:variable name="PRANA_FUND_NAME">
              <xsl:choose>
                <xsl:when test ="($PB_NAME = $MappedPBName)">
                  <xsl:value-of select="document('../ReconMappingXml/AccountMapping.xml')/FundMapping/PB[@Name=$PB_NAME]/FundData[@PBFundCode=$PB_FUND_NAME and @PBFundName=$PB_MASTER_FUND_NAME]/@PranaFund"/>
                </xsl:when>
                <xsl:otherwise>
