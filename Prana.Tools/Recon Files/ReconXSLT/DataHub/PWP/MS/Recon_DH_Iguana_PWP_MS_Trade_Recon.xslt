	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity) and contains(COL84,'Cash')!='true'and COL12='TTT'">
						<PositionMaster>
              ...
              <xsl:variable name="varUnderline" select="substring-before($varOption,'/')" />
							...
							<Symbol>
								<xsl:choose>
									...
									<xsl:when test="$Asset='Option'">
										<xsl:value-of select="concat($varUnderline,'/',$varOptionDate,'/',$varOptionYear,' ',$varStrikePrice)"/>
									</xsl:when>
