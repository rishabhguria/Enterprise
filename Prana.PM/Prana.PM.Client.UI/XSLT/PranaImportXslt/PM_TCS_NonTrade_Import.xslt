        ...
        <xsl:if test="number($varQuantity)">
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_PRE">
              <xsl:choose>
                <xsl:when test="COL12 ='INTEREST ON DEBIT BALANCE'">
                  <xsl:value-of select="'Interest_Expense'"/>
                </xsl:when>
                <xsl:when test="COL12 ='INTEREST ON DEBIT BALANCE'">
                  <xsl:value-of select="'Interest_Payable'"/>
                </xsl:when>
                <xsl:when test="COL12 ='SHORT INTEREST REBATE'">
                  <xsl:value-of select="'Interest_Expense'"/>
                </xsl:when>

                <xsl:when test="COL12 ='TICKET CHARGE' or COL12 ='Ticket Charge'">
                  <xsl:value-of select="'Ticket fees Paid'"/>
                </xsl:when>
                <xsl:when test="COL8 ='Cash Deposit' or contains(COL8,'Deposit')">
                  <xsl:value-of select="'CASH_DEP'"/>
                </xsl:when>
                <xsl:when test="COL8 ='Cash Withdraw' or contains(COL8,'Withdraw')">
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_POST">
              <xsl:choose>
                ...
                <xsl:when test="COL12 ='SHORT INTEREST REBATE' or COL12 ='TICKET CHARGE' or COL12 ='INTEREST ON DEBIT BALANCE'">
                  <xsl:value-of select="'Cash'"/>
                </xsl:when>
                
                <xsl:when test="COL8 ='Withdraw'or COL8 ='Deposit'">
                  <xsl:value-of select="'Cash'"/>
                </xsl:when>
            ...
            <JournalEntries>
              <xsl:value-of select="concat($PRANA_ACRONYM_NAME_PRE, ':' , $varAmount , '|' , $PRANA_ACRONYM_NAME_POST, ':' , $varAmount)"/>
            </JournalEntries>
            ...
            <Date>
              <xsl:call-template name ="FormatDate">
                <xsl:with-param name="DateTime" select="$varDate">
                </xsl:with-param>
              </xsl:call-template>
