		...
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:variable name="varAsset">
					<xsl:choose>
						<xsl:when test="normalize-space(COL7)= 'equity'">
							<xsl:value-of select="'Equity'" />
						</xsl:when>
						<xsl:when test="normalize-space(COL7) = 'put' or  normalize-space(COL7) = 'call'">
				...
				<xsl:if test="$varAsset='EquityOption'">
					...
					<PositionMaster>
						...
						<xsl:variable name="varExpiryYear">
							<xsl:value-of select="substring(substring-after(normalize-space(COL4),' '),1,4)"/>
						</xsl:variable>
