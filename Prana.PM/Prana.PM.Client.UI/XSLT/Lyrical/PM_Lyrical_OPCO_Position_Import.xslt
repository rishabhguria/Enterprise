		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:variable name="varPosition">
					<xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL1),'|'),'|'),'|'),'|'),'|'),'|'),'|'),'|'),'|'),'|'),'|'),'|'),'|')"/>
				</xsl:variable>
				...
				<xsl:if test="number($NetPosition) and not(contains(substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL1),'|'),'|'),'|'),'|'),'|'),'|'),'USD') ='true') ">
					...
					<PositionMaster>
						...
						<xsl:variable name="CostBase2">
							<xsl:value-of select="$CostBasis div $NetPosition"/>
						</xsl:variable>

						<CostBasis>

							<xsl:choose>
								<xsl:when test="$CostBase2 &gt; 0">
									<xsl:value-of select="$CostBase2"/>
								</xsl:when>
								<xsl:when test="$CostBase2 &lt; 0">
									<xsl:value-of select="$CostBase2 * (-1)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
