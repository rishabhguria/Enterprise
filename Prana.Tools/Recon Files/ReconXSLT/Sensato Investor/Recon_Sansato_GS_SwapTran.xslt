	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//Comparision">
				<xsl:if test="COL18 = 'OP' and COL1 != 'DATA: Custody Transaction (with Header)' and COL1 != 'Advisor:' and COL1 != 'Fund:' and COL1 != 'Business Date:' and COL1 != 'Run Date:' and COL1 != 'Report Code:' and COL1 != 'Client ID' ">
					<PositionMaster>
						...
						<xsl:choose>
							<xsl:when test="number(COL29) and number(COL30) and number(COL30) &lt; 0 ">
								<NetNotionalValue>
									<xsl:value-of select="(COL29 * COL30) *(-1)"/>
								</NetNotionalValue>
							</xsl:when>
							<xsl:when test="number(COL29) and number(COL30) and number(COL30) &gt; 0 ">
								<NetNotionalValue>
									<xsl:value-of select="(COL29 * COL30)"/>
								</NetNotionalValue>
							</xsl:when>
							<xsl:otherwise>
								<NetNotionalValue>
