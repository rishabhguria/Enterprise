        ...
        <xsl:if test="number($NetPosition)" >
          <PositionMaster>
			...
			<xsl:variable name="varPrincipal">
               <xsl:value-of select="$CostBasis * $NetPosition"/>
            </xsl:variable>

			<StampDuty>
				<xsl:choose>
                <xsl:when test="contains($varSymbol, '-LON') and ($varSide = 'BY' or $varSide = 'CS')">
                  <xsl:value-of select="$varPrincipal * 0.005"/>
                </xsl:when>
            ...
            <Commission>
              <xsl:choose>
			   <xsl:when test="contains($varSymbol, '-') or (not(contains($varSymbol, 'O'))) ">
                  <xsl:value-of select="$varPrincipal * 0.0008"/>
                </xsl:when> 
               <xsl:when test="not(contains($varSymbol, 'O')) or not(contains($varSymbol, '-'))">
                  <xsl:value-of select="$NetPosition * 0.05"/>
                </xsl:when>  
				 
				<xsl:when test="contains($varSymbol, 'O')">
                  <xsl:value-of select="$NetPosition * 3"/>
                </xsl:when>  				
                <xsl:otherwise>
                  <xsl:value-of select="0"/>
                </xsl:otherwise>
			...
			<OrfFee>
				<xsl:choose>
                <xsl:when test="contains($varSymbol, 'O:')">
                  <xsl:value-of select="$NetPosition * 0.0269"/>
                </xsl:when>
			...
			<SecFee>
				<xsl:choose>
                <xsl:when test="not(contains($varSymbol, '-')) and ($varSide = 'SS' or $varSide = 'SL') ">
                   <xsl:value-of select="$varPrincipal * 0.0008"/>
                </xsl:when>
