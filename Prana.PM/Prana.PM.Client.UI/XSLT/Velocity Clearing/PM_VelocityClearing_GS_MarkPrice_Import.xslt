        ...
        <xsl:if test="number($MarkPrice)">
          <PositionMaster>
            ...
            <xsl:variable name = "varAsset">
              <xsl:choose>
                <xsl:when test="substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL3),' '),' '),' '),' '),' ')='P' or substring-after(substring-after(substring-after(substring-after(substring-after(normalize-space(COL3),' '),' '),' '),' '),' ')='C'">
                  <xsl:value-of select="'EquityOption'"/>
                </xsl:when>	
                <xsl:when test="contains(COL3,'%')">
                  <xsl:value-of select="'FixedIncome'"/>
                </xsl:when>		
                <xsl:when test="not(contains(COL3,'%'))">
