...
namespace Prana.CorporateAction.NameChangeRule
{
    static class DBManager
    {
        ...
        internal static bool SaveCAWithSymbolAndCompanyNameChange(List<CAOnProcessObjects> caOnProcessObjectList, IAllocationServices allocationServices, IClosingServices closingServices, int userID)
        {
            bool isSaved = true;
            try
            {
                foreach (CAOnProcessObjects obj in caOnProcessObjectList)
                {
                    ...
                    Logger.LoggerWrite("Corporate Action (Name Change) data saving starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    Int64 symbol_PK_Underlying = SecurityMasterSymbolIDGenerator.GenerateSymbolPKID();

                    //Saves data in securitymaster
                    Logger.LoggerWrite("Corporate Action (Name Change): saving in Security Master DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    ...
                    Logger.LoggerWrite("Corporate Action (Name Change): saved in Security Master DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    ...
                    foreach (List<AllocationGroup> taxlotSave in taxlotsChunk)
                    {
                        ...
                        Logger.LoggerWrite("Corporate Action (Name Change): New groups from Chunk " + chunkNo + "saving in client DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                        affectedPositions2 = allocationServices.SaveGroups(taxlotSave, userID);
                        chunkNo++;
                    }
                    Logger.LoggerWrite("Corporate Action (Name Change): New groups saved in client DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    //Saves the closing data into PM_Taxlots and PM_Taxlotclosing
                    Logger.LoggerWrite("Corporate Action (Name Change): Closed data saving in client DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    affectedPositions3 = closingServices.SaveCloseTradesData(obj.ClosingData);
                    Logger.LoggerWrite("Corporate Action (Name Change): Closed data saved in client DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    ...
                    Logger.LoggerWrite("Corporate Action (Name Change): Taxlots affected due to Corporate action, saving in client DB...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
                    affectedPositions4 = AllocationDataManager.SaveCAWiseCloseData(caCloseData);
                    Logger.LoggerWrite("Corporate Action (Name Change): Taxlots affected due to Corporate action, saved in client DB", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);

                    Logger.LoggerWrite("Corporate Action (Name Change) data saving completed", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
        ...
        internal static bool UndoNameChange(CAOnProcessObjects caOnProcessObject, IClosingServices closingServices, IAllocationServices allocationServices)
        {
            string caIds = caOnProcessObject.CorporateActionIDs;
            int affectedPositions2 = 0;

            Logger.LoggerWrite("Undo Corporate Action (Name Change) Delete Data from Security Master starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            int affectedPositions1 = SecMasterDataManager.UndoNameChange(caIds);
            Logger.LoggerWrite("Undo Corporate Action (Name Change) Unwinding positions starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            bool isSuccessfullyUnwinded = UnWindPositions(caIds, allocationServices);

            Logger.LoggerWrite("Undo Corporate Action (Name Change) Delete Groups starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            DataTable deleteGroups = FetchGroupsToDeleteForCAs(caIds);

            affectedPositions2 = allocationServices.DeleteGroupsFromCA(deleteGroups);

            Logger.LoggerWrite("Undo Corporate Action (Name Change) Delete CA Information from client side starts...", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
            int affectedPositions3 = DeleteNameChangeData(caIds);
            Logger.LoggerWrite("Corporate Action (Name Change): Undo data completed", LoggingConstants.CATEGORY_FLAT_FILE_TRACING);
