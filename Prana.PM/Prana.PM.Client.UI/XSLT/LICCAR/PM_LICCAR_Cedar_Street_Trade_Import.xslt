...
>
  ...
  <xsl:template name="Option">
    ...
    <xsl:variable name="UnderlyingSymbol">
      <xsl:value-of select="substring-before(normalize-space(COL17),' ')"/>
    </xsl:variable>
    <xsl:variable name="ExpiryDay">
      <xsl:value-of select="substring(substring-after(normalize-space(COL17),' '),5,2)"/>
    </xsl:variable>
    <xsl:variable name="ExpiryMonth">
      <xsl:value-of select="substring(substring-after(normalize-space(COL17),' '),3,2)"/>
    </xsl:variable>
    <xsl:variable name="ExpiryYear">
      <xsl:value-of select="substring(substring-after(normalize-space(COL17),' '),1,2)"/>
    </xsl:variable>
    <xsl:variable name="PutORCall">
      <xsl:value-of select="substring(substring-after(normalize-space(COL17),' '),7,1)"/>
    </xsl:variable>
    <xsl:variable name="StrikePrice">
      <xsl:value-of select="format-number(substring(substring-after(normalize-space(COL17),' '),8) div 1000,'##.00')"/>
    </xsl:variable>
        ...
        <xsl:if test="number($NetPosition) " >
          <PositionMaster>
            ...
            <xsl:variable name="varAsset">
              <xsl:choose>
                <xsl:when test="string-length(normalize-space(COL17)) &gt; 9">
            ...
            <xsl:variable name="PB_FUND_NAME" select="'Cedar Street Fund LP  Class A'"/>
            ...
            <SideTagValue>
              <xsl:choose>
                <xsl:when test="$varAsset='EquityOption'">
                  <xsl:choose>
                    <xsl:when test="$varSide='Buy New'">
                      <xsl:value-of select="'A'"/>
                    </xsl:when>
                    <xsl:when test="$varSide ='Sell New'">
                      <xsl:value-of select="'D'"/>
                    </xsl:when>
                    <xsl:when test="$varSide = 'Sell Short New'">
                      <xsl:value-of select="'C'"/>
                    </xsl:when>
                    <xsl:when test="$varSide = 'Cover Short New'">
                      <xsl:value-of select="'B'"/>
                    </xsl:when>
                  </xsl:choose>
                </xsl:when>
