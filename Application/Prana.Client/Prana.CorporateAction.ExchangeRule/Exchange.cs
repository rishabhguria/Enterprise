...
{
    public class Exchange : ICorporateActionBaseRule
    {
        ...
        public void ValidateCAInfo(ref DataRowCollection rows)
        {
            try
            {
                foreach (DataRow row in rows)
                {
                    ...
                    {
                        if (Convert.ToDouble(row[CorporateActionConstants.CONST_ExchangeRatio].ToString()) <= 0 || Convert.ToDouble(row[CorporateActionConstants.CONST_ExchangeRatio].ToString()) >= 100)
                        {
                            row.SetColumnError(columns[CorporateActionConstants.CONST_ExchangeRatio], "% of Shares Involved should be greater than 0 and less than 100.");
                            continue;
                        }
                    }
                    else
                    {
                        row.SetColumnError(columns[CorporateActionConstants.CONST_ExchangeRatio], "% of Shares Involved should be greater than 0 and less or equal to 100.");
                        continue;
                    }
        ...
        public CAPreviewResult PreviewCorporateActions(DataRowCollection caRows, ref TaxlotBaseCollection modifiedTaxlotList, string commaSeparatedAccountIds, CAPreferences caPref, int brokerId)
        {
            ...
            try
            {
                ...
                foreach (DataRow corporateActionRow in caRows)
                {
                    ...
                    GetModifiedTaxlotForExchangeWithDifferentSecurity(corporateActionRow, ref caModifiedTaxlots, ref taxlotsClosedInFutureStr, ref taxlotsCAAppliedInFutureStr, ref taxlotsBoxedPositionStr, commaSeparatedAccountIds, caPref);
        ...
        private void GetModifiedTaxlotForExchangeWithDifferentSecurity(DataRow corporateActionRow, ref TaxlotBaseCollection caModifiedTaxlots, ref string taxlotsClosedInFutureStr, ref string taxlotsCAAppliedInFutureStr, ref string taxlotsBoxedPositionStr, string commaSeparatedAccountIds, CAPreferences caPref)
        {
            try
            {
                ...
                double exchangeRatio = Convert.ToDouble(corporateActionRow[CorporateActionConstants.CONST_ExchangeRatio]);
                ...
                List<string> positionalExchangeTaxlotId = AllocationDataManager.GetFuturePositionalCorpActionTaxlotId(date);
                //check whether taxlot is already closed or not
                CheckTaxlotStatus(ref taxlotsClosedInFutureStr, ref taxlotsCAAppliedInFutureStr, ref taxlotsBoxedPositionStr, taxlotsClosedInFuture, taxlotsCAAppliedInFuture, taxlotsBoxedPosition, date, taxlotList, openTaxlotCount, positionalExchangeTaxlotId);
                ...
                if (String.IsNullOrEmpty(taxlotsClosedInFutureStr) && String.IsNullOrEmpty(taxlotsCAAppliedInFutureStr) && String.IsNullOrEmpty(taxlotsBoxedPositionStr))
                {
                    ...
                    foreach (KeyValuePair<int, List<TaxlotBase>> kvp in AccountwisetaxlotBaseDict)
                    {
                        ...
                        // this is used to sum up quantity accountwiase of all the new addition taxlots
                        decimal accountTotalQty_AdditionTaxlots = 0;

                        // this is used to sum up quantity accountwiase of all the original taxlots
                        decimal accountTotalQty_OriginalTaxlots = 0;

                        // this is used to sum up quantity accountwiase of all the withdrawal taxlots
                        decimal accountTotalQty_WithdrawalTaxlots = 0;
                        ...
                        for (int i = 0; i < accountTaxlotCount; i++)
                        {
                            ...
                            // qty of old security
                            double taxLotOpenQty = taxlotOriginal.OpenQty;
                            double avgPrice = taxlotWithdrawal.AvgPrice;
                            taxlotWithdrawal.OldAveragePrice = taxlotWithdrawal.AvgPrice;
                            //taxlotWithdrawal.NewTaxlotOpenQty = Convert.ToString(taxLotOpenQty * (exchangeRatio / 100));
                            //taxlotWithdrawal.OpenQty = taxLotOpenQty * (exchangeRatio / 100);
                            ...
                            taxlotWithdrawal.OpenQty = Math.Round(taxLotOpenQty * (exchangeRatio / 100), quantityToRoundoff);

                            taxlotWithdrawal.OpenTotalCommissionandFees = (taxlotBase.OpenTotalCommissionandFees * (exchangeRatio / 100));

                            double withdrawlNotionalChange = originalNotionalValue * (exchangeRatio / 100);

                            double openTotalCommissionandFeesWithdrawal = taxlotBase.OpenTotalCommissionandFees * (exchangeRatio / 100);
                            ...
                            else
                            {
                                ...
                                //Negative commission for withdrawal taxlot
                            ...
                            //allGroupWithdrawal.TransactionType = TransactionTypeNew.StockEX.ToString();//CachedData.GetInstance().vlTransactionType.
                            ...
                            double newQty = taxlotOriginal.OpenQty * newSharesReceivedRatio * (exchangeRatio / 100);
                            //taxlotAddition.NewTaxlotOpenQty = Convert.ToString(newQty);
                            //taxlotAddition.OpenQty = newQty;
                            ...
                            double openTotalCommissionandFees = (taxlotAddition.OpenTotalCommissionandFees * (exchangeRatio / 100));

                            // If  UseNetNotional is set true then Set the Commission to Zero and update the Notional Value Accordingly
                            if (caPref.UseNetNotional)
                            {
                                payReceiveAddition = (originalNotionalValue * (exchangeRatio / 100)) + (openTotalCommissionandFees * GetSideMultiplier(taxlotAddition.PositionType));
                                taxlotAddition.OpenTotalCommissionandFees = 0;
                            }
                            else
                            {
                                taxlotAddition.OpenTotalCommissionandFees = openTotalCommissionandFees;
                                payReceiveAddition = (originalNotionalValue * (exchangeRatio / 100));
                            }
                            ...
                            //allGroupAddition.TransactionType = TransactionTypeNew.StockEX.ToString();
                            ...
                            {
                                accountTotalQty_OriginalTaxlots = accountTotalQty_OriginalTaxlots + Convert.ToDecimal(taxlotOriginal.OpenQty);

                                accountTotalQty_WithdrawalTaxlots = accountTotalQty_WithdrawalTaxlots + Convert.ToDecimal(taxlotWithdrawal.OpenQty);

                                accountTotalQty_AdditionTaxlots = accountTotalQty_AdditionTaxlots + Convert.ToDecimal(taxlotAddition.OpenQty);
                                ...
                                if (accountTaxlotCount.Equals(i + 1))
                                {
                                    ...
                                    integralQty = Math.Truncate(accountTotalQty_AdditionTaxlots);
                                    // decimal part if quantity is fractional
                                    residualQty = (accountTotalQty_AdditionTaxlots - integralQty);
                                    // if residual qty > 0                           
                                    if (residualQty > 0)
                                    {
                                    ...
                                    decimal Diff_OriginalAndWithdrawal = 0;
                                    decimal integralQty_OriginalTaxlot = 0;
                                    decimal residualQty_OriginalTaxlot = 0;

                                    Diff_OriginalAndWithdrawal = accountTotalQty_OriginalTaxlots - accountTotalQty_WithdrawalTaxlots;

                                    integralQty_OriginalTaxlot = Math.Truncate(Diff_OriginalAndWithdrawal);
                                    // decimal part if quantity is fractional
                                    residualQty_OriginalTaxlot = (Diff_OriginalAndWithdrawal - integralQty_OriginalTaxlot);
                                    // if residual qty > 0                           
                                    if (residualQty_OriginalTaxlot > 0)
                                    {
                                        GenerateCashInLieuTaxlots(corporateActionRow, caModifiedTaxlots, closingPrice, taxlotOriginal, residualQty_OriginalTaxlot, ref allGroupOrig, adjustCashinLieuatAccountLevel);
                                        //AddGroup(allGroupOrig, ApplicationConstants.CA_FRACTIONALADDITION);
                                    }
                                }
                            }
                        }
            ...
            //return taxlotsClosedInFuture.ToString().TrimEnd(new char[1] { ',' });
        }
        ...
        private void AddTransactionSource(AllocationGroup allocGroup)
        {
            try
            {
                allocGroup.TransactionSource = TransactionSource.CAStockExchange;
                allocGroup.TransactionSourceTag = (int)TransactionSource.CAStockExchange;
            }
            catch (Exception ex)
            {
                // Invoke our policy that is responsible for making sure no secure information
        ...
        private void GenerateCashInLieuTaxlots(DataRow corporateActionRow, TaxlotBaseCollection caModifiedTaxlots, double closingPrice, TaxlotBase taxlotBase, decimal residualQty, ref AllocationGroup newGroupAddition, bool adjustCashinLieuatAccountLevel)
        {
            try
            {
                ...
                taxlotFractionalShareWithdrawal.OriginalPurchaseDate = taxlotFractionalShareWithdrawal.ProcessDate;// taxlotBase.OriginalPurchaseDate;
                ...
                taxlotFractionalShareWithdrawal.NotionalValue = taxlotFractionalShareWithdrawal.OpenQty * closingPrice * taxlotFractionalShareWithdrawal.AssetMultiplier;
                ...
                allGroupFractionalShareWithdrawal.CounterPartyID = 0;
                allGroupFractionalShareWithdrawal.VenueID = 0;
                ...
                //allGroupFractionalShareWithdrawal.TransactionType = TransactionTypeNew.StockEX.ToString();
        ...
        public bool SaveCorporateAction(string corporateActionListString, TaxlotBaseCollection updatedTaxlots, int userID)
        {
            ...
            try
            {
                ...
                foreach (KeyValuePair<string, TaxlotBaseCollection> keyValue in TaxlotsCacheManager.Instance.GetCAWiseTaxlots())
                {
                    ...
                    ///These taxlots are generated by posttradecachemanager, hence these would be sent and saved by posttradecachemanager.
                    ...
                    foreach (AllocationGroup GroupOrig in allGroupOrig)
                    {
                        ...
                        //taxlotToclose.AvgPrice = taxlotToclose.AvgPrice;
                    ...
                    foreach (AllocationGroup GroupFractionalAddition in allGroupFractionalSharesAddition)
                    {
                        TaxLot taxlotToclose = DeepCopyHelper.Clone<TaxLot>(GroupFractionalAddition.TaxLots[0]);
                        taxlotToclose.Update(GroupFractionalAddition.TaxLots[0]);
                        //Modifying all dates to the effectivedate, as the original transactions are closed on the effectivedate only
                        taxlotToclose.AUECLocalDate = caOnProcessObject.FromDate.Date;
                        taxlotToclose.AUECModifiedDate = caOnProcessObject.FromDate.Date;
                        taxlotToclose.ProcessDate = caOnProcessObject.FromDate.Date;
                ...
                isSuccessful = DBManager.SaveCAExchangeWithDifferentSecurity(caOnProcessObjectList, _allocationServices, _closingServices, userID);
            }
            catch (Exception ex)
            {
                // Invoke our policy that is responsible for making sure no secure information
        ...
        public bool UndoCorporateActions(string caIDs, TaxlotBaseCollection taxlots)
        {
            ...
            try
            {
                ...
                {
                    isSuccessful = DBManager.UndoExchange(requestObject, _closingServices, _allocationServices);
                }
            }
            catch (Exception ex)
            {
