...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
  ...
  <xsl:template match="/">
    <Groups>
      ...
      <xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail[(TaxLotState = 'Allocated') and (UDASecurityTypeName!='cfeu' and UDASecurityTypeName!='cfus')]">
  ...
  <xsl:template name="TaxLotIDBuilder">
    ...
    <xsl:variable name="varClientAccount">
      <xsl:choose>
        <xsl:when test ="FundName = 'Citi Newtyn Partners LP:522-91K57'">
          <xsl:value-of select ="'52291K57'"/>
        </xsl:when>
        <xsl:when test ="FundName = 'Citi Newtyn TE Partners LP:522-91K58'">
    ...
    <xsl:variable name="varSecurityID">
	  ...
	  <xsl:variable name="varSecurityType">
		  <xsl:choose>
			  <xsl:when test ="$PRANA_SYMBOL_TYPE!=''">
				  <xsl:value-of select ="$PRANA_SYMBOL_TYPE"/>
			  </xsl:when>
 ...
 <xsl:if test ="ClosingAlgo != 'HIFO'">
    ...
    <Group 
		RowHeader="FALSE"	RecordType ="{$varRecordType}" TransactionType ="{$varTransactionType}" ClientAccount="{FundAccntNo}" AccountType="{$varACType}"
        ClientReference="{EntityID}" AmendNo="" Instruction="{$varInstruction}" BrokerCode="{$Broker}"
        ...
        Commission="{$varCommission}" NetAmount="{NetAmount}" MemoField1="" MemoField2="" RepoRate=""
