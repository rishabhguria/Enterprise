...
>
	...
	<xsl:template name="Future">
		...
		<xsl:if test="contains(COL9,'FUT')">
			...
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(normalize-space(COL9),' '),3,5)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-after(normalize-space(COL9),' '),6,7)"/>
			</xsl:variable>
	...
	<xsl:template name="FutureOption">
		<xsl:param name="Symbol"/>
		<xsl:param name="Suffix"/>
		<xsl:if test="contains(COL9,'FOP')">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL9,' ')"/>
			</xsl:variable>

			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(normalize-space(COL9),' '),1,3)"/>
			</xsl:variable>

			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring-after(substring-after(substring-after(COL9,' '),' '),' ')"/>
			</xsl:variable>
			...
			<xsl:value-of select="concat($UnderlyingSymbol,' ',$MonthCode,$PutORCall,$StrikePrice)"/>
		</xsl:if>
	</xsl:template>
