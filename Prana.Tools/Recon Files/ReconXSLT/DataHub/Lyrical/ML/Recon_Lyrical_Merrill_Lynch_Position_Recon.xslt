	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Position) and (not(contains($SymbolChk,'SD1')) and not(contains($SymbolChk,'SD2')) and not(contains($SymbolChk,'SD3')))">
						...
						<PositionMaster>
							...
							<Symbol>
								<xsl:choose>
									...
									<xsl:when test ="substring(COL1,11,9)='2G2918310'">
										<xsl:value-of select="'ETN'"/>
									</xsl:when>

									<xsl:when test ="substring(COL1,11,9)='90386H404'">
										<xsl:value-of select="'LYRIX'"/>
									</xsl:when>
							...
							<xsl:variable name="MktVal">
								<xsl:value-of select="substring-before(substring-after(substring-after(normalize-space(COL1),'+'),'+'),'+')"/>
							</xsl:variable>

							<xsl:variable name="MktValPre">
								<xsl:value-of select="substring($MktVal,1,string-length($MktVal)-2)"/>
							</xsl:variable>

							<xsl:variable name="MktValSuff">
								<xsl:value-of select="substring($MktVal,string-length($MktVal)-1)"/>
							</xsl:variable>

							<xsl:variable name="varMarketValue">
								<xsl:value-of select="concat($MktValPre,'.',$MktValSuff)"/>
							</xsl:variable>
							...
							<xsl:variable name ="MarkPrice">
								<xsl:call-template name="Translate">
									<xsl:with-param name="Number" select="$varCost"/>
								</xsl:call-template>
							</xsl:variable>
