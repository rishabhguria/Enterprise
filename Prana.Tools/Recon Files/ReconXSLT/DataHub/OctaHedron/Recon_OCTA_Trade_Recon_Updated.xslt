...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template match="/">
        ...
        <xsl:choose>
          <xsl:when test="COL2='Trades'">
            <PositionMaster>
              <xsl:variable name="PB_NAME">
                <xsl:value-of select="'MSFSOCTA'"/>
              </xsl:variable>
              ...
              <Quantity>
                <xsl:choose>
                  <xsl:when test="$Quantity &gt; 0">
                    <xsl:value-of select="format-number($Quantity,'#.0000')"/>
                  </xsl:when>

                  <xsl:when test="$Quantity &lt; 0">
                    <xsl:value-of select="format-number($Quantity * -1,'#.0000')"/>
                  </xsl:when>
              ...
              <xsl:variable name="TotalCommissionandFees">
                <xsl:call-template name="Translate">
                  <xsl:with-param name="Number" select="COL28 + COL66 + COL30"/>
                </xsl:call-template>
              </xsl:variable>
