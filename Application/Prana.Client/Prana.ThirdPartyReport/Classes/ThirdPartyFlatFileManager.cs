...
namespace Prana.ThirdPartyReport
{
    public class ThirdPartyFlatFileManager
    {
        ...
        /// <summary>
        /// This method filters the data as given SP now fetches data for FIX also
        /// </summary>
        /// <param name="thirdpartyID"></param>
        /// <param name="thirdPartyAccountID"></param>
        /// <param name="inputDate"></param>
        /// <param name="companyID"></param>
        /// <param name="l2DataReq"></param>
        /// <param name="auecIds"></param>
        /// <param name="exportOnly"></param>
        /// <param name="thirdPartyTypeID"></param>
        /// <param name="dateType"></param>
        /// <param name="fileFormatID"></param>
        /// <returns></returns>
        public static async Task<ThirdPartyFlatFileDetailCollection> GetFFThirdPartyAccountDetailsFiltered(int thirdpartyID, StringBuilder thirdPartyAccountID, DateTime inputDate, int companyID, bool l2DataReq, StringBuilder auecIds, bool exportOnly, int thirdPartyTypeID, int dateType, int fileFormatID, bool includeSent)
        {
            try
            {
                ThirdPartyFlatFileDetailCollection collectionIntermediateResult = await GetFFThirdPartyAccountDetails(thirdpartyID, thirdPartyAccountID, inputDate, companyID, l2DataReq, auecIds, exportOnly, thirdPartyTypeID, dateType, fileFormatID, includeSent);

                ThirdPartyFlatFileDetailCollection result = new ThirdPartyFlatFileDetailCollection();


                if (dateType == 0)
                {
                    foreach (ThirdPartyFlatFileDetail item in collectionIntermediateResult)
                    {
                        if ((item.TaxLotState != PranaTaxLotState.Sent || (item.TaxLotState == PranaTaxLotState.Sent && DateTime.Parse(item.ProcessDate).Date == inputDate.Date)) &&
                            (item.TaxLotState != PranaTaxLotState.Ignore || (item.TaxLotState == PranaTaxLotState.Ignore && DateTime.Parse(item.ProcessDate).Date == inputDate.Date)))
                            result.AddItem(item);
                ...
                else if (dateType == 1)
                {
                    foreach (ThirdPartyFlatFileDetail item in collectionIntermediateResult)
                    {
                        if ((item.TaxLotState != PranaTaxLotState.Sent || (item.TaxLotState == PranaTaxLotState.Sent && DateTime.Parse(item.TradeDate).Date == inputDate.Date)) &&
                            (item.TaxLotState != PranaTaxLotState.Ignore || (item.TaxLotState == PranaTaxLotState.Ignore && DateTime.Parse(item.TradeDate).Date == inputDate.Date)))
                            result.AddItem(item);
        ...
        /// </remarks>
        public async static Task<ThirdPartyFlatFileDetailCollection> GetFFThirdPartyAccountDetails(int thirdpartyID, StringBuilder companyFundIDs, DateTime inputDate, int companyID, bool l2DataReq, StringBuilder auecIds, bool exportOnly, int thirdPartyTypeID, int dateType, int fileFormatID, bool includeSent)
        {
