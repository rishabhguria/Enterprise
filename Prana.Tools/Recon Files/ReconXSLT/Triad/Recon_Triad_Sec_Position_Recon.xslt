			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:variable name ="varQuantity">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="substring(COL1,78,18)"/>
					</xsl:call-template>
				</xsl:variable>
				...
				<xsl:if test ="$varQuantity">
					...
					<PositionMaster>
						...
						<xsl:variable name="PB_SYMBOL_NAME">
							<xsl:value-of select="substring(COL1,335,20)"/>
						</xsl:variable>
						...
						<xsl:variable name="Symbol" select="normalize-space(substring(COL1,657,30))"/>

						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="substring(COL1,656,1) ='C' or (substring(COL1,656,1)='P')">
							...
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="Symbol" select="substring(COL1,657,16)"/>
						...
						<xsl:variable name="Quantity">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring(COL1,78,18) div 100000"/>
							</xsl:call-template>
						</xsl:variable>
						<Quantity>
							<xsl:choose>
						...
						<xsl:variable name="MarkPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring(COL1,59,18) div 1000000000"/>
							</xsl:call-template>
						</xsl:variable>
						<MarkPrice>
							<xsl:choose>
						...
						<xsl:variable name="MarketValue">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="substring(COL1,569,17) div 100"/>
							</xsl:call-template>
						</xsl:variable>
						<MarketValue>
							<xsl:choose>
