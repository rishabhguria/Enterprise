      ...
      <xsl:for-each select="//Comparision">
        <xsl:variable name="varDividend">
          <xsl:call-template name="Translate">
            <xsl:with-param name="Number" select="normalize-space(substring-before(substring-after(COL5,'Cash Dividend'),'shares'))"/>
          </xsl:call-template>
        </xsl:variable>
        <xsl:choose>
            ...
            <PositionMaster>
             ...
             <xsl:variable name="PRANA_FUND_NAME">
              ...
              <DividendBase>
				...
				<xsl:variable name="varWHT">
					<xsl:choose>
						<xsl:when test="contains(COL5,'W/H') and  not(contains(COL5,'Corp'))">
							<xsl:value-of select="substring-before(substring-after(COL5,'W/H At Source '),' ')"/>
						</xsl:when>
						<xsl:when test="contains(COL5,'W/H At Source') and contains(COL5,'Less Corp Act')">
							<xsl:value-of select="substring-before(substring-after(COL5,'W/H At Source '),' ') + substring-before(substring-after(COL5,'Less Corp Act Fee '),' ')"/>
						</xsl:when>
				...
				<DividendLocal>
					<xsl:choose>
						<xsl:when test="number($varWHT)">
							<xsl:value-of select="$varWHT"/>
						</xsl:when>
              ...
              <xsl:variable name="varRecordDate">
                <xsl:value-of select="normalize-space(COL77)"/>
              </xsl:variable>
