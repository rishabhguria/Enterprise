				...
				<xsl:if test="number($Quantitiy)">
					<PositionMaster>
						...
						<PortfolioAccount>
							...
							<xsl:value-of select="'ABC partners'"/>
						</PortfolioAccount>
						...
						<xsl:variable name="EndingMarketValueLocal">
							<xsl:call-template name="Translate">
						...
						<EndingMarketValueLocal>
							<xsl:choose>
								<xsl:when test="number($EndingMarketValueLocal)">
									<xsl:value-of select="$EndingMarketValueLocal"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
