...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
  ...
  <xsl:template match="/ThirdPartyFlatFileDetailCollection">
    <ThirdPartyFlatFileDetailCollection>
      ...
      <xsl:for-each select="ThirdPartyFlatFileDetail[CounterParty != 'DASH']">
        ...
        <ThirdPartyFlatFileDetail>
          ...
          <mkt_blt>
            <xsl:value-of select ="'EID'"/>
				...
				<xsl:when test ="Asset = 'EquityOption' and PutOrCall = 'P'">
					<mkt_blt>
						<xsl:value-of select="'EOP'"/>
					</mkt_blt>
				</xsl:when>
				<xsl:when test ="Asset = 'EquityOption' and PutOrCall = 'C'">
					<mkt_blt>
          ...
          <xsl:choose>
			...
			<xsl:choose>
				<xsl:when test ="Asset='EquityOption'">
					<Symbol>
						<xsl:value-of select="normalize-space(substring(OSIOptionSymbol,1,6))"/>
					</Symbol>
				</xsl:when>
				<xsl:otherwise>
          <Symbol>
          ...
          <rec_del>
			...
			<xsl:choose>
				...
				<xsl:when test ="Asset = 'EquityOption' and Exchange = 'AMEX'">
					<exec_serv>
						<xsl:value-of select="'AEXP'"/>
					</exec_serv>
				</xsl:when>
				<xsl:when test ="Asset = 'EquityOption' and Exchange = 'CBOE'">
					<exec_serv>
						<xsl:value-of select="'CHXO'"/>
					</exec_serv>
				</xsl:when>
				<xsl:when test ="Asset = 'EquityOption' and Exchange = 'ISE'">
					<exec_serv>
						<xsl:value-of select="'ISE'"/>
					</exec_serv>
				</xsl:when>
				<xsl:when test ="Asset = 'EquityOption' and Exchange = 'BOX'">
					<exec_serv>
