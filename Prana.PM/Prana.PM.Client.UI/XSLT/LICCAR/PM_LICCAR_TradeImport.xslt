...
<xsl:value-of select="$nYear"/>
	...
	<xsl:variable name="vardate">
 	<xsl:call-template name="FormatDate">
 		<xsl:with-param name="DateTime" select="normalize-space(COL22)"/>
 	</xsl:call-template>
	</xsl:variable>
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="ExpiryDay">
			<xsl:value-of select="substring-before(substring-after(normalize-space(COL22),'/'),'/')"/>			
		</xsl:variable>
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring-before(normalize-space(COL22),'/')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring(substring-after(substring-after(normalize-space(COL22),'/'),'/'),3,2)"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(normalize-space(COL23),1,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(normalize-space(COL13),'#.00')"/>
		</xsl:variable>
				...
				<xsl:if test="number($varNetPosition)">
					...
					<PositionMaster>
						...
						<xsl:variable name="AssetType">
							<xsl:choose>
								<xsl:when test="normalize-space(COL21)= 'Option'">
							...
							<xsl:choose>
								<xsl:when test="$AssetType='EquityOption'">
									<xsl:choose>
										...
										<xsl:when test="$varSide = 'Cancel Sell'">
											<xsl:value-of select="'C'"/>
										</xsl:when>
										<xsl:when test="$varSide = 'Cancel Buy'">
											<xsl:value-of select="'B'"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="''"/>
