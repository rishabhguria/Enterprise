...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[Asset='Equity' and IsSwapped='true']">
				...
				<xsl:if test="(1=position()) or(preceding-sibling::*[1]/PBUniqueID != PBUniqueID)">
					<xsl:if test="(CounterParty != 'Undefined' and CounterParty != 'REBL' and CounterParty!='CorpAction' and CounterParty!='Transfer')">
	...
	<xsl:template name="TaxLotIDBuilder">
      ...
      <Group
					...
					Transactionlevel="B" ClientTradeRefNo.="{PBUniqueID}" AssociatedTradeId="{PBUniqueID}" ExecutionAccount="038098638" AccountID="038098638"
					...
					SettlementCCY="{CurrencySymbol}"	ExchangeCode="" Quantity="{$QtySum}" 	Price="{format-number(AveragePrice,'###.0000')}"	PriceIndicator="G" Principal="{format-number($GroupGrsAmt,'###.00')}"
					ExecutingBrokerCommission="{format-number($CommissionSum,'###.00')}" ExecutingBrokerCommissionIndicator="F"
					MSFees="{$GroupTotalFees}" NotRequired=""
					MSfeesindicator.="F" DividendEntitlementPercent="" Spread="" NetAmount="{format-number($GroupNetAmt,'###.00')}" HearsayInd="N" Custodian="{$Custodian}" MoneyManager="" Bookid=""
					DealId="" TaxLotId="" Originaltaxlottransactiondate="" OriginalTaxlottransactionprice="" TaxLotCloseoutMethod="" PriceFXRate="'1'" AcquisitionDate="" Comments="" SwapReferenceNo=""
          BasketId="" PriceCurrency="{CurrencySymbol}" ResetIndicator="" SNSreference="" ResearchFee="" ResearchFeeIndicator="" LEI="" ClientStrategy2UsedforPEPS=""
				...
				<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
					...
					<xsl:variable name="AccountId">
						<xsl:choose>
              <xsl:when test="AccountName ='Boothbay Absolute Return Strategies' and IsSwapped='true' ">
                <xsl:value-of select="'06178G9X7'"/>
              </xsl:when>
              <xsl:when test="AccountName ='Boothbay Diversified Alpha Master Fund' and IsSwapped='true' ">
              ...
              <xsl:when test="$THIRDPARTY_FUND_NAME!=''">
					...
					<ThirdPartyFlatFileDetail
								TaxLotState="{TaxLotState}" TransactionType = "SW002" TransactionStatus="{$varTaxlotState}" BuySellIndicator="" LongShortIndicator="" PositionType="{$Sidevar}"
								Transactionlevel="A" ClientTradeRefNo.="{concat(PBUniqueID,$AccountId)}" AssociatedTradeId="{PBUniqueID}" ExecutionAccount="038098638" AccountID="{$AccountId}"
								BrokerCounterparty="{$CPVar}" SecurityIdentifierType="{$varSecType}" SecurityIdentifier="{$varSymbol}" SecurityDescription="{FullSecurityName}" TradeDate="{TradeDate}" SettlementDate="{SettlementDate}"
								SettlementCCY="{CurrencySymbol}"	ExchangeCode="" Quantity="{AllocatedQty}" Price="{format-number(AveragePrice,'###.0000')}" PriceIndicator="G" Principal="{format-number($TaxlotGrsAmt,'###.00')}"
								ExecutingBrokerCommission="{format-number(CommissionCharged,'###.00')}" ExecutingBrokerCommissionIndicator="F"
								MSFees="{$TaxFees}"  NotRequired=""
								
								MSfeesindicator.="F" DividendEntitlementPercent="" Spread="" NetAmount="{format-number(NetAmount,'###.00')}" HearsayInd="N" Custodian="{$Custodian}" MoneyManager="" Bookid=""
					      DealId="" TaxLotId="" Originaltaxlottransactiondate="" OriginalTaxlottransactionprice="" TaxLotCloseoutMethod="" PriceFXRate="'1'" AcquisitionDate="" Comments="" SwapReferenceNo=""
                BasketId="" PriceCurrency="{CurrencySymbol}" ResetIndicator="" SNSreference="" ResearchFee="" ResearchFeeIndicator="" LEI="" ClientStrategy2UsedforPEPS=""
								EntityID="{EntityID}" />
					<!--</xsl:if>-->
