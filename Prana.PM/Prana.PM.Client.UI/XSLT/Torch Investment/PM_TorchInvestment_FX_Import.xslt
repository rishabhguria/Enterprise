		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
			    ...
			    <xsl:variable name="PB_CURRENCY_NAME">
			    	<xsl:value-of select="substring-before(normalize-space(COL2),'CURNCY')"/>
			    </xsl:variable>

				<xsl:if test="number($varFXRate) and contains(COL2,'CURNCY') and (contains($PB_CURRENCY_NAME,'AUD') or contains($PB_CURRENCY_NAME,'NZD') or contains($PB_CURRENCY_NAME,'EUR') or contains($PB_CURRENCY_NAME,'GBP'))">
