...
Modified By : Pooja                                                                                                                                        
Description: Add FundNavEndDate column (calulate nav at End Date)      
       
      
[PMGetRealizeAndUnrealizePnL_NAV_Testing_] '2014/05/05','2014/05/05' ,'1214,1233,1234,1235,1236,1238,1239,1240,1241','1,2,3,4,5,6,7,8,9','MTM_V0'    
                                                                                          
******************************************************************/        
                         
Create PROCEDURE  [dbo].[PMGetRealizeAndUnrealizePnL_NAV_Testing_]                                                                             
(                                                                                                                                                                                                                                                  
...
Case                                                                                        
 ...
 Then                                  
  Case                                                                                               
   ...
   Then                                        
    Case                                         
     When DateDiff(day,SplitTab.EffectiveDate,G.ProcessDate)=0                                           Then IsNull(G.AvgPrice * TaxlotOpenQty * IsNUll(SM.Multiplier,0) * dbo.GetSideMultiplier(PT.OrderSideTagValue),0)                                     
...
exec P_MW_GetNAVForEachFund_WithAccruedDividend @StartDate,@Fund,@ReportId    

-------------------------------------Add Table which contain Fund wise End Date Nav -------
 create table #FundsNAVEndDate      
 (     
  FundName varchar(200)    
 ,FundId varchar(200)    
 ,Date datetime    
...
INSERT INTO #FundsNAVEndDate    
exec P_MW_GetNAVForEachFund_WithAccruedDividend @EndDate,@Fund,@ReportId    
...
Select     
#MTMDataTable.*    
,ISNULL(PM_NAV.MarketValue,0)+ ISNULL(PM_NAV.AccruedDividend,0) as 'FundNAV' 
,ISNULL(PM_NAVEndDate.MarketValue,0)+ ISNULL(PM_NAVEndDate.AccruedDividend,0) as 'FundNAVEndDate' 
,(#MTMDataTable.RealizedPNLMTM_Base+#MTMDataTable.UnRealizedPNLMTM_Base+#MTMDataTable.Dividend) as TotalPNL    
,case      
 when (ISNULL(PM_NAV.MarketValue,0)+ ISNULL(PM_NAV.AccruedDividend,0) <> 0 )    
 then (((#MTMDataTable.RealizedPNLMTM_Base+#MTMDataTable.UnRealizedPNLMTM_Base+#MTMDataTable.Dividend)*100)/(ISNULL(PM_NAV.MarketValue,0)+ ISNULL(PM_NAV.AccruedDividend,0)))     
 else 0    
 END AS PNLPercentByNAV    
from     
#MTMDataTable      
left OUTER JOIN #FundsNAV PM_NAV ON(#MTMDataTable.FundID = PM_NAV.FundID and datediff(d,PM_NAV.date,dbo.F_MW_GetWeekendAdjusted(@StartDate,0,1))=0)    
left OUTER JOIN #FundsNAVEndDate PM_NAVEndDate ON(#MTMDataTable.FundID = PM_NAVEndDate.FundID and datediff(d,PM_NAVEndDate.date,dbo.F_MW_GetWeekendAdjusted(@EndDate,0,1))=0)    
Order By Symbol       
