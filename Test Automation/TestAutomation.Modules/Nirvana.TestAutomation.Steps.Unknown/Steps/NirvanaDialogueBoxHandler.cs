...
{
    class NirvanaDialogueBoxHandler : IUIAutomationTestStep
    {
        public TestResult RunUIAutomationTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                uiAutomationHelper.NirvanaDialogueBoxHandler(testData.Tables[0]);
                if (!string.IsNullOrEmpty(ApplicationArguments.UIAutomationCommonActionResult) && testData.Tables[0].Columns.Contains("VerifyBoxCompulsory") && !string.IsNullOrEmpty(testData.Tables[0].Rows[0]["VerifyBoxCompulsory"].ToString()))
                {
