...
namespace Nirvana.Admin
{
	...
	public class OrderType : System.Windows.Forms.Form
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(OrderType));
			...
			this.grpside.Text = "Add/Update Side";
			// 
			...
			this.txtOrderTypeTagValue.Text = "";
			...
			this.grdOrderTypes.FlatMode = true;
		...
		{	
			...
			Nirvana.Admin.BLL.OrderType orderType = new Nirvana.Admin.BLL.OrderType();
			...
			if(newOrderTypeID == -1)
			{
				errorProvider1.SetError(btnSave, "OrderType Already Exists");
			}
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if(grdOrderTypes.Rows.Count > 0)
				{
					...
					if(MessageBox.Show(this, "Do you want to delete this OrderType?", "Nirvana Alert", MessageBoxButtons.YesNo) == DialogResult.Yes)
					{
						...
						bool chkVarraible = OrderManager.DeleteOrderType(orderTypeID, false);
						if(!(chkVarraible))
						{
							MessageBox.Show(this, "OrderType is referenced in CounterpartyVenue/Compliance.\n Please remove references first to delete it.", "Nirvana Alert");
						}
						else
						{
							BindOrderTypesGrid();
