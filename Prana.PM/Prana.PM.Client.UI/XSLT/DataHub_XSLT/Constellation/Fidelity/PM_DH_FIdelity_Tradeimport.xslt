	...
	<xsl:template match="/">
		<DocumentElement>
            ...
            <SEDOL>
              ...
              <xsl:call-template name="Translate">
                <xsl:with-param name="Number" select="substring(COL1,319,9) div 10"/>
              </xsl:call-template>
						</xsl:variable>				
						...
						<xsl:variable name="TransacationLevy">
							<xsl:call-template name="Translate">
						...
						<xsl:variable name="Positions">
							<xsl:value-of select="$Position div 10000"/>
						</xsl:variable>				
              ...
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:value-of select="0"/>
						...
						<Strategy>
							<xsl:choose>
								<xsl:when test="$varNFXRate &gt; 0">
									<xsl:value-of select="$varNFXRate"/>
								</xsl:when>
								<xsl:when test="$varNFXRate &lt; 0">
									<xsl:value-of select="$varNFXRate * (-1)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
