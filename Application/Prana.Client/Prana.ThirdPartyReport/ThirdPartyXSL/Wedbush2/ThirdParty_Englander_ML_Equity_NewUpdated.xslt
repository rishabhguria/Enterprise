	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail">
				<!--<xsl:if test ="FundName = 'Monarch Cap LLC' or FundName = 'Stadium Capital LLC' or FundName='Roundabout:846-52315D3' or FundName='Roundabout:141-65535'">-->
					<ThirdPartyFlatFileDetail>
						...
						<exec_brkr>
							<xsl:choose>
								<xsl:when test =" CounterParty='RODM'">
									<xsl:value-of select="161"/>
								</xsl:when>
								<xsl:when test =" CounterParty ='ATLE'">
									<xsl:value-of select="443"/>
								</xsl:when>
								<xsl:when test =" CounterParty ='BTIG'">
									<xsl:value-of select="501"/>
								</xsl:when>
								<xsl:when test =" CounterParty ='DAP'">
									<xsl:value-of select="501"/>
								</xsl:when>
								<xsl:when test =" CounterParty ='LTCO'">
									<xsl:value-of select="226"/>
								</xsl:when>
								<xsl:when test =" CounterParty ='BARC'">
									<xsl:value-of select="229"/>
								</xsl:when>
								<xsl:when test =" CounterParty ='ELEV'">
									<xsl:value-of select="750"/>
								</xsl:when>
								<xsl:when test ="CounterParty ='IN'">
									<xsl:value-of select="67"/>
								</xsl:when>
								
								<xsl:when test ="CounterParty ='AXIO'">
