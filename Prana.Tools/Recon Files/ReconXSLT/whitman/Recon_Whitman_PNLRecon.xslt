...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Future">
		...
		<xsl:if test="contains(COL84,'FUTURE')">
			...
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-before(substring-after(normalize-space(COL72),' '),' '),2)"/>
			</xsl:variable>
	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Cash) and contains(COL2,'FPL')">
					<PositionMaster>
						...
						<!--<xsl:variable name="Futures">
							<xsl:choose>
								<xsl:when test="$Asset='Future'">
									<xsl:choose>
										<xsl:when test="string-length(COL26)">
						...
						<!--<BloombergSYmbol>   <bloombergSymbol tag not contain in PNL recon>
							<xsl:choose>
								<xsl:when test="$PRANA_SYMBOL_NAME!=''">
									<xsl:value-of select="''"/>
								</xsl:when>
