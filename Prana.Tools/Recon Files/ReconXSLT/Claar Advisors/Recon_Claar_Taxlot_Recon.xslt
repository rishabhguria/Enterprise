  ...
  <xsl:template name="MonthCode">
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring(substring-after(normalize-space(COL3),'___'),3,2)"/>
		</xsl:variable>
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring(substring-after(normalize-space(COL3),'___'),1,2)"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after(normalize-space(COL3),'___'),7,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL3),'___'),8) div 1000,'##.00')"/>
		</xsl:variable>
				...
				<xsl:if test="number($varQuantity)">
					...
					<PositionMaster>
						...
						<Side>
                         ...
                         <xsl:when test="$varAssetType='EquityOption'">
                           <xsl:choose>
                             <xsl:when test="normalize-space(COL11) = 'L'">
                               <xsl:value-of select ="'Buy to Open'"/>
                             </xsl:when>
                             <xsl:when test="normalize-space(COL11) = 'S'">
						...
						<PBSymbol>
						 ...
						 <xsl:variable name="varAsset">
                          <xsl:choose>
                            <xsl:when test="normalize-space(COL5) = 'OPTN'">
                              <xsl:value-of select="'EquityOption'"/>
                            </xsl:when>
							<xsl:when test="normalize-space(COL5) = 'STOCK'">
                              <xsl:value-of select="'Equity'"/>
                            </xsl:when>
							<xsl:when test="normalize-space(COL5) = 'DEBT'">
