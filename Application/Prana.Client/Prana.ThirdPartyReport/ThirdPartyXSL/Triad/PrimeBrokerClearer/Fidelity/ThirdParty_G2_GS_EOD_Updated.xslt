 	...
 	<xsl:variable name ="varAllocationState">
	  ...
	  <xsl:variable name="VarComm">
		  <xsl:choose>
			  ...
			  <!--<xsl:when test="$varSettlFxRate!=0 and FXConversionMethodOperator_Taxlot='M'">
				  <xsl:value-of select="format-number($Commission * $varSettlFxRate,'##.00')"/>
			  </xsl:when>

			  <xsl:when test="$varSettlFxRate!=0 and FXConversionMethodOperator_Taxlot='D'">
				  <xsl:value-of select="format-number($Commission div $varSettlFxRate,'##.00')"/>
			  </xsl:when>-->
			  <xsl:otherwise>
				  <xsl:value-of select="format-number($Commission * $varSettlFxRate ,'##.00')"/>
			  </xsl:otherwise>
	  ...
	  <xsl:variable name="GrossAmount">
		  <xsl:choose>
			  ...
			  <xsl:otherwise>
				  <xsl:value-of select="GrossAmount div $varSettlFxRate"/>
			  </xsl:otherwise>
		  </xsl:choose>
	  </xsl:variable>
