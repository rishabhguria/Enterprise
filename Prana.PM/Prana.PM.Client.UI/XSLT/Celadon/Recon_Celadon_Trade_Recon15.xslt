...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
  ...
  <xsl:template name="Option">
    ...
    <xsl:if test="contains(COL18,'CALL ') or contains(COL18,'PUT ')">
      <xsl:variable name="var">
        <xsl:value-of select="substring-after(normalize-space(COL39),' ')"/>
      </xsl:variable>
      <xsl:variable name="UnderlyingSymbol">
        <xsl:value-of select="normalize-space(COL36)"/>
      </xsl:variable>
      <xsl:variable name="ExpiryDay">
        <xsl:value-of select="substring($var,5,2)"/>
      </xsl:variable>
      <xsl:variable name="ExpiryMonth">
        <xsl:value-of select="substring($var,3,2)"/>
      </xsl:variable>
      <xsl:variable name="ExpiryYear">
        <xsl:value-of select="substring($var,1,2)"/>
      </xsl:variable>
  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:if test="number($Quantity)and normalize-space(COL2)='71211520' or normalize-space(COL2)='20190789' or normalize-space(COL2)='77751741'">
          <PositionMaster>
            ...
            <xsl:variable name="Asset" select="COL18"/>
            <Asset>
              <xsl:choose>
                <xsl:when test="contains($Asset,'CALL ') or contains($Asset,'PUT ')">
            ...
            <!--<xsl:variable name="Cusip" select="COL19"/>-->
