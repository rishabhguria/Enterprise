					...
					<xsl:variable name ="varAllocationState">
						<xsl:choose>
							...
							<xsl:when test ="TaxLotState = 'Deleted'">
								<xsl:value-of  select="concat('CXL',TradeRefID)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of  select="''"/>
							</xsl:otherwise>
		   ...
		   <xsl:variable name="BrokerCode">
            ...
            <xsl:choose>
              <xsl:when test="CurrencySymbol='GBP'">
                <xsl:value-of  select="'GB-UNITTRST'"/>
              </xsl:when>
              <xsl:when test="CurrencySymbol='JPY'">
                <xsl:value-of  select="'HK-XHKCHKH1'"/>
              </xsl:when>
              <xsl:when test="CurrencySymbol='HKD'">
                <xsl:value-of  select="'JP-JJSDJPJT'"/>
              </xsl:when>
              <xsl:otherwise>
            ...
            <xsl:choose>
              <xsl:when test="CurrencySymbol='GBP'">
                <xsl:value-of  select="'CREST'"/>
              </xsl:when>
              <xsl:when test="CurrencySymbol='JPY'">
                <xsl:value-of  select="'CCAS'"/>
              </xsl:when>
              <xsl:when test="CurrencySymbol='HKD'">
                <xsl:value-of  select="'JSDC'"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="'DTC'"/>
              </xsl:otherwise>
