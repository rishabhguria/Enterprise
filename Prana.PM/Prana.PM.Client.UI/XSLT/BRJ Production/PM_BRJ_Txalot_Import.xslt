	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and COL2!='Cash'">
					<PositionMaster>
						...
						<xsl:variable name ="Asset">
							<xsl:choose>
								<xsl:when test=" COL2='Option' and string-length(COL4 &gt; 20)">
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='FixedIncome' and string-length(COL4)='9'">
									<xsl:value-of select="''"/>
								</xsl:when>
								<xsl:when test="$Asset='Equity' and string-length(COL4)='7'">
									<xsl:value-of select="''"/>
								</xsl:when>
						...
						<IDCOOptionSymbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='Equity' and string-length(COL4)='7' and COL1!='USD'">
									<xsl:value-of select="''"/>
								</xsl:when>
								<xsl:when test="$Symbol!='*'">
									<xsl:value-of select="''"/>
					 ...
					 <xsl:variable name="Costbasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$COL12 div $Position"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
