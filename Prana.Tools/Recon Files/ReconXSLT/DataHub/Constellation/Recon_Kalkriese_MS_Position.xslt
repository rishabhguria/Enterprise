...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
        ...
        <xsl:choose>
          <xsl:when test="substring(COL8,2,1) = '1'">
            <xsl:value-of select="substring-before(COL8,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,2,1) = '2'">
            <xsl:value-of select="substring-before(COL8,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,3,1) = '1'">
            <xsl:value-of select="substring-before(COL8,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,3,1) = '2'">
            <xsl:value-of select="substring-before(COL8,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,4,1) = '1'">
            <xsl:value-of select="substring-before(COL8,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,4,1) = '2'">
            <xsl:value-of select="substring-before(COL8,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,5,1) = '1'">
            <xsl:value-of select="substring-before(COL8,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,5,1) = '2'">
            <xsl:value-of select="substring-before(COL8,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,6,1) = '1'">
            <xsl:value-of select="substring-before(COL8,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL8,6,1) = '2'">
            <xsl:value-of select="substring-before(COL8,'2')"/>
          </xsl:when>
        </xsl:choose>
			</xsl:variable>
	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity) and contains(COL50,'CASH')!='true' and COL2='038CAB4S0'">
						<PositionMaster>
