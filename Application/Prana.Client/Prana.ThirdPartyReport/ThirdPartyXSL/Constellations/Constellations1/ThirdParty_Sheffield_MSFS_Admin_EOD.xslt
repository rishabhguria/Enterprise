...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:if test="TaxLotState != 'Deleted' and (AccountName='Sheffield Partners LP-JPM' or AccountName='Sheffield Partners LP-MS' or AccountName='Sheffield Partners LP-MS SWAP' or AccountName='Sheffield-IPO') and CounterParty!='CorpAction' and CounterParty!='Transfer' and CounterParty!='Washsale' and CounterParty!='BOXcollapse' and CounterParty!='SwapReset'">
			...
			<xsl:variable name="varCustodian1">
				<xsl:choose>
					<xsl:when test="AccountMappedName='038Q5197' and Asset='Equity' and IsSwapped='true'">
						<xsl:value-of select="'MSSW'"/>
					</xsl:when>
					<xsl:when test="AccountMappedName='038Q51978'and IsSwapped='false'">
			...
			<Group 
					...
					translevel="B" ClientRef="{PBUniqueID}" Associated="{PBUniqueID}" ExecAccount="038308300" AccountID="038308300"
					ExecBkr="{$CPVar}" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{FullSecurityName}" TDate="{TradeDate}" SDate="{SettlementDate}"
					SCCY="{SettlCurrency}"	ExCode="" qty="{$QtySum}" 	Price="{format-number($varAveragePriceSettl,'###.0000')}"	type="G" prin="{format-number($varGroupGrsAmtSettl,'###.00')}" 
				...
				<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID and CounterParty!='CorpAction' and CounterParty!='Transfer' and CounterParty!='Washsale' and CounterParty!='BOXcollapse' and CounterParty!='SwapReset' and AccountName != 'Quantum Partners LP']">
					...
					<ThirdPartyFlatFileDetail
								...
								translevel="A" ClientRef="{concat(PBUniqueID,'038Q51978')}" Associated="{PBUniqueID}" ExecAccount="038308300" AccountID="{'038Q51978'}"
								ExecBkr="{$CPVar}" SecType="{$varSecType}" SecID="{$varSymbol}" desc="{FullSecurityName}" TDate="{TradeDate}" SDate="{SettlementDate}"
								SCCY="{SettlCurrency}"	ExCode="" qty="{AllocatedQty}" Price="{format-number($varAveragePriceSettl,'###.0000')}" type="G" prin="{format-number($varGroupGrsAmtSettl,'###.00')}"
								comm="{format-number($varCOMM,'###.00')}" comtype="F"
								Taxfees="{$varTaxFees}" 
								Tax2="0"
								feesind="F" interest="{$varAccruedInterest}" interestindicator="{$varInterestindicator}" netamount="{format-number($varNetamountSettl,'###.00')}" hsyind="Y" custbkr="{$varCustodian1}" mmgr="" bookid=""
		...
		<xsl:variable name="AccountId_Group">
			...
			<xsl:variable name="varCustodian1">
				<xsl:choose>
					...
					<xsl:when test="(AccountMappedName='038Q51978' or AccountMappedName='10238924') and Asset='FX'">
						<xsl:value-of select="''"/>
					</xsl:when>
					<xsl:when test="AccountMappedName='038Q51978'and IsSwapped='false'">
						<xsl:value-of select="'MSCO'"/>
