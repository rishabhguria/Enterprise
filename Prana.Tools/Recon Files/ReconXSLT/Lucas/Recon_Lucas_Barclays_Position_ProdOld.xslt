      ...
      <xsl:for-each select="Comparision">
        ...
        <xsl:if test="$PB_DISTRIBUTION_ID = 'lucas' and $QUANTITY != 0 ">
          <PositionMaster>
            ...
            <xsl:if test="Column15 &lt; 0">
              <Side>
                <xsl:value-of select="'Sell'"/>
              </Side>
              <Quantity>
                <xsl:value-of select="Column15*(-1)"/>
              </Quantity>
            </xsl:if>
            <xsl:if test="Column15 &gt; 0">
              <Side>
