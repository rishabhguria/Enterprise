      ...
      <xsl:for-each select="//Comparision">
        ...
        <xsl:if test="$varSymbol != '' and ($varFund = '87010' or $varFund = '87017') and number($varFormatQty)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="varOptExpiration_Day" select="substring(COL1,53,2)" />
            ...
            <xsl:choose>
              ...
              <xsl:when test ="$varCallPutCode !=' ' ">
                <xsl:variable name="varThirdFriday">
                  <xsl:value-of select =" my:Now(number(concat('20',$varOptExpiration_Year)),number($varOptExpiration_Month))"/>
                </xsl:variable>

                <xsl:variable name="varIsFlex">
                  <xsl:choose>
                    <xsl:when test="(substring-before(substring-after($varThirdFriday,'/'),'/') + 1) = number($varOptExpiration_Day)">
                ...
                <xsl:variable name="varDate">
                  <xsl:choose>
                    <xsl:when test="substring($varOptExpiration_Day,1,1)='0'">
                      <xsl:value-of select="substring($varOptExpiration_Day,2,1)"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="$varOptExpiration_Day"/>
                    </xsl:otherwise>
                  </xsl:choose>
                </xsl:variable>
                <Symbol>
                  <xsl:choose>
                    ...
                    <xsl:otherwise>
                      <xsl:value-of select="concat('O:',$varSymbol,' ',$varOptExpiration_Year,$varMonthCodeOption,$varStrike, 'D', $varDate)"/>
                    </xsl:otherwise>
                  </xsl:choose>
                </Symbol>
              </xsl:when>
