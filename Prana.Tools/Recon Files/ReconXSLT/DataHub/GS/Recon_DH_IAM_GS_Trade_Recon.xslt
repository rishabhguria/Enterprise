...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-after(normalize-space(COL8),' '),7,1),'C') or contains(substring(substring-after(normalize-space(COL8),' '),7,1),'P')">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity)">
						<PositionMaster>
							...
							<xsl:variable name="Sedol" select="COL8"/>
							...
							<xsl:variable name ="Date" select="normalize-space(COL9)"/>
							<TradeDate>
