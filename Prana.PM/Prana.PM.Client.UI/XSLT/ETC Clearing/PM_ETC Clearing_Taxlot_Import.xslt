...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL4,'Option') and (contains(substring(substring-before(normalize-space(COL5),' '),1,1),'C') or contains(substring(substring-before(normalize-space(COL5),' '),1,1),'P')) ">
					...
					<PositionMaster>
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='FixedIncome' and string-length(COL2)='9'">
									<xsl:value-of select="COL2"/>
								</xsl:when>
					 ...
					 <xsl:variable name="Costbasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="$varCostbasis div $Position"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:choose>
										...
										<xsl:when test="$Costbasis &lt; 0">
											<xsl:value-of select="($Costbasis div 100) * (-1)"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="0"/>
										</xsl:otherwise>
								...
								<xsl:when test="$Asset='FixedIncome'">
									<xsl:choose>
										<xsl:when test="$Costbasis &gt; 0">
											<xsl:value-of select="$Costbasis * 100"/>
										</xsl:when>
										<xsl:when test="$Costbasis &lt; 0">
											<xsl:value-of select="($Costbasis * 100) * (-1)"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="0"/>
										</xsl:otherwise>
