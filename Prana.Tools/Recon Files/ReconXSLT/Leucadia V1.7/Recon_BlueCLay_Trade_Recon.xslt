	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-before(COL5,' '),1,1),C) or contains(substring(substring-before(COL5,' '),1,1),P)">
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL20),' '),' '),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(normalize-space(COL20),' '),' '),' '),2),'#.00')"/>
			</xsl:variable>
					...
					<PositionMaster>
							...
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:call-template name="Option">
										...
										<xsl:with-param name="Symbol" select="substring-before(COL20,' ')"/>
									</xsl:call-template>
								</xsl:when>
						...
						<Side>
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:choose>
										<xsl:when test="contains($Side,'Y')">
											<xsl:value-of select="'Buy to Open'"/>
										</xsl:when>
										<xsl:when test="contains($Side,'S')">
											<xsl:value-of select="'Sell to Close'"/>
										</xsl:when>
										<xsl:when test="contains($Side,'Cover')">
						...
						<xsl:variable name="NetNotionalValue">
							<xsl:value-of select="($Quantity * $AvgPrice) + $Commission + $Fees"/>
						</xsl:variable>
						<NetNotionalValue>
							<xsl:choose>
