  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($varCash) and COL2 !='*'">
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME">
              <xsl:choose>
                <xsl:when test="$PB_ACRONYM_NAME ='WireIn'">
                  <xsl:value-of select="'CashTransferIn'"/>
                </xsl:when>

                <xsl:when test="$PB_ACRONYM_NAME ='WireOut'">
                  <xsl:value-of select="'CashTransferOut'"/>
                </xsl:when>

                <xsl:when test="$PB_ACRONYM_NAME ='InterestIncome'">
            ...
            <xsl:variable name="Month">
              <xsl:call-template name="MonthsCode">
                <xsl:with-param name="varMonth" select="substring-before(substring-after(COL1,'-'),'-')"/>
              </xsl:call-template>
            </xsl:variable>
            ...
            <Date>
              <xsl:value-of select="concat($Month,'/',$varDay,'/',$varYear,20)"/>
            </Date>
