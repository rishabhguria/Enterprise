		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				...
				<PositionMaster>
					...
					<xsl:variable name="varUnderlying" select="normalize-space(COL21)"/>
					...
					<xsl:variable name="Strike_Price" select="COL18"/>
						...
						<xsl:variable name ="varCallPutCode">
							<xsl:choose>
								<xsl:when test ="normalize-space(COL20)='Call'">
									<xsl:value-of select ="'1'"/>
								</xsl:when>
								<xsl:when test ="normalize-space(COL20)='Put'">
									<xsl:value-of select ="'0'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select ="''"/>
					...
					<xsl:choose>
						<xsl:when test ="$varCallPutCode!='' and string-length($varPBSymbol)=21 and contains($varPBSymbol,'/')= false">
							<Symbol>
								<xsl:value-of select="''"/>
							</Symbol>
							<IDCOOptionSymbol>
						...
						<xsl:when test ="$varCallPutCode!='' and string-length($varPBSymbol)=22 and  contains($varPBSymbol,'/')= false">
							<Symbol>
								<xsl:value-of select="''"/>
							</Symbol>
							<IDCOOptionSymbol>
						...
						<xsl:when test ="$varCallPutCode!='' and contains($varPBSymbol,'/')!= false">
							<Symbol>
					...
					<xsl:choose>
						<xsl:when test="$varSide = 'BO' and $varAssetClass !='Option'">
							<SideTagValue>
								<xsl:value-of select="'1'"/>
							</SideTagValue>
						</xsl:when>
						<xsl:when test="$varSide='SC'and $varAssetClass !='Option'">
							<SideTagValue>
								<xsl:value-of select="'2'"/>
							</SideTagValue>
						</xsl:when>
						<xsl:when test="$varSide='SS' and $varAssetClass !='Option'">
							<SideTagValue>
