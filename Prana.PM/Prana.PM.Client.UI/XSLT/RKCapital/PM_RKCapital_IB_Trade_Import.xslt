	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and contains(normalize-space(COL26),'TRADE')">
					...
					<PositionMaster>
						...
						<SideTagValue>							
							<xsl:choose>
								<xsl:when test="contains(normalize-space(COL15),'BUY')">
									<xsl:value-of select="'1'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL15),'SELL')">
						...
						<xsl:variable name="varDate">
							<xsl:value-of select="substring(normalize-space(COL12),7,2)"/>
						</xsl:variable>

						<xsl:variable name="varMonth">
							<xsl:value-of select="substring(normalize-space(COL12),5,2)"/>
						</xsl:variable>

						<xsl:variable name="varYear">
							<xsl:value-of select="substring(normalize-space(COL12),1,4)"/>
						</xsl:variable>
