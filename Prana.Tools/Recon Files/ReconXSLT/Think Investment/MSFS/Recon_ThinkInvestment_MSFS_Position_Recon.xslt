	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL16,'Options')">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(normalize-space(COL3),'1')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(normalize-space(COL3),$UnderlyingSymbol),5,2)"/>
			</xsl:variable>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(normalize-space(COL3),$UnderlyingSymbol),7,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL3),$UnderlyingSymbol),8) div 1000,'#.00')"/>
			</xsl:variable>
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and not(contains(COL16,'Cash'))">
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								...
								<!--<xsl:when test="normalize-space(COL16)='Equities - Swap'">
									<xsl:value-of select="'OptionSwap'"/>
								</xsl:when>-->
								
								<xsl:when test="contains(normalize-space(COL48),'Equity Swap')">
									<xsl:value-of select="'EquitySwap'"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL48),'Cash Equity')">
									<xsl:value-of select="'Equity'"/>
								</xsl:when>
								<xsl:when test="contains(COL16,'Futures')">
									<xsl:value-of select="'Future'"/>
								</xsl:when>
								<xsl:when test="COL42='Option On Commodity'">
									<xsl:value-of select="'FutureOption'"/>
								</xsl:when>
								<xsl:when test="contains(COL16,'FX Forward')">
						...
						<xsl:variable name ="varFXMonths">
							<xsl:call-template name="Date">
								<xsl:with-param name="Month" select="substring-before(COL22,'/')"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<xsl:variable name="varFXForward">
							<xsl:choose>
								<xsl:when test="$varCurrency='EUR' or $varCurrency='GBP' or $varCurrency='NZD' or $varCurrency='AUD'">
									<xsl:value-of select="concat($varCurrency,'/','USD',' ',$varFXMonths,'/',$varFXDay,'/',$varFXYear)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="concat('USD','/',$varCurrency,' ',$varFXMonths,'/',$varFXDay,'/',$varFXYear)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						...
						<xsl:variable name="varFutureYear">
							<xsl:value-of select="substring(substring-before(substring-after(substring-after(normalize-space(COL15),' '),' '),' '),5,1)"/>
						</xsl:variable>

						<xsl:variable name ="varFutureMonthCode">
							<xsl:call-template name="FutureCode">
								<xsl:with-param name="FMonth" select="substring(substring-before(substring-after(substring-after(normalize-space(COL15),' '),' '),' '),1,3)"/>
							</xsl:call-template>
						</xsl:variable>

						<xsl:variable name="varFuture">
							<xsl:choose>
								<xsl:when test="contains(COL16,'Futures') and COL12='INR'">
									<xsl:value-of select="concat($varFutureSymbol,' ',$varFutureMonthCode,'-NSF')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="concat($varFutureSymbol,' ',$varFutureMonthCode)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						...
						<Symbol>
							<xsl:choose>
								...
								<!--<xsl:when test="$Asset='OptionSwap'">
									<xsl:value-of select="COL3"/>
								</xsl:when>-->
