	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($NetPosition) and COL61='RESET'">
					<PositionMaster>
						...
						<xsl:variable name="PRANA_SYMBOL_NAME">
							<xsl:value-of select="document('../ReconMappingXML/SymbolMapping.xml')/SymbolMapping/PB[@Name='BEMAP']/SymbolData[@PBCompanyName=$PB_SYMBOL_NAME]/@PranaSymbol"/>
						</xsl:variable>
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="COL61='RESET'">
						...
						<xsl:variable name="Symbol" select="normalize-space(COL49)"/>

						<xsl:variable name ="varBaseCurrency" select="COL16"/>

						<xsl:variable name ="varSettleCurrency" select="COL17"/>

				<xsl:variable name="varPBFxRate">
					<xsl:call-template name="Translate">
						...
						<xsl:choose>
								<xsl:when test ="COL17 ='USD'">
									<xsl:value-of select ="number($varFXRate)"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<xsl:variable name="varCostBasis" select="number(COL32)"/>
						...
						<xsl:variable name="Side" select="COL27"/>
						<SideTagValue>
						...
						<xsl:if test="$Asset='EquitySwap'">
							...
							<OrigTransDate>
								<xsl:value-of select ="concat(substring-before(substring-after(COL23,'-'),'-'),'/',substring-before(COL23,'-'),'/',substring-after(substring-after(COL23,'-'),'-'))"/>
							</OrigTransDate>
							...
							<!--<xsl:choose>
								...
								<xsl:value-of select="substring-after(substring-after(COL23,'/'),'/')"/>
									</xsl:when>
								<xsl:otherwise>
											<xsl:value-of select="substring-after(substring-after(COL23,'-'),'-')"/>
								</xsl:otherwise>
							...
							<!--<xsl:choose>
									<xsl:when test="contains(COL23,'/')">
								<xsl:choose>
									<xsl:when test="string-length(number(substring-before(substring-after(COL23,'/'),'/'))) = 1">
										<xsl:value-of select="concat(0,substring-before(substring-after(COL23,'/'),'/'))"/>
									</xsl:when>
									<xsl:otherwise>
							...
							<!--<xsl:choose>
									<xsl:when test="contains(COL23,'/')">
								<xsl:choose>
									<xsl:when test="string-length(number(substring-before(COL23,'/'))) = 1">
										<xsl:value-of select="concat(0,substring-before(COL23,'/'))"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="substring-before(COL23,'/')"/>
									</xsl:otherwise>
