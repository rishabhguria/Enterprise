				  ...
				  <xsl:choose>
					  ...
					  <xsl:otherwise>
						...
						<xsl:variable name="UnitCost">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL16 div COL7"/>
							</xsl:call-template>
						</xsl:variable>

						<xsl:variable name="UnitCost1">
							<xsl:choose>
							<xsl:when test="COL4='Option'">
								<xsl:value-of select="$UnitCost div 100"/>
							</xsl:when>

								<xsl:when test="normalize-space(COL4)='Conv'">
									<xsl:value-of select="$UnitCost * 100"/>
								</xsl:when>
								<xsl:when test="normalize-space(COL4)='Corp'">
									<xsl:value-of select="$UnitCost * 100"/>
								</xsl:when>
								<xsl:when test="normalize-space(COL4)='Govt'">
									<xsl:value-of select="$UnitCost * 100"/>
								</xsl:when>
								<xsl:when test="normalize-space(COL4)='Muni'">
									<xsl:value-of select="$UnitCost * 100"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<UnitCost>
							<xsl:choose>
								<xsl:when test="number($UnitCost1)">
