...
CREATE Procedure [dbo].[P_GetOpenPos_Daily_Cash_Cushing]                        
(                                 
@ThirdPartyID int,                                            
...
Max(TradeDate) As TaxlotTradeDate,
PT.ExpirationDate  
...
UPdate #TempGroupedOpenPosTable      
Set UnrealizedPNL = (OpenPositions * SideMultiplier * AssetMultiplier) * (MarkPrice - 
Case        
	When OpenPositions <> 0 And AssetMultiplier <> 0        
	Then (TotalCost_Local/OpenPositions) /AssetMultiplier        
	Else 0        
...
'' AS TradeDate,
CONVERT(VARCHAR(10), EODCash.Date, 101) AS TaxlotTradeDate, 
'' As ExpirationDate,
1 As UnitCost,
1 AS UnrealizedPNL,
1 AS UnitCostLocal
...
Max(EODCash.TaxlotTradeDate) AS TaxlotTradeDate,
