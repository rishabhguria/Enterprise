...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template name ="CurrencyCode">
		...
		<xsl:choose>
			...
			<xsl:when test="$varcurrencyDes='AUSTRALIAN DOLLAR'">
				<xsl:value-of select="'AUD'"/>
			</xsl:when>

			<xsl:when test="$varcurrencyDes='SWEDISH KRONA'">
				<xsl:value-of select="'SEK'"/>
			</xsl:when>

			<xsl:when test="$varcurrencyDes='DANISH KRONER'">
				<xsl:value-of select="'DKK'"/>
			</xsl:when>
			
			<xsl:when test="$varcurrencyDes='JAPANESE YEN'">
				<xsl:value-of select="'JPY'"/>
			</xsl:when>


			<xsl:when test="$varcurrencyDes='NEW POLISH ZLOTY'">
				<xsl:value-of select="'PLN'"/>
			</xsl:when>

			<xsl:when test="$varcurrencyDes='UK POUND STERLING'">
				<xsl:value-of select="'GBP'"/>
			</xsl:when>
			
			<xsl:when test="$varcurrencyDes='NORWEGIAN KRONE'">
				<xsl:value-of select="'NOK'"/>
			</xsl:when>

			<xsl:when test="$varcurrencyDes='HONG KONG DOLLAR'">
				<xsl:value-of select="'HKD'"/>
			</xsl:when>
			
			<xsl:when test="$varcurrencyDes='EURO'">
				<xsl:value-of select="'EUR'"/>
			</xsl:when>
			
			<xsl:when test="$varcurrencyDes='SWISS FRANC'">
				<xsl:value-of select="'CHF'"/>
			</xsl:when>
			
			<xsl:when test="$varcurrencyDes='CANADIAN DOLLAR'">
