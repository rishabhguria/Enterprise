  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      <xsl:for-each select="//Comparision">
		  ...
		  <xsl:if test="number(COL4) and number( COL2='11520755' or COL2='41954500' or COL2='64860812')">
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_SYMBOL_NAME">
              <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='GOLDMAN']/SymbolData[@PBCompanyName=$PB_COMPANY_NAME]/@PranaSymbol"/>
            </xsl:variable>
            ...
            <Description>
			  ...
			  <xsl:variable name ="varCallPut">
				  <xsl:choose>
					  <xsl:when test="substring-before(COL18,' ') = 'PUT' or substring-before(COL18,' ') = 'CALL'">
						  <xsl:value-of select="1"/>
					  </xsl:when>
					  <xsl:otherwise>
              ...
              <xsl:choose>
                <xsl:when test="COL5 != '' and $varCallPut = 1">
            ...
            <xsl:variable name="varExpirationYear">
			  ...
			  <xsl:variable name="varDateNo">
				  <xsl:if test="$varCallPut = 1">
					  <xsl:value-of select="substring(COL5,($varUnderlyingLength + 3),2)"/>
				  </xsl:if>
			  </xsl:variable>
            ...
            <!--<xsl:variable name ="varFees">
              <xsl:value-of select="number(COL10)+number(COL11)+number(COL12)+number(COL13)+number(COL14)"/>
            </xsl:variable>
