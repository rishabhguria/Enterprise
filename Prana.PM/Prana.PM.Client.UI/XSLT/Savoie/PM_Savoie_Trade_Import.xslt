...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="string-length(COL8) &gt; 15">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL8,' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(COL8,' '),5,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(COL8,' '),3,2)"/>
			</xsl:variable>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(COL8,' '),7,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(COL8,' '),8) div 1000,'#.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($NetPosition)">
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="string-length(COL8) &gt; 15">
								...
								<xsl:when test="COL11='0'">
									<xsl:choose>
										<xsl:when test="$Asset='Equity'">
											<xsl:value-of select="COL12 div COL10"/>
										</xsl:when>
										<xsl:when test="$Asset='Option'">
											<xsl:value-of select="(COL12 div COL10) div 100"/>
										</xsl:when>
										<xsl:otherwise>
						...
						<xsl:variable name="varCommission">
							<xsl:choose>
								<xsl:when test="$Asset='Option' and COL12 &lt; 0">
									<xsl:value-of select="COL12 + ($NetPosition * $varCostBasis)* 100"/>
								</xsl:when>
								<xsl:when test="$Asset='Option' and COL12 &gt; 0">
									<xsl:value-of select="COL12 - ($NetPosition * $varCostBasis)* 100"/>
								</xsl:when>
								<xsl:when test="$Asset='Equity' and COL12 &lt; 0">
									<xsl:value-of select="COL12 + ($NetPosition * $varCostBasis)"/>
								</xsl:when>
								<xsl:when test="$Asset='Equity' and COL12 &gt; 0">
									<xsl:value-of select="COL12 - ($NetPosition * $varCostBasis)"/>
								</xsl:when>
