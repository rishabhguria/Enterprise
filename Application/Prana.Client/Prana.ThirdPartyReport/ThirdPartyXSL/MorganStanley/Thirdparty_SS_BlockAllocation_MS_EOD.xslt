	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<xsl:variable name="AccountId_Group">
			...
			<!--<xsl:variable name="SettlementDate">
				<xsl:value-of select="concat(substring-after(substring-after(SettlementDate,'/'),'/'),substring-before(SettlementDate,'/'),substring-after(substring-before(SettlementDate,'/'),'/'))"/>
			</xsl:variable>
			<xsl:variable name="TradeDate">
				<xsl:value-of select="concat(substring-after(substring-after(TradeDate,'/'),'/'),substring-before(TradeDate,'/'),substring-after(substring-before(TradeDate,'/'),'/'))"/>
			</xsl:variable>-->

			<Group 
					TransactionType = "TR001" TransactionStatus="{$varTaxlotStateTx}" BuySellIndicator="" LongShortIndicator="" PositionType="{$Sidevar}"
					TransactionLevel="B" ClientTradeRefNo="{PBUniqueID}" AssociatedTradeId="{PBUniqueID}" ExecutionAccount="038222222" AccountId="038222222"
					...
					SettlementCCY="{CurrencySymbol}"	ExchangeCode="" Quantity="{$QtySum}" 	PriceAmount="{format-number(AveragePrice,'###.0000')}"	PriceIndicator="G" Principal="{format-number($GroupGrsAmt,'###.00')}" 
					CommissionAmount="{format-number($CommissionSum,'###.00')}" CommissionIndicator="F" 
					Taxorfees="{$GroupTotalFees}" 
					Tax2="0"
					TaxorFeesindicator="F" Interest="0" Interestindicator="" NetAmount="{format-number($GroupNetAmt,'###.00')}" HearsayIND="" Custodian="{$Custodian}" MoneyManager="" Bookid=""
					DealID="" TaxlotID="" OriginalTaxLotTransactionDate="" OriginalTaxlotTransactionPrice="" TaxlotCloseOutMethod="" ExchangeRate="" AcquisitionDate="" Comments="" FXOrderIndicator="" RowHeader="true"
				...
				<xsl:for-each select="/ThirdPartyFlatFileDetailCollection/ThirdPartyFlatFileDetail[PBUniqueID = $I_GroupID]">
					...
					<!--<xsl:variable name="SettlementDate1">
						<xsl:value-of select="concat(substring-after(substring-after(SettlementDate,'/'),'/'),substring-before(SettlementDate,'/'),substring-after(substring-before(SettlementDate,'/'),'/'))"/>
					</xsl:variable>
					...
					<ThirdPartyFlatFileDetail
								TaxLotState="{TaxLotState}" TransactionType = "TR001" TransactionStatus="{$varTaxlotState}" BuySellIndicator="" LongShortIndicator="" PositionType="{$Sidevar}"
								TransactionLevel="A" ClientTradeRefNo="{concat(PBUniqueID,$AccountId)}" AssociatedTradeId="{PBUniqueID}" ExecutionAccount="038222222" AccountId="{$AccountId}"
								BrokerCounterparty="{$CPVar}" SecurityIdentifierType="{$varSecType}" SecurityIdentifier="{$varSymbol}" SecurityDescription="{FullSecurityName}" TradeDate="{TradeDate}" SettlementDate="{SettlementDate}"
								SettlementCCY="{CurrencySymbol}"	ExchangeCode="" Quantity="{AllocatedQty}" PriceAmount="{format-number(AveragePrice,'###.0000')}" PriceIndicator="G" Principal="{format-number($TaxlotGrsAmt,'###.00')}"
								CommissionAmount="{format-number(CommissionCharged,'###.00')}" CommissionIndicator="F"
								Taxorfees="{$TaxFees}" 
								Tax2="0"
								TaxorFeesindicator="F" Interest="0" Interestindicator="" NetAmount="{format-number(NetAmount,'###.00')}" HearsayIND="" Custodian="{$Custodian}" MoneyManager="" Bookid=""
								DealID="" TaxlotID="" OriginalTaxLotTransactionDate="" OriginalTaxlotTransactionPrice="" TaxlotCloseOutMethod="" ExchangeRate="" AcquisitionDate="" Comments="" FXOrderIndicator="" RowHeader="true"
								EntityID="{EntityID}" />
					<!--</xsl:if>-->
				</xsl:for-each>
			...
			<xsl:variable name="TradeDate">
				<xsl:value-of select="concat(substring-after(substring-after(TradeDate,'/'),'/'),substring-before(TradeDate,'/'),substring-after(substring-before(TradeDate,'/'),'/'))"/>
					...
					<ThirdPartyFlatFileDetail
								...
								Tax2="0"
								TaxorFeesindicator="F" Interest="0" Interestindicator="" NetAmount="{format-number(NetAmount,'###.00')}" HearsayIND="" Custodian="{$Custodian}" MoneyManager="" Bookid=""
								DealID="" TaxlotID="" OriginalTaxLotTransactionDate="" OriginalTaxlotTransactionPrice="" TaxlotCloseOutMethod="" ExchangeRate="" AcquisitionDate="" Comments="" FXOrderIndicator=""
								EntityID="{EntityID}" />
					<!--</xsl:if>-->
				</xsl:for-each>
