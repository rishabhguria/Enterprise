...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[contains(Asset,'Future')!='true' and (Symbol!='PISXX' and Symbol!='DREUSTB')]">
				...
				<ThirdPartyFlatFileDetail>
					...
					<ACCOUNT>
						...
						<!--<xsl:choose>
							<xsl:when test="contains(FundName,'_')">
								<xsl:value-of select="substring-after(FundName,'_')"/>
					...
					<!--<SECURITY_SUB_TYPE>
						<xsl:choose>
							<xsl:when test="Asset!='FX'">
								<xsl:value-of select="translate(UDASecurityTypeName,',','')"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="''"/>
							</xsl:otherwise>
