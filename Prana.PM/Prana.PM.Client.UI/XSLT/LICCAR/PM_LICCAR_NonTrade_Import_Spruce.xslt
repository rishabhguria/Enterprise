    ...
    <DocumentElement>
      ...
      <xsl:for-each select="//PositionMaster">
	   ...
	   <xsl:variable name="varDescription">
        ...
        <xsl:if test="number($varCash) and ($varDescription = 'Withdraw' or $varDescription = 'Debit/Credit Interest' or $varDescription = 'SL Rebate/Fee')">
          ...
          <PositionMaster>
            ...
            <xsl:variable name = "PRANA_PRE_ACRONYM_NAME" >
              <xsl:choose>
                <xsl:when test="$varDescription='Withdraw'">
                  <xsl:value-of select="'CASH_WDL'"/>
                </xsl:when>
				<xsl:when test="$varDescription='Debit/Credit Interest'  and $varCash  &gt; 0">
                  <xsl:value-of select="'Cash'"/>
                </xsl:when>
				<xsl:when test="$varDescription='Debit/Credit Interest'  and $varCash  &lt; 0">
                  <xsl:value-of select="'Interest_Expense'"/>
                </xsl:when>
			   <xsl:when test="$varDescription='SL Rebate/Fee'">
                  <xsl:value-of select="'Cash'"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="''"/>
