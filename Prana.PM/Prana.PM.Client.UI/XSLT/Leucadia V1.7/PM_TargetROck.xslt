...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL2,'PUT') or contains(COL2,'CALL') ">
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name ="Asset" select="COL36"/>
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="string-length(COL6) &gt;20">
									<xsl:choose>
										<xsl:when test ="COL4='Buy Long'">
											<xsl:value-of select ="'A'"/>
										</xsl:when>
										<xsl:when test ="COL4='Cover Short'">
											<xsl:value-of select ="'B'"/>
										</xsl:when>
										<xsl:when test ="COL4='Sell Long'">
											<xsl:value-of select ="'D'"/>
										</xsl:when>
										<xsl:when test ="COL4='Short Sale'">
											<xsl:value-of select ="'C'"/>
										</xsl:when>
									</xsl:choose>
								</xsl:when>
						...
						<xsl:variable name="UnitCost">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="((COL12 + COL13 + COL14 + COL15) div COL10) div 100"/>
							</xsl:call-template>
						</xsl:variable>

						<xsl:variable name="UnitCostWithQuantity">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="(COL12 + COL13 + COL14 + COL15) div COL10"/>
							</xsl:call-template>
						</xsl:variable>
						...
						<CostBasis>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="$UnitCostWithQuantity &gt; 0">
											<xsl:value-of select="$UnitCostWithQuantity"/>
										</xsl:when>
										<xsl:when test="$UnitCostWithQuantity &lt; 0">
											<xsl:value-of select="$UnitCostWithQuantity * (-1)"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="0"/>
										</xsl:otherwise>
