...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring-before(COL1,' '),'CALL') or contains(substring-before(COL1,' '),'PUT') ">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL3,' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(COL3,' '),5,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(COL3,' '),3,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-after(COL3,' '),1,2)"/>
			</xsl:variable>

			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-before(COL1,' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
			
				<xsl:value-of select="format-number(substring(substring-after(COL3,' '),8,5),'#.00')"/>
			</xsl:variable>
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and  not(contains(COL3,'Accrued')) and  not(contains(COL2,'Yes')) ">
					...
					<PositionMaster>
						...
						<xsl:variable name ="Asset">
							<xsl:choose>
								<xsl:when test="contains(substring-before(COL1,' '),'CALL') or contains(substring-before(COL1,' '),'PUT')">
						...
						<AccountName>
							...
							<xsl:value-of select="'Hyphen Partners LP-MS'"/>
						</AccountName>
