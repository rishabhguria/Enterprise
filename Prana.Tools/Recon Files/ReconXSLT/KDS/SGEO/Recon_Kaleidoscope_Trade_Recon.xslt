...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="substring-before(substring-after(substring-after(COL5,' '),' '),' ') ='P' or substring-before(substring-after(substring-after(COL5,' '),' '),' ') ='C'">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL5,' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(COL5,' '),3,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(COL5,' '),1,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-after(COL5,' '),7,2)"/>
			</xsl:variable>
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring-after(substring-after(substring-after(COL5,' '),' '),' '),'#.00')"/>
			</xsl:variable>
					...
					<PositionMaster>
						...
						<IDCOOptionSymbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:value-of select="concat(COL39,'U')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="''"/>
								</xsl:otherwise>
						...
						<Side>
							<xsl:choose>
								<xsl:when test="$Asset ='EquityOption'">
									<xsl:choose>
										<xsl:when test="$Side ='B' and COL41 ='O'">
											<xsl:value-of select="'Buy to Open'"/>
										</xsl:when>

										<xsl:when test="$Side ='B' and COL41 ='C'">
											<xsl:value-of select ="'Buy to Close'"/>
										</xsl:when>

										<xsl:when test="$Side ='S' and COL41 ='O'">
											<xsl:value-of select="'Sell to Open'"/>
										</xsl:when>

										<xsl:when test="$Side ='S' and COL41 ='C'">
