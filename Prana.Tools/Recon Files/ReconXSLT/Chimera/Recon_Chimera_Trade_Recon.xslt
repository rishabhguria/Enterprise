			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) ">
					<PositionMaster>
						...
						<AccountName>
							<xsl:choose>
								<xsl:when test ="COL1='ARXJ1209'">
									<xsl:value-of select ="'Walleye WIF'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select ="'Walleye WOF'"/>
								</xsl:otherwise>
							</xsl:choose>
						...
						<!--<xsl:variable name="AvgPrice">
							<xsl:choose>
									<xsl:when test ="contains($Symbol,' ')">
									<xsl:value-of select="$AvgPriceValue * 100"/>
								</xsl:when>

								<xsl:otherwise>
									<xsl:value-of select="$AvgPriceValue"/>
								</xsl:otherwise>
