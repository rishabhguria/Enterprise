...
{
    public class SelectTradeWorkingSubsBlotter : BlotterUIMap, ITestStep
    {
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            {
                SamsaraHelperClass.CaptureMyScreen("", ApplicationArguments.TestCaseToBeRun, "SelectTradeWorkingSubsBlotter");
                _res.IsPassed = false;
            ...
            try
            {
                ...
                if (!DataUtilities.checkList)
                {
                    if (index < 0)
                    {
                        ...
                        throw new Exception("Trade not found during SelectTradeWorkingSubsBlotter step. [Symbol= " + dr["Symbol"] + "], Quantity = [" + dr["Quantity"] + "] Side = [" + dr["Side"] + "]\nRecon Error: " + String.Join("\n\r", errors));
                    }
                }
