	...
	<xsl:template match="/">
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($NetPosition) and contains(normalize-space(COL21),'@') and $PRANA_FUND_NAME!=''">
					<PositionMaster>
						...
						<xsl:variable name="varNetPosition">
							<xsl:value-of select="$NetPosition div 100000"/>
						</xsl:variable>
						...
						<xsl:variable name="CostBasis">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="($varCostBasis div 100) div $varNetPosition "/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
						...
						<xsl:variable name="varDay">
							<xsl:value-of select="substring(COL10,7)"/>
						</xsl:variable>
