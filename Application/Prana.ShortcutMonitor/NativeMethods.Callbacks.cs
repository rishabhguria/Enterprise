...
namespace Prana.ShortcutMonitor
{
    public static partial class NativeMethods
    {
        ...
        /// procedure does not call CallNextHookEx, the return value should be zero. 
        /// </returns>
        private static IntPtr KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
        {
            ...
            if (nCode >= 0)
            {
                ...
                if (s_KeyDown != null && (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN))
                {
                    ...
                    for (int i = 65; i < 91; i++)
                    {
                        if ((keys[i] & 0x80) != 0 && i != (int)keyData)
                        {
                            oldKey = (Keys)i;
                            break;
                        }
                    }
                    bool isDownControl = (GetAsyncKeyState(VK_CONTROL) != 0 ? true : false);
                    bool isDownALT = (GetAsyncKeyState(VK_MENU) != 0 ? true : false);
                    bool isDownShift = (GetAsyncKeyState(VK_SHIFT) != 0 ? true : false);
            ...
            return CallNextHookEx(s_KeyboardHookHandle, nCode, new IntPtr(wParam), lParam);
        }
