...
CREATE PROCEDURE [dbo].[PMGetAllSymbolsBusinessAdjustedDayMarkPriceForGivenDate]                                              
 (                                                                                
  @Date DateTime,                                                                                                                       
...
MAX(AUEC.ExchangeIdentifier) as AUECIdentifier                                                            
 ...
 FROM                                                                 
  [dbo].T_Group with (nolock )LEFT JOIN T_AUEC AUEC ON T_Group.AUECID = AUEC.AUECID                                        
...
TPATAF.VsCurrencyID  
 ...
 (Select PMDMP.Symbol, PMDMP.FinalMarkPrice, PMDMP.Date, PMDMP.ForwardPoints from PM_DayMarkPRice PMDMP         
  INNER JOIN #BusinessAdjustedSymbolWiseMaxDate symbolDate ON DateDiff(d,symbolDate.MaxDate,PMDMP.Date) = 0 and symbolDate.Symbol = PMDMP.Symbol WHERE PMDMP.FundID=0 ) PMDMP1        
