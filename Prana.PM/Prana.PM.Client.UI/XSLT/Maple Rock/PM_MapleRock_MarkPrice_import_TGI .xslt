        ...
        <xsl:if test="number($MarkPrice)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="varMarkPrice">
              <xsl:call-template name="Translate">
                <xsl:with-param name="Number" select="$varMarketValue div COL9"/>
              </xsl:call-template>
            </xsl:variable>
            <MarkPrice>
              <xsl:choose>
            ...
            <xsl:variable name="vraDay">
              <xsl:value-of select="substring-before(COL29,'-')"/>
            </xsl:variable>
            <xsl:variable name="vraMonth">
              <xsl:call-template name="Date">
                <xsl:with-param name="Month" select="substring-before(substring-after(COL29,'-'),'-')"/>
              </xsl:call-template>              
            </xsl:variable>
            
            <xsl:variable name="vraYear">
              <xsl:value-of select="substring-after(substring-after(COL29,'-'),'-')"/>
            </xsl:variable>
            <Date>
              <xsl:value-of select="concat($vraMonth,'/',$vraDay,'/',$vraYear)"/>
            </Date>
