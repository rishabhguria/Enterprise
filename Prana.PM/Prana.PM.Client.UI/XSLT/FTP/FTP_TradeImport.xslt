...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs"
	...
	<!--<xsl:template name ="FormateDate">
		...
		<xsl:variable name ="Day" select ="substring($DateIn,5,2)">
		</xsl:variable>
		<xsl:variable name ="Month" select ="substring($DateIn,7,2)">
		</xsl:variable>
		<xsl:variable name ="Year" select ="substring($DateIn,1,4)">
		</xsl:variable>
		<xsl:choose>
			<xsl:when test ="contains($DateIn,'[YYYY][MM][DD]')">
			...
			<xsl:for-each select="//PositionMaster">
				<!--<xsl:variable name ="PositionDate">
					<xsl:call-template name ="FormateDate">
						<xsl:with-param name ="DateIn" select="COL13"/>
					</xsl:call-template>
				</xsl:variable>-->
				...
				<xsl:if test="number($Position)">
					...
					<PositionMaster>
						...
						<xsl:variable name="AvgPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL181"/>
							</xsl:call-template>
						</xsl:variable>
						<CostBasis>
							<xsl:choose>
						...
						<ISIN>
							<xsl:value-of select ="normalize-space(COL195)"/>
						</ISIN>

						<CUSIP>
							<xsl:value-of select ="normalize-space(COL179)"/>
						</CUSIP>
						...
						<SideTagValue>
							<xsl:choose>
								<xsl:when test="normalize-space(COL31)='PURCHASED'">
									<xsl:value-of select="'1'"/>
								</xsl:when>

								<xsl:when test="normalize-space(COL31)='SOLD'">
