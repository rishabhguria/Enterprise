  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
        <xsl:if test ="contains(COL16, 'DIVIDEND')= false and normalize-space(COL2)!='Fund'">
          <PositionMaster>
            ...
            <xsl:variable name = "PRANA_ACRONYM_NAME">
              <xsl:choose>
                <xsl:when test="normalize-space(COL11)='WITHDRAWAL' and substring(normalize-space(COL16),1,8)!='TRANSFER'">
                  <xsl:value-of select="'Misc_Fees'"/>
                </xsl:when>
                <xsl:when test="normalize-space(COL11)='WITHDRAWAL' and substring(normalize-space(COL16),1,8)='TRANSFER'">
                  <xsl:value-of select="'CASH-WDL'"/>
                </xsl:when>
                <xsl:when test="normalize-space(COL11)='DEPOSIT' and substring(normalize-space(COL16),1,8)='TRANSFER'">
            ...
            <Description>
              <xsl:value-of select ="concat(COL11,' ',COL16)"/>
            </Description>
