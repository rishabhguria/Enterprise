	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				<xsl:if test="normalize-space(substring(COL1,30,1)) != ''">
					<PositionMaster>
						<xsl:variable name = "PB_FUND_NAME" >
							<xsl:value-of select="substring(COL1,22,8)"/>
						</xsl:variable>
						<xsl:variable name="PRANA_FUND_NAME">
							<xsl:value-of select="document('../ReconMappingXml/FundMapping.xml')/FundMapping/PB[@Name='Tradestation']/FundData[@PBFundCode=$PB_FUND_NAME]/@PranaFund"/>
						</xsl:variable>
						<xsl:choose>
						...
						<xsl:variable name = "Notional" >
							<xsl:value-of select="normalize-space(substring(COL1,118,16))* normalize-space(substring(COL1,82,14))"/>
						</xsl:variable>
						
						<xsl:choose>
							<xsl:when test="substring(COL1,134,1) = 'B'">
								<SideTagValue>
							...
							<xsl:when test="substring(COL1,134,1) = 'S'">
								...
								<MiscFees>
									<xsl:value-of select=".0000169 * $Notional"/>
								</MiscFees>
							</xsl:when>
							<xsl:otherwise>
								<SideTagValue>
