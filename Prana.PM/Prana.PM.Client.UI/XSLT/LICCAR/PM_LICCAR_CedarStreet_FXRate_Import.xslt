<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:my="put-your-namespace-uri-here" version="1.0">
	...
	<xsl:template match="/">
		...
		<xsl:variable name="varmarketValue">
				...
				<xsl:variable name="varReportingMarketValue">
					<xsl:call-template name="Translate">
				...
				<xsl:variable name="varFXRate">
					<xsl:call-template name="Translate">
						<xsl:with-param name="Number" select="$varmarketValue div $varReportingMarketValue"/>
					</xsl:call-template>
				</xsl:variable>
      
				<xsl:if test="number($varFXRate) and COL11 = 'Cash'">
		...
		<PositionMaster>
            ...
            <xsl:variable name="varForexPrice">
              <xsl:choose>
                <xsl:when test="normalize-space(COL9) = 'AUD' or normalize-space(COL9) = 'NZD' or normalize-space(COL9) = 'GBP' or normalize-space(COL9) = 'EUR'">
