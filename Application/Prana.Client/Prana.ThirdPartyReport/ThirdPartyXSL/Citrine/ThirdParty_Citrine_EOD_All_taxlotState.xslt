					...
					<ThirdPartyFlatFileDetail>
						...
						<CitcoCode>
							<xsl:choose>
								...
								<xsl:when test="(substring($varKey,1,4) = 'Comd' or substring($varKey,1,4) = 'COMD' or $varKey = 'Curncy' or $varKey = 'CURNCY') and Asset = 'Future' and Exchange != 'LME'">
									<xsl:value-of select="'CMDFUT'"/>
								</xsl:when>
								<xsl:when test="(substring($varKey,1,4) = 'Comd' or substring($varKey,1,4) = 'COMD' or $varKey = 'Curncy' or $varKey = 'CURNCY') and Asset = 'FutureOption'">
									<xsl:value-of select="'CMDFUTOPT'"/>
								</xsl:when>
								<xsl:when test="(substring($varKey,1,4) = 'Comd' or substring($varKey,1,4) = 'COMD') and Exchange = 'LME' ">
						...
						<Commodity>
							<xsl:value-of select="UDASubSectorName"/>
						</Commodity>
						...
						<Carry>
							<xsl:choose>
								<xsl:when test ="TradeAttribute1 = '' or TradeAttribute1 = '*'">
						...
						<PromptDate>
							<xsl:choose>
								<xsl:when test="PromptDateCheck = '3W' and (Exchange = 'LME' or Exchange = 'LME-FO')">
									<xsl:choose>
										<xsl:when test ="(Asset = 'Future' and Exchange ='LME') or (Asset = 'FutureOption' and Exchange ='LME-FO') ">
								...
								<xsl:when test="PromptDateCheck = '3M' and (Exchange = 'LME' or Exchange = 'LME-FO')">
									<xsl:value-of select="'3M'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test ="Exchange = 'LME' or Exchange = 'LME-FO'">
											<xsl:value-of select="PromptDate"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:choose>
												<xsl:when test="number(substring($varPromptDate,1,1))">
						...
						<Put_Call>
							<xsl:value-of select="normalize-space(substring(PutOrCall,1,1))"/>
						</Put_Call>
						...
						<Strike>
							...
							<xsl:choose>
								<xsl:when test="number(StrikePrice)">
									<xsl:value-of select="number(StrikePrice)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="''"/>
								</xsl:otherwise>
						...
						<ClearingBroker>
							<xsl:choose>
								<xsl:when test ="AccountName = '05A-00189'">
									<xsl:value-of select ="'Jefferies'"/>
								</xsl:when>
								<xsl:when test ="AccountName = '0VQ-954118'">
									<xsl:value-of select ="'Jefferies'"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<PromptDate1>
							<xsl:value-of select="concat(substring-before(substring-after(PromptDate,'/'),'/'),substring-before(PromptDate,'/'),substring-after(substring-after(PromptDate,'/'),'/'))"/>
						</PromptDate1>

						<LastTradeDate>
							<xsl:choose>
								<xsl:when test ="Exchange = 'LME' or Exchange = 'LME-FO'">
									<xsl:value-of select="concat(substring-before(substring-after(LastTradeDate,'/'),'/'),substring-before(LastTradeDate,'/'),substring-after(substring-after(LastTradeDate,'/'),'/'))"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="concat(substring-before(substring-after(PromptDate,'/'),'/'),substring-before(PromptDate,'/'),substring-after(substring-after(PromptDate,'/'),'/'))"/>
								</xsl:otherwise>
						...
						<DeliveryDate>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test ="Exchange = 'LME' or Exchange = 'LME-FO'">
											<xsl:value-of select="concat(substring-before(substring-after(PromptDate,'/'),'/'),substring-before(PromptDate,'/'),substring(substring-after(substring-after(PromptDate,'/'),'/'),1,4))"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="concat(substring-before(substring-after(DeliveryDate,'/'),'/'),substring-before(DeliveryDate,'/'),substring(substring-after(substring-after(DeliveryDate,'/'),'/'),1,4))"/>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:otherwise>
							</xsl:choose>
