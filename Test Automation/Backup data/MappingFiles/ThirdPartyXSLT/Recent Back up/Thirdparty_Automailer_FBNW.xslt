...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/NewDataSet">
		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[CounterParty='FBNW']">
				...
				<ThirdPartyFlatFileDetail>
					...
					<xsl:variable name="PB_NAME" select="'FBNW'"/>
					<xsl:variable name = "PRANA_FUND_NAME">
						<xsl:value-of select="AccountName"/>
					</xsl:variable>

					<xsl:variable name ="THIRDPARTY_FUND_CODE1">
					...
					<AccountName>
						<xsl:choose>
							<xsl:when test="$THIRDPARTY_FUND_CODE1!=''">
								<xsl:value-of select="$THIRDPARTY_FUND_CODE1"/>
							</xsl:when>
							<xsl:otherwise>
