...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/NewDataSet">
        ...
        <RDpartyBIC>
          <xsl:value-of select="'R/D party BIC'"/>
        </RDpartyBIC>

        <RDpartylocalCDID>
          <xsl:value-of select="'R/D party local CD ID'"/>
        </RDpartylocalCDID>

        <RDpartylocalcode>
          <xsl:value-of select="'R/D party local code'"/>
        </RDpartylocalcode>

        <BSPartyBIC>
          <xsl:value-of select="'B/S Party BIC'"/>
        </BSPartyBIC>

        <BSPartyDesc>
          <xsl:value-of select="'B/S Party Desc'"/>
        </BSPartyDesc>

        <BSPartyAcctatCustodian>
          <xsl:value-of select="'B/S Party Acct at Custodian'"/>
        </BSPartyAcctatCustodian>

        <PlaceofSettleBIC>
          <xsl:value-of select="'Place of Settle BIC'"/>
        </PlaceofSettleBIC>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail [CurrencySymbol !='USD']">
				...
				<xsl:choose>
					<xsl:when test ="TaxLotState!='Amemded'">
						<ThirdPartyFlatFileDetail>
							...
							<LOCALREF>
								<xsl:value-of select="substring(concat('PFJ',EntityID),1,10)"/>
							</LOCALREF>
							...
							<TIRORDERID>
								<xsl:value-of select="substring(concat('BLK', $BLK),1,6)"/>
							</TIRORDERID>
							...
							<SECACCOUNT>
								<xsl:choose>
									<xsl:when test="Side='Buy' or Side='Buy to Open' or Side='Sell' or Side='Sell to Close'">
										<xsl:value-of select="substring(concat(FundAccntNo,'1'),1,9)"/>
									</xsl:when>

									<xsl:when test="Side='Buy to Close' or Side='Buy to Cover' or Side='Sell short' or Side='Sell to Open'">
										<xsl:value-of select="substring(concat(FundAccntNo,'3'),1,9)"/>
									</xsl:when>

									<xsl:otherwise>
										<xsl:value-of select="substring(FundAccntNo,1,9)"/>
									</xsl:otherwise>
                ...
                <xsl:choose>
                  <xsl:when test="CurrencySymbol !='USD'">
                    <xsl:value-of select="concat('/AUTOFX=Y/AUTOFXCUR=',CurrencySymbol)"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="'/UTF=Y'"/>
                  </xsl:otherwise>
                </xsl:choose>
							</NOTES>
