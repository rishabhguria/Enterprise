	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				...
				<xsl:if test="number($varCash) and COL5 = 'FISXX'">

					<PositionMaster>

						<xsl:variable name="PB_NAME">
							<xsl:value-of select="'UMB'" />
						</xsl:variable>
						...
						<xsl:variable name="PRANA_ACRONYM_NAME">
							<xsl:choose>
								<xsl:when test="(contains(COL14, 'Income'))">
									<xsl:value-of select="'FISXX Money Fund'"/>
								</xsl:when>
						...
						<xsl:choose>
						<xsl:when test="(contains(COL14, 'Income'))">
									<xsl:value-of select="concat($PRANA_ACRONYM_NAME, ':' , $AbsCash , '|' , 'Money_Fund_Dividend_Income', ':' , $AbsCash)"/>
								</xsl:when>
								<xsl:when test="$varCash &lt; 0">
									<xsl:value-of select="concat('FISXX Money Fund', ':' , $AbsCash , '|' , 'Cash', ':' , $AbsCash)"/>
								</xsl:when>
								<xsl:when test="$varCash &gt; 0">
									<xsl:value-of select="concat('Cash', ':' , $AbsCash , '|' , 'FISXX Money Fund', ':' , $AbsCash)"/>
								</xsl:when>
