  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
        <xsl:if test="COL27 != '*' and normalize-space(COL27) != 'DIV'">
          <PositionMaster>
            ...
            <xsl:variable name = "PB_ACRONYM_NAME">
				<xsl:choose>
					<xsl:when test="contains(normalize-space(COL27), ' ') != false">
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME">
              <xsl:choose>
                <xsl:when test="$PB_ACRONYM_NAME = 'FEE'">
                  <xsl:value-of select="'Misc_Fees'"/>
                </xsl:when>
                <xsl:when test="$PB_ACRONYM_NAME = 'INT' and COL39 &gt; 0">
                  <xsl:value-of select="'Interest_Income'"/>
                </xsl:when>
                <xsl:when test="$PB_ACRONYM_NAME = 'INT' and COL39 &lt; 0">
                  <xsl:value-of select="'Interest_Expense'"/>
                </xsl:when>
                <xsl:when test="$PB_ACRONYM_NAME = 'JRL' and COL39 &gt; 0">
                ...
                <xsl:when test="$PB_ACRONYM_NAME = 'REB' or COL40 = 'BME'">
            ...
            <Description>
              <xsl:choose>
                <xsl:when test ="($PB_ACRONYM_NAME = 'INT' or COL31 = 'BRKR MG EXPENSE')">
