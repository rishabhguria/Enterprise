...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring(substring-after(normalize-space(COL15),' '),8,8) div 1000,'#.00')"/>
		...
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
		       ...
		       <xsl:variable name="varAsset">
				...
				<xsl:when test="number($Quantity) and ($varAsset !='BOND') and  ($varAsset !='CASH')">
					...
					<PositionMaster>
						...
						<xsl:variable name="AssetType">
							<xsl:choose>
								<xsl:when test="contains($varAsset,'LOPT')">
									<xsl:value-of select="'EquityOption'"/>
								</xsl:when>
								<xsl:when test="contains($varAsset,'EQTY')">
