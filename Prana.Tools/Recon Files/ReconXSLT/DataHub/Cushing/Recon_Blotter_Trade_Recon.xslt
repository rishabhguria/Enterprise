...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="string-length($Symbol) > 8">
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(substring-after(normalize-space($Symbol),' '),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after(normalize-space($Symbol),' '),' '),2),'##.00')"/>
			</xsl:variable>
			...
			<xsl:value-of select="concat('O:',$UnderlyingSymbol,' ',$ExpiryYear,$MonthCodVar,$StrikePrice,'D',$ExpiryDay)"/>
    ...
    <DocumentElement>
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:choose>
          <xsl:when test="number($Quantity) and ($PB_FUND_NAME='CUSHEN' or $PB_FUND_NAME='SRV' or $PB_FUND_NAME='PREMIER')">
            <PositionMaster>
