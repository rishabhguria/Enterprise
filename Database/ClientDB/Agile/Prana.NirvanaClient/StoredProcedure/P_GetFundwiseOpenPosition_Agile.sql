...
*/        
CREATE PROCEDURE [dbo].[P_GetFundwiseOpenPosition_Agile]     
(        
  @ThirdPartyID INT      
...
--And PT.Symbol = 'vns-tc'    
  
Declare @MinDate DateTime  
Set @MinDate = (Select Min(AUECLocalDate) As MinDate From #PositionTable)       
 ...
 EXEC P_GetAllFXConversionRatesFundWiseForGivenDateRange @MinDate ,@InputDate        
...
AND DateDiff(d, AUECLocalDate, FX.DATE) = 0        
   )        
 LEFT OUTER JOIN #FXConversionRatesDayMark AS FX1 ON (        
   ...
   AND FX1.FundID = 0        
AND DateDiff(d, AUECLocalDate, FX1.DATE) = 0        
   )    
  
-- FX End Date   
 LEFT OUTER JOIN #FXConversionRatesDayMark AS FXEndDate ON (        
   FXEndDate.FromCurrencyID = #PositionTable.CurrencyID        
   AND FXEndDate.ToCurrencyID = TC.LocalCurrency        
   AND FXEndDate.FundID = #PositionTable.FundID   
AND DateDiff(d, @InputDate, FXEndDate.DATE) = 0        
   )        
 LEFT OUTER JOIN #FXConversionRatesDayMark AS FX1EndDate ON (        
   FX1EndDate.FromCurrencyID = #PositionTable.CurrencyID        
   AND FX1EndDate.ToCurrencyID = TC.LocalCurrency        
   AND FX1EndDate.FundID = 0        
AND DateDiff(d, @InputDate, FX1EndDate.DATE) = 0        
   )   
...
AND DateDiff(d, AUECLocalDate, SettlFX.DATE) = 0             
   )        
...
AND DateDiff(d, AUECLocalDate, SettlFX1.DATE) = 0         
   )        
...
AND DateDiff(d, AUECLocalDate, FX3.DATE) = 0         
   )        
 LEFT OUTER JOIN #FXConversionRatesDayMark AS FX4 ON (        
   ...
   AND FX4.FundID = 0   
AND DateDiff(d, AUECLocalDate, FX4.DATE) = 0             
   )        
 CROSS APPLY (        
  SELECT CASE         
    WHEN (        
 ...
 CROSS APPLY (        
   SELECT   
    CASE         
     WHEN #PositionTable.CurrencyID = TC.LocalCurrency              
     THEN 1              
    ELSE IsNull(coalesce(FXEndDate.RateValue, FX1EndDate.RateValue), 0)        
    END        
  ) AS DayEndFxRate(Val)        
 CROSS APPLY (        
  SELECT CASE         
 ...
 DROP TABLE #PositionTable ,#TempTaxlotPK,#FXConversionRatesDayMark  
