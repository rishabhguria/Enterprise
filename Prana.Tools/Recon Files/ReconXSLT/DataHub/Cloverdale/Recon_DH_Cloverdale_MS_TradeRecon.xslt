  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:choose>
          <xsl:when test="number($Quantity) and contains(COL84,'Cash')!='true' and not(contains(COL14,'Money Market')) and (normalize-space(COL3)='038CAAPD2' or normalize-space(COL3)='038CDKDA6')">
            <PositionMaster>
              ...
              <xsl:variable name="Underlying">
                <xsl:choose>
                  <xsl:when test="substring(COL19,2,1) = '1'">
                    <xsl:value-of select="substring-before(COL19,'1')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,2,1) = '2'">
                    <xsl:value-of select="substring-before(COL19,'2')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,3,1) = '1'">
                    <xsl:value-of select="substring-before(COL19,'1')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,3,1) = '2'">
                    <xsl:value-of select="substring-before(COL19,'2')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,4,1) = '1'">
                    <xsl:value-of select="substring-before(COL19,'1')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,4,1) = '2'">
                    <xsl:value-of select="substring-before(COL19,'2')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,5,1) = '1'">
                    <xsl:value-of select="substring-before(COL19,'1')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,5,1) = '2'">
                    <xsl:value-of select="substring-before(COL19,'2')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,6,1) = '1'">
                    <xsl:value-of select="substring-before(COL19,'1')"/>
                  </xsl:when>
                  <xsl:when test="substring(COL19,6,1) = '2'">
                    <xsl:value-of select="substring-before(COL19,'2')"/>
                  </xsl:when>
                </xsl:choose>
              </xsl:variable>
