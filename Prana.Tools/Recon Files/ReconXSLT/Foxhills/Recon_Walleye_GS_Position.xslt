...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring(substring-after(normalize-space(COL3),' '),1,3)"/>
		</xsl:variable>
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring(substring-after(normalize-space(COL3),' '),10,2)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring-before(normalize-space(substring(substring-after(normalize-space(COL3),' '),12)),' '),'##.00')"/>
		</xsl:variable>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test ="number($varQuantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="varCusip" select="COL17"/>
