...
{
    public class HubClientConnectionManagerRTPNL : IHubManagerRTPNL
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<HubClientConnectionManagerRTPNL> _logger;
        private readonly IHubContext<ServiceGatewayHubRTPNL> _hubContext;

        public HubClientConnectionManagerRTPNL(IConfiguration configuration,
            ILogger<HubClientConnectionManagerRTPNL> logger,
            IHubContext<ServiceGatewayHubRTPNL> hubManager)
        {
        ...
        public static void AddUserToRTPNLUpdatesList(int userId)
        {
            try
            {
                ...
                var isAdded = _userRtpnlUpdatesList.TryAdd(userId, true);
                if (isAdded)
                    Log.Information($"User {userId} added to the RTPNL update list.");
            }
            catch (Exception ex)
            {
        ...
        public static void RemoveUserFromRTPNLUpdatesList(int userId)
        {
            try
            {
                ...
                if (isRemoved)
                    Log.Information($"User {userId} removed from RTPNL update list.");
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Error occurred while removing user from the list.");
            }
        }
        ...
        public async void SendUserBasedMessage(string topic, RequestResponseModel message)
        {
            try
            {
                ...
                if (!isForcefulBroadcast)
                {
                    ...
                    var connections = HubClientConnectionManagerRTPNL.GetConnections(userId.ToString());
