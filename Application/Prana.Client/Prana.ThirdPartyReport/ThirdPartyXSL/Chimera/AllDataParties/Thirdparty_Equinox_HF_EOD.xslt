...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
    ...
    <ThirdPartyFlatFileDetailCollection>
      ...
      <xsl:for-each select="ThirdPartyFlatFileDetail[(contains(AccountName,'Diamond')='true')]">
        ...
        <xsl:variable name="varOtherBrokerFees_Expo">
          <xsl:choose>
            <xsl:when test="contains(OtherBrokerFees,'E')">
              <xsl:call-template name="ScientificToNumber">
                <xsl:with-param name="ScientificN" select="OtherBrokerFee"/>
              </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
        ...
        <ThirdPartyFlatFileDetail>
          ...
          <BrokerCounterparty>
            <xsl:choose>
              <xsl:when test="CounterParty='RAJA' or CounterParty='RAJA_RJ'">
          ...
          <Principal>
            <xsl:value-of select="format-number(AllocatedQty * AveragePrice * AssetMultiplier,'0.##')"/>
          </Principal>
          ...
          <Taxorfees>
            <xsl:value-of select="format-number($varOtherBrokerFees_Expo + ClearingBrokerFee + StampDuty + TransactionLevy + ClearingFee + TaxOnCommissions + MiscFees + SecFee + OccFee + OrfFee,'0.####')"/>
          </Taxorfees>
          ...
          <xsl:variable name="varNetamount_Expo">
            <xsl:choose>
              <xsl:when test="contains(NetAmount,'E')">
                <xsl:call-template name="ScientificToNumber">
                  <xsl:with-param name="ScientificN" select="NetAmount"/>
                </xsl:call-template>
              </xsl:when>
              <xsl:otherwise>
