  ...
  <xsl:template match="/ThirdPartyFlatFileDetailCollection">
    <ThirdPartyFlatFileDetailCollection>
      ...
      <xsl:for-each select="ThirdPartyFlatFileDetail">
        <xsl:if test="TaxLotState != 'Amemded' and TaxLotState != 'Deleted' and AccountName != 'Maerisland S-1 BNY' and AccountName != 'Maerisland S-1 GSCO'">
          <ThirdPartyFlatFileDetail>
            <!--for system internal use-->
            <RowHeader>
              <xsl:value-of select ="'false'"/>
            ...
            <SecurityType>
			  ...
			  <xsl:variable name ="varSecCurrency">
				  <xsl:choose>
					  <xsl:when test ="(Asset='FX' or Asset='FXForward') and LeadCurrencyName = 'USD'">
			  ...
			  <SecurityCurrency>
				  <xsl:value-of select="$varSecCurrency"/>
			  </SecurityCurrency>
            ...
            <xsl:variable name="varSide">
              <xsl:choose>
                ...
                <xsl:when test="(Side='Sell short' or Side='Sell to Open') and (Asset='FX' or Asset='FXForward')">
                  <xsl:value-of select="'S'"/>
                </xsl:when>
            ...
            <xsl:choose>
              ...
              <xsl:when test ="Asset = 'FutureOption' or Asset = 'Future' or Asset='FX' or Asset='FXForward'">
                <IDSource>
                  <xsl:value-of select="'BLOOMBERG'"/>
                </IDSource>
              </xsl:when>
            ...
            <xsl:variable name="varBBCode">
              <xsl:choose>
                <xsl:when test="contains(BBCode, 'INDEX') != false">
                  <xsl:value-of select="concat(substring-before(BBCode,'INDEX'), 'Index')"/>
                </xsl:when>
                <xsl:when test="contains(BBCode, 'COMDTY') != false">
            ...
            <xsl:choose>
              ...
              <xsl:when test="Asset='FX' or Asset='FXForward'">
                <SecurityID>
                  <xsl:value-of select="concat($varSecCurrency,' Curncy')"/>
                </SecurityID>
              </xsl:when>
              <xsl:when test="Asset='FixedIncome' and ISIN != ''">
                <SecurityID>
            ...
            <Trader>
              <xsl:value-of select="'Default Trader'"/>
            </Trader>
            ...
            <AvgPx>
              <xsl:choose>
                <xsl:when test ="$varMultiplier != '' and number($varMultiplier)">
				  ...
				  <xsl:when test ="Asset = 'FXForward' and LeadCurrencyName = 'JPY'">
			  ...
			  <!--<AvgPx>
				  <xsl:choose>
					  <xsl:when test ="$varMultiplier != '' and number($varMultiplier)">
						  <xsl:value-of select='format-number(AveragePrice * $varMultiplier, "###0.0000")'/>
					  </xsl:when>
					  <xsl:otherwise>
            ...
            <Fund>
				<xsl:choose>
					<xsl:when test ="AccountName = 'Maerisland Master-43001203'">
						<xsl:value-of select ="'TRDNT_M'"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="'TRDNT'"/>
					</xsl:otherwise>
				</xsl:choose>
            </Fund>
            ...
            <!--<Fee>
			  ...
			  <Fee>
				  ...
				  <!--<xsl:value-of select="StampDuty + MiscFees + ClearingFee + OtherBrokerFee + SecFees + TaxOnCommissions + TransactionLevy"/>-->
                ...
                -->
				  <!--As per Alex mail ORF Fee is included, which comes into TransactionLevy from FIX-->
				  <!--
                <xsl:otherwise>
                  <xsl:value-of select="StampDuty + MiscFees + ClearingFee + OtherBrokerFee + SecFees + TaxOnCommissions + TransactionLevy"/>
                </xsl:otherwise>
