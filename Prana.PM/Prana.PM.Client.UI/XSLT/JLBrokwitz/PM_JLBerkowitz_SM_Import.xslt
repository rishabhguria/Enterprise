				...
				<xsl:if test="COL5">
					...
					<PositionMaster>
						<xsl:variable name="PB_NAME">
							<xsl:value-of select="'JLBerkowitz'"/>
						</xsl:variable>
						...
						<xsl:variable name="varDecimal">
							<xsl:value-of select="substring(substring-after(substring-after($Symbol,' '),$varPutORCall),6)"/>
						</xsl:variable>
						...
						<PutOrCall>
							<xsl:choose>
								<xsl:when test="contains(substring(substring-after($Symbol,$UnderlyingSymbol),7,1),'P')">
									<xsl:value-of select="'0'"/>
								</xsl:when>
								<xsl:when test="contains(substring(substring-after($Symbol,$UnderlyingSymbol),7,1),'C')">
						...
						<xsl:variable name="OsiOption">
							<xsl:choose>
								<xsl:when test="string-length(substring-before(COL7,'1'))=4">
									<xsl:value-of select="concat(substring-before(COL7,'1'),(' '),(' '),substring-after(COL7,'1'))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL7,'1'))=3">
									<xsl:value-of select="concat(substring-before(COL7,'1'),(' '),(' '),(' '),substring-after(COL7,'1'))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL7,'1'))=2">
									<xsl:value-of select="concat(substring-before(COL7,'1'),(' '),(' '),(' '),(' '),substring-after(COL7,'1'))"/>
								</xsl:when>
								<xsl:when test="string-length(substring-before(COL7,'1'))=1">
									<xsl:value-of select="concat(substring-before(COL7,'1'),(' '),(' '),(' '),(' '),(' '),substring-after(COL7,'1'))"/>
								</xsl:when>
								<xsl:otherwise>
						...
						<CurrencyID>
							<xsl:choose>
								<xsl:when test="contains(COL2,'USD')">
									<xsl:value-of select="1"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
