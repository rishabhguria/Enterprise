            ...
            <xsl:variable name="PRANA_SUFFIX_NAME">
              <xsl:value-of select="document('../ReconMappingXML/SymbolSuffixMapping.xml')/SymbolMapping/PB[@Name=$PB_NAME]/SymbolData[@PBSuffixCode=PB_SUFFIX_NAME]/@PranaSuffixCode"/>
						...
						<xsl:variable name="Bond">
							<xsl:choose>
								<xsl:when test="contains(COL60,'BOND')">
              ...
              <xsl:value-of select="COL9" />
						...
						<xsl:variable name="Date">
							<xsl:choose>
								<xsl:when test="string-length(substring-before(substring-after(substring-after(normalize-space(COL6),'/'),'/'),' ')) =4">
									<xsl:value-of select="concat($Month1,'/',substring-before(normalize-space(COL8),'-'),'/',substring-after(substring-after(COL8,'-'),'-'))"/>
								</xsl:when>
								<!--<xsl:when test="string-length(substring-before(substring-after(substring-after(normalize-space(COL6),'/'),'/'),' ')) =''">
					...
					<xsl:when test="string-length(substring-before(substring-after(substring-after(normalize-space(COL6),'-'),'-'),' ')=2)">
						<xsl:value-of select="concat($Month1,'/',substring-before(normalize-space(COL8),'-'),'/',20,substring-after(substring-after(COL8,'-'),'-'))"/>
					</xsl:when>
								--><!--<xsl:when test="string-length(substring-before(substring-after(substring-after(normalize-space(COL6),'/'),'/'),' ')=2)">
									<xsl:value-of select="concat($Month1,'/',substring-before(normalize-space(COL8),'-'),'/',substring-after(substring-after(COL8,'-'),'-'))"/>
