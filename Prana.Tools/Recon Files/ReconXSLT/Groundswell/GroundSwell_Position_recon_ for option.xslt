          ...
          <PositionMaster>
            ...
            <xsl:variable name="PB_SYMBOL_NAME">
			  ...
			  <xsl:variable name="PRANA_CUSIP_SYMBOL">
				  <xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name=$PB_NAME1]/SymbolData[@CusipSymbol=$PB_CUSIP_SYMBOL]/@PranaSymbol"/>
			  </xsl:variable>
            ...
            <Symbol>
				<xsl:choose>
					<xsl:when test="$varCallPut = 1">
						<xsl:call-template name="ConvertBBCodetoTicker">
							<xsl:with-param name="varBBCode" select="''"/>
						</xsl:call-template>
					</xsl:when>
					...
					<xsl:otherwise>
						<xsl:choose>
							...
							<xsl:when test="COL3='' or contains(COL3,'/') or contains(COL3,$Apos)">
								<xsl:value-of select="COL2"/>
							</xsl:when>
