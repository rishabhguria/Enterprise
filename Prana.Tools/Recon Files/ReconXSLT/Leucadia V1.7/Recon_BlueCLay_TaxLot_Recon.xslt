					...
					<PositionMaster>
						...
						<AccountName>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select ="'002491751 - Blue Clay Capital Master Fund LTD'"/>
								</xsl:otherwise>
							</xsl:choose>
						</AccountName>
						...
						<xsl:variable name="NetNotionalValue">
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:value-of select="($COLE * $COLF) * 100"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$COLE * $COLF"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						...
						<NetNotionalValueBase>
							<xsl:choose>
								<xsl:when test="number($COLH)">
									<xsl:value-of select="number($COLH)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
						...
						<xsl:variable name="MarketValue">
							<xsl:choose>
								<xsl:when test="$Asset='EquityOption'">
									<xsl:value-of select="($COLE * COL7) * 100"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$COLE * COL7"/>
								</xsl:otherwise>
							</xsl:choose>
						...
						<MarketValueBase>
							<xsl:choose>
								<xsl:when test="number($COLJ)">
									<xsl:value-of select="number($COLJ)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
