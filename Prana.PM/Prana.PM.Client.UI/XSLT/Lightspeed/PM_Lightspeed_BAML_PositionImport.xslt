...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-before(COL10,' '),1,1),C) or contains(substring(substring-before(COL10,' '),1,1),P)">
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-before(normalize-space(COL10),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring-after(substring-after(substring-after(normalize-space(COL10),' '),' '),' ') ,'#.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="(string-length(COL20) &gt; 20) and contains(COL5,'B')">
									<xsl:value-of select="'EquityOption'"/>
								</xsl:when>
								<xsl:when test="COL5='0'">
									<xsl:value-of select="'Equity'"/>
								</xsl:when>
								<xsl:when test="COL5='4'">
						...
						<xsl:variable name="Symbol">
							<xsl:value-of select="translate(normalize-space(COL20),$DopubleQuote,'')"/>
						</xsl:variable>
						...
						<!--<IDCOOptionSymbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:value-of select="concat(normalize-space(COL20),'U')"/>
								</xsl:when>
