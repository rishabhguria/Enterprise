...
CREATE PROCEDURE [dbo].[P_GetTransactions_TotalCommissionAndRate] (
...
BEGIN
	...
	FROM V_Taxlots VT
	INNER JOIN @FromAUECDatesTable FromAUECDates ON VT.AUECID = FromAUECDates.AUECID
	INNER JOIN @ToAUECDatesTable ToAUECDates ON VT.AUECID = ToAUECDates.AUECID
	...
	SELECT VT.TaxLotID AS TaxLotID
			...
			WHEN VT.CurrencyID <> CF.LocalCurrency
				THEN CASE 
						WHEN IsNull(VT.FXRate, 0) <> 0
							THEN VT.FXRate
						When ISNull((coalesce(FXDayRatesForTradeDate.RateValue, FXDayRatesForTradeDate1.RateValue)), 0) > 0 And VT.FXConversionMethodOperator = 'M'
						Then ISNull((coalesce(FXDayRatesForTradeDate.RateValue, FXDayRatesForTradeDate1.RateValue)), 0)
						When ISNull((coalesce(FXDayRatesForTradeDate.RateValue, FXDayRatesForTradeDate1.RateValue)), 0) > 0 And VT.FXConversionMethodOperator = 'D'
						Then 1/ISNull((coalesce(FXDayRatesForTradeDate.RateValue, FXDayRatesForTradeDate1.RateValue)), 0)
						Else 0
						END
			ELSE 1
			END AS FXRate
		...
		,CASE 
			...
			THEN (VT.TaxLotQty * VT.AvgPrice * SM.Multiplier) + VT.AccruedInterest
			ELSE VT.TaxLotQty * VT.AvgPrice * SM.Multiplier 
		END As GrossNotionalValue
						...
						WHEN IsNull(VT.FXRate, 0) <> 0
							THEN CASE 
									WHEN VT.FXConversionMethodOperator = 'M'
										THEN ((VT.TaxLotQty * VT.AvgPrice * SM.Multiplier) + VT.AccruedInterest) * IsNull(VT.FXRate, 0)
									WHEN VT.FXConversionMethodOperator = 'D'
										AND VT.FXRate > 0
										THEN ((VT.TaxLotQty * VT.AvgPrice * SM.Multiplier) + VT.AccruedInterest) * 1 / VT.FXRate
									END
						ELSE IsNull(((VT.TaxLotQty * VT.AvgPrice * SM.Multiplier) + VT.AccruedInterest) * IsNull(coalesce(FXDayRatesForTradeDate.RateValue, FXDayRatesForTradeDate1.RateValue), 0), 0)
						END
			ELSE (VT.TaxLotQty * VT.AvgPrice * SM.Multiplier) + VT.AccruedInterest
							...
							THEN CASE 
									WHEN IsNull(VT.FXRate, 0) <> 0
										THEN CASE 
												...
												END
									ELSE IsNull((VT.TaxLotQty * VT.AvgPrice * SM.Multiplier + (VT.AccruedInterest + (VT.SideMultiplier * VT.TotalExpenses))) * IsNull(coalesce(FXDayRatesForTradeDate.RateValue, FXDayRatesForTradeDate1.RateValue), 0), 0)
									END
		...
		,VT.OptionPremiumAdjustment
		,VT.Commission + VT.SoftCommission + VT.OtherBrokerFees AS TotalCommission
		,CASE 
			WHEN VT.CurrencyID <> CF.LocalCurrency
				THEN  
				CASE 
						WHEN IsNull((VT.TaxLotQty * VT.AvgPrice * SM.Multiplier), 0) <> 0
							THEN CASE 
									WHEN T_Asset.Assetid = 8
										THEN ((VT.Commission + VT.SoftCommission + VT.OtherBrokerFees ) / ((VT.TaxLotQty * VT.AvgPrice * SM.Multiplier) + VT.AccruedInterest)) *10000
									ELSE     (VT.Commission + VT.SoftCommission + VT.OtherBrokerFees ) / (VT.TaxLotQty * VT.AvgPrice *SM.Multiplier )*10000 
									END
						ELSE 0
						END
			ELSE CASE 
						WHEN IsNull(VT.TaxLotQty, 0) <> 0
							THEN ( VT.Commission + VT.SoftCommission + VT.OtherBrokerFees ) / VT.TaxLotQty
						ELSE 0
						END
	...
	LEFT OUTER JOIN V_UDA_DynamicUDA UDA ON SM.Symbol_PK = UDA.Symbol_PK
	CROSS APPLY (
		SELECT CASE 
	...
	CROSS APPLY (
		SELECT CASE 
				WHEN COALESCE(VT.FXConversionMethodOperator, 'M') <> 'D'
	...
	CROSS APPLY (
		SELECT CASE 
				WHEN T_Asset.Assetid = 8
					THEN VT.TaxLotQty * VT.AvgPrice * SM.Multiplier + (VT.AccruedInterest + (VT.SideMultiplier * VT.TotalExpenses))
				ELSE VT.TaxLotQty * VT.AvgPrice * SM.Multiplier + VT.SideMultiplier * VT.TotalExpenses
				END
