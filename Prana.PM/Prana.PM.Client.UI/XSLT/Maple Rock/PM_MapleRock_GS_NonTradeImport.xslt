  ...
  <xsl:template name="Translate">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//PositionMaster">
        ...
        <xsl:if test="number($Cash) and contains(normalize-space(COL18), 'PB VARIATION')">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_PRE">
              <xsl:choose>
				  <xsl:when test="contains($varSubAccountDesc, 'PB VARIATION') and $Cash &gt; 0 ">
			...
			<xsl:choose>
                <xsl:when test="$Cash &gt; 0">
                  <xsl:value-of select="'VM Receipt'"/>
                </xsl:when>
                <xsl:when test="$Cash &lt; 0">
                  <xsl:value-of select="'VM Call'"/>
                </xsl:when>
              </xsl:choose>
            </Description>
			<CurrencyName>
