...
{
    class EnterAllocatePreference : AllocationUIMap, ITestStep
    {
        /// <summary>
        ...
        public TestResult RunTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                if (testData.Tables[sheetIndexToName[0]].Rows.Count > 0)
                {
                    ...
                    SetPreferenceFromEditPrefUI(row);
                    AllocateUnallocatePinTab.Click(MouseButtons.Left);
                    AllocateByPreference(row);
                    AllocateUnallocatePinTab.Click(MouseButtons.Left);
                    SetForceAllocation(row);
                }
            }
            catch (Exception ex)
            {
                SamsaraHelperClass.CaptureMyScreen("", ApplicationArguments.TestCaseToBeRun, "EnterAllocatePreference");
                _res.IsPassed = false;
        ...
        private void SetPreferenceFromEditPrefUI(DataRow row)
        {
            try
            {
        ...
        private void AllocateByPreference(DataRow row)
        {
            try
            {
        ...
        /// </summary>
        /// <param name="row"></param>
        private void SetForceAllocation(DataRow row)
        {
            try
            {
