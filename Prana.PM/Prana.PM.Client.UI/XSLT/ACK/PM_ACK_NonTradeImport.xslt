				...
				<xsl:if test="number($varQuantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="varCurrencyName">
                           <xsl:choose>
								<xsl:when test="COL4='CANADIAN DOLLAR'">
						...
						<JournalEntries>
							<xsl:choose>
								<xsl:when test="contains(normalize-space(COL18),'SECURITIES LENDING FEE')">
									<xsl:value-of select="concat('SL FEE', ':' , $varAmount , '|' , 'Cash', ':' , $varAmount)"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL18),'SECURITIES LENDING REBATE')">
									<xsl:value-of select="concat('Cash', ':' , $varAmount , '|' , 'REB INC', ':' , $varAmount)"/>
								</xsl:when>
								<xsl:when test="contains(normalize-space(COL18),'DEBIT INTEREST')">
									<xsl:value-of select="concat('Interest_Payable', ':' , $varAmount , '|' , 'Cash', ':' , $varAmount)"/>
								</xsl:when>
						     	<xsl:when test="contains(normalize-space(COL18),'INTEREST ON SHORT SALE PROCEEDS') and $varQuantity &gt;0">
