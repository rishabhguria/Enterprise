...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-before(COL11,' '),1,1),C) or contains(substring(substring-before(COL11,' '),1,1),P)">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(normalize-space(COL16),' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring-before(substring-after(normalize-space(COL16),'/'),'/')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring-before(substring-after(substring-after(normalize-space(COL16),' '),' '),'/')"/>
			</xsl:variable>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(substring-after(substring-after(normalize-space(COL16),' '),' '),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(normalize-space(COL16),' '),' '),' '),2),'#.00')"/>
			</xsl:variable>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="MarketBaseValue">
							<xsl:call-template name="Translate">
						...
						<MarketValueBase>
							<xsl:choose>
								<xsl:when test="$MarketBaseValue &gt; 0">
									<xsl:value-of select="$MarketBaseValue"/>
								</xsl:when>
								<xsl:when test="$MarketBaseValue &lt; 0">
									<xsl:value-of select="$MarketBaseValue * (-1)"/>
								</xsl:when>
