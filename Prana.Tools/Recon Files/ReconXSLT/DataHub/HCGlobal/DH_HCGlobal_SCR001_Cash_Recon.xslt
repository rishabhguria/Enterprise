...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="Option"> 
		...
		<xsl:variable name="var">
			<xsl:value-of select="normalize-space(substring-after($varSymbol,' '))"/>
		</xsl:variable>

		<xsl:variable name="UnderlyingSymbol">
			<xsl:value-of select="normalize-space(substring-before(COL3,' '))"/>
		</xsl:variable>
		...
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="concat('20',substring($Date,7,2))"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after($var,' '),1,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">			
				<xsl:value-of select="format-number(substring(substring-after($var,' '),2),'#.00')"/>
		</xsl:variable>
        ...
        <xsl:choose>
          <xsl:when test="number($Quantity)">
            ...
            <PositionMaster>
              ...
              <Currency>
                <xsl:value-of select="normalize-space(COL52)"/>
              </Currency>
