...
{
    class RemoveAllSecurities : RebalancerUIMap, IUIAutomationTestStep
    {
        public TestResult RunUIAutomationTest(DataSet testData, Dictionary<int, string> sheetIndexToName)
        {
            ...
            try
            {
                ...
                {
                    uiAutomationHelper.HandleExpandedState(ApplicationArguments.mapdictionary["RASExpander"].AutomationUniqueValue, ApplicationArguments.mapdictionary["HeaderSite"].AutomationUniqueValue, "ToggleState_On");
                    uiAutomationHelper.CommonAction(testData.Tables[0], "OPEN_REBALANCER", "RebalancerWindow");
                }
                else
                {
                    throw new Exception("RemoveAllSecurities DataTable is Null");
                }
            }
            catch (Exception ex)
            {
