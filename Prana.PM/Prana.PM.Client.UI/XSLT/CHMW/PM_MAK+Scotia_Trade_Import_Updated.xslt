        ...
        <xsl:if test="number($NetPosition) and (COL7='BL' or COL7='SS' or COL7='SL' or COL7='CS')">
          ...
          <PositionMaster>
            ...
            <PositionSettlementDate>
				<xsl:choose>
					<!--<xsl:when test="string-length(substring-after(substring-after(normalize-space(COL8),'-'),'-')=2)">
						<xsl:value-of select="concat($Month1,'/',substring-before(normalize-space(COL8),'-'),'/',20,substring-after(substring-after(COL8,'-'),'-'))"/>
					</xsl:when>
					
					<xsl:when test="string-length(substring-after(substring-after(normalize-space(COL8),'-'),'-')=4)">
