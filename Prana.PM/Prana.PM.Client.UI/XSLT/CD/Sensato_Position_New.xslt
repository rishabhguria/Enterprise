		...
		<DocumentElement>
			...
			<xsl:for-each select="//PositionMaster">
				...
				<PositionMaster>
					...
					<PBSymbol>
						<xsl:value-of select ="concat('--',COL28)"/>
					</PBSymbol>
					...
					<xsl:choose>
						<xsl:when test="number(COL7) and number(COL7) &lt; 0 ">
							<NetPosition>
								<xsl:value-of select="normalize-space(COL7) *(-1)"/>
							</NetPosition>
						</xsl:when>
						<xsl:when test=" number(COL7) and number(COL7) &gt; 0">
							<NetPosition>
					...
					<!--Prise ie CostBasis-->
					<xsl:choose>
						<xsl:when test="number(COL10) and number(COL10) &lt; 0 ">
							<CostBasis>
								<xsl:value-of select="number(COL10) *(-1)"/>
							</CostBasis>
						</xsl:when>
						<xsl:when test="number(COL10) and number(COL10) &gt; 0 ">
							<CostBasis>
