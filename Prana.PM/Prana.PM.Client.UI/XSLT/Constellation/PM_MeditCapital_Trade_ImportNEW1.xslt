  ...
  <msxsl:script language="C#" implements-prefix="my">
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring-before(substring-after(normalize-space(substring-after(normalize-space(COL2),'/')),'/'),' ')"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-after(normalize-space(substring-after(normalize-space(COL2),' ')),' '),1,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(substring(substring-after(normalize-space(substring-after(normalize-space(COL2),' ')),' '),2),'#.00')"/>
		...
		<DocumentElement>
					...
					<PositionMaster>
						...
						<xsl:variable name="AssetType">
							<xsl:choose>
								...
								<xsl:when test="(contains(COL18,'STOCK') and (COL21 = 'FALSE'))">
									<xsl:value-of select="'Equity'"/>
								</xsl:when>
								<xsl:when test="(contains(COL18,'STOCK') and (COL21 = 'TRUE'))">
            ...
            <Symbol>
              <xsl:choose>
				...
				<xsl:when test="string-length(COL5) = 7">
                  <xsl:value-of select="''" />
                </xsl:when>
            ...
            <xsl:variable name = "varAccount" >
              <xsl:choose>
                <xsl:when test="contains(COL21,'TRUE')">
                  <xsl:value-of select="'Medit Capital GS 065443806'"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="'Medit Capital GS 065443806'"/>
                </xsl:otherwise>
							...
							<xsl:choose>
								<xsl:when test="$AssetType='EquityOption'">
									<xsl:choose>
										...
										<xsl:when test="contains($varSide,'Cover')">
											<xsl:value-of select="'B'"/>
										</xsl:when>
									</xsl:choose>
								</xsl:when>
...
<xsl:variable name="varFXRate">
						...
						<xsl:variable name="varCurrency" select="COL20"/>
            ...
            <!--Equity Swap-->
            <xsl:if test="$AssetType='EquitySwap'">
              ...
              <xsl:variable name="SettleDate">
                <xsl:value-of select='my:NowSwap(number($Year),number($Month),number($Day))'/>
              </xsl:variable>
