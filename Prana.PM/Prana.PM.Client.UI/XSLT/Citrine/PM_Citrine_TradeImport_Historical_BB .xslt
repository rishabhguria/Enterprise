        ...
        <xsl:if test="COL1 != 'Confirmed'">
          <PositionMaster>
			  ...
			  <xsl:variable name="varFutOptionExpiry">
				  <xsl:if test="$varAssetCategory = 'Option'">
					  ...
					  <xsl:variable name ="varExYear">
						  <xsl:choose>
							  <xsl:when test ="string-length(normalize-space(COL17)) = 2">
								  <xsl:value-of select ="concat('201', substring(COL17,2))"/>
							  </xsl:when>
							  <xsl:otherwise>
								  <xsl:value-of select ="concat('20', substring(COL17,2))"/>
							  </xsl:otherwise>
						  </xsl:choose>
					  </xsl:variable>
					  <xsl:choose>
						  <xsl:when test ="$varExchange = 'LME'">
							  <xsl:value-of select ="substring-before(my:Now(number($varExYear),number($varMonthNo)),' ')"/>
						  </xsl:when>
            ...
            <xsl:variable name="varOptionYear">
			  ...
			  <xsl:variable name ="varCode" select="substring(COL24,1,2)"/>

			  <xsl:variable name="PRANA_Strike_Multiplier">
				  <xsl:value-of select="document('../ReconMappingXml/UnderlyingCodeMapping.xml')/UnderlyingMapping/PB[@Name='Client Imports']/SymbolData[@UnderlyingCode=$varCode]/@StrikeMul"/>
			  </xsl:variable>

			  <xsl:variable name="strikeFutureOption">
			  <xsl:choose>
				  <xsl:when test ="number($PRANA_Strike_Multiplier)">
					  <xsl:value-of select="number(substring-after(COL24,' '))*$PRANA_Strike_Multiplier"/>
				  </xsl:when>
				  <xsl:otherwise>
					  <xsl:value-of select="number(substring-after(COL24,' '))"/>
				  </xsl:otherwise>
			  </xsl:choose>
			  </xsl:variable>
			  ...
			  <xsl:variable name="varFutureOption">
				  <xsl:choose>
					  ...
					  <xsl:when test="normalize-space(COL8) = 'Option' and COL13 != 'LME'">
						  <xsl:value-of select="translate(concat(substring(COL24,1,2),' ',substring(COL24,3,2),substring(COL24,5,1),$strikeFutureOption),$varSmall,$varCapital)"/>
					  </xsl:when>
					  <xsl:otherwise>
						  <xsl:value-of select="''"/>
					  </xsl:otherwise>
            ...
            <xsl:variable name="varDescription">
			  ...
			  <xsl:variable name="PRANA_Multiplier">
				  <xsl:value-of select="document('../ReconMappingXml/PriceMulMapping.xml')/PriceMulMapping/PB[@Name='Client Imports']/MultiplierData[@PranaRoot=$varRoot]/@Multiplier"/>
			  </xsl:variable>
            ...
            <!--<xsl:choose>
			  ...
			  <xsl:variable name="BB_Key">
				  <xsl:value-of select="document('../ReconMappingXml/PriceMulMapping.xml')/PriceMulMapping/PB[@Name='JPM']/MultiplierData[@PranaRoot=$varRoot]/@BBSuffix"/>
			  </xsl:variable>
            ...
            <SideTagValue>
			  ...
			  <CounterPartyID>
				  <xsl:choose>
					  ...
					  <xsl:when test ="$varExBroker = 'New Edge'">
					  ...
					  <xsl:when test ="$varExBroker = 'UBSS'">
					  ...
					  <xsl:when test ="$varExBroker = 'SocGen'">
						  <xsl:value-of select ="12"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'ANZ'">
						  <xsl:value-of select ="2"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'Assigned'">
						  <xsl:value-of select ="9"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'Tradition'">
						  <xsl:value-of select ="113"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'ABN'">
						  <xsl:value-of select ="115"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'ABN / Sunrise'">
						  <xsl:value-of select ="116"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'Sunrise'">
						  <xsl:value-of select ="117"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'HSBC'">
						  <xsl:value-of select ="5"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'INTLFCStone'">
						  <xsl:value-of select ="14"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'Delphi'">
						  <xsl:value-of select ="26"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'Wells Fargo'">
						  <xsl:value-of select ="120"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'JVA'">
						  <xsl:value-of select ="121"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'JVA (Marex)'">
						  <xsl:value-of select ="119"/>
					  </xsl:when>
					  <xsl:when test ="$varExBroker = 'AMT'">
						  <xsl:value-of select ="122"/>
					   </xsl:when>
					  <xsl:when test ="$varExBroker = 'Standard Chartered'">
