...
--    select dbo.[F_MW_GetMDReturnNOF_WPSReport]('10/13/2014','7/7/2015', 'Fund', 'T4E001012')           
-- =============================================          
CREATE FUNCTION [dbo].[F_MW_GetMDReturnNOF_WPSReport]           
(          
 @fromDate datetime,            
 @toDate datetime,          
...
(fundname varchar(max),payoutdate datetime,TransactionType varchar(max),TransactionTypeID int, MasterCategoryName varchar(max),  SubCategoryName varchar(max), Name varchar(max),cashvalue float)          
          
insert into @InvestorCash          
SELECT Funds.FundName,Journal.TransactionDate AS PayOutDate,TransactionType,SubAccounts. TransactionTypeID, MasterCategoryName,SubCategoryName,Name, case when Journal.CurrencyID = @BaseCurrencyID then Journal.CR- Journal.DR       
else isnull(CurrencyConversionRate.ConversionRate,1)*(Journal.CR- Journal.DR) end AS CashValue          
...
UPDATE @tempPNL SET DR=-DR          
 ...
 set @result = @BMVsecurities + @BMVcash              
 End            
