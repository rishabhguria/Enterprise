...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!--CCK   I152300090.000-->
	<!--CALL BRBR  H182300035.000   EXP 08/18/2023-->
	...
	<xsl:template name="Option">
		...
		<xsl:variable name="ExpiryMonth">
			<xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after(normalize-space(COL11),' '),' '),' '),' '),'/')"/>
		</xsl:variable>
		<xsl:variable name="ExpiryYear">
			<xsl:value-of select="substring(substring-after(substring-after(normalize-space(COL11),'/'),'/'),3,2)"/>
		</xsl:variable>
		<xsl:variable name="PutORCall">
			<xsl:value-of select="substring(substring-before(normalize-space(COL11),' '),1,1)"/>
		</xsl:variable>
		<xsl:variable name="StrikePriceBeforeDecimal">
			<xsl:value-of select="substring-before(substring(substring-after(substring-after(normalize-space(COL11),' '),' '),6),'.')"/>
		</xsl:variable>
		<xsl:variable name="StrikePriceAfterDecimal">
			<xsl:value-of select="substring-before(substring-after(substring(substring-after(substring-after(normalize-space(COL11),' '),' '),6),'.'),' EXP')"/>
		</xsl:variable>
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="format-number(concat($StrikePriceBeforeDecimal,'',$StrikePriceAfterDecimal) div 1000,'##.00')"/>
		</xsl:variable>
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="$varAssetType='EquityOption' and COL1 !='CCY'">
					...
					<PositionMaster>
						...
						<xsl:variable name="varExpiryYear">
							<xsl:value-of select="substring(substring-after(substring-after(normalize-space(COL11),'/'),'/'),1,4)"/>
						</xsl:variable>
						...
						<xsl:variable name="VarUDA">
							<xsl:choose>
								<xsl:when test="$varPutORCall='P'">
									<xsl:value-of select="'Equity Put'"/>
								</xsl:when>
								<xsl:when test="$varPutORCall='C'">
									<xsl:value-of select="'Equity Call'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="'Undefined'"/>
								</xsl:otherwise>
