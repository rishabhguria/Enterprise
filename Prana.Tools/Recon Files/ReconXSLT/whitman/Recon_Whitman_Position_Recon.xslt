...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-after(substring-after(substring-after(COL2,' '),' '),' '),1,1),P) or contains(substring(substring-after(substring-after(substring-after(COL2,' '),' '),' '),1,1),C) ">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL2,' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after(substring-after(COL2,' '),' '),4,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring(substring-after(substring-after(COL2,' '),' '),1,2)"/>
			</xsl:variable>
			<xsl:variable name="ExpiryYear">
				<xsl:value-of select="substring(substring-after(substring-after(COL2,' '),' '),9,2)"/>
			</xsl:variable>
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(COL2,' '),' '),' '),2),'#.00')"/>
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select="'FutOPT'"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>

						<xsl:variable name="varFutMonthsCode">
							<xsl:call-template name="MonthsCode">
								<xsl:with-param name="varMonth" select="substring-before(COL13,' ')"/>
							</xsl:call-template>
						</xsl:variable>

						<xsl:variable name="varFutOptMonthsCode">
							<xsl:choose>
								<xsl:when test="COL5!=''">
									<xsl:call-template name="MonthsCode">
										<xsl:with-param name="varMonth" select="substring-before(substring-after(normalize-space(COL13),' '),' ')"/>
									</xsl:call-template>
								</xsl:when>
							</xsl:choose>							
						</xsl:variable>
						...
						<xsl:variable name="varFutYear">
							<xsl:value-of select="substring(substring-before(substring-after(COL13,' '),' '),2,1)"/>
						</xsl:variable>

						<xsl:variable name="varFutOptYear">
							<xsl:choose>
								<xsl:when test="COL5='P' or COL5='C'">
									<xsl:value-of select="substring(substring-before(substring-after(substring-after(COL13,' '),' '),' '),2,1)"/>
								</xsl:when>
							</xsl:choose>
						</xsl:variable>
						
						<xsl:variable name="Future">
							<xsl:value-of select="concat($varFutMonthsCode,$varFutYear)"/>
						</xsl:variable>
						...
						<xsl:variable name="FutureOpt">
							<xsl:choose>
								<xsl:when test="COL5='P' or COL5='C'">
									<xsl:value-of select="concat($varFutOptMonthsCode,$varFutOptYear)"/>
								</xsl:when>
							</xsl:choose>
							
						</xsl:variable>

						<xsl:variable name ="StrickPrise">
						...
						<!--<xsl:variable name="Symbol">
							<xsl:choose>
								...
								<xsl:when test="COL5!='' or COL5!='*'">
									<xsl:value-of select="concat($UnderlyingSymbol,' ',$FutureOpt,$PUTORCALL,$StrickPrise)"/>
								</xsl:when>
								--><!--<xsl:otherwise>
									<xsl:value-of select="concat($UnderlyingSymbol,' ',$Future)"/>
								</xsl:otherwise>--><!--
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='FutOPT'">
									<xsl:value-of select="concat($varUnderlyingSymbol,' ',$FutureOpt,$PUTORCALL,$StrickPrise)"/>
								</xsl:when>

								<xsl:when test="$Asset='Fut'">
									<xsl:value-of select="concat($varUnderlyingSymbol,' ',$Future)"/>
								</xsl:when>
						...
						<xsl:variable name="Day" select="substring(COL8,8,2)"/>
