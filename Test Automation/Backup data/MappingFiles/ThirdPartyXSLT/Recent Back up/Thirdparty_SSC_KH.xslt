...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs">
	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail[contains(AccountName,'KH')='true']">
				...
				<ThirdPartyFlatFileDetail>
					...
					<Broker>
						<xsl:choose>
							<xsl:when test="contains(AccountName,'BTIG')">
								<xsl:value-of select="'BTIG'"/>
							</xsl:when>
							<xsl:when test="contains(AccountName,'GS')">
								<xsl:value-of select="'GSCO'"/>
							</xsl:when>
							<xsl:when test="contains(AccountName,'Pershing')">
								<xsl:value-of select="'PERSH'"/>
							</xsl:when>
							<xsl:when test="contains(AccountName,'Tech')">
								<xsl:value-of select="'PERSH'"/>
							</xsl:when>
							<xsl:when test="contains(AccountName,'US Bank')">
								<xsl:value-of select="'US Bank'"/>
							</xsl:when>
							<xsl:when test="contains(AccountName,'Morgan Stanley')">
