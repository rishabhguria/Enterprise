          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_PRE">
              <xsl:choose>
				  ...
				  <xsl:when test="$varSubAccountDesc ='GCL'and $Cash  &lt; 0 ">
					  <xsl:value-of select="'ClearingExpenses'"/>
				  </xsl:when>

				  <xsl:when test="$varSubAccountDesc ='WDR'and $Cash  &lt; 0 ">
					  <xsl:value-of select="'CASHTRA'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='WDR'and $Cash  &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='BUY' and $Cash  &lt; 0 ">
					  <xsl:value-of select="'MoneyMarketSecurity'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='SELL' and $Cash  &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
            ...
            <xsl:variable name="Description" select="normalize-space(COL3)"/>
            <Description>
