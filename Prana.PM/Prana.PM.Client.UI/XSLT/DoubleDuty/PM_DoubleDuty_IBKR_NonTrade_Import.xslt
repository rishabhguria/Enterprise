        ...
        <xsl:if test="number($Cash)">
          ...
          <PositionMaster>
            ...
            <xsl:variable name="PRANA_ACRONYM_NAME_PRE">
              <xsl:choose>
				  <xsl:when test="$varSubAccountDesc ='CINT' and $Cash &gt; 0 ">
					  <xsl:value-of select="'Cash'"/>
				  </xsl:when>
				  <xsl:when test="$varSubAccountDesc ='DINT' and $Cash &lt; 0 ">
          ...
          <xsl:variable name="varCurrentDate">
            <xsl:value-of select="concat($varMonth,'/',$varDay, '/', $varYear)"/>
			 ...
			 <xsl:variable name="CurrencyName">
			  ...
			  <FXRate>
			 <xsl:choose>
                 <xsl:when test="$CurrencyName='AUD' or $CurrencyName='EUR' or $CurrencyName='GBP' or $CurrencyName='NZD' or $CurrencyName='USD'">
						  <xsl:value-of select="$Cash div number(COL24)"/>
					  </xsl:when>
               <xsl:otherwise>
                <xsl:value-of select="number(COL24) div $Cash"/>
                </xsl:otherwise>
              </xsl:choose>
			  </FXRate>
