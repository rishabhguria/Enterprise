	...
	<xsl:template match="/ThirdPartyFlatFileDetailCollection">
		...
		<ThirdPartyFlatFileDetailCollection>
			...
			<xsl:for-each select="ThirdPartyFlatFileDetail">
				...
				<xsl:if test="Side!='Buy' and Side!='Sell' and AccountName='Clover SMA: Hatteras'">
					...
					<ThirdPartyFlatFileDetail>
						...
						<SecurityDescription>
							<xsl:choose>
								<xsl:when test="Asset='EquityOption'">
									<xsl:value-of select="concat(PutOrCall,' ',UnderlyingSymbol,' ',$ExpirationMonth,' ',format-number(StrikePrice,'#.00000'),' ',ExpirationDate)"/>
								</xsl:when>
								<xsl:otherwise>
