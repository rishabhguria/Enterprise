...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
  ...
  <xsl:template name="TaxLotIDBuilder">
    ...
    <xsl:variable name="varClosingMethod">
	  ...
	  <Group 
			  AccountNumber="{AccountName}" Strategy ="{Strategy}" TradeDate = "{TradeDate}" SettleDate = "{SettlementDate}" Side="{Side}"
        Ticker ="{Symbol}" CUSIP ="{CUSIP}" Issuer="" OtherDescription ="{FullSecurityName}" Coupon="{Coupon}" Maturity="{$varExpirationDate}"
        OriginalFace="" TradeQuantity="{AllocatedQty}" Factor="{AssetMultiplier}" Price="{AveragePrice}" TradeFXRate="{ForexRate}"
