				...
				<xsl:if test="COL1!='tradeDate'">
					<PositionMaster>
						...
						<xsl:variable name="varFxForwardMonth">
							<xsl:if test="COL6='NDF'">
								<xsl:call-template name="tempMonth">
									<xsl:with-param name="paramMonthCode" select="substring-before(COL15,'/')"/>
								</xsl:call-template>								
							</xsl:if>
						</xsl:variable>
						...
						<xsl:variable name="PRANA_FUND_NAME">
							<!--<xsl:value-of select="document('../ReconMappingXml/FundMapping.xml')/FundMapping/PB[@Name='GARDENIA']/FundData[@PBFundCode=$PB_FUND_NAME]/@PranaFund"/>-->
						</xsl:variable>
