...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="COL10='OPTION'">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before($Symbol,'/')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring(substring-after($Symbol,'/'),6,2)"/>
			</xsl:variable>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after($Symbol,'/'),8,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring-before(substring(substring-after($Symbol,'/'),9),'-'),'#.00')"/>
			</xsl:variable>
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:variable name="MarkPrice">
					<xsl:value-of select="translate(translate(translate(COL6,'(',''),')',''),',','') div translate(translate(translate(COL4,'(',''),')',''),',','')"/>
				</xsl:variable>
				...
				<xsl:if test="number($MarkPrice)">
					...
					<PositionMaster>
						...
						<xsl:variable name="COL1" select="translate(translate(COL1,'[',''),']','')"/>
