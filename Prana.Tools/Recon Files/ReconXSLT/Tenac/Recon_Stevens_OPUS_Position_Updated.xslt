	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				<xsl:if test ="number(COL8) and COL2!='Cash '">
					<PositionMaster>
						...
						<xsl:variable name ="Asset">
							<xsl:choose>
								...
								<xsl:when test ="COL2='Interest Rate Swap'">
						...
						<xsl:variable name="varCusip" select="COL3"/>
						...
						<xsl:variable name ="NetQuantity">
							<xsl:choose>
								<xsl:when test ="COL2 = 'Forward Currency Contract' and COL8 &gt; 0">
						...
						<Quantity>
							<xsl:choose>
								<xsl:when test ="COL2 = 'Forward Currency Contract' and (substring($varCusip,1,3) !='AUD'  or substring($varCusip,1,3) !='GBP' or substring($varCusip,1,3) !='NZD' or substring($varCusip,1,3) !='EUR')">
						...
						<!--<Side>
							<xsl:choose>
								<xsl:when test="COL2='Forward Currency Contract'">
									<xsl:choose>
										<xsl:when test="$NetQuantity &gt; 0">
											<xsl:value-of select="'Sell'"/>
										</xsl:when>
										<xsl:when test="$NetQuantity &lt; 0">
											<xsl:value-of select="'Buy'"/>
						...
						<MarketValue>
							<xsl:choose>
								<xsl:when test ="number($NetMarketValue)">
									<xsl:value-of select ="$NetMarketValue"/>
								</xsl:when>								
								<xsl:otherwise>
									<xsl:value-of select ="0"/>
						...
						<MarketValueBase>
							<xsl:choose>
								<xsl:when test ="number($NetMarketValueBase)">
