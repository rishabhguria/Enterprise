					...
					<xsl:when test="number($Quantity)">
						<PositionMaster>
							...
							<xsl:variable name="PRANA_UNDERLYING_NAME">
								<xsl:value-of select="document('../../ReconMappingXML/UnderlyingMapping.xml')/UnderlyingMapping/PB[@Name=$PB_NAME]/SymbolData[@InstrmentCode=$PB_PBContract]/@UnderlyingCode"/>
							</xsl:variable>


							<xsl:variable name="varFutureYear">
								<xsl:value-of select="substring(substring-before(substring-after(normalize-space(COL11),' '),' '),2,1)"/>
							</xsl:variable>

							<xsl:variable name ="varFutureMonthCode">
								<xsl:call-template name="FutureCode">
									<xsl:with-param name="FMonth" select="substring-before(normalize-space(COL10),' ')"/>
								</xsl:call-template>
							</xsl:variable>

							<xsl:variable name="varSymbol">
								<xsl:value-of select="concat($PRANA_UNDERLYING_NAME,' ',$varFutureMonthCode,$varFutureYear)"/>
							</xsl:variable>
