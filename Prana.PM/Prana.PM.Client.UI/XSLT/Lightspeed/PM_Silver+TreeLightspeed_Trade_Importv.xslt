...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="COL5='Option - Index' or COL5='Option - Equity'">
			
			<xsl:variable name="UnderlyingSymbol">
				<xsl:choose>
					<xsl:when test="contains(COL6,'Equity')">
						<xsl:value-of  select="substring-before(substring-after(COL6,' '),' ')"/>
					</xsl:when>
					<xsl:when test="contains(COL6,'Index')">			
			...
			<xsl:variable name="ExpiryDay">
				<xsl:choose>
					<xsl:when test="contains(COL6,'Index')">
						<xsl:choose>
							<xsl:when test="contains(COL6,'US')">
								<xsl:value-of select="substring-before(substring-after(substring-after(substring-after(COL6,' '),' '),'/'),'/')"/>
							</xsl:when>
							<xsl:otherwise>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL6),'/'),'/'),' '),' '),1,1)"/>
			</xsl:variable>
			
			<xsl:variable name="StrikePrice">
				<xsl:choose>
					<xsl:when test="contains(COL6,'Index')">
						<xsl:value-of select="substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL6),'/'),'/'),' '),' '),2)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="substring(substring-before(substring-after(substring-after(substring-after(normalize-space(COL6),'/'),'/'),' '),' '),2)"/>
					</xsl:otherwise>
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="COL5='Option - Index' or COL5='Option - Equity'">
									<xsl:value-of select="'EquityOption'"/>
								</xsl:when>
								<xsl:when test="COL5='Option - Commodity Future'">
									<xsl:value-of select="'FutureOption'"/>
								</xsl:when>
								<xsl:when test="contains(COL5,'Future')">
						...
						<xsl:variable name="VarFutureUnder">
							<xsl:value-of select="substring(substring-before(COL6,' '),1,2)"/>
						</xsl:variable>

						<xsl:variable name="VarFutureMonthCode">
							<xsl:value-of select="substring(substring-before(COL6,' '),3)"/>
						</xsl:variable>

						<xsl:variable name="VarFutureStrick">
							<xsl:value-of select="substring-before(substring-after(COL6,' '),' ')"/>
						</xsl:variable>
						...
						<xsl:variable name="varFuture">
							<xsl:choose>
								<xsl:when test="$Asset='FutureOption'">
									<xsl:value-of select="concat($VarFutureUnder,' ',$VarFutureMonthCode,$VarFutureStrick)"/>
								</xsl:when>
							</xsl:choose>
						</xsl:variable>
						...
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='Equity'">
									<xsl:choose>
										<xsl:when test="string-length($varSymbol)=1 and contains(COL6,'HK')">
											<xsl:value-of select="concat(concat('000',$varSymbol),'-HKG')"/>
										</xsl:when>
										<xsl:when test="string-length($varSymbol)=2 and contains(COL6,'HK')">
											<xsl:value-of select="concat(concat('00',$varSymbol),'-HKG')"/>
										</xsl:when>
										<xsl:when test="string-length($varSymbol)=3 and contains(COL6,'HK')">
											<xsl:value-of select="concat(concat('0',$varSymbol),'-HKG')"/>
										</xsl:when>
										<xsl:when test="string-length($varSymbol)=4 and contains(COL6,'HK')">
											<xsl:value-of select="concat($varSymbol,'-HKG')"/>
										</xsl:when>
										<xsl:otherwise>
										<xsl:value-of select="$PRANA_SYMBOL_NAME"/>
										</xsl:otherwise>
						...
						<FundName>
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select ="'Silver Tree'"/>
								</xsl:otherwise>
							</xsl:choose>
						</FundName>
						...
						<xsl:variable name="Year">
							<xsl:value-of select="substring(COL11,1,4)"/>
						</xsl:variable>
