  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:choose>
          <xsl:when test="number($Quantity) and (COL11 !='*' or COL11 !='')">
            <PositionMaster>
              ...
              <xsl:variable name="PB_FUND_NAME" select="'NTG 036518'"/>
              ...
              <xsl:variable name="varMarkPriceUsd">
                <xsl:call-template name="Translate">
              ...
              <xsl:variable name="varMarkPrice">
                <xsl:choose>
                  <xsl:when test="COL14 ='USD'">
                    <xsl:value-of select="$varMarkPriceUsd"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:choose>
                      <xsl:when test="$Quantity !='0'">
                        <xsl:value-of select ="($varMarketValue div $Quantity)"/>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="0"/>
                      </xsl:otherwise>
              ...
              <xsl:variable name="MarketValueBaseUsd">
                <xsl:call-template name="Translate">
                  <xsl:with-param name="Number" select="COL13"/>
                </xsl:call-template>
              </xsl:variable>

              <xsl:variable name="varMarketValueBaseUsd">
                <xsl:choose>
                  <xsl:when test="COL14 ='USD'">
                    <xsl:value-of select="$MarketValueBaseUsd"/>
                  </xsl:when>
                  <xsl:otherwise>
              ...
              <MarketValueBase>
                <xsl:choose>
                  <xsl:when test="number($varMarketValueBaseUsd)">
                    <xsl:value-of select="$varMarketValueBaseUsd"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="0"/>
                  </xsl:otherwise>
