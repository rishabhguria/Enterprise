...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(substring(substring-after(substring-after(substring-after(COL30,' '),' '),' '),1,1),'C') or contains(substring(substring-after(substring-after(substring-after(COL30,' '),' '),' '),1,1),'P')">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(normalize-space(COL30),' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring-before(substring-after(COL30,'/'),'/')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring-before(substring-after(substring-after(COL30,' '),' '),'/')"/>
			</xsl:variable>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(substring-after(substring-after(COL30,' '),' '),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(COL30,' '),' '),' '),2),'##.00')"/>
			</xsl:variable>
	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test ="number(COL5) and  COL1 != 'Account Number'">
					...
					<PositionMaster>
						...
						<xsl:variable name="PRANA_FUND_NAME">
							<xsl:value-of select="document('../ReconMappingXml/FundMapping.xml')/FundMapping/PB[@Name='GOLDMAN SACHS EXECUTION and CLEARING']/FundData[@PBFundName=$PB_FUND_NAME]/@PranaFund"/>
						</xsl:variable>

						<xsl:variable name="PRANA_STRATEGY_NAME">
							<xsl:value-of select="document('../ReconMappingXml/StrategyMapping.xml')/StrategyMapping/PB[@Name='GOLDMAN SACHS EXECUTION and CLEARING']/StrategyData[@PranaFundName=$PRANA_FUND_NAME]/@PranaStrategy"/>
						</xsl:variable>
						...
						<xsl:variable name="PRANA_SYMBOL_NAME">
							<xsl:value-of select="document('../ReconMappingXml/SymbolMapping.xml')/SymbolMapping/PB[@Name='GOLDMAN SACHS EXECUTION and CLEARING']/SymbolData[@PBCompanyName=$PB_Symbol]/@PranaSymbol"/>
						</xsl:variable>

						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="COL16='OPTION' and string-length(COL17 &gt; 20)">
							...
							<xsl:choose>
								...
								<xsl:when test ="$Asset='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="Symbol" select="COL17"/>
						...
						<xsl:choose>
							...
							<xsl:when test="COL5 &gt; 0 and COL2='OA'">
								<Side>
									<xsl:value-of select="'Buy to Close'"/>
								</Side>
							</xsl:when>
