	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//PositionMaster">
				...
				<xsl:if test="number($Position) and string-length(COL8) &gt; 10">
					...
					<PositionMaster>
						...
						<PutOrCall>
							<xsl:choose>
								<xsl:when test="contains(substring(substring-after(normalize-space(COL8),' '),7,1),'P')">
									<xsl:value-of select="'0'"/>
								</xsl:when>
								<xsl:when test="contains(substring(substring-after(normalize-space(COL8),' '),7,1),'C')">
						...
						<xsl:variable name="OsiOption">
							<xsl:choose>
								...
								<xsl:when test="string-length(substring-before(COL8,' ')=5)">
