	...
	<xsl:template name="Option">
		...
		<xsl:if test="COL2='OPTION'  and COL23='Funds'">
			...
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring-before(substring-after(normalize-space(COL4),' '),' '),'00.00')"/>
			</xsl:variable>
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<xsl:when test="COL2='ETF' or COL2='COMMON' or COL2='INTEREST_RATE_SWAP'">
									<xsl:value-of select="'Equity'"/>
								</xsl:when>
								<xsl:when test="COL2='FUTURES'">
									<xsl:value-of select="'Future'"/>
								</xsl:when>
								<xsl:when test="COL2='OPTION' and (COL23='' or COL23='*')">
									<xsl:value-of select="'FutureOption'"/>
								</xsl:when>
								<xsl:when test="COL2='OPTION'  and COL23='Funds'">
									<xsl:value-of select="'EquityOption'"/>
								</xsl:when>
								<xsl:when test="COL2='GOVT BOND' and COL23='Government'">
									<xsl:value-of select="'FixedIncome'"/>
								</xsl:when>
								<xsl:when test="COL2='INTEREST_RATE_SWAP' and (COL23='' or COL23='*')">
						...
						<xsl:variable name="Root_Code">
							<xsl:choose>
								<xsl:when test="string-length(COL29)= 5 and $Asset='FutureOption'">
									<xsl:value-of select="substring(COL29,1,2)"/>
								</xsl:when>

								<xsl:when test="string-length(COL29)= 5 and $Asset='Future'">
									<xsl:value-of select="substring(COL29,1,3)"/>
								</xsl:when>
								<xsl:when test="contains(COL29,' ')">
									<xsl:value-of select="substring-before(COL29,' ')"/>
								</xsl:when>
								<xsl:when test="string-length(COL29)= 4">
									<xsl:value-of select="substring(COL29,1,2)"/>
								</xsl:when>
						...
						<xsl:variable name="MONTH_CODE">
							<xsl:choose>
								<xsl:when test="string-length(COL29)= 5">
									<xsl:value-of select="substring(COL29,4,1)"/>
								</xsl:when>
								<xsl:when test="string-length(COL29)= 4">
									<xsl:value-of select="substring(COL29,3,1)"/>
								</xsl:when>
								<xsl:when test="contains(COL29,' ')">
									<xsl:value-of select="substring(substring-after(COL29,' '),1,1)"/>
								</xsl:when>
						...
						<xsl:variable name ="Year">
							<xsl:choose>
								<xsl:when test="string-length(normalize-space(COL29))= 5">
									<xsl:value-of select="substring(COL29,5,1)"/>
								</xsl:when>
								<xsl:when test="string-length(normalize-space(COL29))= 4 and substring(COL29,4,1)='8'">
									<xsl:value-of select="concat('1',substring(COL29,4,1))"/>
								</xsl:when>
								<xsl:when test="string-length(normalize-space(COL29))= 4">
									<xsl:value-of select="substring(COL29,4,1)"/>
								</xsl:when>
								<xsl:when test="contains(COL29,' ')">
									<xsl:value-of select="substring(substring-after(COL29,' '),2,1)"/>
								</xsl:when>
						...
						<xsl:variable name="varFUTUnderlyng">
							<xsl:choose>
								...
								<!--<xsl:when test="string-length(normalize-space(COL29))= 4">
									<xsl:value-of select="substring(normalize-space(COL29),1,2)"/>
								</xsl:when>-->
								<!--<xsl:when test="contains(COL29,' ')">
									<xsl:value-of select="substring(substring-after(COL29,' '),2,1)"/>
								</xsl:when>-->
						...
						<xsl:variable name="varFutCode">
							<xsl:choose>
								<xsl:when test="string-length(COL29)= 5">
									<xsl:value-of select="substring(COL29,3)"/>
								</xsl:when>
								<xsl:when test="string-length(COL29)= 4">
									<xsl:value-of select="substring(COL29,3)"/>
								</xsl:when>
						...
						<xsl:variable name="Symbol" select="COL29"/>
						<Symbol>
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="Symbol" select="COL29"/>
						...
						<xsl:variable name="MarkPrice">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="format-number(COL8,'0.#######')"/>
							</xsl:call-template>
