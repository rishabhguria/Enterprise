...
Modified Date: 8 September 2015  
Modiffied by : Pankaj Sharma  
Description : TotalCostASZero_Futures is now INT in the table T_reportPreferences with the following cases  
1) TotalCostASZero_Futures = 0 means total cost will not be Zero for Futures  
2) TotalCostASZero_Futures = 1 means total cost will be Zero for Futures  
3) TotalCostASZero_Futures = 2 means commission has to be considered for Futures  
...
CREATE Procedure [dbo].[P_MW_GetUnrealizedPNL]                    
(                    
@Rundate datetime,                
...
If(@SearchString <> '' and @paramGroupByLevel1<>'Select' AND (@paramGroupByLevel2<>'Select' OR @paramGroupByLevel3<>'Select' OR @paramGroupByLevel4<>'Select'))        
 BEGIN        
  ...
  CASE         
   ...
   THEN ISNULL(((EndingMarketValueBase/TotalEndingMarketValueBase)*100),0)        
   ELSE 0.0        
  END        
