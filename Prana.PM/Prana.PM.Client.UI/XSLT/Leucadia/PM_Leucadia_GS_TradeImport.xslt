            ...
            <xsl:choose>
			  ...
			  <xsl:variable name="Month">
				  <xsl:call-template name="MonthName">
					  <xsl:with-param name="Month" select="substring-before(COL9,' ')"/>
				  </xsl:call-template>
			  </xsl:variable>

			  <PositionStartDate>
				  <xsl:value-of select="concat($Month,'/',substring-before(substring-after(COL9,' '),','),'/',substring-after(substring-after(COL9,' '),' '))"/>
			  </PositionStartDate>

			  <xsl:variable name="Month1">
				  <xsl:call-template name="MonthName">
					  <xsl:with-param name="Month" select="substring-before(COL10,' ')"/>
				  </xsl:call-template>
			  </xsl:variable>

			  <PositionSettlementDate>
				  <xsl:value-of select="concat($Month1,'/',substring-before(substring-after(COL10,' '),','),'/',substring-after(substring-after(COL10,' '),' '))"/>
			  </PositionSettlementDate>
