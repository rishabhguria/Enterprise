	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Position) and contains(COL1,'Total')!='true' and (not(contains(COL1,'Cash')) and not(contains(COL1,'SSGA')))">
					...
					<PositionMaster>
						...
						<xsl:variable name="Cost" select="number(COL6) div number(COL5)"/>
						...
						<xsl:variable name="varMarketValue" select ="format-number($MarketValue,'##.####')"/>
						<MarketValue>
							<xsl:choose>
