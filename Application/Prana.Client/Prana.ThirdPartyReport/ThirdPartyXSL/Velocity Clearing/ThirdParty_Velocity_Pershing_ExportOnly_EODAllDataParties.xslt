  ...
  <xsl:template match="/NewDataSet">
    <ThirdPartyFlatFileDetailCollection>
      ...
      <ThirdPartyFlatFileDetail>
        ...
        <GPF2IDCode>
          <xsl:value-of select="'GPF2IDCode'"/>
        </GPF2IDCode>


        <GPF2Amount>
          <xsl:value-of select="'GPF2Amount'"/>
        </GPF2Amount>

        <GPF2CurrencyCode>
          <xsl:value-of select="'GPF2CurrencyCode'"/>
        </GPF2CurrencyCode>

        <GPF2AddSubtract>
          <xsl:value-of select="'GPF2AddSubtract'"/>
        </GPF2AddSubtract>

        <GPF3IDCode>
          <xsl:value-of select="'GPF3IDCode'"/>
        </GPF3IDCode>

        <GPF3Amount>
          <xsl:value-of select="'GPF3Amount'"/>
        </GPF3Amount>

        <GPF3CurrencyCode>
          <xsl:value-of select="'GPF3CurrencyCode'"/>
        </GPF3CurrencyCode>

        <GPF3AddSubtract>
          <xsl:value-of select="'GPF3AddSubtract'"/>
        </GPF3AddSubtract>

        <GPF4IDCode>
          <xsl:value-of select="'GPF4IDCode'"/>
        </GPF4IDCode>

        <GPF4Amount>
          <xsl:value-of select="'GPF4Amount'"/>
        </GPF4Amount>

        <GPF4CurrencyCode>
          <xsl:value-of select="'GPF4CurrencyCode'"/>
        </GPF4CurrencyCode>

        <GPF4AddSubtract>
          <xsl:value-of select="'GPF4AddSubtract'"/>
        </GPF4AddSubtract>

        <GPF5IDCode>
          <xsl:value-of select="'GPF5IDCode'"/>
        </GPF5IDCode>

        <GPF5Amount>
          <xsl:value-of select="'GPF5Amount'"/>
        </GPF5Amount>

        <GPF5CurrencyCode>
          <xsl:value-of select="'GPF5CurrencyCode'"/>
        </GPF5CurrencyCode>

        <GPF5AddSubtract>
          <xsl:value-of select="'GPF5AddSubtract'"/>
        </GPF5AddSubtract>
      ...
      <xsl:for-each select="ThirdPartyFlatFileDetail">
       ...
       <xsl:variable name="varPosition" select="position()" />
		  ...
		  <xsl:choose>
		    <xsl:when test="CounterParty='JSES'">
                <xsl:value-of select="'JSEB'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='CSIN'">
                <xsl:value-of select="'8430'"/>
              </xsl:when>
			   <xsl:when test="CounterParty='NMRA'">
                <xsl:value-of select="'NMFI'"/>
              </xsl:when>
			   <xsl:when test="CounterParty='BDDK'">
                <xsl:value-of select="'JJKE'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='TWDI'">
                <xsl:value-of select="'JJKE'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='BEST'">
                <xsl:value-of select="'JPMS'"/>
              </xsl:when>
			  <xsl:when test="CounterParty='2396'">
                <xsl:value-of select="'MIZU'"/>
              </xsl:when>
			  <xsl:otherwise>
        ...
        <SECACCOUNT>
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:choose>
				<xsl:when test="CustomOrdering='2' and AccountName='SMW Local 12'">
                    <xsl:value-of select="'XVL001209'"/>
                  </xsl:when>
                  <xsl:when test="CustomOrdering='2' and $varAccountName != ''">
                    <xsl:value-of select="$varAccountName"/>
                  </xsl:when>				  
