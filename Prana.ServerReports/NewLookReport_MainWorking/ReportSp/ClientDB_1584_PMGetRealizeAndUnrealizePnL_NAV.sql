PRINT N'Started executing ClientDB_1584_PMGetRealizeAndUnrealizePnL_NAV.sql'
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PMGetRealizeAndUnrealizePnL_NAV]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PMGetRealizeAndUnrealizePnL_NAV]
PRINT N'Deleted PMGetRealizeAndUnrealizePnL_NAV EXISTING sp'
GO       
...
[PMGetRealizeAndUnrealizePnL_NAV] '2/1/2014','3/10/2014' ,'1267,1272','1,2,3,4,5,6,7,8,9','MTM_V0',1                     
                    
Declare @NAVOfCurrentOrPriorDay Int                    
1 for Current and 2 for Prior Day                    
...
Set @BaseCurrencyID=(select BaseCurrencyID from T_Company where companyid <>-1)                                                     
                                                                                                                        
Declare @DefaultAUECID int                                                                              
Set @DefaultAUECID=(select DefaultAUECID from T_Company where companyid <>-1)                                                                          
...
--Local Realized PnL of closed Trade without Commission                              
 ...
 (Open_CloseTag = 'C' and  Asset = 'EquityOption' and @IncludeCommissionInPNL_EquityOption =0)  or                            (Open_CloseTag = 'C' and Asset not in ('Future','FutureOption','FX','Equity','EquityOption') and @IncludeCommissionInPNL_Other =
...
DROP table #FXConversionRates, #AUECYesterDates, #AUECBusinessDatesForEndDate,#MTMDataTable,#TempSymbol                                
END   

go
PRINT 'Done! ClientDB_1584_PMGetRealizeAndUnrealizePnL_NAV.sql'