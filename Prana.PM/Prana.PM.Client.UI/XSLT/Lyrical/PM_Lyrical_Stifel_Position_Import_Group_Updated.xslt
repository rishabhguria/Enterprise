	...
	<xsl:template match="/">
		<DocumentElement xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:my="put-your-namespace-uri-here">
			...
			<xsl:for-each select="//PositionMaster[generate-id() = generate-id(key('grouped', concat(COL6, '|', COL1))[1])]">
				...
				<xsl:variable name="groupedNetPosition" select="sum(key('grouped', concat($symbol, '|', $accountname))/COL12)"/>
				<xsl:variable name="groupedCostBasis" select="sum(key('grouped', concat($symbol, '|', $accountname))/COL14)"/>
            ...
            <sum>
                <xsl:for-each select="key('grouped', concat($symbol, '|', $accountname))">
                    <value>
                        <xsl:value-of select="number(COL12) * number(COL14)"/>
                    </value>
                </xsl:for-each>
            </sum>
        </xsl:variable>
        ...
        <xsl:variable name="weightedAvgPrice">
            <xsl:choose>
                <xsl:when test="$groupedNetPosition != 0">
                    <xsl:value-of select="$groupedCostBasis div $groupedNetPosition"/>
                </xsl:when>
