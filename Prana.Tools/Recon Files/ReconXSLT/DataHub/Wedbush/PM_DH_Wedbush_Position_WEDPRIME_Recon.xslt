...
xmlns:msxsl="urn:schemas-microsoft-com:xslt"
  ...
  <xsl:template name="ConvertBBCodetoTicker">
    ...
    <xsl:variable name="varRoot">
        ...
        <xsl:choose>
          <xsl:when test="substring(COL3,2,1) = '1'">
            <xsl:value-of select="substring-before(COL3,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,2,1) = '2'">
            <xsl:value-of select="substring-before(COL3,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,3,1) = '1'">
            <xsl:value-of select="substring-before(COL3,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,3,1) = '2'">
            <xsl:value-of select="substring-before(COL3,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,4,1) = '1'">
            <xsl:value-of select="substring-before(COL3,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,4,1) = '2'">
            <xsl:value-of select="substring-before(COL3,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,5,1) = '1'">
            <xsl:value-of select="substring-before(COL3,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,5,1) = '2'">
            <xsl:value-of select="substring-before(COL3,'2')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,6,1) = '1'">
            <xsl:value-of select="substring-before(COL3,'1')"/>
          </xsl:when>
          <xsl:when test="substring(COL3,6,1) = '2'">
            <xsl:value-of select="substring-before(COL3,'2')"/>
          </xsl:when>
        </xsl:choose>
      </xsl:variable>
    ...
    <DocumentElement>
      <xsl:for-each select="//Comparision">
        ...
        <xsl:variable name="PRANA_FUND_NAME">
          <xsl:value-of select="document('../../../MappingFiles/ReconMappingXml/FundMapping.xml')/FundMapping/PB[@Name='Wedbush Securities']/FundData[@PBFundCode=$PB_FUND_NAME]/@PranaFund"/>
        </xsl:variable>
          ...
          <PositionMaster>
            ...
            <MarkPrice>
			  ...
			  <xsl:variable name="varMarketValue">
				  <xsl:call-template name="Translate">
					  <xsl:with-param name="Number" select="COL4 * $varMarkPrice"/>
				  </xsl:call-template>
			  </xsl:variable>

			  <xsl:variable name="varMarketValueOption">
				  <xsl:choose>
					  <xsl:when test="$varCallPut = 1">
						  <xsl:value-of select="$varMarketValue * 100"/>
					  </xsl:when>
					  <xsl:otherwise>
			  ...
			  <MarketValue>
				  <xsl:value-of select="$varMarketValueOption"/>
			  </MarketValue>
