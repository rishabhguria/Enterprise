...
>
	...
	<xsl:template name="Option">
		...
		<!-- Normalize spacing -->
		<xsl:variable name="cleaned" select="normalize-space($varSymbol)"/>

		<!-- Option Type -->
		<xsl:variable name="PutOrCall">
			<xsl:choose>
				<xsl:when test="contains($cleaned, 'PUT')">
					<xsl:value-of select="'P'"/>
				</xsl:when>
				<xsl:when test="contains($cleaned, 'CALL')">
					<xsl:value-of select="'C'"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
		...
		<xsl:variable name="UnderlyingSymbol">
			<xsl:value-of select="substring-before(substring-after($cleaned, '- '), ' ')"/>
		</xsl:variable>

		<!-- Strike Price -->
		<xsl:variable name="StrikePrice">
			<xsl:value-of select="normalize-space(substring-before(substring-after($cleaned, '@'), 'EXP'))"/>
		</xsl:variable>

		<!-- Expiry Date: MM/DD/YYYY -->
		<xsl:variable name="ExpiryDate" select="normalize-space(substring-after($cleaned, 'EXP'))"/>
		<xsl:variable name="ExpiryMonth" select="substring($ExpiryDate, 1, 2)"/>
		<xsl:variable name="ExpiryDay" select="substring($ExpiryDate, 4, 2)"/>
		<xsl:variable name="ExpiryYear" select="substring($ExpiryDate, 9, 2)"/>
		...
		<!-- Final Output -->
		<xsl:value-of select="concat('O:', $UnderlyingSymbol, ' ', $ExpiryYear, $MonthCodVar, format-number($StrikePrice, '##0.00'), 'D', $Day)"/>
	</xsl:template>
	<xsl:template match="/">
							...
							<xsl:choose>
								...
								<xsl:when test="$AssetType='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="varSymbol" select="normalize-space(COL15)" />
									</xsl:call-template>
								</xsl:when>
                ...
                <xsl:otherwise>
                  <xsl:value-of select="0"/>
						...
						<xsl:variable name="TotalCommissionandFees">
							<xsl:call-template name="Translate">
								<xsl:with-param name="Number" select="COL73 + number(COL74) + number(COL75) + number(COL80) + number(COL76)"/>
							</xsl:call-template>
						</xsl:variable>
