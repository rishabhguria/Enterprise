  ...
  <xsl:template match="/">
    ...
    <DocumentElement>
      ...
      <xsl:for-each select ="//Comparision">
        ...
        <xsl:if test="number($Quantity) and contains(COL84,'Cash')!='true'">
          <PositionMaster>
            ...
            <xsl:variable name="Side" select="COL13"/>
			  <xsl:variable name="varSide" select="COL31"/>
			  <xsl:variable name="varRebalancing" select="COL14"/>

            <Side>
				<xsl:choose>
					<xsl:when test="$varRebalancing='Rebalancing'">
						<xsl:choose>
							<xsl:when test="$varSide='L' and $Quantity &gt; 0">
								<xsl:value-of select="'Buy'"/>
							</xsl:when>
							<xsl:when test="$varSide='L' and $Quantity &lt; 0">
								<xsl:value-of select="'Sell'"/>
							</xsl:when>
							<xsl:when test="$varSide='S' and $Quantity &lt; 0">
