	...
	<xsl:template match="/DocumentElement">
		<DocumentElement>
			...
			<xsl:for-each select="Comparision">
				...
				<xsl:if test ="substring(normalize-space(COL1),1,16)!='6320100329POSTNV' and  substring(normalize-space(COL1),1,7)!='T002376'and $varQty !=''">
				...
				<xsl:value-of select="substring(COL1,1,8)"/>
					...
					<xsl:choose>
						<xsl:when test="boolean(number($varMarkPrice))">
							<MarkPrice>
								<xsl:value-of select='format-number($varMarkPrice, "###,###,###.#####" )'/>
							</MarkPrice>
						</xsl:when>
						<xsl:otherwise>
							<MarkPrice>
