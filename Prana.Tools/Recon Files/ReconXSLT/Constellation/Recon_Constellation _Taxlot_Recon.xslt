...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL8,'CALL') or contains(COL8,'PUT')  ">
			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(substring-after(normalize-space(COL8),' '),' ')"/>
			</xsl:variable>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-before(COL8,' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring-before(substring-after(substring-after(substring-after(normalize-space(COL8),' '),' '),' '),' '),'#.00')"/>
	...
	<xsl:template match="/">
		...
		<DocumentElement>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity) and not(contains(COL3,'CASH'))">
					...
					<PositionMaster>
						...
						<xsl:variable name="PB_SUFFIX_NAME">
							<xsl:value-of select="substring-after(COL7,'_')"/>
						</xsl:variable>
						...
						<xsl:variable name="Asset">
							<xsl:choose>
								<!--<xsl:when test="string-length(COL7) &gt; 0">
									<xsl:value-of select="'Option'"/>
								</xsl:when>-->
								<xsl:when test="contains(normalize-space(COL3),'OPTIONS')">
									<xsl:value-of select="'Option'"/>
								</xsl:when>
						...
						<xsl:variable name="TradeDate" select="normalize-space(COL9)"/>
						<TradeDate>
