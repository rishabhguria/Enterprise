...
>
	...
	<xsl:template name="Option">
		...
		<xsl:if test="contains(COL4,'Option') and contains(COL24,'/')">

			<xsl:variable name="UnderlyingSymbol">
				<xsl:value-of select="substring-before(COL24,' ')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryDay">
				<xsl:value-of select="substring-after(substring-before(COL24,'/'),'/')"/>
			</xsl:variable>
			<xsl:variable name="ExpiryMonth">
				<xsl:value-of select="substring-before(substring-after(substring-after(COL24,' '),' '),'/')"/>
			</xsl:variable>
			...
			<xsl:variable name="PutORCall">
				<xsl:value-of select="substring(substring-after(substring-after(substring-after(COL24,'/'),'/'),' '),1,1)"/>
			</xsl:variable>
			<xsl:variable name="StrikePrice">
				<xsl:value-of select="format-number(substring(substring-after(substring-after(substring-after(COL24,'/'),'/'),' '),2),'#.00')"/>
			</xsl:variable>
			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($Quantity)">
					...
					<PositionMaster>
							...
							<xsl:choose>
								...
								<xsl:when test="$Asset='EquityOption'">
									<xsl:call-template name="Option">
										<xsl:with-param name="Symbol" select="COL24"/>
						...
						<xsl:variable name="PB_FUND_NAME" select="COL35"/>
