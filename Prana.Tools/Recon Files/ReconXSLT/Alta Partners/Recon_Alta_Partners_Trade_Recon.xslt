			...
			<xsl:for-each select ="//Comparision">
				...
				<xsl:if test="number($varQuantity)">
					...
					<PositionMaster>
						...
						<CounterParty>
							<xsl:choose>
								<xsl:when test="$PB_BROKER_NAME='INCA'">
									<xsl:value-of select="'INCA'"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0"/>
								</xsl:otherwise>
						...
						<Side>
							<xsl:choose>
							    ...
							    <xsl:when test="normalize-space(COL9) = 'Buy to Close'">
						...
						<TradeDate>
							<xsl:value-of select="concat(substring(normalize-space(COL6),5,2),'/',substring(normalize-space(COL6),7,2),'/',substring(normalize-space(COL6),1,4))"/>
						</TradeDate>
						
						<SettlementDate>
							<xsl:value-of select="concat(substring(normalize-space(COL7),5,2),'/',substring(normalize-space(COL7),7,2),'/',substring(normalize-space(COL7),1,4))"/>
						</SettlementDate>
