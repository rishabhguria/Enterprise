	...
	<xsl:template match="/">
		<DocumentElement>
			<xsl:for-each select ="//Comparision">
				...
				<xsl:choose>
					<xsl:when test="number($Quantity)  and contains(COL51,'CASH') !='true' and (normalize-space(COL2)='38514592' or normalize-space(COL2)='038514584'or normalize-space(COL2)='038CDNI64')">
						<PositionMaster>
							...
							<xsl:variable name="Asset">
								<xsl:choose>
									...
									<xsl:when test="normalize-space(COL52)='EQS'">
										<xsl:value-of select="'EquitySwap'"/>
									</xsl:when>
									<xsl:when test="COL51='Equity'">
							...
							<xsl:choose>
							    <xsl:when test="contains(COL55,'/')">
								<xsl:value-of select="substring(substring-after(substring-after(normalize-space(COL55),'/'),'/'),3,2)"/>
								</xsl:when>
								<xsl:otherwise>
								<xsl:value-of select="substring(substring-before(COL55,'-'),3,2)"/>
								</xsl:otherwise>
							</xsl:choose>
							</xsl:variable>
							...
							<xsl:variable name="varMonth">
								<xsl:choose>
									<xsl:when test="contains(COL55,'/')">
										<xsl:value-of select="substring-before(normalize-space(COL55),'/')"/>
									</xsl:when>
									<xsl:otherwise>
								<xsl:value-of select="substring-before(substring-after(COL55,'-'),'-')"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:variable>
							...
							<xsl:variable name="varDay">
								<xsl:choose>
									<xsl:when test="contains(COL55,'/')">
								<xsl:value-of select="substring-before(substring-after(normalize-space(COL55),'/'),'/')"/>
									</xsl:when>
									<xsl:otherwise>
								<xsl:value-of select="substring-after(substring-after(COL55,'-'),'-')"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:variable>
              
							<xsl:variable name="varPutOrCallStrike">
								<xsl:choose>
									<xsl:when test="contains(COL6,'CALL') or contains(COL6,'PUT')">
										<xsl:value-of select="substring-after(substring-after(normalize-space(COL14),' '),' ')"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="''"/>
									</xsl:otherwise>
							...
							<xsl:variable name="varBBCode">
								<xsl:choose>
									<xsl:when test="string-length($varMonth)=1">
									<xsl:value-of select="concat($varUnderlying,' ','0',$varMonth,'/',$varDay,'/',$VarYear,' ',$varPutOrCallStrike)"/>							
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="concat($varUnderlying,' ',$varMonth,'/',$varDay,'/',$VarYear,' ',$varPutOrCallStrike)"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:variable>

							<xsl:variable name="varFwdDay" select="substring-before(substring-after(COL86,'/'),'/')"/>
							<xsl:variable name="varFwdMonth" select="substring-before(COL86,'/')"/>
							<xsl:variable name="varFwdYear" select="substring-after(substring-after(COL86,'/'),'/')"/>
							...
							<Symbol>
								<xsl:choose>
									...
									<xsl:when test="$Asset='EquitySwap'">
										<xsl:value-of select="concat('EQS.',COL14)"/>
									</xsl:when>
									<xsl:when test="$Asset='FX'">
										<xsl:value-of select="concat(COL8,'Fwd',' ',$varFwdMonth,'/',$varFwdDay,'/',$varFwdYear)"/>
									</xsl:when>

									<xsl:when test="$Asset='Option' and contains(COL51,'- Listed')">
