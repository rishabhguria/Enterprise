  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//PositionMaster">
		  ...
		  <xsl:if test="number(COL6) and normalize-space(normalize-space(COL4)) != 'DIV'and normalize-space(normalize-space(COL4)) != 'PDST'">
			  <PositionMaster>
				  ...
				  <xsl:variable name="PRANA_ACRONYM_NAME">
					  <xsl:choose>
						 <!--<xsl:when test="contains(COL3,'SHORT INT')">
							  <xsl:value-of select="'Interest_Income'"/>
						  </xsl:when>-->
						  <!--<xsl:when test="normalize-space(COL3) = 'INTR'">-->
						  
						  <xsl:when test="(contains(COL3,'INT') and normalize-space(COL4)='INTR') and (COL6 &lt; 0)">
							  <xsl:value-of select="'Interest_Expense'"/>
						  </xsl:when>
						  <xsl:when test="contains(COL3,'OCC') or contains(COL3,'FEE') or normalize-space(COL5) = 'OTHER INC/EXP'">
						  ...
						  <!--<xsl:when test="normalize-space(COL5) = 'CONTRIBUTIONS' or normalize-space(COL5) = 'OTHER INC/EXP'">
							  <xsl:value-of select="'CASH-DEP'"/>
						  </xsl:when>-->
						  
						  <xsl:when test="((normalize-space(COL4) = 'FEDWT') or (normalize-space(COL4) = 'JRL' or normalize-space(COL5) = 'WITHDRAWALS')) and (COL6 &lt; 0)  ">
