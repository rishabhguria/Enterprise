  ...
  <msxsl:script language="C#" implements-prefix="my">
	...
	<xsl:template name="ConvertBBCodetoTicker">
		...
		<xsl:variable name="varMonthCode">
			<xsl:value-of select="substring(COL1,($varUnderlyingLength + 5),1)"/>
		</xsl:variable>
  ...
  <xsl:template match="/">
    <DocumentElement>
      <xsl:for-each select="//Comparision">
        ...
        <xsl:if test="number(COL4) and  contains(COL2,' Spot')=false">
			<!--<xsl:if test="number(COL4) and normalize-space(not(contains(COL15,'Cash and Equivalents')))">-->
          <PositionMaster>
            ...
            <PBSymbol>
			  ...
			  <AccountName>
				  <xsl:choose>
					  ...
					  <xsl:otherwise>
						  <xsl:value-of select="'Wedbush Partner:2MA00293'"/>
					  </xsl:otherwise>
				  </xsl:choose>
			  </AccountName>
            ...
            <xsl:variable name ="varCallPut">
			  ...
			  <xsl:variable name="varUnderlying">
				  <xsl:choose>
					  <xsl:when test="COL1 != '' and COL1 != '*' and $varCallPut = 1">
						  <xsl:value-of select="substring-before(COL1,'1')"/>
					  </xsl:when>
            ...
            <xsl:variable name="varDateNo">
              <xsl:if test="$varCallPut = 1">
                <xsl:value-of select="substring(COL1,($varUnderlyingLength + 3),2)"/>
              </xsl:if>
            </xsl:variable>

            <xsl:variable name="varExpirationYear">
              <xsl:if test="$varCallPut = 1">
                <xsl:value-of select="substring(COL1,($varUnderlyingLength + 1),2)"/>
              </xsl:if>
            </xsl:variable>

            <xsl:variable name="varStrikePriceString">
              <xsl:value-of select="substring-after(substring-after(COL1,$varUnderlying),$varMonthCode)"/>
            </xsl:variable>
						  ...
						  <xsl:choose>
							  <xsl:when test="$varCallPut = 1">
								  <xsl:call-template name="ConvertBBCodetoTicker">
									  <xsl:with-param name="varBBCode" select="COL1"/>
								  </xsl:call-template>
							  </xsl:when>
							  <xsl:otherwise>
