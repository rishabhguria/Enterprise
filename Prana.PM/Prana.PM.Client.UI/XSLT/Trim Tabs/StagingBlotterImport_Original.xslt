        ...
        <xsl:if test="$varSide != 'Side' and translate(normalize-space(COL2),'&quot;','')!='USD' and translate(normalize-space(COL2),'&quot;','')!='CASH'">
          <PositionMaster>
            ...
            <xsl:variable name ="varBBG">
              <xsl:value-of select ="concat(translate(translate(normalize-space(COL2),'&quot;',''),$vLowercaseChars_CONST,$vUppercaseChars_CONST),' EQUITY')"/>
            </xsl:variable>
...
<xsl:when test ="COL6 = 'PF-1'">
            ...
            <xsl:variable name="varEndTime">
              <xsl:choose>
                <xsl:when test="$varEDateTimeCheck !='' or $varEDateTimeCheck !='*'">
