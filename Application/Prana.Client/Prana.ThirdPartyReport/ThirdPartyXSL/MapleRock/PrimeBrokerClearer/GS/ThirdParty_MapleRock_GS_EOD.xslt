...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
	...
	<xsl:template match="/">
		<Groups>
			...
			<!--<xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail[Asset!='FX' and Asset!='FXForward' and ((Asset='FixedIncome' and (TransactionType='Buy' or TransactionType='Sell')) or Asset!='FixedIncome')]">-->
			<xsl:for-each select="/NewDataSet/ThirdPartyFlatFileDetail[Asset!='FX' and Asset!='FXForward']">
	...
	<xsl:template name="TaxLotIDBuilder">
		...
		<!--<xsl:variable name ="PB_SettleCurrency">
			<xsl:value-of select="document('../ReconMappingXml/SettlementCurrencyMapping.xml')/SettleCurrencyMapping/PB[@Name=$PB_NAME]/SymbolData[@UDACountry=$UDA_Country]/@SettleCurrency"/>
		</xsl:variable>-->
		...
		<xsl:choose>
			<xsl:when test="Asset='Equity' and IsSwapped='true'">
				<xsl:value-of select="'0052392750'"/>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:variable name="Stamp">
			<xsl:choose>
				<xsl:when test="CurrencySymbol = 'HKD' or CurrencySymbol = 'JPY' or CurrencySymbol = 'GBP' or CurrencySymbol = 'EUR'">
					<xsl:value-of select="StampDuty + TransactionLevy + SecFee + ClearingFee + MiscFees + TaxOnCommissions"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''"/>
				</xsl:otherwise>
		...
		<Group 
				OrderNumber ="" Cancelcorrectindicator ="" AccountNumber="{$varFund}" SecurityIdentifier="{$varSymbol}"
				Broker="{$varCounterParty}" Custodian="GSCO" TransactionType="{$varTransactionType}" CurrencyCode="{$Curr}"
				TradeDate = "{TradeDate}" SettleDate = "{SettlementDate}"
				Quantity="{AllocatedQty}" Commission="{$varCommission}" Price="{$Price}" AccruedInterest="{$AccruedInterest}" 
				TradeTax="{$varATradeTax}" MiscMoney="{MiscFees}" NetAmount="{$NetAmount}" Principal="{GrossAmount}" Description="{CompanyName}"
				SecurityType="{$varProductCode}" CountrySettlementCode="" ClearingAgent="" SECFee="" RepoOpenSettleDate="" RepoMaturityDate=""
		  ...
		  EntityID="{EntityID}" TaxLotState="{$varTaxlotState}" TaxLotState1="" FileHeader="FALSE" FileFooter="FALSE">
