		...
		<ThirdPartyFlatFileDetailCollection>
			<xsl:for-each select="ThirdPartyFlatFileDetail[AccountName != 'Rebal Risk Analysis' and AccountName != 'SB####']">
				<ThirdPartyFlatFileDetail>
					...
					<Advisor>
						<xsl:choose>
							<xsl:when test="AccountName ='AGEN 0082' or AccountName ='AGEN 0217' or AccountName ='IB 6160' or AccountName ='PIPR 7165'">
								<xsl:value-of select="'ATI-Air T'"/>
							</xsl:when>
							<xsl:when test="AccountName ='IB 0034' or AccountName ='IB 9812' or AccountName ='PIPR 6076' ">
								<xsl:value-of select="'ATI-Space Age'"/>
							</xsl:when>
							<xsl:when test="AccountName ='AGEN 0095' or AccountName ='CIBC 0472' ">
								<xsl:value-of select="'BCCM-AO II'"/>
							</xsl:when>							
							<xsl:when test="AccountName ='AGEN 0097' or AccountName ='CIBC 4860'">
								<xsl:value-of select="'BCCM-Pan Africa'"/>
							</xsl:when>
							<xsl:when test="AccountName ='CIBC 9246' or AccountName ='GSCO 3141'">
								<xsl:value-of select="'BCLAY-CO I'"/>
							</xsl:when>
							<xsl:when test="AccountName ='CIBC 1374' or AccountName ='GSCO 4449'">
								<xsl:value-of select="'BCLAY-CO III'"/>
							</xsl:when>
							<xsl:when test="AccountName ='GSCO 1751' ">
								<xsl:value-of select="'BCLAY-Master'"/>
							</xsl:when>							
							<xsl:when test="AccountName ='CIBC 1399' or AccountName ='GSCO 0924' ">
